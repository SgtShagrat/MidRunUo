...
namespace Server
{
	...
	#region Delegates
    public delegate void RaceChangeHandler( Mobile m, Race oldRace, Race newRace ); // mod by Dies Irae
	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable
	{
		...
		private bool m_Slowed;//edit by Arlas
		private FrozenTimer m_FrozenTimer;
		private SlowedTimer m_SlowedTimer;
        ...
        {
            #region mod by Dies Irae
            if( m_RaceChangeHandler != null )
                m_RaceChangeHandler( this, oldRace, Race );
            #endregion
        }

        #region mod by Dies Irae
        private static RaceChangeHandler m_RaceChangeHandler;

        public static RaceChangeHandler RaceChangeHandler
        {
            get { return m_RaceChangeHandler; }
            set { m_RaceChangeHandler = value; }
        }
        #endregion
        ...
        #region [custom stone enchant system by Dies Irae]
        public virtual void UpdateStonePowers()
        {
        }

        public virtual void UpdateStonePower( int bitmask )
        {
        }

        public virtual int GetStonePower( int bitmask )
        {
            return 0;
        }
        #endregion

        #region [custom resistance system by Dies Irae]
        public virtual int CustElectricResistance { get { return 0; } }
        public virtual int CustFireResistance { get { return 0; } }
        public virtual int CustGeneralResistance { get { return 0; } }
        public virtual int CustImpactResistance { get { return 0; } }
        public virtual int CustMentalResistance { get { return 0; } }
        public virtual int CustVenomResistance { get { return 0; } }

        public virtual int GetCustomResistance( CustomResType type )
        {
            return 0;
        }
        #endregion
		...
		public virtual void OnAosSingleClick( Mobile from )
		{
			...
			if( opl.Header > 0 )
			{
				...
				from.Send( new MessageLocalized( m_Serial, Body, MessageType.Label, hue, 3, opl.Header, Name, opl.HeaderArgs, from.Language ) );
			}
		}
		...
		public int VirtualArmorMod
		{
			get
			{
				return m_VirtualArmorMod + GetStatOffset( StatType.AR ); // mod by Dies Irae
			}
			set
			{
		...
		public void SlowSwing( TimeSpan duration )
		{
			if( !m_Slowed )
			{
				m_Slowed = true;

				m_SlowedTimer = new SlowedTimer( this, duration );
				m_SlowedTimer.Start();
			}
		}
		...
		public virtual bool CanRegenMana { get { return this.Alive; } }//edit by Arlas
		...
		private class ManaTimer : Timer
		{
			...
			protected override void OnTick()
			{
				if( m_Owner.CanRegenMana )//&& !m_Owner.NoNextManaGain )// m_Owner.Alive )
					m_Owner.Mana++;

				//m_Owner.NoNextManaGain = false;//edit by Arlas
		...
		private class SlowedTimer : Timer
		{
			private Mobile m_Mobile;

			public SlowedTimer( Mobile m, TimeSpan duration )
				: base( duration )
			{
				this.Priority = TimerPriority.TwentyFiveMS;
				m_Mobile = m;
		...
		private class CombatTimer : Timer
		{
			...
			protected override void OnTick()
			{
				if( DateTime.Now > m_Mobile.m_NextCombatTime )
				{
                    ...
                    if( m_Mobile.Player && !m_Mobile.OutRange( combatant, weapon.MinRange ) ) // mod by Dies Irae
                        return;
					...
					{							
						/*if ( !attacker.npctemplate  and !(attacker.paralyzed))
						if (attacker.stamina<5)
							SendSysMessage(attacker,"You are too tired to combat.");
							PerformAction(attacker, ANIM_STAND);
							return 0;
						endif*/
						if ( m_Mobile.Player )
						{
							if ( m_Mobile.Stam < 5 )
							{
								m_Mobile.SendMessage( m_Mobile.Language == "ITA"? "Sei troppo stanco per combattere." : "You are too tired to combat." );
								m_Mobile.m_NextCombatTime = DateTime.Now + TimeSpan.FromSeconds( 1.0 );
								return;
							}

							if( DateTime.Now > m_Mobile.LastMoveTime + TimeSpan.FromSeconds( 0.5 ) )
							{
								NetState ourState = m_Mobile.m_NetState;//prova per sincronizzazione on hit
								if( ourState != null )
								{
									ourState.Sequence = 0;

									if ( ourState.StygianAbyss )
										ourState.Send( new MobileUpdate( m_Mobile ) );
									else
										ourState.Send( new MobileUpdateOld( m_Mobile ) );

									m_Mobile.ClearFastwalkStack();
								}
							}
						}
		...
		[CommandProperty( AccessLevel.GameMaster )]		
		public Mobile LastAttacker{get;set;} //who bring last attack
		#endregion
		...
		public virtual void Attack( Mobile m )
		{
			if( CheckAttack( m ) )
				Combatant = m;
			//sincro dell'attacker
			if( Player && m_NetState != null && DateTime.Now > LastMoveTime + TimeSpan.FromSeconds( 0.5 ) )
			{
			...
			if (m.Player && DateTime.Now > m.LastMoveTime + TimeSpan.FromSeconds( 0.5 ))//sincro del defender
			{
				NetState defState = m.m_NetState;
				if( defState != null )
				{
					defState.Sequence = 0;

					if ( defState.StygianAbyss )
						defState.Send( new MobileUpdate( m ) );
					else
						defState.Send( new MobileUpdateOld( m ) );
		...
		#region mod by Magius(CHE): moved from PlayerMobile
		...
		protected virtual bool OnMove( Direction d )
		{
			...
			//if (Player)
			//	NoNextManaGain = true;//edit by Arlas
			return true;
		}
		...
		public virtual bool CheckShove( Mobile shoved )
		{
			if( (m_Map.Rules & MapRules.FreeMovement) == 0 )
			{
				...
				#region mod by Dies Irae
				if( shoved.Body.HandleShove )
				{
					SendMessage( "Thou shoved easily that away." );
					return true;
				}
				#endregion
					...
					else
					{
						if( Stam == StamMax )
						{
							...
							else
							{
								#region mod by Dies Irae
								int staminaLoss = 2;
								//int ESI = shoved.Str; // PlayerWalking->GetStat( 0 );  // Strength (+ Bonus)
								//int ECX = Str; // MobileInTheWay->GetStat( 0 ); // Strength (+ Bonus)
								//if( ESI > ECX )
								//{
								//	//StaminaLoss = (MobileInTheWay->GetStat(0) * 10) / PlayerWalking->GetStat(0);
								//	staminaLoss = ( ESI * 10 ) / ECX;
								//}
								// SetFatigue( GetFatigue() - StaminaLoss );
								Stam -= staminaLoss;
								#endregion
							}
						...
						else if ( Stam > 20 )
						{
							...
							if( ECX >= ESI )
							{
								//staminaLoss = ( ESI * 10 ) / ECX;
								Stam -= staminaLoss;
								SendMessage( "You are stronger than that, so you shoved that away.");
								RevealingAction();
								return true;
							}
							#endregion
		...
		public virtual void Resurrect()
		{
			if( !Alive )
			{
			    ...
			    int shroudId = Race.GhostShroudId( this ); // mod by Dies Irae
				for( int i = m_Items.Count - 1; i >= 0; --i )
				{
					...
					if( item.ItemID == /*0x204E*/ shroudId ) // mod by Dies Irae
						item.Delete();
				}
		...
		public virtual void OnAfterDelete()
		{
			...
			if( m_SlowedTimer != null )//edit by Arlas
				m_SlowedTimer.Stop();
		...
		public virtual void OnDeath( Container c )
		{
			...
			if( !m_Player )
			{
				EventSink.InvokeNonPlayerDeath( new MobileDeathEventArgs( this, c ) );
			...
			else
			{
                ...
                // mod by Dies Irae : each race can define a different shroud itemid
			    int shroudId = Race.GhostShroudId( this );

                Item deathShroud = new Item( /* 0x204E */ shroudId );
				...
				EventSink.InvokePlayerDeath( new MobileDeathEventArgs( this , c ) );
		...
		public virtual void Use( Item item )
		{
			...
			//DisruptiveAction(); edit by Arlas
		...
		public virtual void Lift( Item item, int amount, out bool rejected, out LRReason reject )
		{
			...
			if( from.AccessLevel >= AccessLevel.GameMaster || DateTime.Now >= from.NextActionTime )
			{
				if( from.CheckAlive() )
				{
					//from.DisruptiveAction(); edit by arlas
		...
		public virtual void DamageMana( int amount, Mobile from )
		{
			...
			if( amount > 0 )
			{
				...
				if( m_Spell != null )
					m_Spell.OnCasterDamaged( from, amount );
		...
		public virtual void DamageStam( int amount, Mobile from )
		{
			if( !CanBeDamaged() || m_Deleted )
				return;
		...
		public void Heal( int amount, Mobile from, bool message )
		{
			...
			if( message && amount > 0 && m_NetState != null )
				m_NetState.Send( new MessageLocalizedAffix( Serial.MinusOne, -1, MessageType.Label, 0x3B2, 3, 1008158, "", AffixType.Append | AffixType.System, amount.ToString(), "", from.Language ) );
		}
		...
		public DateTime CreationTime
		{
            ...
            set { m_CreationTime = value; } // mod by Dies Irae
		}
		...
		public virtual void AddItem( Item item ) // mod by Dies Irae
		{
		...
		public virtual void RevealingAction( bool disrupt )
		{
			if( m_Hidden && m_AccessLevel == AccessLevel.Player )
				Hidden = false;
		...
		{
			RevealingAction( false );//edit by Arlas
		}
		...
		{
			to.Send( new MessageLocalized( m_Serial, Body, MessageType.Regular, m_SpeechHue, 3, number, Name, "", to.Language ) );
		}

		public void SayTo( Mobile to, int number, string args )
		{
			to.Send( new MessageLocalized( m_Serial, Body, MessageType.Regular, m_SpeechHue, 3, number, Name, args, to.Language ) );
		}
        ...
        #region mod by Dies Irae
        public bool HasAnyGump()
        {
            NetState ns = m_NetState;

            return ns != null && new List<Gump>( ns.Gumps ).Count > 0;
        }

        public bool HasAnyMenu()
        {
            NetState ns = m_NetState;

            return ns != null && new List<IMenu>( ns.Menus ).Count > 0;
        }

        public bool HasAnyStringQuery()
        {
            NetState ns = m_NetState;

            return ns != null && new List<StringQueries.StringQuery>( ns.StringQueries ).Count > 0;
        }

	    public static bool PreAosDebug = false;

        public void DebugMessage( string format, params object[] args )
        {
            if( PreAosDebug )
                SendMessage( string.Format( "Debug: {0}", string.Format( format, args ) ) );
        }

        public void DebugOverheadMessage( string format, params object[] args )
        {
            if( PreAosDebug )
                PublicOverheadMessage( MessageType.Regular, 37, true, string.Format( format, args ) );
        }
        #endregion

        #region mod by Dies Irae : string queries and menu support
        public IMenu FindMenu( Type type )
        {
            NetState ns = m_NetState;

            if( ns != null )
            {
                foreach( IMenu menu in ns.Menus )
                {
                    if( type.IsAssignableFrom( menu.GetType() ) )
                    {
                        return menu;
                    }
                }
        ...
        public bool HasMenu( Type type )
        {
            return ( FindMenu( type ) != null );
        }

        public StringQueries.StringQuery FindStringQuery( Type type )
        {
            NetState ns = m_NetState;

            if( ns != null )
            {
                foreach( StringQueries.StringQuery stringQuery in ns.StringQueries )
                {
                    if( type.IsAssignableFrom( stringQuery.GetType() ) )
                    {
                        return stringQuery;
                    }
                }
        ...
        public bool HasStringQuery( Type type )
        {
            return ( FindStringQuery( type ) != null );
        }

        public bool SendStringQuery( StringQueries.StringQuery sq )
        {
            return SendStringQuery( sq, false );
        }

        public bool SendStringQuery( StringQueries.StringQuery sq, bool throwOnOffline )
        {
            if( m_NetState != null )
            {
		...
		public virtual bool CanAreaHarmful( Mobile target )
		{
			return true;
		}
		...
		/*
		[CommandProperty( AccessLevel.Counselor, AccessLevel.GameMaster )]
		public bool NoNextManaGain//edit By Arlas
		{
			get
			{
				return m_NoNextManaGain;
			}
			set
			{
				m_NoNextManaGain = value;
			}
		}
		*/
		/// <summary>
        ...
        #region Mod by Magius(CHE)
        /// <summary>
        /// Specify is this is a virtual Player with socket emulation.
        /// </summary>
        protected bool IsSocketEmulated = false;
        #endregion
		...
		public NetState NetState
		{
			...
			set
			{
				if( m_NetState != value )
				{
					...
					if( m_NetState == null && !IsSocketEmulated ) //Mod by Magius(CHE) RPV Recorder
					{
						OnDisconnected();
		...
		#region mod by Magius(CHE): inverse cansee
		private bool CanSeePreventRecursion = false;
		#endregion
		...
		public virtual bool CanSee( Item item )
		{
			...
			#region mod by Magius(CHE): inverse cansee
			if (!CanSeePreventRecursion)
			{
				CanSeePreventRecursion = true;
				if ( ! item.CanBeSeenBy( this ) )
					return false;
				CanSeePreventRecursion = false;
			}
			#endregion
			...
			if( item.Parent != null )
			{
				...
				#region Instance Loot by Dies Irae
		...
		public virtual void OnPoisoned( Mobile from, Poison poison, Poison oldPoison )
		{
			if( poison != null )
			{
				...
				{
					this.LocalOverheadMessage( MessageType.Regular, 0x3F, 1042857 + (poison.RealLevel * 2) );
					this.NonlocalOverheadMessage( MessageType.Regular, 0x3F, 1042858 + (poison.RealLevel * 2), Name );
				}
				else if ( poison.Level >= 34 )//Blocco
				{
					this.LocalOverheadMessage( MessageType.Regular, 0x429, 1042857 + (poison.RealLevel * 2) );
					this.NonlocalOverheadMessage( MessageType.Regular, 0x429, 1042858 + (poison.RealLevel * 2), Name );
				}
				else if ( poison.Level >= 29 )//Paralisi
				{
					this.LocalOverheadMessage( MessageType.Regular, 0x35, 1042857 + (poison.RealLevel * 2) );
					this.NonlocalOverheadMessage( MessageType.Regular, 0x35, 1042858 + (poison.RealLevel * 2), Name );
				}
				...
				{
					this.LocalOverheadMessage( MessageType.Regular, 0x3, 1042857 + (poison.RealLevel * 2) );
					this.NonlocalOverheadMessage( MessageType.Regular, 0x3, 1042858 + (poison.RealLevel * 2), Name );
				}
				else
				{
        ...
        #region mod by Dies Irae
        private Container m_Waistpack;

        [CommandProperty( AccessLevel.GameMaster )]
        public Container Waistpack
        {
            get
            {
                if( m_Waistpack != null && !m_Waistpack.Deleted && m_Waistpack.Parent == this )
                    return m_Waistpack;

                return ( m_Waistpack = ( FindItemOnLayer( Layer.Waist ) as Container ) );
            }
        }
        #endregion
		...
		{
			if( item == null || item.Deleted ) // mod by Dies Irae
				return false;
		...
		public int ShortTermMurders
		{
			...
			set
			{
                int oldValue = m_ShortTermMurders; // mod by Dies Irae
				...
				if( m_ShortTermMurders != value )
				{
                    ...
                    OnShortTermMurdersChange( oldValue ); // mod by Dies Irae
				}
			}
		}

		public virtual void OnShortTermMurdersChange( int oldValue )
		{
		}
			...
			if( m_Map != null )
			{
				...
				foreach( NetState state in eable )
				{
					if( state.Mobile.CanSee( this ) && (noLineOfSight || state.Mobile.InLOS( this )) )
					{
						if( p == null )
						{
							if( ascii || m_ForceAscii ) // mod by Dies Irae
		...
		public void PublicOverheadMessage( MessageType type, int hue, int number, string args, bool noLineOfSight )
		{
			if( m_Map != null )
			{
				...
				foreach( NetState state in eable )
				{
					...
					{
						if( p == null )
							p = Packet.Acquire( new MessageLocalized( m_Serial, Body, type, hue, 3, number, Name, args, Language ) );
		...
		public void PublicOverheadMessage( MessageType type, int hue, int number, AffixType affixType, string affix, string args, bool noLineOfSight )
		{
			if( m_Map != null )
			{
				...
				foreach( NetState state in eable )
				{
					...
					{
						if( p == null )
							p = Packet.Acquire( new MessageLocalizedAffix( m_Serial, Body, type, hue, 3, number, Name, affixType, affix, args, Language ) );
		...
		public void PrivateOverheadMessage( MessageType type, int hue, int number, string args, NetState state )
		{
			...
			state.Send( new MessageLocalized( m_Serial, Body, type, hue, 3, number, Name, args, Language ) );
		}
		...
		public void LocalOverheadMessage( MessageType type, int hue, int number, string args )
		{
			...
			if( ns != null )
				ns.Send( new MessageLocalized( m_Serial, Body, type, hue, 3, number, Name, args, Language ) );
		}
		...
		public void SendLocalizedMessage( int number )
		{
			...
			if( ns != null )
				//ns.Send( MessageLocalized.InstantiateGeneric( number, ns.Language ) );
				ns.Send( new MessageLocalized( Serial.MinusOne, -1, MessageType.Regular, 0x3B2, 3, number, "System", "", Language ) );
		}
		...
		public void SendLocalizedMessage( int number, string args, int hue )
		{
			if( hue == 0x3B2 && (args == null || args.Length == 0) )
			{
				...
				if( ns != null )
					ns.Send( MessageLocalized.InstantiateGeneric( number, Language ) );
			}
			else
			{
				NetState ns = m_NetState;
				...
				if( ns != null )
					ns.Send( new MessageLocalized( Serial.MinusOne, -1, MessageType.Regular, hue, 3, number, "System", args, Language ) );
			}
		}
		...
		public void SendLocalizedMessage( int number, bool append, string affix, string args, int hue )
		{
			...
			if( ns != null )
				ns.Send( new MessageLocalizedAffix( Serial.MinusOne, -1, MessageType.Regular, hue, 3, number, "System", (append ? AffixType.Append : AffixType.Prepend) | AffixType.System, affix, args, Language ) );
		}
        ...
        #region md by Dies Irae
        public virtual bool PlayerDebug { get { return false; } }

 		[CommandProperty( AccessLevel.GameMaster )]
		public virtual LanguageType TrueLanguage
 		{
            get { return TextHelper.SystemLanguage; }
            set { }
 		}

        public void SendLangMessage( int index, params object[] args )
        {
            TextHelper.SendLangMessage( this, TrueLanguage, index, args );
        }

        public void SendLangMessage( int index )
        {
            TextHelper.SendLangMessage( this, TrueLanguage, index );
        }

        public void SendLangMessage( int index, MessageHues hue, object[] args )
        {
            TextHelper.SendLangMessage( this, TrueLanguage, hue, index, args );
        }

        public void SendLangMessage( int index, MessageHues hue )
        {
            TextHelper.SendLangMessage( this, TrueLanguage, hue, index );
        }

        public void LocalLangOverheadMessageTo( int index, params object[] args )
        {
            TextHelper.LocalLangOverheadMessageTo( this, TrueLanguage, index, args );
        }

        public void LocalLangOverheadMessageTo( int index )
        {
            TextHelper.LocalLangOverheadMessageTo( this, TrueLanguage, index );
        }

	    public void SendAdvMessage( string formatEnu, string formatIta, params object[] args )
        {
            SendAdvMessage( 0x3B2, String.Format( formatEnu, args ), String.Format( formatIta, args ) );
        }

        public void SendAdvMessage( int hue, string textEnu, string textIta )
        {
            NetState ns = m_NetState;

            if( ns != null )
            {
                ns.Send( new AsciiMessage( Serial.MinusOne, -1, MessageType.Regular, hue, 3, "System", Language == "ITA" ? textIta : textEnu ) );
            }
        }

        public void SendAdvMessage( string textEnu, string textIta )
        {
            SendAdvMessage( 0x3B2, textEnu, textIta );
        }
        #endregion
		...
		#region InRange
		public bool OutRange( Point2D p, int range )
		{
		...
		public bool OutRange( Point3D p, int range )
		{
			return (p.m_X <= (m_Location.m_X - range))
				|| (p.m_X >= (m_Location.m_X + range))
				|| (p.m_Y <= (m_Location.m_Y - range))
		...
		public bool OutRange( IPoint2D p, int range )
		{
        ...
        #region mod by Dies Irae
	    private static bool m_ForceAscii = true;

        public static bool ForceAscii { get { return m_ForceAscii; } set { m_ForceAscii = value; } }
        #endregion
		...
		public virtual void OnSingleClick( Mobile from )
		{
		    ...
		    string trueTitle = GetTitle( from ); // mod by Dies Irae

            if( ClickTitle && /* Title != null && Title.Length > 0*/ !string.IsNullOrEmpty( trueTitle ) )
                suffix = trueTitle /* Title */;
			...
			if( !Core.AOS )
			{
				if( ShowFrozenStatus && ( Frozen || Paralyzed || ( m_Spell != null && m_Spell.IsCasting ) ) )
					val += (Language == "ITA" ? " (paralizzato)" : " (frozen)");
				if( ShowInvulStatus && Blessed && ( !Player || AccessLevel == AccessLevel.Player ) )
					val += (Language == "ITA" ? " (invulnerabile)" : " (invulnerable)");
			}
			#endregion
	    ...
	    #region mod by Dies Irae
        /// <summary>
        /// Enable or disable "[tag]" status when this mobile is single clicked.
        /// </summary>
        ...
        /// <summary>
        /// Enable or disable "(invulnerable)" status when this mobile is single clicked.
        /// </summary>
        public virtual bool ShowInvulStatus
        {
            get { return true; }
        }

        /// <summary>
        /// Enable or disable "(frozen)" status when this mobile is single clicked.
        /// </summary>
        public virtual bool ShowFrozenStatus
        {
            get { return true; }
        }

	    private string m_TitleIta;

	    [CommandProperty( AccessLevel.GameMaster )]
	    public string TitleIta
	    {
	        get { return m_TitleIta; }
	        set
	        {
	            m_TitleIta = value;
	            InvalidateProperties();
	    ...
	    public virtual string GetTitle( Mobile from )
	    {
            if( m_TitleIta == null )
                return m_Title;

	        return from.TrueLanguage == LanguageType.Ita ? m_TitleIta : m_Title;
	    }
	    #endregion
		...
		public virtual Item ChestArmor
		{
			get
			{
