...
namespace Server
{
	public class ScriptCompiler
	{
		...
		public static string[] GetReferenceAssemblies()
		{
			...
			//var defframeworkpath = Path.GetDirectoryName( new Object().GetType().Assembly.Location );
			...
			if( File.Exists( path ) )
			{
				using( StreamReader ip = new StreamReader( path ) )
				{
					...
					while( (line = ip.ReadLine()) != null )
					{
						...
						{
							if ( File.Exists(Path.Combine( Core.BaseDirectory,line ) ) )
								list.Add( Path.Combine( Core.BaseDirectory,line) ); //load assembly from exe path
							else
								list.Add( line ); // assume load assembly from GAC
		...
		public static string GetDefines()
		{
            ...
            #region Magius(CHE) Do not Compile Scripts at Runtime
            foreach( string defs in Core.AdditionalDefines )
                AppendDefine( ref sb, "/d:" + defs );
            #endregion
        ...
        {
            #region Magius(CHE) Display definitions compile
            Console.Write( "Scripts: Compiled with ( " );
            Console.Write( GetDefines().Replace( "/d:", "" ) );
            Console.WriteLine( " )" );
            #endregion
			
            #region Magius(CHE) Do not Compile Scripts at Runtime
            var iffirst = true;
            foreach( var asse in Directory.GetFiles( "Scripts/Output/", "*.dll" ) )
            {
                if( Path.GetFileName( asse ).StartsWith( "[Debug]" ) )
                {
                    if( iffirst )
                    {
                        Console.WriteLine( "Scripts: Loading \"IDE\" assemblies (debug)..." );
                        iffirst = false;
                    }
                    Console.WriteLine( " - " + asse );
                    var toload = Path.GetFullPath( asse );
                    if( !m_AdditionalReferences.Contains( toload ) )
                    {
                        var asm = Assembly.LoadFrom(toload);
                        m_MultipleScriptsLoaded.Add( asm );
                        m_AdditionalReferences.Add( Path.GetFullPath( asse ) );
                    }
                }
            }

            /*foreach (var dfile in Directory.GetFiles("Scripts/Output/", "*.ToDebug"))
                m_DebuggedProject.Add(Path.GetFileNameWithoutExtension(dfile));*/
            ...
            if( Core.SkipCompile )
            {
                if( File.Exists( "Scripts/Output/Scripts.CS.dll" ) )
                {
                    try
                    {
                        ...
                        Console.WriteLine( "Scripts cached and skipped!" );
						
						/*foreach(var file in Directory.GetFiles("Scripts/Output/","*.dll"))
						{
							if (!Path.GetFileName(file).Equals("Scripts.CS.dll", StringComparison.InvariantCultureIgnoreCase))
							{
								Console.WriteLine( " - Scripts dll: " + Path.GetFileName(file) + " loaded." );
								var asm =  Assembly.LoadFrom( file );
								m_AdditionalReferences.Add( file );	
								m_MultipleScriptsLoaded.Add( asm );
							}
						}*/
            ...
            #region CompilationTimeCheck - Magius(CHE)
            ...
            if( files.Length == 0 && !Core.SkipCompile ) // mod by Dies Irae
			{
...
#endif
			{
...
#if !MONO_noneedanymore_MAGIUSCHE
                ...
                {
                    watch.Stop();
                    Console.WriteLine("Scripts: Compilation completed in {0:F2} seconds.", watch.Elapsed.TotalSeconds );
                }
                #endregion
        ...
        #region MultipleScriptsLoading - Magius(CHE)
        private static List<Assembly> m_MultipleScriptsLoaded = new List<Assembly>();
        #endregion
		...
		public static bool Compile( bool debug, bool cache )
		{
            ...
            #region MultipleScriptsLoading - Magius(CHE)
            foreach( var msl in m_MultipleScriptsLoaded )
                assemblies.Add( msl );
            #endregion
            ...
            #region vs debugging mod by Magius(CHE)
            if( Core.ExitAfterCompile )
                return true;
            #endregion

#if Framework_3_5
			Console.WriteLine("Scripts: use Framework v3.5." );
#endif
        ...
        private static string FormatDelegate( MethodInfo info )
        {
            return info == null ? "null" : String.Format( "{0}.{1}", info.DeclaringType.FullName, info.Name );
        }
	    ...
	    public static void Invoke( string method )
		{
            ...
            using( StreamWriter op = new StreamWriter( Path.Combine( "Logs", "methodInvoker-profile.log" ), true ) )
            {
        ...
        #region Mod by Magius(CHE) - Debugger Scripts
        private static List<string> m_DebuggedProject = new List<string>();
        ...
        /// <returns>false cause to break current dir scan</returns>
        public static bool GetScripts( List<string> list, string path, string filter )
        {
            #region Mod by Magius(CHE) - Debugger Scripts

            //if (m_DebuggedProject.Count > 0)			
            {
                //var toload = Directory.GetFiles(path, filter);

                List<string> proj = new List<string>();
                proj.AddRange( Directory.GetFiles( path, "*.csproj" ) );
                proj.AddRange( Directory.GetFiles( path, "*.vbproj" ) );
                if( proj.Count > 0 && Path.GetFileNameWithoutExtension( proj[ 0 ] ) != "Scripts" )
                {
                    //open project file...
                    XDocument projdoc = XDocument.Load( proj[ 0 ] );
                    string projdllname = null;
                    XElement melem = projdoc.Element( "{http://schemas.microsoft.com/developer/msbuild/2003}Project" );
                    if( melem != null )
                        foreach( XElement elem in melem.Elements( "{http://schemas.microsoft.com/developer/msbuild/2003}PropertyGroup" ) )
                        {
                            XElement saa = elem.Element( "{http://schemas.microsoft.com/developer/msbuild/2003}AssemblyName" );
                            if( saa != null )
                            {
                                projdllname = saa.Value;
                                break;
                            }
                        }

                    // if (m_DebuggedProject.Contains(Path.GetFileNameWithoutExtension(proj[0])))
                    if( !string.IsNullOrEmpty( projdllname ) )
                    {
                        //this dir is under bugging mode... so skip all files from here.
                        string dll = "Scripts/Output/" + projdllname + ".dll";
                        if( File.Exists( dll ) )
                        {
                            Console.Write( "(Debug Project: " + Path.GetFileNameWithoutExtension( proj[ 0 ] ) + "..." );
