...
namespace Server.Items
{
	public class SecureTradeContainer : Container
	{
		...
		public override bool CheckHold( Mobile m, Item item, bool message, bool checkItems, int plusItems, int plusWeight )
		{
            ...
            Mobile from = Trade.From.Container == this ? Trade.From.Mobile : Trade.To.Mobile;
            if( item is IIdentificable && !( (IIdentificable)item ).IsIdentifiedFor( from ) )
            {
                if( message )
                    from.SendMessage( "You cannot trade an unidentified item." );
                return false;
            }
            else if( item is ITasteIdentificable && !( (ITasteIdentificable)item ).IsIdentifiedFor( from ) )
            {
                if( message )
                    from.SendMessage( "You cannot trade an unidentified item." );
                return false;
		...
		public override void OnItemAdded( Item item )
        {
            #region mod by Dies Irae
            Mobile from = Trade.From.Container == this ? Trade.From.Mobile : Trade.To.Mobile;
            Mobile to = Trade.From.Container == this ? Trade.To.Mobile : Trade.From.Mobile;

            if( item is IIdentificable && ( (IIdentificable)item ).IsIdentifiedFor( from ) )
                ( (IIdentificable)item ).AddIdentifier( to );
            else if( item is ITasteIdentificable && ( (ITasteIdentificable)item ).IsIdentifiedFor( from ) )
                ( (ITasteIdentificable)item ).AddIdentifier( to );
            #endregion
