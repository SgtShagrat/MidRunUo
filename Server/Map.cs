...
namespace Server
{
	...
	public sealed class Map : IComparable, IComparable<Map>
	{
		...
		public static Map Parse( string value )
		{
            ...
            Console.Error.WriteLine("### ERROR: Invalid map name: " + value);
		    Console.Error.WriteLine("" + new System.Diagnostics.StackTrace());
            throw new Exception("Invalid map name: " + value);
		}
		...
		private class SectorEnumerator : IPooledEnumerator, IDisposable
		{
			...
			public bool MoveNext()
			{
				while ( true )
				{
					...
					if ( m_CurrentIndex >= m_CurrentList.Count ) // mod by Dies Irae : possibile crash by Smjert
					{
						++m_ySector;
