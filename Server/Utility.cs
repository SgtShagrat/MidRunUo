...
#pragma warning restore 618
        ...
        public static int Dice( string str )
        {
            ...
            numDice = ToInt32( str.Substring( start, index - start ) );
            ...
            numSides = ToInt32( str.Substring( start, index - start ) );
            ...
            bonus = ToInt32( str.Substring( start, index - start ) );

            if( negative )
                bonus *= -1;

            return Dice( numDice, numSides, bonus );
        }
		...
		public static int RandomList( params int[] list )
		{
			return list[m_Random.Next( list.Length )];
        		...
        		public static string RandomStringList( params string[] list )
		{
			return list[m_Random.Next( list.Length )];
		}
        ...
        private static string DebugLogsFolder = "DebugLogs";

        public static void Log( string file, string toLog )
        {
            Log( file, toLog, null );
        }
        ...
        {
            ScriptCompiler.EnsureDirectory( DebugLogsFolder );

            try
            {
                using( StreamWriter op = new StreamWriter( Path.Combine( DebugLogsFolder, file ), true ) )
                {
                    op.WriteLine( string.Format( format, args ) );
                }
            }
            catch
            {
