...
namespace Server
{
	...
	public struct Body
	{
		...
		private static Byte[] m_Dimensions;
		...
		static Body()
		{
			if ( File.Exists( "Data/bodyTable.cfg" ) )
			{
				...
				{
					m_Types = new BodyType[2048];//edit by arlas
			...
			if ( File.Exists( "Data/bodyDimensions.cfg" ) )
			{
				using ( StreamReader ip = new StreamReader( "Data/bodyDimensions.cfg" ) )
				{
					m_Dimensions = new Byte[2048];//edit by arlas
					...
					while ( (line = ip.ReadLine()) != null )
					{
						...
						{
							int bodyID = int.Parse( split[0] );
							byte dimension = byte.Parse( split[1] );

							if ( bodyID >= 0 && bodyID < m_Dimensions.Length )
								m_Dimensions[bodyID] = dimension;
						}
						catch
						{
							Console.WriteLine( "Warning: Invalid bodyDimension entry:" );
							Console.WriteLine( line );
						}
					}
				}
			...
			{
				Console.WriteLine( "Warning: Data/bodyDimension.cfg does not exist" );

				m_Dimensions = new Byte[0];
			}
		}
		...
		public int Dimension
		{
			get
			{
				if ( m_BodyID >= 0 && m_BodyID < m_Dimensions.Length )
					return (int)m_Dimensions[m_BodyID];
				else
					return (3);
			}
		}
