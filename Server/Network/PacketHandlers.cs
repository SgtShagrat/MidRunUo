...
namespace Server.Network
{
	...
	public static class PacketHandlers
	{
		...
		static PacketHandlers()
		{
            ...
            PacketHandlers.Register( 0xAC, 0, true, new OnPacketReceive( StringQueryResponse ) ); // mod by Dies Irae
		}
	    ...
	    public static void StringQueryResponse( NetState state, PacketReader pvSrc )
	    {
	        ...
	        int menuID = pvSrc.ReadUInt16(); ///  unknown
	        ...
	        foreach( StringQuery stringQuery in state.StringQueries )
	        {
	            if( stringQuery.Serial == serial )
	                state.RemoveStringQuery( stringQuery );
		...
		public static void Language( NetState state, PacketReader pvSrc )
		{
			...
			if ( state.Mobile != null && state.Mobile.Language == null)
