...
namespace Server.Network
{
	...
	public sealed class MessageLocalized : Packet
	{
		...
		{
			return InstantiateGeneric( number, "ITA" );
		}

		public static MessageLocalized InstantiateGeneric( int number, string language )
		{
			...
			if ( cache != null && index >= 0 && index < cache.Length )
			{
				...
				if ( p == null )
				{
					cache[index] = p = new MessageLocalized( Serial.MinusOne, -1, MessageType.Regular, 0x3B2, 3, number, "System", "", language );
					p.SetStatic();
				}
			}
			else
			{
				p = new MessageLocalized( Serial.MinusOne, -1, MessageType.Regular, 0x3B2, 3, number, "System", "", language );
			}

			return p;
		}

		public MessageLocalized( Serial serial, int graphic, MessageType type, int hue, int font, int number, string name, string args ) : this( serial, graphic, type, hue, font, number, name, args, "ITA" )
		{
		}

		public MessageLocalized( Serial serial, int graphic, MessageType type, int hue, int font, int number, string name, string args, string language ) : base( 0xC1 )
		{
			...
			try
			{
                ...
                string text = language == "ITA" ? StringList.LocalizationIta.SplitFormat( number, args ) : StringList.Localization.SplitFormat( number, args );
    ...
    public sealed class CustomScrollMessage : Packet
    {
        public CustomScrollMessage( string text )
            : base( 0xA6 )
        {
            if( text == null ) text = "";
            ...
            m_Stream.Write( (ushort)0xA908 );
            m_Stream.Write( (ushort)0x6013 );
            m_Stream.Write( (ushort)text.Length );
            m_Stream.WriteAsciiFixed( text, text.Length );
        }
    } 
	...
	public sealed class MobileStatusExtended : Packet
	{
		...
		public MobileStatusExtended( Mobile m, NetState ns ) : base( 0x11 )
		{
            ...
            m_Stream.Write( (byte) m.Hunger );
			m_Stream.Write( (byte) m.Thirst );
            #endregion
	...
	public sealed class MobileStatus : Packet
	{
		...
		public MobileStatus( Mobile beholder, Mobile beheld, NetState ns ) : base( 0x11 )
		{
			...
			if ( beholder == beheld )
			{
                ...
                m_Stream.Write( (byte)beheld.Hunger );
                m_Stream.Write( (byte)beheld.Thirst );
                #endregion
    ...
    public sealed class MessageLocalizedAffix : Packet
    {
        public MessageLocalizedAffix( Serial serial, int graphic, MessageType messageType, int hue, int font, int number, string name, AffixType affixType, string affix, string args )
            : this( serial, graphic, messageType, hue, font, number, name, affixType, affix, args, "ITA" )
        {
        }

        public MessageLocalizedAffix( Serial serial, int graphic, MessageType messageType, int hue, int font, int number, string name, AffixType affixType, string affix, string args, string language )
            : base( 0xCC )
        {
            #region modifica by Dies Irae
            string text = language == "ITA" ? StringList.LocalizationIta[ number ] : StringList.Localization[ number ];
            ...
            if( text != null )
            {
                ...
                Packet p = new AsciiMessage( serial, graphic, messageType, hue, font, name, text );
                EnsureCapacity( 3 );
                m_Stream = p.UnderlyingStream;
                return;
            }
	...
	public abstract class Packet
	{
			...
			if ( m_CompiledBuffer == null )
			{
				...
				else
				{
					if ( (m_State & State.Warned) == 0 )
					{
						...
						try
						{
							using ( StreamWriter op = new StreamWriter( "Logs/net_opt.log", true ) )
							{
		...
		private void InternalCompile( bool compress )
		{
            ...
            PreCompressedBuffer = null; // mod by Magius

			if ( compress )
			{
				#region mod by Magius
                PreCompressedBuffer = new byte[length];
                Buffer.BlockCopy(m_CompiledBuffer, 0, PreCompressedBuffer, 0, PreCompressedBuffer.Length);
				#endregion
				...
				if ( m_CompiledBuffer == null )
				{
					...
					using ( StreamWriter op = new StreamWriter( "Logs/compression_overflow.log", true ) )
					{
        ...
        private byte[] PreCompressedBuffer{get;set;}

        public byte[] RawUncompressedBuffer(out int length)
        {
            var InternalCompiledBuffer = m_CompiledBuffer;
            var InternalCompiledBuffer_Len = m_CompiledLength;
            var InternalRawLen = m_Length;
            var compressed = PreCompressedBuffer != null;
            var compiled = compressed || InternalCompiledBuffer != null;

            var buff = InternalCompiledBuffer;
            var len = InternalCompiledBuffer_Len;

            if (!compiled)
            {
                // Packet not Compiled! Compile it.

                var isDynaLen = (InternalRawLen == 0);

                buff = new byte[UnderlyingStream.Length];
                len = buff.Length;
                Array.Copy(UnderlyingStream.UnderlyingStream.GetBuffer(), buff, buff.Length);

                if (isDynaLen)
                {
                    // Packet is dynamic Write len {0} into 1L.
                    buff[1] = (byte) (((ushort) len) >> 8);
                    buff[2] = (byte) ((ushort) len);
                }
                else
                {
                    //Packet is static.
            ...
            else
            {
                ...
                if (compressed)
                {
                    //Packet seems to be compressed. Uncompress it
                    buff = PreCompressedBuffer;
                    len = PreCompressedBuffer.Length;
                }
            }
            length = len;
            return buff;
