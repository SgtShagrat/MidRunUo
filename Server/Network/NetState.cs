...
namespace Server.Network {
	...
	public class NetState {
        ...
        #region mod by Dies Irae : stringQuery support
        private static int m_StringQueryCap = 512;

        private List<StringQuery> m_StringQueries;


        public static int StringQueryCap
        {
            get { return m_StringQueryCap; }
            set { m_StringQueryCap = value; }
        }

        public IEnumerable<StringQuery> StringQueries
        {
            get
            {
                return m_StringQueries;
            }
        }
        ...
        public void AddStringQuery( StringQuery sq )
        {
            ...
            {
                WriteConsole( "Exceeded string query cap, disconnecting..." );
                Dispose();
            }
            else
            {
        ...
        public void RemoveStringQueryAt( int index )
        {
            if( m_StringQueries == null )
                return;
        ...
        public void RemoveStringQuery( StringQuery stringQuery )
        {
            if( m_StringQueries == null )
                return;

            m_StringQueries.Remove( stringQuery );
        }

        public void ClearStringQueries()
        {
            if( m_StringQueries != null )
            {
                m_StringQueries.Clear();
            }
        }
        #endregion
		...
		public NetState( Socket socket, MessagePump messagePump )
		{
            ...
            m_StringQueries = new  List<StringQuery>(); // mod by Dies Irae
            ...
            {
                //atipical netstate...
                m_Address = new IPAddress( new byte[] { 0, 0, 0, 0 } );
                m_ToString = "(Internal Socket)";
            }
			#endregion
            else
            {
		...
		public virtual void Send( Packet p ) {
			...
			if ( buffer != null ) {
				...
				using ( StreamWriter op = new StreamWriter( "Logs/null_send.log", true ) )
				{
		...
		public virtual bool Flush() { // mod by Magius ( made virtual )
		...
		public virtual bool CheckAlive() { // mod by Dies Irae : made virtual
			if ( m_Socket == null )
				return false;
		...
		private void OnReceive( IAsyncResult asyncResult ) {
			...
			try {
				...
				if ( byteCount > 0 )
                {
                    #region SocketPatcketInterceptor System - Magius(CHE)
                    var remainbytes = new byte[Math.Max(byteCount - 1,0)];

                    if (remainbytes.Length > 0)
                        Array.Copy(m_RecvBuffer, 1, remainbytes, 0, remainbytes.Length);

                    var args = new ClientPacketInterceptedEventArgs(Socket, m_RecvBuffer[0], remainbytes);
                    EventSink.InvokeClientPacketIntercepted(args);

                    if (args.Intercepted)
                    {
                        Intercepted = true;
                        Console.WriteLine("Login: {0}: Client intercepted. Removing from server. ", this);
                        Dispose(false);
                        return;
                    }
                    #endregion
		...
		public static void TraceException( Exception ex ) {
			try {
				using ( StreamWriter op = new StreamWriter( "Logs/network-errors.log", true ) ) {
        ...
        internal bool Intercepted { get; set; } // SocketPatcketInterceptor System - Magius(CHE)

		public virtual void Dispose( bool flush ) {
			if ( /* m_Socket == null */ m_Disposing ) { // mod by Magius
				return;
			}
            ...
            if (Intercepted)
            {
                //do not shutdown or close socket.
            }
            else
            #endregion
            {
		...
		public static void ProcessDisposedQueue() {
			...
			while ( breakout < 200 && m_Disposed.Count > 0 ) {
                ...
                ns.m_StringQueries.Clear(); // mod by Dies Irae : string queries support
				...
				if ( a != null ) {
					...
					ns.WriteConsole( "Disconnected. [{0} Online] [Mobile: {1}]", m_Instances.Count,m ); // mod by Magius
                } else {
