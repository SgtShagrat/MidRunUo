...
namespace Server
{
    ...
    public class Region : IComparable
	{
        ...
        #region mod by Dies Irae
        public static List<Region> FindAll( Point3D p, Map map )
        {
            if( map == null )
                return null;

            var list = map.GetSector( p ).RegionRects;
            var resList = new List<Region>();
            ...
            for( var i = 0; i < list.Count; ++i )
            {
                ...
                if( regRect.Contains( p ) )
                    resList.Add( regRect.Region );
            }

            return resList.Count > 0 ? resList : null;
        }
        #endregion
		...
		{
			FireRegionEvent("OnEnter",m); //mod my Magius(CHE): massive custom eventhandling
		}

		public virtual void OnExit( Mobile m )
		{
			FireRegionEvent("OnExit",m); //mod my Magius(CHE): massive custom eventhandling
		}
		
		#region mod my Magius(CHE): massive custom eventhandling
		public class OnRegionEventHandlerArgs
		{
			public readonly Region Region;
			public readonly string Event;
			public readonly object[] Arguments;
			public object Return;
			public OnRegionEventHandlerArgs(Region region, string eventname,object returns,params object[] args)
			{
				Region = region;
				Return = returns;
				Arguments = args;
		...
		public delegate void OnRegionEventHandler(OnRegionEventHandlerArgs e);
		public event OnRegionEventHandler OnRegionEvent;
		protected void FireRegionEvent(string eventname, params object[] args)
		{
			FireRegionEvent<object>(eventname,args);
		}
		protected T FireRegionEvent<T>(string eventname, params object[] args)
		{
			if (OnRegionEvent!=null)				
			{
				var evarg = new OnRegionEventHandlerArgs ( this, eventname, null, args);
				OnRegionEvent(evarg);
				return (T) evarg.Return;
			}
			return default(T);
		}
		#endregion
		...
		public virtual void MakeGuard( Mobile focus, bool isBackup )
		{
			if ( m_Parent != null )
				m_Parent.MakeGuard( focus, isBackup );
		}
		...
		internal static void Load()
		{
			...
			else
			{
                ...
                SaveModifications( doc ); // mod by Dies Irae
			}
		...
		public Region( XmlElement xml, Map map, Region parent )
		{
            ...
            #region mod by Dies Irae : la priorità va letta anke per le subregions
            // if ( parent == null )
                // ReadInt32( xml, "priority", ref m_Priority, false );

		    ReadPriority( xml, ref m_Priority, false );
            #endregion
        ...
        public virtual int GetSolidHue()
        {
            return -1;
        }

        public static bool RegionsFileHasBeenModified = false;

        private static void SaveModifications( XmlDocument document )
        {
            if( !RegionsFileHasBeenModified )
                return;

            string fileName = String.Format( "Regions_{0}.xml", GetTimeStamp() );
        ...
        public static bool ReadDouble( XmlElement xml, string attribute, ref double value )
        {
            return ReadDouble( xml, attribute, ref value, true );
        }

        public static bool ReadDouble( XmlElement xml, string attribute, ref double value, bool mandatory )
        {
            string s = GetAttribute( xml, attribute, mandatory );
            ...
            catch( Exception ex )
            {
                ...
                Console.WriteLine( "Could not parse double attribute '{0}' in element '{1}'", attribute, xml.Name );
                return false;
            }
        ...
        public static bool ReadInt32FromHex( XmlElement xml, string attribute, ref int value, bool mandatory )
        {
            string s = GetAttribute( xml, attribute, mandatory );
            ...
            try
            {
                value = Utility.ToInt32( s );
            }
            catch( Exception ex )
            {
                Console.WriteLine( s );
                Console.WriteLine( "Could not parse hexadecimal integer attribute '{0}' in element '{1}'", attribute, xml.Name );
                Console.WriteLine( ex.ToString() );
                return false;
            }
        ...
        public static bool ReadPriority( XmlElement xml, ref int value, bool mandatory )
        {
            string s = GetAttribute( xml, "priority", mandatory );
            if( s == null )
                return false;

            try
            {
                if( Enum.IsDefined( typeof( PriorityEnum ), s ) )
                    value = (int)Enum.Parse( typeof( PriorityEnum ), s, true );
                else
                    value = XmlConvert.ToInt32( s );
            }
            catch
            {
                Console.WriteLine( "Could not parse integer attribute 'priority' in element '{0}'", xml.Name );
                return false;
            }
