...
namespace Server.Gumps
{
	public class Gump
	{
        ...
        public void AddBlackAlphaRegion( int x, int y, int width, int height )
        {
            AddImageTiled( x + 10, y + 10, width - 20, height - 20, 2624 );
            AddAlphaRegion( x + 10, y + 10, width - 20, height - 20 );
        }

        public void AddBlackAlphaRegion( int width, int height )
        {
            AddImageTiled( 10, 10, width - 20,  height - 20, 2624 );
            AddAlphaRegion( 10, 10, width - 20,  height - 20 );
        }

        public void AddBlackAlphaRegionRect( int width, int height )
        {
            AddImageTiled( 5, 10, width - 10,  height - 20, 2624 );
            AddAlphaRegion( 5, 10, width - 10,  height - 20 );
        }

        public void AddOldHtmlHued( int x, int y, int width, int height, string text, int hue )
        {
            Add( new GumpHtml( x, y, width, height, HtmlColor( String.Format( "<BIG>{0}", text ), hue ), false, false ) );
        }

        public void AddOldScrollableHtmlHued( int x, int y, int width, int height, string text, int hue )
        {
            Add( new GumpHtml( x, y, width, height, HtmlColor( String.Format( "<BIG>{0}", text ), hue ), false, true ) );
        }

        public void AddOldHtml( int x, int y, int width, int height, string text )
        {
            Add( new GumpHtml( x, y, width, height, String.Format( "<BIG>{0}", text ), false, false ) );
        }

        public void AddFirstCharHuedHtml( int x, int y, int width, int height, string text, int hue, bool ascii )
        {
            AddHtml( x, y, width, height, String.Format( ascii ? "<BIG>{0}" : "{0}", text ), false, false );
            AddHtml( x, y, 20, height, HtmlColor( String.Format( ascii ? "<BIG>{0}" : "{0}", text[ 0 ] ), hue ), false, false );
        }

        public string HtmlColor( string text, int color )
        {
            return String.Format( "<BASEFONT COLOR=#{0:X6}>{1}</BASEFONT>", color, text );
        }

        public string HtmlCenter( string text )
        {
            return String.Format( "<CENTER>{0}</CENTER>", text );
        }

        public string HtmlAscii( string text )
        {
            return String.Format( "<BIG>{0}", text );
        }

        public string HtmlRight( string text )
        {
            return String.Format( "<DIV ALIGN=RIGHT>{0}</DIV>", text );
        }

        public string HtmlLeft( string text )
        {
		...
		public virtual void SendTo( NetState state )
		{
			state.AddGump( this );
