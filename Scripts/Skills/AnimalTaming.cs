			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( targeted is Mobile )
				{
					if ( targeted is BaseCreature )
					{
						...
						else if( creature.RequiredRace != null && from.Race != creature.RequiredRace )
						{
							from.SendMessage( from.Language == "ITA" ? "Solo {0} può addomesticare questa creatura!" : "Only {0} can tame this creature!", creature.RequiredRace.PluralName );
						}
						#endregion
						...
						else if ( CheckMastery( from, creature ) || from.Skills[SkillName.AnimalTaming].Value >= creature.MinTameSkill )
						{
							...
							else if ( creature.CanAngerOnTame && 0.95 - bonus >= Utility.RandomDouble() )
							{
							...
							else
							{
								...
								{
									from.LocalOverheadMessage( MessageType.Emote, 0x59, true, string.Format( "You start to tame {0}.", creature.Name ?? "the creature" ) );
									from.NonlocalOverheadMessage( MessageType.Emote, 0x59, true, string.Format( "*begins taming {0}*", creature.Name ?? "a creature" ) );
								}
			...
			private class InternalTimer : Timer
			{
				...
				protected override void OnTick()
				{
					...
					else
					{
						...
						int bonus = 0;
						if (ClassSystem.IsDruid( m_Tamer ))
							bonus += 5;
						if (m_Tamer.Skills[ SkillName.AnimalTaming ].Value >= 100 )
							bonus += 5;

						if ( CheckMastery( m_Tamer, m_Creature ) || alreadyOwned || m_Tamer.CheckTargetSkill( SkillName.AnimalTaming, m_Creature, minSkill - 25.0, minSkill + 25.0 - bonus ) )
						{
							...
							if( m_Creature.Criminal )
								m_Creature.Criminal = false;
							if ( m_Tamer.Combatant == m_Creature )
								m_Tamer.Combatant = null;

							NotorietyHandlers.ClearAggressors( m_Creature );
							NotorietyHandlers.ClearAggressed( m_Creature );
							#endregion
