...
namespace Server.SkillHandlers
{
	public class AnimalLore
	{
				...
				else if ( targeted is BaseCreature )
				{
					...
					if ( !c.IsDeadPet )
					{
						if ( c.Body.IsAnimal || c.Body.IsMonster || c.Body.IsSea )
						{
							...
							else if( Utility.Random( 101 ) >= from.Skills[SkillName.AnimalLore].Value )
							{
								from.CheckSkill( SkillName.AnimalLore, 0.95 );
								from.SendLocalizedMessage( 500334 ); // You can't think of anything you know offhand.
							}
							else if( !Core.AOS )
							{
								from.CheckSkill( SkillName.AnimalLore, 0.01 );
								OldSkillHandler( from, c );
							}
							else
							{
		...
		private static void OldSkillHandler( Mobile from, BaseCreature c )
		{
			if( c.Controlled && c.ControlMaster == from )
				c.PrivateOverheadMessage( MessageType.Regular, 0x3B2, Gethappy( c ), from.NetState );

			from.SendMessage( from.Language == "ITA" ? "E' {0}." : "This creature is of {0} gender.", c.Female ? (from.Language == "ITA" ? "una femmina" : "female") : (from.Language == "ITA" ? "un maschio" : "male"));

			from.SendMessage( from.Language == "ITA" ? "Ha {0} giorni." : "It is {0} days old.", Math.Floor(c.Age).ToString());

			from.SendLocalizedMessage( FindFood( c ) );

			if( c.Meat > 0 )
				from.SendLocalizedMessage( 1042909 ); //You could slaughter this creature for meat.
			if( c.Hides > 0 )
				from.SendLocalizedMessage( 1042910 ); //If this creature were dead you could use its hides for leather.
			if( c.Wool > 0 )
				from.SendLocalizedMessage( 1042912 ); //You could use this creature for its wool.
			if( c.Feathers > 0 )
				from.SendLocalizedMessage( 1048138 ); //This creature is sometimes used for it's feathers.
			if( c is PackHorse || c is PackLlama )
				from.SendLocalizedMessage( 1042911 ); //This creature does well at carrying heavy loads.
			else if( c.Meat + c.Hides + c.Wool + c.Feathers == 0 )
				from.SendLocalizedMessage( 1042915 ); //Although you know this creature, you can't seem to think of any uses for it.

			// from.SendLocalizedMessage( Hunger( c ) );

			from.SendLocalizedMessage( GetStr( c ) );

			if( c.IsPregnant )
			{
				if( c.DeliveryTimeInUoDays > 0 )
					from.SendMessage( from.Language == "ITA" ? "Lei è incinta. Il bambino nascerà tra {0} giorni circa." : "This female is pregnant. The baby will be born in about {0} days.", c.DeliveryTimeInUoDays );
				else
					from.SendMessage( from.Language == "ITA" ? "Lei è incinta. Il bambino sta per nascere! Sbrigati!" : "This female is pregnant. The baby will be born soonly! Hurry up!" );
			}
		}

		public static int FindFood( BaseCreature c )
		{
			if( c == null )
				return 3000340; // None

			if( ( c.FavoriteFood & FoodType.FruitsAndVegies ) != 0 )
				return 1042904; // You sense that it would delight in fruit for a meal.
			else if( ( c.FavoriteFood & FoodType.GrainsAndHay ) != 0 )
				return 1042903; // You sense that it likes to eat grass.
			else if( ( c.FavoriteFood & FoodType.Gold ) != 0 )
				return 1042902; // Strangely, you cannot recall what this creature might eat.
			else if( ( c.FavoriteFood & FoodType.Fish ) != 0 )
				return 1042908; // This creature will eat fish.
			else if( ( c.FavoriteFood & FoodType.Meat ) != 0 )
				return 1042907; // This creature devours meat for its meals.
			else
				return 1042902; // Strangely, you cannot recall what this creature might eat.
		}

		public static int Gethappy( BaseCreature c )
		{
			return ( c == null || !c.Controlled || c.Loyalty == 0 ) ? 1061643 : 1043271 + ( c.Loyalty / 10 );
		}

		public static int GetStr( BaseCreature c )
		{
			int phys = c.Str / 20;
			if( phys > 10 )
				phys = 10;
			...
			return 1038045 + ( phys * 11 ) + agile;
		}

		public static int Hunger( BaseCreature c )
		{
			int hunger = c.Hunger;

			if( hunger == 0 )
				return 1042916; // It looks like it is starving.
			else if( hunger < 3 )
				return 1042917; // It looks like it's near starving.
			else if( hunger < 5 )
				return 1042918; // It looks pretty hungry.
			else if( hunger < 7 )
				return 1042919; // It looks hungry.
			else if( hunger < 10 )
				return 1042920; // It looks moderately well fed.
			else if( hunger < 12 )
				return 1042921; // It looks well fed.
			else if( hunger < 17 )
				return 1042922; // It looks plump.
			else if( hunger < 20 )
				return 1042923; // It looks like it gets plenty to eat.
			else
				return 1042924; // It looks replete, as if it has just eaten.
		}
		#endregion
	}
