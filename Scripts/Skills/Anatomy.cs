	...
	public class Anatomy
	{
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if( targeted == from )
				{
					from.SendLangMessage( FoodDecayTimer.GetHungerMessage( from ) );
				}
				#endregion
				else if ( targeted is Mobile )
				{
					...
					if( Utility.Random( 101 ) < from.Skills[SkillName.Anatomy].Value )
					{
						from.CheckSkill( SkillName.Anatomy, 0.01 );
					...
					{
						from.CheckSkill( SkillName.Anatomy, 0.95 );
