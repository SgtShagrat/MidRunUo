...
namespace Server.SkillHandlers
{
	public class Stealth
	{
		...
		public static TimeSpan OnUse( Mobile m )
		{
			...
			{
				m.LocalOverheadMessage( Network.MessageType.Regular, 0x22, 501237 ); // You can't seem to hide right now.
			...
			*/
			...
			else
			{
				...
				double minSkill = -20.0 + ( armorRating * 2 );
				double maxSkill = 80.0 + ( armorRating * 2 );

				double chance = ( ( value - minSkill ) / ( maxSkill - minSkill ) ) - malus;

				if( m.PlayerDebug )
					m.SendMessage( "Stealth debug: malus: {0:F2} - value {1:F1} - chance {2:F2}.", malus, value, chance );
				#endregion

				if( armorRating >= /* (Core.AOS ? 42 : 26) */ 42 )	//I have a hunch '42' was chosen cause someone's a fan of DNA
				{
					m.SendLocalizedMessage( 502727 ); // You could not hope to move quietly wearing this much armor.
					m.RevealingAction( true );
				}
				else if( m.CheckSkill( SkillName.Stealth, chance ) ) // -20.0 + (armorRating * 2),(Core.AOS ? 60.0 : 80.0) + (armorRating * 2) ) )
				{
