...
namespace Server.SkillHandlers
{
	...
	public class TrackWhatGump : Gump
	{
		...
		{
			m_From = from;
			m_Success = from.CheckSkill( SkillName.Tracking, 10.0, 100.0 );

            #region mod by Dies Irae: paladin system
	...
	public class TrackWhoGump : Gump
	{
		...
		public static void DisplayTo( bool success, Mobile from, int type )
		{
		    ...
		    bool isPaladine = ClassSystem.IsPaladine( from ); // mod by Dies Irae: paladin system

			TrackTypeDelegate check = m_Delegates[type];

            if( !isPaladine ) // mod by Dies Irae: paladin system
			    from.CheckSkill( SkillName.Tracking, 21.1, 100.0 ); // Passive gain

			// int range = 5 + (int)(from.Skills[SkillName.Tracking].Value / ( Core.AOS ? 10 : 4 ) ); // mod by Dies Irae

            int range = (int)( 0.0045 * Math.Pow( from.Skills[ SkillName.Tracking ].Value, 2.0 ) );
            if( range < 5 )
                range = 5;
            else if( range > 45 )
                range = 45;
			...
			foreach ( Mobile m in from.GetMobilesInRange( range ) )
			{
                ...
                // paladin system: paladine track only evil beings
                if( isPaladine && !( m.Karma < 0 || m.Kills > 0 ) )
                    continue;
                #endregion
		...
		private static bool CheckDifficulty( Mobile from, Mobile m )
		{
            ...
            if( ClassSystem.IsPaladine( from ) )
            {
                double paladinChance = Math.Max( from.Skills[ SkillName.Chivalry ].Value / 100.0, 0.5 ); // from 0.5 to 1.0

                paladinChance += ( m.Skills[ SkillName.Necromancy ].Value / 100.0 ) * 0.25; // 25% bonus to find a skilled necromancer
                paladinChance += ( from.Karma / (double)15000 ) * 0.25; // 25% bonus if paladin is karma filled
                paladinChance += RPGPaladinSpell.IsSuperVulnerable( m ) ? 0.25 : 0.0; // 25% bonus to find evil ones

                return Utility.RandomDouble() < paladinChance;
            }
            #endregion
		    ...
		    #region mod by Dies Irae
		    if( TransformationSpellHelper.UnderTransformation( m, typeof( EvilAvatarSpell ) ) )
		        divisor -= 200;
		    #endregion
	...
	public class TrackTimer : Timer
	{
		...
		public TrackTimer( Mobile from, Mobile target, int range, QuestArrow arrow ) : base( TimeSpan.FromSeconds( 0.25 ), TimeSpan.FromSeconds( 2.5 ) )
		{
		    ...
		    m_End = DateTime.Now + TimeSpan.FromSeconds( 40 ); // mod by Dies Irae
		}
		...
		protected override void OnTick()
		{
            ...
            #endregion
            else if ( m_From.NetState == null || m_From.Deleted || !m_From.Alive || m_Target.Deleted || m_From.Map != m_Target.Map || !m_From.InRange( m_Target, m_Range ) || DateTime.Now > m_End )
			{
