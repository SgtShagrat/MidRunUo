		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( targeted is Mobile )
				{
                    ...
                    if( from == targeted )
                    {
                        from.LocalOverheadMessage( MessageType.Regular, 0x3B2, 500398 ); // Perhaps just asking would work better.
                        return;
                    }
                    // if( targeted is Beggar) // TODO implement beggar class
                    #endregion
			...
			private class InternalTimer : Timer
			{
				...
				protected override void OnTick()
				{
					...
					else if( Utility.Random( 101 ) < m_From.Skills[SkillName.Begging].Value )
					{
						m_From.CheckSkill( SkillName.Begging, 0.01 );
					...
					{
						m_From.CheckSkill( SkillName.Begging, 0.95 );
