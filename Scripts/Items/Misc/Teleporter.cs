...
namespace Server.Items
{
	public class Teleporter : Item
	{
        ...
        #region mod by Dies Irae [Twin Teleporters]
        private Teleporter m_Link;
        ...
        public Teleporter Link
        {
            ...
            set
            {
                Teleporter oldValue = m_Link;
                if( oldValue != value )
                {
                    m_Link = value;

                    OnLinkChanged( oldValue );
                }
            }
        }

        private void OnLinkChanged( Teleporter oldValue )
        {
            UpdateTwins();
        }
        ...
        private void UpdateTwins()
        {
            if( m_Link != null )
            {
                m_MapDest = m_Link.Map;
                m_PointDest = m_Link.Location;

                m_Link.MapDest = Map;
                m_Link.m_PointDest = Location;

                PublicOverheadMessage( MessageType.Regular, 0x3B2, true, "* my twin sync *" );
                m_Link.PublicOverheadMessage( MessageType.Regular, 0x3B2, true, "* my twin sync *" );
            }
        }
        ...
        {
            CommandSystem.Register( "TeleLink", AccessLevel.GameMaster, new CommandEventHandler( Link_OnCommand ) );
        }

        [Usage( "TeleLink" )]
        [Description( "Links two targeted teleporters together." )]
        private static void Link_OnCommand( CommandEventArgs e )
        {
            ...
            e.Mobile.SendMessage( "Target the first teleporter to link." );
        }

        private static void Link_OnFirstTarget( Mobile from, object targeted )
        {
            Teleporter tele = targeted as Teleporter;

            if( tele == null )
            {
                from.BeginTarget( -1, false, TargetFlags.None, new TargetCallback( Link_OnFirstTarget ) );
                from.SendMessage( "That is not a teleporter. Try again." );
            }
            else
            {
                from.BeginTarget( -1, false, TargetFlags.None, new TargetStateCallback( Link_OnSecondTarget ), tele );
                from.SendMessage( "Target the second teleporter to link." );
            }
        }

        private static void Link_OnSecondTarget( Mobile from, object targeted, object state )
        {
            Teleporter first = (Teleporter)state;
            Teleporter second = targeted as Teleporter;
            ...
            else
            {
                first.Link = second;
                second.Link = first;
                from.SendMessage( "The teleporters have been linked." );
            }
        }
        #endregion
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
                case 4:
			    {
                    m_Link = reader.ReadItem() as Teleporter;
			        goto case 3;
			    }
				case 3:
				{
