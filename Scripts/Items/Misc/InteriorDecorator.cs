		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( targeted is Item && InteriorDecorator.CheckUse( m_Decorator, from ) )
				{
					...
					else if ( !house.IsLockedDown( item ) && !house.IsSecure( item ) && !isDecorableComponent && !( item is AddonComponent ) && !( item is AddonContainerComponent ) )
					{
			...
			private static int GetFloorZ( Item item )
			{
			    ...
			    #region mod by Dies Irae : townhouses
			    if ( z == int.MinValue )
