...
using Corex = Midgard.Engines.Races.Core;
...
namespace Server.Items
{
	...
	public enum CorpseFlag
	{
        ...
        /// <summary>
        /// This corpse is of one hireable?
        /// </summary>
        IsHireable              = 0x00000040, // mod by Dies Irae
	}
	...
	public class Corpse : Container, ICarvable
	{
		...
		private int			m_LeechableHp;		// hp che il necro può prendere
		...
		public string CorpseName
		{
			...
			set { m_CorpseName = value; InvalidateProperties(); }
		}
		
		[CommandProperty( AccessLevel.GameMaster )]
		public bool NoBones
		{
			get { return GetFlag( CorpseFlag.NoBones ); }
			set { SetFlag( CorpseFlag.NoBones, value ); }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public bool IsHireable
		{
			get { return GetFlag( CorpseFlag.IsHireable ); }
			set { SetFlag( CorpseFlag.IsHireable, value ); }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public int GoldOnDeath { get; set; }

		#region edit By Arlas [Necro]
		[CommandProperty( AccessLevel.GameMaster )]
		public int LeechableHp
		{
			get { return m_LeechableHp; }
			set { m_LeechableHp = value; }
		}
		#endregion

		[CommandProperty( AccessLevel.GameMaster )]
		public Mobile HireMaster { get; set; }
		#endregion
		...
		public virtual void BeginDecay( TimeSpan delay )
		{
		...
		public static string GetCorpseName( Mobile m )
		{
			...
			if( m is BaseCreature && ( (BaseCreature)m ).HasCustomCorpseName )
				return ( (BaseCreature)m ).CustomCorpseName;
			#endregion
		...
		public static Container Mobile_CreateCorpseHandler( Mobile owner, HairInfo hair, FacialHairInfo facialhair, List<Item> initialContent, List<Item> equipItems )
		{
			...
			if ( shouldFillCorpse )
			{
				for ( int i = 0; i < initialContent.Count; ++i )
				{
					...
					if ( /* Core.AOS && */ owner.Player && item.Parent == owner.Backpack || ( owner.Body.IsHuman && c is DecoCorpse ) )
						c.AddItem( item );
					else
						c.DropItem( item );
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			#region mod by Dies Irae
			writer.Write( (int) GoldOnDeath );
			writer.Write( HireMaster );
			#endregion
		}
				...
				case 11:
					{
						...
						#region mod by Dies Irae
						GoldOnDeath = reader.ReadInt();
						HireMaster = reader.ReadMobile();
						#endregion
...
//			int noto = NotorietyHandlers.CorpseNotoriety( from, this );
//			return ( noto == Notoriety.Innocent || noto == Notoriety.Ally );
		...
		{
			#region modifica by Dies Irae
			if( from is PlayerMobile && m_Owner is PlayerMobile && from != m_Owner )			
				from.PublicOverheadMessage( MessageType.Regular, 0x3B2, true, "*Looting*" );
			#endregion
        ...
        {
            Open( from, checkSelfLoot, false );
        }
        #endregion

		public virtual void Open( Mobile from, bool checkSelfLoot, bool forceSelfLoot )
		{
			if ( forceSelfLoot || from.AccessLevel > AccessLevel.Player || from.InRange( this.GetWorldLocation(), 2 ) )
			{
		...
		public override void OnDoubleClick( Mobile from )
		{
            ...
            if( from == HireMaster )
            {
                if( GoldOnDeath > 0 )
                {
                    Gold rest = new Gold( GoldOnDeath );
                    from.AddToBackpack( rest );
                    from.SendMessage( "You recover the rest of payment form the mercenary body." );
                }
            }

            if( Midgard.Misc.RessSystem.IsImmune( from ) )
                Midgard.Misc.RessSystem.EndRessImmune( from );
            #endregion

			Open( from, from.AccessLevel > AccessLevel.Player || Core.AOS ); // mod by Dies Irae
		}
		...
		public void Carve( Mobile from, Item item )
		{
			...
			else if ( ((Body)Amount).IsHuman && ItemID == 0x2006 )
			{
				...
				if( m_Owner.Race == Corex.Drow || m_Owner.Race == Corex.HighElf || m_Owner.Race == Corex.HalfElf )
					DropItem( new OrecchieDiElfo() );
				else if( m_Owner.Race == Corex.Vampire )
					DropItem( new PolvereDiVampiro() );
				else if( m_Owner.Race == Corex.FairyOfAir || m_Owner.Race == Corex.FairyOfEarth || m_Owner.Race == Corex.FairyOfFire || m_Owner.Race == Corex.FairyOfWater || m_Owner.Race == Corex.FairyOfWood )
					DropItem( new AliDiFata() );
                ...
                if( m_Killer is PlayerMobile )
                    dummy = m_Killer;
                else if( m_Killer is BaseCreature )
                    dummy = ( (BaseCreature)m_Killer ).GetMaster();

                if( dummy != null && dummy is PlayerMobile )
                {
                    head.Owner = m_Owner;
                    head.Killer = dummy;
                ...
                if( m_Owner is Midgard.Engines.BountySystem.Murderer )
                    head.HeadType = HeadType.Murderer;
                #endregion
