...
namespace Server.Items
{
	...
	public class Key : Item
	{
		...
		public virtual bool UseOn( Mobile from, ILockable o )
		{
			if ( o.KeyValue == this.KeyValue )
			{
				...
				else
				{
					...
					if ( o is Item )
					{
                        ...
                        item.PublicOverheadMessage( 0, 0, true, "* Click *" );
		...
		private class UnlockTarget : Target
		{
			...
			{
				m_Key = key;
				CheckLOS = true; // mod by Dies Irae
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				//edit by Arlas
				else if ( targeted is TrapableExplosionWoodenBox )
				{
					number = -1;
					TrapableContainer cont = (TrapableContainer) targeted;
					if (cont.TrapType == TrapType.ExplosionTrap)
					{
						from.SendMessage("You removed the trap.");
						cont.TrapType = TrapType.None;
					}
					else
					{
						from.SendMessage("You put the trap on.");
				...
				else if ( targeted is ILockable )
				{
					if ( m_Key.UseOn( from, (ILockable) targeted ) )
					{
                        ...
                        if( targeted is BaseDoor )
                        {
                            ...
                            // BaseDoor link = door.Link;

                            if( door.UseChainedFunctionality )
                            {
                                foreach( BaseDoor t in door.GetChain() )
                                {
                                    if( t != null && t != door )
                                        t.PublicOverheadMessage( 0, 0, true, "* Click *" );
                                }
                            }
                            /*
                            if( link != null )
                                m_Key.UseOn( from, link );
                            */
                        }
                        #endregion
					}
