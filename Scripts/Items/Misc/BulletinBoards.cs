...
namespace Server.Items
{
	...
	public abstract class BaseBulletinBoard : Item
	{
		...
		private static TimeSpan ThreadDeletionTime = TimeSpan.FromDays( 365 );
        ...
        #region mod by Dies Irae : NPC bounty system

        private const string QuestAuthorTag = "";

        public static BulletinMessage[] PostQuest( Mobile questMobile, int rangeToPost, string subject, string[] Message )
        {
            // List of posts that are posted on bulletin boards
            List<BulletinMessage> PostList = new List<BulletinMessage>();

            if( questMobile != null && questMobile.Alive )
            {
                // Try to find any bulletin boards in the range provided
                IPooledEnumerable eable = questMobile.Map.GetItemsInRange( questMobile.Location, rangeToPost );

                try
                {
                    BulletinMessage questPost = null;

                    foreach( Item item in eable )
                    {
                        if( item is BaseBulletinBoard )
                        {
                            BaseBulletinBoard bb = (BaseBulletinBoard)item;

                            Direction dir = questMobile.GetDirectionTo( bb.Location );
                            string QuestDirection = string.Empty;

                            switch( dir )
                            {
                                case Direction.North: QuestDirection = "South"; break;
                                case Direction.East: QuestDirection = "West"; break;
                                case Direction.South: QuestDirection = "North"; break;
                                case Direction.West: QuestDirection = "East"; break;
                                case Direction.Up: QuestDirection = "South East"; break;
                                case Direction.Right: QuestDirection = "South West"; break;
                                case Direction.Down: QuestDirection = "North West"; break;
                                case Direction.Left: QuestDirection = "North East"; break;
                            }

                            // Check how far the quest mobile is from the posted message 
                            double dist = questMobile.GetDistanceToSqrt( bb.Location );
                            string QuestDistance = string.Empty;

                            // Determine the verbal representation of the distance to be travelled 
                            if( dist < 50 )
                                QuestDistance = "a short distance to the ";
                            else if( dist < 200 )
                                QuestDistance = "a fair distance to the ";
                            else
                                QuestDistance = "a great distance to the ";

                            // Each bulletin board needs to get its own unique post 
                            string[] directionTag = new string[]{   " ", "Begin your quest by travelling", QuestDistance, QuestDirection };

                            // Make the complete message 
                            string[] message = new string[ Message.Length + directionTag.Length ];
                            Message.CopyTo( message, 0 );
                            directionTag.CopyTo( message, Message.Length );

                            // Create the bulletin message
                            questPost = new BulletinMessage( questMobile, null, subject, message );

                            // Change the author to the quest author tag value
                            questPost.PostedName = QuestAuthorTag;

                            // Add the post to the bulletin board
                            bb.AddItem( questPost );

                            // Add the bulletin board to the list of boards where the quest is posted.
                            PostList.Add( questPost );
                        }
                    }
                }
                finally
	...
	public class BulletinMessage : Item
	{
		...
		public string PostedName
		{
            ...
            set { m_PostedName = value; } // mod by Dies Irae
		}
