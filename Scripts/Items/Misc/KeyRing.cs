...
namespace Server.Items
{
	public class KeyRing : Item
	{
        ...
        public void DisplayInfoTo( Mobile from )
        {
            if( Keys != null && Keys.Count > 0 )
            {
                from.SendMessage( "That ring contains:" );
                foreach( Key key in Keys )
                {
                    if( !string.IsNullOrEmpty( key.Description ) )
                        from.SendMessage( key.Description );
                }
            }
            else
                from.SendMessage( "There is no key inside." );
        }

		public override void OnDoubleClick( Mobile from )
		{
            // mod by Dies Irae
            // http://forum.uosecondage.com/viewtopic.php?f=7&t=4919
            // KeyRings no longer must be in a backpack to be used.
            if( !Core.AOS && ( !IsAccessibleTo( from ) || !from.InRange( GetWorldLocation(), 3 ) ) )
            {
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
                ...
                // http://forum.uosecondage.com/viewtopic.php?f=7&t=4919
                // KeyRings no longer must be in a backpack to be used.
                if( !Core.AOS && ( !m_KeyRing.IsAccessibleTo( from ) || from.InRange( m_KeyRing.GetWorldLocation(), 3 ) ) )
                {
                    from.SendLocalizedMessage( 502436 ); // That is not accessible.
                }
                if ( Core.AOS && ( m_KeyRing.Deleted || !m_KeyRing.IsChildOf( from.Backpack ) ) )
				{
					from.SendLocalizedMessage( 1060640 ); // The item must be in your backpack to use it.
					return;
				}
