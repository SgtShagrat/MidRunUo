	...
	public class GuildDeed : Item
	{
		...
		private class InternalPrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				{
				    bool isSeer = from.AccessLevel >= AccessLevel.Seer; // mod by Dies Irae

				    BaseHouse house = BaseHouse.FindHouseAt( from );

					if ( !isSeer && house == null )
					{
						from.SendLocalizedMessage( 501138 ); // You can only place a guildstone in a house.
					}
					else if ( !isSeer && house.FindGuildstone() != null )
					{
						from.SendLocalizedMessage( 501142 );//Only one guildstone may reside in a given house.
					}
					else if ( !isSeer && !house.IsOwner( from ) )
					{
						from.SendLocalizedMessage( 501141 ); // You can only place a guildstone in a house you own!
					}
					else
					{
						...
						if( !Midgard.Misc.GuildsHelper.IsValidGuildLeader( from ) )
						{
							from.SendMessage( from.Language == "ITA" ? "Non sei un valido Leader di Gilda." : "You cannot be a valid Guild Leader." );
							return;
						}
						#endregion
