	...
	public class ColoredAnvil : Item
    {
        ...
        */
            ...
            : base( 0xFAF )
        {
            if( Core.AOS )
            {
                Hue = CraftResources.GetHue( (CraftResource)Utility.RandomMinMax(
                                                                (int)CraftResource.DullCopper,
                                                                (int)CraftResource.Valorite ) );
            }
            else
            {
                Hue = CraftResources.GetHue( (CraftResource)Utility.RandomMinMax(
                                                                (int)CraftResource.OldDullCopper,
                                                                (int)CraftResource.OldRadiantDiamond ) );
            }
