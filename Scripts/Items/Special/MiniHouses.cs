			...
			for (int y = 0; y < size; ++y)
			{
				for (int x = 0; x < size; ++x)
				{
					...
					if (info.Graphics[num] != 0x1)
						AddComponent( new LocalizedAddonComponent( info.Graphics[num++], info.LabelNumber ), size - x - 1, size - y - 1, 0 );
					#endregion
				}
			}
		}
		...
		private static MiniHouseInfo[] m_Info = new MiniHouseInfo[]
			{
				...
				#region Modifica by Dies Irae
				/* Gingerbread House                 */ new MiniHouseInfo( 1077395, 11238, 11237, 11239, 0x1 )
				#endregion
			};
