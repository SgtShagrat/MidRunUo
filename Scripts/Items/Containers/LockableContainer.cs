...
namespace Server.Items
{
	public abstract class LockableContainer : TrapableContainer, ILockable, ILockpickable, ICraftable, IShipwreckedItem
	{
		...
		public override void Deserialize( GenericReader reader )
		{
            ...
            Utility.Log( "lockableContainers.log", "serial {0:X4} - type {1} - loc {2} - req skill {3:F2} - lock level {4:F2} - max lock level {5:F2}.",
                Serial, GetType().Name, Location,  m_RequiredSkill, m_LockLevel, m_MaxLockLevel );
		}
		...
		public override int OnCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CraftItem craftItem, int resHue )
		{
			if ( from.CheckSkill( SkillName.Tinkering, -5.0, 15.0 ) )
			{
				...
				if ( LockLevel == 0 )
					LockLevel = -1;
				else if ( LockLevel > 100 )
					LockLevel = 100;

				if ( RequiredSkill > 100 )
					RequiredSkill = 100;

                /* mod by Dies Irae: cosi' possono arrivare a 100 anche con casse craftate
				if ( MaxLockLevel > 95 )
					MaxLockLevel = 95;
                */
			}
