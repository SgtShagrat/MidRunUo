	...
	public abstract class TrapableContainer : BaseContainer, ITelekinesisable
	{
		...
		public virtual bool ExecuteTrap( Mobile from )
		{
			if ( m_TrapType != TrapType.None )
			{
				...
				switch ( m_TrapType )
				{
					case TrapType.ExplosionTrap:
					{
						...
						if ( from.InRange( loc, 3 ) )
						{
							...
							if ( m_TrapLevel > 0 )
								damage = Utility.RandomMinMax( 10, 20 ) * m_TrapLevel;
							else
								damage = m_TrapPower;
				...
				if (!( this is TrapableExplosionWoodenBox ))
				{
					m_TrapPower = 0;
					m_TrapLevel = 0;
				}
		...
		public override int OnCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CraftItem craftItem, int resHue )
		{
			if ( this is TrapableExplosionWoodenBox )
			{
				...
				{
					from.SendMessage( "Your tinker skill was sufficient to make the item trapable." ); // Your tinker skill was sufficient to make the item lockable.	

					Key key = new Key( KeyType.Copper, Key.RandomValue() );
					key.Name = "a trap installer";
					...
					TrapType = TrapType.None;//x evitare che craftandolo non possano aprirlo

					TrapPower = level - 10;
					TrapLevel = (int)(level/20);

					//RequiredSkill = level - 8;
					//LockLevel = level - 10;
					//MaxLockLevel = level + Utility.RandomMinMax(30,40);

				}
				else
				{
					from.SendMessage( "Your tinker skill was insufficient to make the item trapable." );
				}
			}
