...
namespace Server.Items
{
	public abstract class BaseDoor : Item, ILockable, ITelekinesisable
	{
		...
		public bool Locked
		{
			...
			set
			{
			    bool oldValue = m_Locked;
                if( value != oldValue )
                {
                    m_Locked = value;
                    OnLockedChanged(oldValue);
                }
			}
		}
		...
		public void OnTelekinesis( Mobile from )
		{
            ...
            if ( !TrapOnOpen || !ExecuteTrap( from ) ) // mod by Dies Irae
			    Use( from );
		}
        ...
        public virtual void OnLockedChanged( bool oldValue )
        {
            if( UseChainedFunctionality )
            {
                foreach( BaseDoor t in GetChain() )
                    t.Locked = Locked;
            }
            else
            {
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !TrapOnOpen || !ExecuteTrap( from ) ) // mod by Dies Irae
				Use( from );
        }

        #region mod by Dies Irae : trap system
        ...
        {
            if( TrapType != TrapType.None && IsDoorEnemy( from ) )
            {
        ...
        public virtual bool CanPlaceTrap( Mobile from )
        {
            if( from == null )
                return false;

            return ( from.AccessLevel > AccessLevel.Player ) || IsAccessibleTo( from );
        }

        public virtual bool CanRemoveTrap( Mobile from )
        {
            if( from == null )
                return false;
        ...
        public virtual void RemoveTrap( Mobile from, bool force )
        {
            if( !force )
            {
                if( from == null || !CanRemoveTrap( from ) )
                    return;
            }
        ...
        public Type DeedType
        {
            get
            {
                switch( TrapType )
                {
                    case TrapType.ExplosionTrap: return typeof( ExplosionTrapDeed );
                    case TrapType.DartTrap: return typeof( DartTrapDeed );
                    case TrapType.PoisonTrap: return typeof( PoisonTrapDeed );
                    default: return null;
                }
            }
        }
        ...
        public void Redeed( Mobile from )
        {
			...
			BaseCraftableDoorTrapDeed deed = Loot.Construct( DeedType ) as BaseCraftableDoorTrapDeed;
            if ( deed != null )
			{
				from.AddToBackpack( deed );
                from.SendMessage( "A door trap deed has been placed in your backpack!" );
			}
        }

        public virtual bool IsDoorEnemy( Mobile from )
        {
            ...
            if( t != null )
                return t.IsDoorEnemy( from, this );
            #endregion

            return true;
        }

        public static double GetDifficultyToPlace( int level )
        {
            double scalar = 1.0;
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            #region mob by Dies Irae
            // Version 2: traps
            writer.Write( CanBeTrapped );
            writer.Write( TrapCharges );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
            {
                ...
                case 2:
                {
                    CanBeTrapped = reader.ReadBool();
                    TrapCharges = reader.ReadInt();
                    goto case 1;
                }
                case 1:
                {
