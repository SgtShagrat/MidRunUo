...
namespace Server.Items
{
	...
	public abstract class BaseHouseDoor : BaseDoor, ISecurable
	{
		...
		public BaseHouseDoor( DoorFacing facing, int closedID, int openedID, int openedSound, int closedSound, Point3D offset ) : base( closedID, openedID, openedSound, closedSound, offset )
		{
		    ...
		    IsInternal = false; // mod by Dies Irae
		}
		...
		public override void OnOpened( Mobile from )
		{
			...
			if ( house != null && house.IsFriend( from ) && from.AccessLevel == AccessLevel.Player && house.CanRefresh( from ) && house.RefreshDecay() )
			#endregion
        ...
        #region mod by Dies Irae
        public override bool CanPlaceTrap( Mobile from )
        {
            BaseHouse house = FindHouse();
            ...
            if( base.CanPlaceTrap( from ) && ( Key.ContainsKey( from.Backpack, KeyValue ) || house.IsCoOwner( from ) || house.IsOwner( from ) ) )
                return true;
        ...
        public override bool ExecuteTrap( Mobile from )
        {
            ...
            {
                from.SendMessage( "You know how to use this door without arming the trap." );
                return false;
            }
        ...
        public override bool IsDoorEnemy( Mobile from )
        {
            BaseHouse house = FindHouse();
            ...
            if( Key.ContainsKey( from.Backpack, KeyValue ) || house.HasAccess( from ) || house.IsCoOwner( from ) || house.IsOwner( from ) )
                return false;

            return base.IsDoorEnemy( from );
        }
        #endregion
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            writer.Write( (bool) IsInternal ); // You are not allowed to access this.
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			{
                case 2:
                    IsInternal = reader.ReadBool(); // You are not allowed to access this.
			        goto case 1;
				case 1:
				{
