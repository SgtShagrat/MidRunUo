...
namespace Server.Items
{
    ...
    #region mod by Dies Irae : ClothingMagicalAttribute
    public enum ClothingMagicalAttribute
    {
        None,
    ...
    public abstract class BaseClothing : Item, IDyable, IScissorable, IFactionItem, ICraftable, IWearableDurability, IEngravable, IGuildUniform, ISetItem, IStoneEnchantItem, IIdentificable, IRepairable, IResourceItem
	{
		#region Factions
		...
		public ClothingQuality Quality
		{
			get
			{
                ...
                if( CustomQuality >= Server.Quality.Exceptional )
                    return ClothingQuality.Exceptional;
                else if( CustomQuality <= Server.Quality.Low )
                    return ClothingQuality.Low;
                #endregion

                return m_Quality;
			}
			set{ m_Quality = value; InvalidateProperties(); InvalidateSecondAgeNames(); } // mod by Dies Irae
		}
        ...
        */
		...
		public CraftResource Resource
		{
			...
			set{ m_Resource = value; Hue = CraftResources.GetHue( m_Resource ); InvalidateProperties(); InvalidateSecondAgeNames(); } // mod by Dies Irae
		}
		...
		public override bool CanEquip( Mobile from )
		{
			...
			{
                if( Core.AOS && RequiredRace != null && from.Race != RequiredRace )
                {
		...
		public static void ValidateMobile( Mobile m )
		{
			for ( int i = m.Items.Count - 1; i >= 0; --i )
			{
				...
				if ( item is BaseClothing )
				{
                    ...
                    #region mod by Dies Irae
                    else if( clothing.RequiredRace != null && clothing.RequiredRace == Race.Elf && !Midgard.Engines.Races.Core.IsElfRace( m.Race ) )
                    {
                        m.SendMessage( "Only {0} may use this.", clothing.RequiredRace.PluralName );
                    }
                    #endregion
		...
		public virtual int OnHit( BaseWeapon weapon, int damageTaken )
		{
			...
			//parent.Say( "VESTITO absorbed {0} points of damage.", Absorbed );
			...
			if( parent != null )//&& parent.PlayerDebug )
			{
				...
				{//protection + armor indossata. circa 30 AR
					if (damageTaken > 70 )//cap danno massimo
						damageTaken = 70;
					Absorbed = (int)((double)(parent.ArmorRating/2));

					if ( weapon.Layer == Layer.OneHanded )
						Absorbed = (int)((double)(parent.ArmorRating/4));

					//parent.Say( "PROTECTIONsuVESTITO absorbed {0} points of damage.", Absorbed );
			...
			if ( 25 > Utility.Random( 100 ) && !IsGuildItem ) // 25% chance to lower durability
			{
		...
		public BaseClothing( int itemID, Layer layer, int hue ) : base( itemID )
		{
            ...
            #region mod by Dies Irae
            m_MagicalCharges = 0;
            m_MagicalAttribute = ClothingMagicalAttribute.None;
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if( IsGuildItem )
			{
				if( m_OwnerGuild != null )
					list.Add( "guild item : {0}", m_OwnerGuild.Name );
				if( GuildedOwner != null && GuildedOwner.Name != null )
					list.Add( 1041602, GuildedOwner.Name ); // Owner ~1_val~
			}
			#endregion
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            MidgardSerialize( writer ); // mod by Dies Irae version 8
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 5:
				{
                    ...
                    if( version < 8 )
                    {
                        // IsGuildUniform	= 0x00001000
                        // Guild			= 0x00002000
                        // GuildedOwner		= 0x00004000		
                        if( ( (int)flags & 0x00001000 ) != 0 )
                            IsGuildItem = reader.ReadBool();

                        if( ( (int)flags & 0x00002000 ) != 0 )
                            m_OwnerGuild = reader.ReadGuild();

                        if( ( (int)flags & 0x00004000 ) != 0 )
        ...
        private ClothingMagicalAttribute m_MagicalAttribute;

        [CommandProperty( AccessLevel.GameMaster )]
        public ClothingMagicalAttribute MagicalAttribute
        {
        ...
        public virtual void OnPreAoSAdded( object parent )
        {
            ...
            if( parent is Mobile )
            {
                ...
                if( Parent == from && from.Player )
                {
                    ...
                    if( m_MagicalAttribute != ClothingMagicalAttribute.None || this is BaseOldMagicWizardsHat || this is BaseBodySashOfStrength )
                    {
        ...
        public virtual void OnPreAoSRemoved( object parent )
        {
            ...
            if( parent is Mobile )
            {
                ...
                if( m_MagicalAttribute == ClothingMagicalAttribute.Invisibility && m.Hidden )
                {
        ...
        public virtual void OnPreAosUse( Mobile from )
        {
            ...
            if( !from.CanBeginAction( typeof( BaseClothing ) ) )
            {
        ...
        public bool DoPreAosMagicalEffect( Mobile target )
        {
            if( m_MagicalAttribute != ClothingMagicalAttribute.None && m_MagicalCharges > 0 )
            {
                switch( m_MagicalAttribute )
                {
                    case ClothingMagicalAttribute.Clumsiness:
                        DoClumsiness( target );
                        return true;
                    case ClothingMagicalAttribute.Feeblemindedness:
                        DoFeeblemindedness( target );
                        return true;
                    case ClothingMagicalAttribute.Weakness:
                        DoWeakness( target );
                        return true;
                    case ClothingMagicalAttribute.Agility:
                        DoAgility( target );
                        return true;
                    case ClothingMagicalAttribute.Cunning:
                        DoCunning( target );
                        return true;
                    case ClothingMagicalAttribute.Strength:
                        DoStrength( target );
                        return true;
                    case ClothingMagicalAttribute.Curses:
                        DoCurses( target );
                        return true;
                    case ClothingMagicalAttribute.NightEyes:
                        DoNightEyes( target );
                        return true;
                    case ClothingMagicalAttribute.Blessings:
                        DoBlessings( target );
                        return true;
                    case ClothingMagicalAttribute.SpellReflection:
                        DoSpellReflection( target );
                        return true;
                    case ClothingMagicalAttribute.Invisibility:
        ...
        private void ApplyDelayTo( Mobile from )
        {
            ...
            Timer.DelayCall( GetUseDelay, new TimerStateCallback( ReleaseClothingLock_Callback ), from );
        }

        private void ReleaseClothingLock_Callback( object state )
        {
        ...
        #region [t2a effects]
        public virtual void DoClumsiness( Mobile from )
        {
        ...
        public virtual void DoFeeblemindedness( Mobile from )
        {
        ...
        public virtual void DoWeakness( Mobile from )
        {
        ...
        public virtual void DoAgility( Mobile from )
        {
            string name = String.Format( "[Magic] {0} Offset", StatType.Dex );
            StatMod mod = from.GetStatMod( name );
        ...
        public virtual void DoCunning( Mobile from )
        {
            string name = String.Format( "[Magic] {0} Offset", StatType.Int );
        ...
        public virtual void DoStrength( Mobile from )
        {
            string name = String.Format( "[Magic] {0} Offset", StatType.Str );
        ...
        public virtual void DoCurses( Mobile from )
        {
        ...
        public virtual void DoNightEyes( Mobile from )
        {
        ...
        public virtual void DoBlessings( Mobile from )
        {
            string nameS = String.Format( "[Magic] {0} Offset", StatType.Str );
            string nameD = String.Format( "[Magic] {0} Offset", StatType.Dex );
            string nameI = String.Format( "[Magic] {0} Offset", StatType.Int );
        ...
        public virtual void DoSpellReflection( Mobile from )
        {
        ...
        public virtual void DoInvisibility( Mobile from )
        {
        ...
        public virtual void DoProtection( Mobile from )
        {
        }
        #endregion
		...
		if ( language == "ITA" )
		{
			...
			// fortified
			if( this is IFortificable && ( (IFortificable)this ).IsFortified )
				builder.Append( "irrobustita " );
		...
		if ( m_MaxHitPoints > 0 )
		{
			if ( ((double)m_HitPoints/(double)m_MaxHitPoints) <= 0.2 )
				messageq = from.Language == "ITA" ? "E' uno straccio.": "It's rubbish.";
			else if ( ((double)m_HitPoints/(double)m_MaxHitPoints) <= 0.5 )
				messageq = from.Language == "ITA" ? "E' certamente da rattoppare.": "It needs repair, obviously.";
        ...
        public virtual bool IsMagical
        {
            get
            {
                return MagicalAttribute != ClothingMagicalAttribute.None && MagicalCharges > 0;
            }
        }
        ...
        public int BaseArmorRating
        {
            get
            {
                return m_ArmorBase == -1 ? ArmorBase : m_ArmorBase;
            }
        ...
        public virtual double ArmorRating
        {
            get
            {
                ...
                return Math.Max( 0, ScaleArmorByDurability( ar ) );
            }
        }
        ...
        public virtual double ScaleArmorByQuality( double armor )
        {
            if( Quality == ClothingQuality.Exceptional )
                return armor + 2;
            ...
            if( CustomQuality <= Server.Quality.Low )
                return armor - 2;
            else if( CustomQuality > Server.Quality.Superior )
                return armor + 2;
        ...
        private static readonly double[] m_ArmorScalars = { 0.07, 0.07, 0.14, 0.14, 0.14, 0.44 };
        ...
        private void MidgardSerialize( GenericWriter writer )
        {
            ...
            SetSaveFlag( ref flags, MidgardFlag.MagicEffect, m_MagicalAttribute != ClothingMagicalAttribute.None );
        ...
        private void MidgardDeserialize( GenericReader reader, int version )
        {
            ...
            {
                if( GetSaveFlag( flags, MidgardFlag.MagicEffect ) )
                    m_MagicalAttribute = (ClothingMagicalAttribute)reader.ReadInt();
            ...
            {
                if( ( (int)flags & 0x00000001 ) != 0  )
                    m_MagicalAttribute = (ClothingMagicalAttribute)reader.ReadInt();

                if( ( (int)flags & 0x00000002 ) != 0 )
                    m_MagicalCharges = reader.ReadInt();

                if( ( (int)flags & 0x00000004 ) != 0 )
                    reader.ReadBool();

                if( ( (int)flags & 0x00000008 ) != 0  )
                    m_IdentifiersList = reader.ReadStrongMobileList();

                if( ( (int)flags & 0x00000010 ) != 0  )
                    m_StoneEnchantItemState = new StoneEnchantItem( reader );
            }
        }
        #endregion
        ...
        public void AddIdentifier( Mobile from )
        {
            ...
            if( Core.Debug )
            {
                Utility.Log( "BaseClothingAddIdentifier.log", "IdentifiersList for clothing {0}", Serial.ToString() );
                foreach( Mobile m in m_IdentifiersList )
                    Utility.Log( "BaseClothingAddIdentifier.log", m.Name );
            }
        }
        ...
        public virtual bool Repair( Mobile from, BaseTool tool )
        {
            ...
            if( system.CraftItems.SearchForSubclass( GetType() ) == null && !IsSpecialClothing( system ) )
            {
        ...
        public bool IsSpecialClothing( CraftSystem system )
        {
            // Armor repairable but not craftable

            if( system is DefTailoring )
            {
                return ( this is BearMask )
                       || ( this is DeerMask );
            }
