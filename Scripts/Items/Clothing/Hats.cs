...
{
    public abstract class BaseHat : BaseClothing, IShipwreckedItem, IFortificable
    {
        ...
        public override void Serialize( GenericWriter writer )
        {
            ...
            writer.Write( IsFortified );
            writer.Write( FortifyArmor );
            writer.Write( FortifyDexMalus );
        ...
        public override void Deserialize( GenericReader reader )
        {
            ...
            switch( version )
            {
                case 2:
                    {
                        IsFortified = reader.ReadBool();
                        FortifyArmor = reader.ReadDouble();
                        FortifyDexMalus = reader.ReadInt();
                        goto case 1;
                    }
                case 1:
                    {
        ...
        #region IFortificable
        [CommandProperty( AccessLevel.GameMaster )]
        public override double ArmorRating
        {
            get { return IsFortified ? FortifyArmor : base.ArmorRating; }
        }

        [CommandProperty( AccessLevel.GameMaster )]
        public override int BaseDexBonus
        {
            get { return IsFortified ? FortifyDexMalus : base.BaseDexBonus; }
        }
        ...
        [CommandProperty( AccessLevel.GameMaster )]
        public double FortifyArmor { get; private set; }

        [CommandProperty( AccessLevel.GameMaster )]
        public int FortifyDexMalus { get; private set; }

        public void Fortify( Mobile from, Item combinedWith )
        {
            if( !IsChildOf( from.Backpack ) )
                return;

            from.SendMessage( "Fortify this hat with what?" );
            from.Target = new InternalTarget( this );
        }

        private void EndFortify( Mobile from, BaseArmor with )
        {
            if( from == null || with == null || with.Deleted || !with.IsChildOf( from.Backpack ) )
                return;

            if( !from.CheckSkill( SkillName.Tailoring, 0, 100.0 ) )
            {
                from.SendMessage( "You fail and ruin the hat." );
                if( !Deleted )
                    Delete();
            }
            else
            {
                DefTailoring.CraftSystem.PlayCraftEffect( from );

                IsFortified = true;
                FortifyArmor = with.ArmorRating;
                FortifyDexMalus = with.DexBonus;

                if( !with.Deleted )
                    with.Delete();

                InvalidateSecondAgeNames();
                from.SendMessage( "You were able to fortify the hat." );
            }
        }

        private class InternalTarget : Target
        {
            private readonly BaseHat m_Owner;

            public InternalTarget( BaseHat owner )
            ...
            {
                if( o != null && o is BaseArmor && IsValidHelm( (BaseArmor)o ) )
                    m_Owner.EndFortify( from, (BaseArmor)o );
                else
                    from.SendMessage( "That's not a helmet." );
            }

            private static bool IsValidHelm( BaseArmor item )
            {
                Type t = item.GetType();
    ...
    public class SkullCap : BaseHat
    {
        ...
        public override string OldInitHits { get { return "1d10+50"; } }
        #endregion
    ...
    {
        public override int ArmorBase { get { return 5; } } // mod by Dies Irae : pre-aos stuff
