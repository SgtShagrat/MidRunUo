	...
	public class CommodityDeed : Item
	{
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			*/
			if ( m_Commodity != null )
            {
                if( ( (ICommodity)m_Commodity ).Description != null ) // Mod By Dies Irae
                    list.Add(  ( (ICommodity)m_Commodity ).Description );
                else if( ( (ICommodity)m_Commodity ).DescriptionNumber > 0 )
                    list.Add( 1060658, "#{0}\t{1}", ( (ICommodity)m_Commodity ).DescriptionNumber, m_Commodity.Amount ); // ~1_val~: ~2_val~
            }
		}
