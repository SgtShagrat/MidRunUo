...
namespace Server.Items
{
	...
	[TypeAlias( "Server.Items.PitcherAle", "Server.Items.PitcherCider", "Server.Items.PitcherLiquor",
		"Server.Items.PitcherMilk", "Server.Items.PitcherWine", "Server.Items.PitcherWater",
				"Server.Items.PitcherJuice", "Server.Items.PitcherMead", "Server.Items.GlassPitcher" )] // mod by Dies Irae
	...
	public abstract class BaseBeverage : Item, IHasQuantity
	{
		...
		public virtual void Pour_OnTarget( Mobile from, object targ )
		{
			...
			{
                #region mod by Dies Irae per i Vampiri
                if( from.Race == Midgard.Engines.Races.Core.Vampire )
                {
                    from.SendMessage( from.Language == "ITA" ? "Non trarresti alcun giovamento dal bere questo liquido!" : "This liquid is not useful to you!" );
                    return;
                }
                #endregion
				...
				if ( from.Thirst >= 20 )
				{
					from.SendMessage( from.Language == "ITA" ? "Non hai sete." : "You are not thirsty at all." );
					return;
				}
				...
				{
					from.Thirst = 20;
					from.SendMessage( from.Language == "ITA" ? "Hai bevuto a sazietà, ma ora sei pieno!" : "You drink so much, but now you're full!" );
				}
				else
				{
					from.Thirst = iThirst;
					...
					if ( iThirst < 5 )
						from.SendMessage( from.Language == "ITA" ? "Hai bevuto, ma sei ancora molto assetato." : "You drink but are still thirsty." );
					else if ( iThirst < 10 )
						from.SendMessage( from.Language == "ITA" ? "Hai bevuto, e cominci ad essere soddisfatto." : "You drink and are more satisfied." );
					else if ( iThirst < 15 )
						from.SendMessage( from.Language == "ITA" ? "Dopo aver bevuto, ti senti molto meno assetato." : "You drink and are less thirsty." );
					else
						from.SendMessage( from.Language == "ITA" ? "Dopo aver bevuto ti senti soddisfatto." : "You drink and are satisfied." );
				}

				// Modifica per il computo dello stat loss da cibo.
				Midgard.Engines.FoodDecaySystem.FoodDecayTimer.ComputeFoodStatLoss( from );
				...
				if ( player != null )
				{
					...
					if ( qs != null )
					{
						...
						if ( obj != null && !obj.Completed )
						{
							...
							if ( vat.X >= 6007 && vat.X <= 6018 && vat.Y >= 1157 && vat.Y < 1170 &&
							#endregion
