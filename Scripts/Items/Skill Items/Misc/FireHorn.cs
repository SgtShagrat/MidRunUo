...
namespace Server.Items
{
	public class FireHorn : Item
	{
		...
		public void Use( Mobile from, IPoint3D loc )
		{
			...
			if ( targets.Count > 0 )
			{
				...
				for ( int i = 0; i < targets.Count; ++i )
				{
					...
					if ( !Core.AOS && m.CheckSkill( SkillName.MagicResist, 0.0, Core.AOS ? 120.0 : 100.0 ) ) // mod by Dies Irae
					{
						toDeal *= 0.5;
        ...
        private const double DelayOfReuse = 6.0;
        private const int SulfurusAshUsed = 15;
        private const int MaxRange = 7;
        private const double BreakChance = 0.05;

        public void UseOld( Mobile from, IPoint3D loc )
        {
            if( !CheckUse( from ) )
                return;

            from.BeginAction( typeof( FireHorn ) );
            Timer.DelayCall( TimeSpan.FromSeconds( DelayOfReuse ), new TimerStateCallback( EndAction ), from );

            int music = from.Skills[ SkillName.Musicianship ].Fixed;
            int prov = from.Skills[ SkillName.Provocation ].Fixed;
            int entice = from.Skills[ SkillName.Discordance ].Fixed;
            int peace = from.Skills[ SkillName.Peacemaking ].Fixed;

            int sum = music + prov + entice + peace;   // from 0 to 4000
            int mean = (int)( sum / 40.0 );            // from 0 to 100
            ...
            from.Backpack.ConsumeUpTo( typeof( SulfurousAsh ), SulfurusAshUsed );
            from.PlaySound( 0x15F );
            Effects.SendPacket( from, from.Map, new HuedEffect( EffectType.Moving, from.Serial, Serial.Zero, 0x36D4, from.Location, loc, 5, 0, false, true, 0, 0 ) );
            ...
            IPooledEnumerable eable = from.Map.GetMobilesInRange( new Point3D( loc ), range );
            ...
            if( targets.Count > 0 )
            {
                ...
                foreach( Mobile m in targets )
                {
                    ...
                    if( m.CheckSkill( SkillName.MagicResist, 0.0, 100.0 ) )
                    {
                        toDeal *= 0.5;
                        m.SendLocalizedMessage( 501783 ); // You feel yourself resisting magical energy.
                    }
                    ...
                    from.DoHarmful( m );
                    MidgardSpellHelper.Damage( TimeSpan.Zero, m, from, toDeal, SpellType.Fire );
                    Effects.SendTargetEffect( m, 0x3709, 10, 30 );
                }
            }
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget( FireHorn horn ) : base( Core.AOS ? 3 : /* 2 */ 10, true, TargetFlags.Harmful )
			{
				m_Horn = horn;
			}
