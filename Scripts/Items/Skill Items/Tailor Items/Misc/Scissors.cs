	...
	public class Scissors : Item
	{
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( targeted is BaseOrgan )
				{
					from.RevealingAction();
					BaseOrgan organ = (BaseOrgan)targeted;
					organ.OpenOrganTo( from );
				}
				else if ( targeted is Vein )
				{
					from.RevealingAction();
					Vein veins = (Vein)targeted;
					veins.DoCutVein( from );
				}
				else if ( targeted is MutationCore )
				{
					from.RevealingAction();
					MutationCore core = (MutationCore)targeted;
					core.OnAfterCut( from );
				}
				else if ( targeted is DiseasedGland )
				{
					from.RevealingAction();
					DiseasedGland gland = (DiseasedGland)targeted;
					gland.OnAftetCut( from );
				}
				#endregion			
				...
				#region modifica by Dies Irae
				else if( Core.AOS && targeted == from ) // if( Core.AOS && targeted == from )
				#endregion
				{
