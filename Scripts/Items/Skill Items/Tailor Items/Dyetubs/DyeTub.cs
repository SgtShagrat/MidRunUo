		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( targeted is Item )
				{
					...
					#region tappeti
					if( item is AddonComponent && CarpetAttribute.Check(((AddonComponent)item).Addon ) && 
					    ( m_Tub.AllowDyables || m_Tub.AllowFurniture ) )
					{
						...
						else
						{
							...
							if( house == null )
								from.SendLocalizedMessage( 1064281 ); // You can dye only carpets placed in your house.
							else if ( !house.IsCoOwner( from ) )
								from.SendLocalizedMessage( 1064282 ); // You must be a co-owner of this house to dye this carpet.
							else
							{
								((AddonComponent)item).Addon.Hue = m_Tub.DyedHue;
					...
					else if( CarpetAttribute.Check( item ) && ( m_Tub.AllowDyables || m_Tub.AllowFurniture ) )
					{
						if ( !from.InRange( m_Tub.GetWorldLocation(), 1 ) )
							from.SendLocalizedMessage( 500446 ); // That is too far away.
						else
						{
							if( item.IsChildOf( from.Backpack ) )
							{
								...
								from.SendLocalizedMessage( 1064283 ); // The carpet deed must be in your backpack to be painted.
						}
					}
					#endregion
			
					#region banners
					else if( item is AddonComponent && BannerAttribute.Check(((AddonComponent)item).Addon ) && 
					    ( m_Tub.AllowDyables || m_Tub.AllowFurniture ) )
					{
						if ( !from.InRange( m_Tub.GetWorldLocation(), 1 ) || !from.InRange( item.GetWorldLocation(), 1 ) )
							from.SendLocalizedMessage( 500446 ); // That is too far away.
						else
						{
							...
							if( house == null )
								from.SendLocalizedMessage( 1064284 ); // You can dye only banners placed in your house.
							else if ( !house.IsCoOwner( from ) )
								from.SendLocalizedMessage( 1064285 ); // You must be a co-owner of this house to dye this banner.
							else
							{
								item.Hue = m_Tub.DyedHue;
								from.PlaySound( 0x23E );
					...
					else if( BannerAttribute.Check( item ) && ( m_Tub.AllowDyables || m_Tub.AllowFurniture ) )
					{
						if ( !from.InRange( m_Tub.GetWorldLocation(), 1 ) )
							from.SendLocalizedMessage( 500446 ); // That is too far away.
						else
						{
							if( item.IsChildOf( from.Backpack ) )
							{
								...
								from.SendLocalizedMessage( 1064286 ); // The banner deed must be in your backpack to be painted.
						}
					}
					#endregion
