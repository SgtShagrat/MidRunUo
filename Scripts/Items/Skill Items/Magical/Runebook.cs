...
namespace Server.Items
{
	public class Runebook : Item, ISecurable, ICraftable
	{
        ...
        */
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( from.InRange( GetWorldLocation(), Core.AOS ? 1 : 2 ) )
			{
		...
		public override bool OnDragDrop( Mobile from, Item dropped )
		{
			if ( dropped is RecallRune )
			{
				...
				{
                    from.SendMessage( "That cannot be done while the book is locked down." );
				}
				else if ( IsOpen( from ) )
				{
	...
	public class RunebookEntry
	{
		...
		public RunebookEntry( GenericReader reader )
		{
            ...
            if( m_House != null )
                Timer.DelayCall( TimeSpan.Zero, new TimerCallback( Verifylocation ) );
		}
        ...
        private void Verifylocation()
        {
            ...
            if( newLocation != m_Location )
                Console.WriteLine( "Warning: changing rune entry location for house to ban location." );
