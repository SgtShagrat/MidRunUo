	...
	public class Moongate : Item
	{
		...
		public override bool OnMoveOver( Mobile m )
		{
			if ( m.Player )
				CheckGate( m, 0 );
            #region mod by Arlas
            else if( m is BaseEscortable || m is TalkingBaseEscortable )
            {
                m.Say( "No! Ho fretta, ma odio la magia!" );
                return false;
            }
            #endregion
		...
		public virtual void UseGate( Mobile m )
		{
			...
			{
				m.SendMessage( m.Language == "ITA" ? "Questo moongate non sembra avere una destinazione." : "This moongate does not seem to go anywhere." );
			}
		}
		...
		public virtual void DelayCallback( Mobile from, int range )
		{
			...
			if ( m_TargetMap != null )
				...
				from.SendMessage( from.Language == "ITA" ? "Questo moongate non sembra avere una destinazione." : "This moongate does not seem to go anywhere." );
		}
