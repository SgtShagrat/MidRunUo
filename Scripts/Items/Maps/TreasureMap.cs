...
namespace Server.Items
{
	public class TreasureMap : MapItem
	{
		...
		private static Type[][] m_SpawnTypes = new Type[][]
		{
			...
			new Type[]{ typeof( LichLord ), typeof( Daemon ), typeof( ElderGazer ), typeof( PoisonElemental ), typeof( BloodElemental ), typeof( Balron ), typeof( DreadSpider ) }, // mod by Dies Irae
		...
		public override void GetProperties( ObjectPropertyList list )
		{
            ...
            #region mod by Dies Irae
            if( Core.AOS )
                list.Add( "Midgard staff: fino a nuovo ordine le mappe di tesori produrranno SOLO tesori OSI style." );
            #endregion
        }
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			{
                if( m_Level == 6 )
                    LabelTo( from, 1063452 ); // a tattered, ingeniously drawn treasure map
                else
                    LabelTo( from, 1041510 + m_Level ); // a tattered, youthful treasure map etc.
