...
namespace Server.Gumps
{
	...
	{
	    public static readonly bool OldStyle = true; // mod by Dies Irae
		...
		private void AddBackground()
		{
			...
			// Charges
            if( OldStyle )
                AddHtml( 140, 40,  80, 18, HtmlAscii( "Charges:" ), false, false );
            else
			...
			// Max charges
            if( OldStyle )
			    AddHtml( 300, 40, 100, 18, HtmlAscii( "Max Charges:" ), false, false );
            else
		...
		private void AddIndex()
		{
            ...
            if( OldStyle )
			    AddOldHtml( 158, 22, 100, 18, "Rename book:" );
            else
		...
		private void AddDetails( int index, int half )
		{
			...
			if ( index < m_Book.Entries.Count )
			{
                ...
                if( OldStyle )
                    AddOldHtml( 150 + ( half * 160 ), 115, 100, 18, "Drop rune" );
                else
			...
			// Description label
            if( OldStyle )
                AddOldHtml( 145 + ( half * 160 ), 60, 115, 17, desc );
            else
		...
		{
            RegisterUse( from ); // mod by Dies Irae
		...
		private class InternalPrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				if ( m_Book.CheckAccess( from ) )
                {
                    ...
                    // You can remove the name on a runebook by setting it to "none"
                    if( !string.IsNullOrEmpty( text ) && text == "none" )
                    {
                        m_Book.Description = null;
                        from.SendMessage( "The book's title has been removed." );
                        return;
                    }
                    #endregion
        ...
        private static Dictionary< Mobile, Point3D> m_UserLocationsDict = new Dictionary<Mobile, Point3D>();

	    #region mod by Dies Irae
	    private static void RegisterUse( Mobile m )
	    {
	        m_UserLocationsDict[ m ] = m.Location;
	    }

	    private static bool CheckLocation( Mobile m )
	    {
            return !m_UserLocationsDict.ContainsKey( m ) || m.InRange( m_UserLocationsDict[ m ], 1 );
	    }
	    #endregion
