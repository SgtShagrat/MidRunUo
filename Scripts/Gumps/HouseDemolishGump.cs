			...
			if ( info.ButtonID == 1 && !m_House.Deleted )
			{
				if ( m_House.IsOwner( m_Mobile ) )
				{
					...
					try
					{
						...
						{
							t.WriteLine( "House deleting process confirmed from {0} (account {1}) in Date {2} - Time {3}. House serial {4}.",
							            m_Mobile.Name, m_Mobile.Account.Username, DateTime.Now.ToShortDateString(), DateTime.Now.ToShortTimeString(), m_House.Serial.ToString() );
						}	
					}
					catch
					{
