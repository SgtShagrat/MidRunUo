		...
		public HouseGumpAOS( HouseGumpPageAOS page, Mobile from, BaseHouse house ) : base( 50, 40 )
		{
			...
			switch ( page )
			{
				case HouseGumpPageAOS.Information:
				{
					...
					AddLabel( 20, 270, LabelHue, "Will collapse on:" );       
					AddLabel( 250, 270, LabelHue, GetDateTime( house.LastRefreshed + house.DecayPeriod ) );
					#endregion
					...
					switch ( index )
					{
						...
						case 1: // Customize This House
						{
							if ( isOwner && isCustomizable )
							{
                                ...
                                else if ( m_House.HasHicthes )
								{
								    string content = "The house can not be customized when hitched creatures are present in the house." +
								                     "Please free them before customizing the house.";
									from.SendGump( new NoticeGump( 1060637, 30720, content, 32512, 320, 180, new NoticeGumpCallback( CustomizeNotice_Callback ), m_House ) );
                                }
                                #endregion
                                else
								{
