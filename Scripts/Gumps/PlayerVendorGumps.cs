...
namespace Server.Gumps
{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1 )
			{
				...
				else
				{
					...
					m_Vendor.HoldGold += (int)( m_VI.Price * 0.9 ); // mod by Dies Irae

					from.SendLocalizedMessage( 503201 ); // You take the item.

                    #region mod by Dies Irae
                    if( m_VI.Item is IIdentificable )
                        ( (IIdentificable)m_VI.Item ).AddIdentifier( from );
                    else if( m_VI.Item is ITasteIdentificable )
                        ( (ITasteIdentificable)m_VI.Item ).AddIdentifier( from );
                    #endregion
                }
			}
			else
	...
	public class PlayerVendorOwnerGump : Gump
	{
		...
		public PlayerVendorOwnerGump( PlayerVendor vendor, Mobile player ) : base( 50, 200 )
		{
			m_Vendor = vendor;
			m_Player = player;
			...
			AddHtmlLocalized( 40, 21, 260, 20, 1038318, 0x7FFF, false, false ); // # of days Vendor salary is paid for: 
			AddLabel( 300, 21, 0x480, ((int)days).ToString() );

			AddHtmlLocalized( 40, 41, 260, 20, 1038319, 0x7FFF, false, false ); // Earth days: 
			AddLabel( 300, 41, 0x480, ((int)(days / 12.0)).ToString() );

			AddHtmlLocalized( 40, 61, 260, 20, 1038321, 0x7FFF, false, false ); // Gold held for you:
			AddLabel( 300, 61, 0x480, m_Vendor.HoldGold.ToString() );
			
			AddHtmlLocalized( 40, 81, 260, 20, 1038322, 0x7FFF, false, false ); // Gold held in my account:
			AddLabel( 300, 81, 0x480, m_Vendor.BankAccount.ToString() );

			AddLabel( 40, 101, 0x480, m_Player.Language == "ITA"? "La mia diaria è:" : "My charge per day is:" );
			AddLabel( 300, 101, 0x480, perDay.ToString() );

			AddHtmlLocalized( 40, 121, 260, 20, 1062509, 0x7FFF, false, false ); // Shop Name:
			AddLabel( 140, 121, 0x66D, vendor.ShopName );

			AddLabel( 40, 141, 0x480, m_Player.Language == "ITA" ? "Visite:" : "Visits:" );
			AddLabel( 140, 141, 0x480, vendor.Visits.ToString() );

			AddButton( 390, 22, 0x15E1, 0x15E5, 1, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 408, 21, 120, 20, 1019068, 0x7FFF, false, false ); // See goods

			AddButton( 390, 42, 0x15E1, 0x15E5, 2, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 408, 41, 120, 20, 1019069, 0x7FFF, false, false ); // Customize

			AddButton( 390, 62, 0x15E1, 0x15E5, 3, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 408, 61, 120, 20, 1062434, 0x7FFF, false, false ); // Rename Shop

			AddButton( 390, 82, 0x15E1, 0x15E5, 4, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 408, 81, 120, 20, 3006217, 0x7FFF, false, false ); // Rename Vendor

			AddButton( 390, 102, 0x15E1, 0x15E5, 5, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 408, 101, 120, 20, 3006123, 0x7FFF, false, false ); // Open Paperdoll

			AddButton( 390, 122, 0x15E1, 0x15E5, 11, GumpButtonType.Reply, 0 );
			AddLabel( 408, 121, 0x480, m_Player.Language == "ITA" ? "Modifica grida" : "Edit shouts" );

			AddButton( 390, 142, 0x15E1, 0x15E5, 12, GumpButtonType.Reply, 0 );
			AddLabel( 408, 141, 0x480, m_Player.Language == "ITA" ? "Messaggio commerciale" : "Commercial message" );
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 11: // Edit Shouts
				{
					from.SendGump( new Midgard.Engines.TalkingPlayerVendorSystem.PlayerVendorShoutGump( from, m_Vendor ) );

					break;
				}
				case 12: // Edit Commercial
				{
					m_Vendor.EditCommercialMessage( from );
	...
	public class NewPlayerVendorOwnerGump : Gump
	{
		...
		public NewPlayerVendorOwnerGump( PlayerVendor vendor, Mobile player ) : base( 50, 200 )
		{
			m_Vendor = vendor;
			m_Player = player;
			...
			#region mod by Dies Irae
			AddButton( 140, 133, 0x15E1, 0x15E5, 11, GumpButtonType.Reply, 0 );
			AddLabel( 40, 130, 0x480, m_Player.Language == "ITA" ? "Modifica grida" : "Edit shouts:" );

			AddLabel( 40, 150, 0x480, m_Player.Language == "ITA" ? "Visite:" : "Visits:" );
			AddLabel( 140, 150, 0x480, vendor.Visits.ToString() );
			#endregion
			...
			if ( vendor is RentedVendor )
			{
				...
				if (m_Player.Language == "ITA")
					AddLabel( 38, 132, 0x480, String.Format( "L'affitto finirà in {0} giorn{1} e {2} or{3}.", days, days != 1 ? "o" : "i", hours, hours != 1 ? "a" : "e" ) );
				else
			...
			AddLabel( 408, 121, 0x480, m_Player.Language == "ITA" ? "Riscuoti Oro" : "Collect Gold" );

			AddButton( 390, 144, 0x15E1, 0x15E5, 7, GumpButtonType.Reply, 0 );
			AddLabel( 408, 141, 0x480, m_Player.Language == "ITA" ? "Licenzialo" : "Dismiss Vendor" );
	...
	public class PlayerVendorCustomizeGump : Gump
	{
		...
		#region mod by Dies Irae
		private bool m_CustomizingHair;
		private bool m_CustomizingFacialHair;
		#endregion
				...
				else
				{
					...
					if ( cat < 100 )
					{
						if ( cat < Categories.Length && cat >= 0 )
						{
                            ...
                            #region mod by Dies Irae
                            else if( m_CustomizingHair )
                                new PVHuePicker( null, m_Vendor, from, true, false ).SendTo( state );
                            else if( m_CustomizingFacialHair )
                                new PVHuePicker( null, m_Vendor, from, false, true ).SendTo( state );
                            #endregion
						}
					}
					else
		...
		private class PVHuePicker : HuePicker
		{
            ...
            public PVHuePicker( Item item, Mobile v, Mobile from, bool hair, bool facialHair )
                : base( ( hair || facialHair ) ? 0xFAB : item.ItemID )
            {
                m_Vendor = v;
                m_Item = item;
                m_Mob = from;
			...
			public override void OnResponse( int hue )
			{
                ...
                if( m_Item != null )
                    ...
                    m_Mob.HairHue = hue;
                else if( m_FacialHair )
                    m_Mob.FacialHairHue = hue;
                #endregion
	...
	{
		public static readonly bool ChangeRaceEnabled = false; // mod by Dies Irae
