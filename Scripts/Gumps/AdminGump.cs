...
namespace Server.Gumps
{
	public enum AdminGumpPage
	{
		...
		AccountDetails_Tags,
		AccountDetails_Email, // modifica by Berto
	...
	public class AdminGump : Gump
	{
		...
		public AdminGump( Mobile from, AdminGumpPage pageType, int listPage, ArrayList list, string notice, object state ) : base( 50, 40 )
		{
			...
			switch ( pageType )
			{
				...
				case AdminGumpPage.Accounts:
				{
					...
					for ( int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index )
					{
                        ...
                        #region mod by Dies Irae
                            ...
                            AddLabelCropped( 252, offset, 120, 20, RedHue, "Jailed" );
                        #endregion
						else
				...
				case AdminGumpPage.AccountDetails:
				{
				    ...
				    AddPageButton( 320, 10, GetButtonID( 5, 40 ), "Email", AdminGumpPage.AccountDetails_Email ); // mod by Berto
                    AddButtonLabeled( 320, 30, GetButtonID( 5, 41 ), "Jail" ); // mod by Dies Irae
                    AddButtonLabeled( 320, 50, GetButtonID( 5, 42 ), "Unjail" ); // mod by Dies Irae
                    break;
				}
				...
				#region berto
				case AdminGumpPage.AccountDetails_Email:
				{
					Account a = state as Account;
					...
					AddHtml( 10, 125, 400, 20, Color( Center( "Change Email" ), LabelColor32 ), false, false );

					AddLabel(  20, 150, LabelHue, "Username:" );
					AddLabel( 200, 150, LabelHue, a.Username );

					AddLabel( 20, 180, LabelHue, "Current Email:" );
					AddLabel( 200, 180, LabelHue, a.Email );

					AddLabel( 20, 210, LabelHue, "New Email:" );
					AddTextField( 200, 210, 160, 20, 0 );

					AddButtonLabeled( 20, 240, GetButtonID( 5, 40 ), "Submit Change" );
		...
		public void AddAccountHeader()
		{
			...
			AddLabel( 200, 42, LabelHue, "Pass:" );
			AddTextField( 250, 42, 150, 20, 1 );

			AddLabel( 200, 64, LabelHue, "Mail:" );
			AddTextField( 250, 64, 150, 20, 2 );
			#endregion
					...
					switch ( index )
					{
						...
						case 504:
						{
                            ...
                            if( Misc.AccountHandler.LockdownLevel > AccessLevel.Player )
                            {
                                ...
                                try
                                {
                                    if( File.Exists( Misc.AccountHandler.LockDownFileName ) )
                                        File.Delete( Misc.AccountHandler.LockDownFileName );

                                    using( StreamWriter op = new StreamWriter( Misc.AccountHandler.LockDownFileName, true ) )
                                        op.WriteLine( Misc.AccountHandler.LockdownLevel );
                                }
                                catch
                                {
                                }
				...
				case 5:
				{
					switch ( index )
					{
						...
						case 6:
						{
							...
							TextRelay emEntry = info.GetTextEntry( 2 ); // modifica by berto

							string un = ( unEntry == null ? null : unEntry.Text.Trim() );
							string pw = ( pwEntry == null ? null : pwEntry.Text.Trim() );
							string em = ( emEntry == null ? null : emEntry.Text.Trim() ); // modifica by berto
							...
							else
							{
								...
								{
									dispAccount = new Account( un, pw ,em); // modifica by berto
						...
						case 40:
						{
							...
							string notice;
							AdminGumpPage page = AdminGumpPage.AccountDetails_Email;

							if ( email == null || email.Length == 0 )
							{
								notice = "You must enter the email.";
							}
							else
							{
								notice = "The email has been changed.";
								a.Email = email;
								page = AdminGumpPage.AccountDetails_Information;
								CommandLogging.WriteLine( from, "{0} {1} changing email of account {2}", from.AccessLevel, CommandLogging.Format( from ), a.Username );
							}
                        ...
                        {
                            string notice;
                            AdminGumpPage page = AdminGumpPage.AccountDetails_Information;

                            Account a = m_State as Account;
                            if( a == null )
                                break;

                            Midgard.Engines.JailSystem.JailSystem.DoJailAccount( from, a, out notice );
                            from.SendGump( new AdminGump( from, page, 0, null, notice, m_State ) );
                            break;
                        }
                        case 42:
                        {
                            ...
                            Midgard.Engines.JailSystem.JailSystem.DoReleaseAccount( from, a, out notice );
                            from.SendGump( new AdminGump( from, page, 0, null, notice, m_State ) );
                            break;
                        }
						#endregion	
