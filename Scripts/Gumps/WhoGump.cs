	...
	public class WhoGump : Gump
	{
		...
		public static List<Mobile> BuildList( Mobile owner, string filter )
		{
			...
			for ( int i = 0; i < states.Count; ++i )
			{
				...
				#region Modifica by Dies Irae per la visibilità tra dev owner e admin
				if( m != null && owner.AccessLevel == AccessLevel.Administrator && m.AccessLevel == AccessLevel.Developer )
				{
					list.Add( m );
					continue;
				}	
			...
			switch ( info.ButtonID )
			{
				...
				default:
				{
					...
					if ( index >= 0 && index < m_Mobiles.Count )
					{
						...
						#region modifica by Dies Irae per l'uso di Who tra dev e admin
						else if ( from.AccessLevel == AccessLevel.Administrator && m.AccessLevel == AccessLevel.Developer )
						{
							from.SendGump( new ClientGump( from, m.NetState ) );
						}
						#endregion
