...
using Midgard.Engines.RegionalSpawningSystem;
...
namespace Server.Regions
{
	...
	{
        #region mod by Dies Irae: custom props
        public virtual bool BlockFields { get; set; }

        public virtual int SolidHue { get; set; }

        public virtual int AlteredLightLevel { get; set; }

        public virtual int ForcedSeason { get; set; }

        public virtual bool TravelRestricted { get; set; }

        public virtual bool CannotRun { get; set; }

        public virtual bool CannotMount { get; set; }

        private static void Region_OnLogin( LoginEventArgs e )
        {
            Mobile m = e.Mobile;

            if( m.AccessLevel < AccessLevel.GameMaster )
            {
                BaseRegion br = m.Region as BaseRegion;
        ...
        public override bool OnBeginSpellCast( Mobile m, ISpell s )
        {
            if( TravelRestricted )
            {
                ...
                {
                    if( ( s is Spells.Seventh.GateTravelSpell || s is Spells.Fourth.RecallSpell
                         || s is Spells.Sixth.MarkSpell || s is Spells.Chivalry.SacredJourneySpell
                         || s is Midgard.Engines.SpellSystem.NaturesPassageSpell ) )
                    {
        ...
        public override bool OnDoubleClick( Mobile m, object o )
        {
            ...
            {
                if( CannotMount && o is EtherealMount )
                    return false;
            }
        ...
        public TimeSpan CustomLogoutDelay { get; set; }

        private string m_RegionalInfo;

        public string RegionalInfo { get { return m_RegionalInfo; } set { m_RegionalInfo = value; } }

        public virtual void DisplayRegionalInfo( Mobile m )
        {
            if( !m.Player )
                return;

            if( m.AccessLevel > AccessLevel.Counselor && m.PlayerDebug)
            {
                m.SendMessage( "Debug: You have entered {0} region with priority {1}.", Name ?? "an unkown", Priority );
                m.SendMessage( "Debug: Regional info is \"{0}\".", m_RegionalInfo ?? "-none-" );
            }

            if( ( (Midgard2PlayerMobile)m ).DisplayRegionalInfo && !String.IsNullOrEmpty( m_RegionalInfo ) )
                m.SendMessage( 0x35, m_RegionalInfo );
        }
        ...
        public override int GetSeason()
        {
            ...
            return base.GetSeason();
        }

        public override int GetSolidHue()
        {
            if( SolidHue > -1 )
                return SolidHue;

            return base.GetSolidHue();
        }

        public override void AlterLightLevel( Mobile m, ref int global, ref int personal )
        {
            if( AlteredLightLevel != -1 )
                global = AlteredLightLevel;
		...
		public static void Configure()
		{
			...
			EventSink.Login += new LoginEventHandler( Region_OnLogin ); // mod by Dies Irae
		}
		...
		public override TimeSpan GetLogoutDelay( Mobile m )
		{
            ...
            if( CustomLogoutDelay > TimeSpan.Zero )
                return CustomLogoutDelay;
            #endregion
        ...
        public virtual int RegionSophistication
        {
            get
            {
                if( m_SpeechFragment != null || !( Parent is BaseRegion ) )
                    return m_RegionSophistication;
                else
                    return ( (BaseRegion)Parent ).RegionSophistication;
            }
        }

        public virtual int RegionAttitude
        {
            get
            {
                if( m_SpeechFragment != null || !( Parent is BaseRegion ) )
                    return m_RegionAttitude;
                else
                    return ( (BaseRegion)Parent ).RegionAttitude;
            }
        }

	    readonly int m_RegionSophistication;

	    readonly int m_RegionAttitude;

	    readonly SpeechFragment m_SpeechFragment;

        public virtual SpeechFragment SpeechFragment
        {
            get
            {
                if( m_SpeechFragment != null )
                    return m_SpeechFragment;
                else if( Parent is BaseRegion )
                    return ( (BaseRegion)Parent ).SpeechFragment;
                else
                    return SpeechFragment.SFBDefault;
            }
        }
        #endregion

        #region [regional spawning by Dies Irae]
        public int SizeOfRegion { get; private set; }

        public void UpdateSizeOfRegion()
        {
            int area = 0;
            ...
            m_SizeOfRegion = blocks.Count; */
        }

        public SpawnDensity Density { get; set; }
        #endregion

		public BaseRegion( XmlElement xml, Map map, Region parent ) : base( xml, map, parent )
		{
		    Density = SpawnDensity.Normal;
            UpdateSizeOfRegion(); // mod by Dies Irae
			...
			#region mod by Dies Irae
            ReadString( xml["info"], "value", ref m_RegionalInfo, false );

			int minDelayLogOut = 0;
			ReadInt32( xml["logoutDelay"], "mins", ref minDelayLogOut, false );
            CustomLogoutDelay = minDelayLogOut > 0 ? TimeSpan.FromMinutes( minDelayLogOut ) : TimeSpan.Zero;

            // if( m_CustomLogoutDelay > TimeSpan.Zero )
            //    Console.WriteLine( "Warning: Region {0} has logout delay of {1} minutes.", this.Name, m_CustomLogoutDelay.TotalMinutes );

			bool cannotRun = false;
			ReadBoolean( xml["cannotRun"], "enabled", ref cannotRun, false );
			CannotRun = cannotRun;

			bool cannotMount = false;
			ReadBoolean( xml["cannotMount"], "enabled", ref cannotMount, false );
			CannotMount = cannotMount;

			bool travelRestricted = false;
			ReadBoolean( xml["travelRestricted"], "enabled", ref travelRestricted, false );
			TravelRestricted = travelRestricted;

			bool blockFields = false;
			ReadBoolean( xml["blockFields"], "enabled", ref blockFields, false );
			BlockFields = blockFields;

            int solidHue = -1;
            ReadInt32( xml[ "solidHue" ], "value", ref solidHue, false );
            SolidHue = solidHue;

            int alteredLightLevel = -1;
            ReadInt32( xml[ "alteredLightLevel" ], "value", ref alteredLightLevel, false );
            AlteredLightLevel = alteredLightLevel;

            if( Parent != null && Parent is BaseRegion && ( (BaseRegion)Parent ).AlteredLightLevel != -1 )
                AlteredLightLevel = ( (BaseRegion)Parent ).AlteredLightLevel;

		    int forcedSeason = -1;
            ReadInt32( xml[ "forcedSeason" ], "value", ref forcedSeason, false );
            ForcedSeason = forcedSeason;
            #endregion

            #region [speech system by Derrick]
            string fragName = null;
            ReadString( xml[ "speechFrag" ], "name", ref fragName, false );
            if( fragName != null )
            {
                FieldInfo field = typeof( RegionalFragment ).GetField( fragName, BindingFlags.NonPublic |
                    BindingFlags.Static | BindingFlags.Public | BindingFlags.IgnoreCase );

                if( field == null )
                    Console.WriteLine( "Unknown SpeechFragment instance {0} for Region {1}", fragName, Name );
                else
                    m_SpeechFragment = field.GetValue( typeof( RegionalFragment ) ) as SpeechFragment;

                ReadInt32( xml[ "speechFrag" ], "sophistication", ref m_RegionSophistication, false );

                ReadInt32( xml[ "speechFrag" ], "attitude", ref m_RegionAttitude, false );
            }
            #endregion
			...
			if ( spawning != null )
			{
			    ...
			    #region [regional spawning by Dies Irae]
			    SpawnDensity density = SpawnDensity.Normal;
                if( !ReadEnum( spawning, "density", ref density, false ) )
                {
                    spawning.SetAttribute( "density", density.ToString() );

                    if( !RegionsFileHasBeenModified )
                        RegionsFileHasBeenModified = true;
                }
                Density = density;

                int area = 0;
                if( !ReadInt32( spawning, "area", ref area, false ) )
                {
                    UpdateSizeOfRegion();
                    spawning.SetAttribute( "area", SizeOfRegion.ToString() );
                ...
                int numToSpawn = 0;
                if( !ReadInt32( spawning, "numToSpawn", ref numToSpawn, false ) )
                    spawning.SetAttribute( "numToSpawn", Midgard.Engines.RegionalSpawningSystem.Core.NumToSpawn( this ).ToString() );
			    #endregion
				...
				foreach ( XmlNode node in spawning.ChildNodes )
				{
					...
					if ( el != null )
					{
					    ...
					    string id = null;
					    if( !ReadString( el, "id", ref id, false ) /* || int.Parse( id ) < 1000000 */ )
					    {
                            id = SpawnEntry.NewSpawn;
                            Console.WriteLine( "Warning: SpawnEntry with null or invalid id detected. Its new serial is: {0}.", id );
                            RegionsFileHasBeenModified = true;
                        ...
                        #region regional spawning by Dies Irae
                        SpawnDensity entryDensity = Density;
                        ReadEnum( spawning, "density", ref entryDensity, false );

                        int forceAmount = 0;
                        ReadInt32( el, "forceAmount", ref forceAmount, false );

                        int weight = amount;
                        if( !ReadInt32( el, "weight", ref weight, false ) )
                        {
                            if( !RegionsFileHasBeenModified )
                                RegionsFileHasBeenModified = true;

                            el.SetAttribute( "weight", amount.ToString() );
                        }
                        #endregion
						...
						Direction oDir = SpawnEntry.InvalidDirection;
						ReadEnum( el["direction"], "value", ref oDir, false );
                        ...
                        #region [regional spawning by Dies Irae]
                        entry.Weight = weight;
                        entry.Density = entryDensity;
                        entry.ForceAmount = forceAmount;
                        #endregion
				...
				if ( list.Count > 0 )
				{
				    ...
				    try
				    {
				        Midgard.Engines.RegionalSpawningSystem.Core.HandleSpawnDefinition( this, ref spawning, ref list );
				    }
				    catch ( Exception e )
				    {
				        Console.WriteLine( e );
