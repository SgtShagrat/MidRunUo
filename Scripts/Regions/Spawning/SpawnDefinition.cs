...
namespace Server.Regions
{
		...
		public static SpawnDefinition GetSpawnDefinition( XmlElement xml )
		{
			switch ( xml.Name )
			{
				...
				case "treasureChest":
				{
					...
					BaseTreasureChest.TreasureLevel oLevel = BaseTreasureChest.TreasureLevel.Level2;
					Region.ReadEnum( xml, "level", ref oLevel, false );

					#region mod by Dies Irae
					int hue = 0;
					Region.ReadInt32( xml, "hue", ref hue, false );					
					#endregion

					return new SpawnTreasureChest( itemID, oLevel, (Midgard.Items.ChestHues)hue ); // mod By Dies Irae
				}
				default:
				{
					return null;
	...
	public abstract class SpawnType : SpawnDefinition
	{
        ...
        {
            return new XElement( "object", new XAttribute( "type", Type.Name ) );
        }
        #endregion
	}
	...
	public class SpawnMobile : SpawnType
	{
		...
		protected override void Init()
		{
            ...
            {
                Console.WriteLine( "Error: Invalid constructor parameterless for type \"{0}\".", Type.Name );
	...
	{
		private readonly Midgard.Items.ChestHues m_Hues; // mod by Dies Irae
		...
		public SpawnTreasureChest( int itemID, BaseTreasureChest.TreasureLevel level, Midgard.Items.ChestHues hues ) : base( typeof( BaseTreasureChest ) )
		{
			m_ItemID = itemID;
			m_Level = level;

			m_Hues = hues; // mod by Dies Irae
		}
		...
		{
			#region mod by Dies Irae
			return new Midgard.Items.MidgardTreasureChest( m_Level, m_ItemID, m_Hues, true, true );
	...
	public class SpawnGroupElement
	{
	    ...
	    #region mod by Dies Irae
	    /*
            <spawnDefinitions>                                          // root
                <spawnGroup name="Healers">                             // group
                    <object type="WanderingHealer" weight ="8" />       // group element
                    <group name="TownAnimals" weight ="2" />            // group element
                </spawnGroup>
            </spawnDefinitions>
        */
	    ...
	    public XElement ToXElement()
	    {
	        if( m_SpawnDefinition is SpawnType )
	        {
	            return new XElement( "object", new XAttribute( "type", ( (SpawnType)m_SpawnDefinition ).Type.Name ),
	                                 new XAttribute( "weight", Weight ) );
	        }
	        else if( m_SpawnDefinition is SpawnGroup )
	        {
	            return new XElement( "group", new XAttribute( "name", ( (SpawnGroup)m_SpawnDefinition ).Name ),
	                                 new XAttribute( "weight", Weight ) );
	        }
	        else
	            return null;
	...
	public class SpawnGroup : SpawnDefinition, IComparable<SpawnGroup>
	{
		private static Hashtable m_Table = new Hashtable();
        ...
        public int CompareTo( SpawnGroup other )
        {
        ...
        */
        ...
        public static XElement AllGroupsToXElement()
        {
            List<SpawnGroup> allGroups = Midgard.Engines.RegionalSpawningSystem.Core.GetSpawnGroups();

	        return new XElement( "spawnDefinitions", from spawnGroup in allGroups
	                                                 where spawnGroup != null
	                                                 select spawnGroup.ToXElement() );
	    }

	    public XElement ToXElement()
	    {
	        return new XElement( "spawnGroup", new XAttribute( "name", Name ),
                                                from spawnGroupElement in m_Elements
	                                            where spawnGroupElement != null
	                                            select spawnGroupElement.ToXElement() );
	    }
	    #endregion
	}
}