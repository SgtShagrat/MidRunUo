...
namespace Server.Regions
{
	public class SpawnPersistence : Item
	{
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			{
				writer.Write( /* (int) */ entry.ID ); // mod by Dies Irae: ID is now a string
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			{
                /* int */ string id = /* reader.ReadInt(); */ reader.ReadString();  // mod by Dies Irae: ID is now a string

                // fucking runuo team... a spawn entry remove throw a key not found exception? Funny.
				// SpawnEntry entry = (SpawnEntry) SpawnEntry.Table[id];
                SpawnEntry entry = SpawnEntry.FindSpawnEntry( id );
