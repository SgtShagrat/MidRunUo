...
{
	public class HouseRegion : GuardedRegion // mod by Dies Irae : was BaseRegion
    {
        #region mod by Dies Irae
        public override bool IsDisabled()
        {
            if( m_House != null && TownFieldSign.IsField( m_House, true ) )
                return false;

            if( m_House != null && m_House is TownHouse )
                return false;

            if( m_House != null && m_House.Public && TownSystem.Find( this ) != null )
                return false;
        ...
        {
            m_House.Sign.PublicOverheadMessage( MessageType.Regular, 37, true, "* showing preview *" );

            List<Point2D> blocks = new List<Point2D>();
            m_PreviewItems = new List<Item>();

            foreach( var rect in GetArea( m_House ) )
            ...
            m_House.Sign.PublicOverheadMessage( MessageType.Regular, 37, true, String.Format( "* showing blocks: {0} *", blocks.Count ) );
            ...
            foreach( var p in blocks )
            {
                ...
                item.Movable = false;
                item.Location = new Point3D( p.X, p.Y, avgz + 10 );
                item.Map = Map;

                m_PreviewItems.Add( item );
            }

            Timer.DelayCall( TimeSpan.FromSeconds( 100 ), new TimerCallback( ClearPreview ) );
        }

        public void ClearPreview()
        {
            m_House.Sign.PublicOverheadMessage( MessageType.Regular, 37, true, "* clearing preview *" );
        ...
        public static readonly bool OldSchoolRules = !Core.AOS; // Mod by Dies Irae

		private BaseHouse m_House;

        /* 
         * moved to LoginLogoutEventManager class because 
         * we cant hide Initialize member of GuardedRegion
         * 
        ...
        */
		...
		public override void OnLocationChanged( Mobile m, Point3D oldLocation )
		{
            ...
            else if( !OldSchoolRules && ( m_House.Public || !m_House.IsAosRules ) && m_House.IsBanned( m ) && m_House.IsInside( m ) ) // mod by Dies Irae : pre-aos stuff
            {
            ...
            else if( !OldSchoolRules && m_House.IsAosRules && !m_House.Public && !m_House.HasAccess( m ) && m_House.IsInside( m ) ) // mod by Dies Irae : pre-aos stuff
            {
                m.Location = m_House.BanLocation;
            ...
            else if( !OldSchoolRules && m_House.IsCombatRestricted( m ) && m_House.IsInside( m ) && !m_House.IsInside( oldLocation, 16 ) ) // mod by Dies Irae : pre-aos stuff
            {
                m.Location = m_House.BanLocation;
		...
		public override bool OnMoveInto( Mobile from, Direction d, Point3D newLocation, Point3D oldLocation )
		{
			...
			else if ( !OldSchoolRules && (m_House.Public || !m_House.IsAosRules) && m_House.IsBanned( from ) && m_House.IsInside( newLocation, 16 ) )
			{
			...
			else if ( !OldSchoolRules && m_House.IsAosRules && !m_House.Public && !m_House.HasAccess( from ) && m_House.IsInside( newLocation, 16 ) )
			{
				if( !Core.SE )
					from.SendLocalizedMessage( 501284 ); // You may not enter.
			...
			else if ( !OldSchoolRules && m_House.IsCombatRestricted( from ) && !m_House.IsInside( oldLocation, 16 ) && m_House.IsInside( newLocation, 16 ) )
			{
		...
		{
		    #region mod by Dies Irae
		    // Objects in houses will not decay. 
		    if( !Core.AOS )
		        return false;
		    #endregion
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
            ...
            // Roadkill: strongboxes came a bit later
			else if ( !BaseHouse.OldSchoolRules && e.HasKeyword( 0x27 ) ) // I wish to place a strongbox
			{
				if ( isOwner )
				{
		...
		public override bool OnDoubleClick( Mobile from, object o )
		{
            ...
            // You will no longer be able to open containers that are withing a house from outside
		    #region mod by Dies Irae
            if( !m_House.IsInside( from ) )
		    {
		        if( o is Item )
		            ((Item)o).SendLocalizedMessageTo( from, 1061637 ); // You are not allowed to access this.
                return false;
		    }
		    #endregion
        ...
        public override bool CanUseStuckMenu( Mobile m )
        {
            if( House != null )
            {
                if( House.IsOwner( m ) || House.IsFriend( m ) || House.IsCoOwner( m ) )
                    return true;
                else
                    return false;
            }
