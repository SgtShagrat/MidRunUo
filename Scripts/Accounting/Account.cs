...
namespace Server.Accounting
{
	public class Account : IAccount, IComparable, IComparable<Account>
	{
        ...
        /// <summary>
        /// Gets if this account is jailed.
        /// </summary>
        public bool Jailed
        {
            get
            {
                JailSystem js = JailSystem.FromAccount( this );
                return js != null && js.Jailed;
            }
        }
        #endregion
		...
		public DateTime Created
		{
			...
			set { m_Created = value; } // mod by Dies Irae
		}
		...
		{
			get { return ( ( m_LastLogin + InactiveDuration ) <= DateTime.Now ); } // mod by Dies Irae
		}
		...
		public TimeSpan TotalGameTime
		{
            ...
            set { m_TotalGameTime = value; } // mod by Dies Irae
		}
        ...
        public void SetPassword( string plainPassword, bool mail )
		{
            ...
            if( mail )
            {
                MailMessage message = new MailMessage( "staff@midgardshard.it", Email, "Account password reset", "Your new password on midgardshard is: " + plainPassword );
                Misc.Email.Send( message );
            }
		}
		...
		public Account( string username, string password , string email ) // modifica by Berto
		{
			m_Username = username;
			
			SetPassword( password );
			m_Email = email; // modifica by Berto
		...
		public Account( XmlElement node )
		{
			...
			m_Email = Utility.GetText( node["email"], "empty" );
			switch ( AccountHandler.ProtectPasswords )
			{
				case PasswordProtection.None:
					{
		    ...
		    CheckExpiredComments(); // mod by Dies Irae

			Accounts.Add( this );
		}

        #region mod by Dies Irae
        public void CheckExpiredComments()
        {
            try
            {
                using( StreamWriter op = new StreamWriter( "Logs/accountsCleanup.log", true ) )
                {
                    for( int i = 0; i < m_Comments.Count; i++ )
                    {
                        AccountComment comment = m_Comments[ i ];

                        if( comment.AddedBy == JailorWarnerName && comment.LastModified < DateTime.Now - JailWarningCleanUpSpan )
                        {
                            op.WriteLine( "{0} account: {1} comment: {2}", DateTime.Now, Username, comment.Content );
        ...
        #region mod by Dies Irae
        public static readonly TimeSpan JailWarningCleanUpSpan = TimeSpan.FromDays( 5.0 );
        public static readonly string JailorWarnerName = "JailSystem-warning";
        #endregion
		...
		public int Limit
		{
			get { return ( Core.AOS ? 6 : 3 /* 5 */ ); } // mod by Dies Irae
		}
