...
namespace Server.Movement
{
	public class MovementImpl : IMovementImpl
	{
		...
		private bool Check( Map map, Mobile m, List<Item> items, int x, int y, int startTop, int startZ, bool canSwim, bool cantWalk, out int newZ )
		{
			...
			bool ignoreDoors = ( m_AlwaysIgnoreDoors || !m.Alive || m.Body.BodyID == 0x3DB || m.IsDeadBondedPet || m.AccessLevel > AccessLevel.Player ); // mod by Dies Irae
			...
			if ( checkDiagonals )
			{
				...
				for ( int i = 0; i < sectors.Count; ++i )
				{
					...
					for ( int j = 0; j < sector.Items.Count; ++j )
					{
					    ...
					    if( m.AccessLevel > AccessLevel.Counselor && item is BaseDoor )
					        continue;
					    #endregion
