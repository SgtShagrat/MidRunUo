	...
	public class ChampionSpawn : Item
	{
		...
		public void AdvanceLevel()
		{
			...
			if( Level < 16 )
			{
                ...
                if( Type == ChampionSpawnType.Labyrinth )
                {
                    if( Map != null && Map != Map.Internal )
                    {
                        ...
                        if( r != null )
                        {
                            ...
                            if( Level >= 16 )
                                message = 1073369; // A minotaur captain bellows, "Meraktus has arrived!"
                            else if( Level >= 12 )
                                message = 1073368; // A minotaur captain barks an order to his fellow scouts, "Aid the others! Kill the invaders!"
                            else if( Level >= 8 )
                                message = 1073367; // A minotaur scout barks an order to his fellow minotaurs, "Invaders are among us! Crush them!"
                            else if( Level >= 4 )
                                message = 1073370; // A minotaur bellows, "I see invaders! Must warn the others!"
...
//			while( m_Creatures.Count < (200 - (GetSubLevel() * 40)) )
			while( m_Creatures.Count < (100 - (GetSubLevel() * 10)) )
			#endregion
			{
				Mobile m = Spawn();
