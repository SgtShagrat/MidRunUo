...
namespace Server.Engines.BulkOrders
{
	...
	public class SmallSmithBOD : SmallBOD
	{
		...
		public static SmallSmithBOD CreateRandomFor( Mobile m )
		{
			...
			if( BodHelper.GeneralBodMode == BodHelper.BodMode.Midgard )
			{
                if( m.Skills[ SkillName.ArmsLore ].Value > 30 && m.Skills[ SkillName.ItemID ].Value > 30 )
                {
                    if( m is Midgard2PlayerMobile && ( (Midgard2PlayerMobile)m ).LastSmithBulkOrderValue > BodHelper.SmithPointsLimit )
                        return BodHelper.CreateRandomSmallBlackFor( m );
                }
			}
			#endregion
		...
		public SmallSmithBOD( SmallBulkEntry entry, BulkMaterialType material, int amountMax, bool reqExceptional )
		{
			this.Hue = 0x44E;
