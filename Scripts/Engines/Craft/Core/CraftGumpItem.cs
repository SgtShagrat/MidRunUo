...
namespace Server.Engines.Craft
{
	public class CraftGumpItem : Gump
	{
		...
		public CraftGumpItem( Mobile from, CraftSystem craftSystem, CraftItem craftItem, BaseTool tool ) : base( 40, 40 )
		{
            ...
            if( craftItem.RequiredTownSystem != null )
            {
                bool canTownCraft = from != null && ( TownSystem.Find( from ) == craftItem.RequiredTownSystem );
                int color = canTownCraft ? OldGold32 : Red32;
                AddHtml( 170, 302 + (m_OtherCount++ * 20), 310, 18, Color( RequiredTownSystemMessage( craftItem.RequiredTownSystem ), color ), false, false );
            }

            if( craftItem.RequiredRace != null )
            {
                bool canRaceCraft = ( from != null && ( from.Race == craftItem.RequiredRace ) );
                int raceColor = canRaceCraft ? GreenYellow32 : Red32;
                AddHtml( 170, 302 + (m_OtherCount++ * 20), 310, 18, Color( RequiredRaceMessage( craftItem.RequiredRace ), raceColor ), false, false );
            }
            #endregion
        }
        ...
        protected const int Red32 = 0xFF0000;

        private TextDefinition RequiredTownSystemMessage( TownSystem system )
        {
            return String.Format( "* Requires the {0} Townstate", system.Definition.TownName );
        }

        private TextDefinition RequiredRaceMessage( Race race )
        {
            return String.Format( "* Required race: {0}", race.Name );
        }
