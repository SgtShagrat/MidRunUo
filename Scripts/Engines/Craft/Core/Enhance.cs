			...
			if ( lastRes >= 0 && lastRes < subRes.Count )
			{
				...
				if ( from.Skills[craftSystem.MainSkill].Value < res.RequiredSkill )
				{
                    if( !Core.AOS && craftSystem.SupportOldMenu )
                        OldCraftMenu.DisplayTo( from, craftSystem, tool, res.Message ); // mod by Dies Irae
                    else
				...
				else
				{
					...
					{
                        if( !Core.AOS && craftSystem.SupportOldMenu )
                            OldCraftMenu.DisplayTo( from, craftSystem, tool, 1061010 ); // mod by Dies Irae
                        else
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( targeted is Item )
				{
                    ...
                    if( !Core.AOS && m_CraftSystem.SupportOldMenu )
                        OldCraftMenu.DisplayTo( from, m_CraftSystem, m_Tool, 1061010 ); // mod by Dies Irae
                    else
