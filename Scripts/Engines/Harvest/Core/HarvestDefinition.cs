...
namespace Server.Engines.Harvest
{
	public class HarvestDefinition
	{
		...
		public HarvestBank GetBank( Map map, int x, int y, int z, int tileID )
		{
            return GetBank( map, x, y, z, tileID, null );
		}

		public HarvestBank GetBank( Map map, int x, int y, int z, int tileID, object targeted )
		{
            ...
            #region mod by Dies Irae: modifica by Dies Irae per le banche con delay differente
            if( bank == null || bank.Vein == null )
            {
                if( targeted != null )
                {
                    double minDelay = MinRespawn.TotalMinutes;
                    double maxDelay = MaxRespawn.TotalMinutes;

                    HarvestSpawnDefinition.GetRespawnDelay( this, targeted, out minDelay, out maxDelay );

                    banks[ key ] = bank = new HarvestBank( this, GetVeinAt( map, x, y, z, tileID ), minDelay, maxDelay );
                }
                else
                    banks[ key ] = bank = new HarvestBank( this, GetVeinAt( map, x, y, z, tileID ) );
            }
            #endregion
		...
		public HarvestVein GetVeinAt( Map map, int x, int y,int z, int tileID )
		{
			if ( m_Veins.Length == 1 )
				return m_Veins[0];

			#region modifica by Dies Irae
			if( m_Skill == SkillName.Lumberjacking )
				return Lumberjacking.System.GetVeinAt( this, map, x, y, z, tileID );
			else if( m_Skill == SkillName.Mining && Core.AOS )
				return Mining.System.GetVeinAt( this, map, x, y, z, tileID );
            else if( m_Skill == SkillName.Mining )
                return Mining.System.GetRegionalVein( this, map, x, y, z, tileID );
			#endregion

			return GetVeinAt( map, x, y );
		}
		...
		{
			return GetBonusResource( null );
		}
		#endregion

		public BonusHarvestResource GetBonusResource( Mobile from ) // modifica by Dies Irae per testare il livello di from
		{
			...
			if( from != null && from.AccessLevel == AccessLevel.Developer )
				randomValue = 90.0 + Utility.RandomDouble() * 10.0;
			#endregion
