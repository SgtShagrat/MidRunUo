...
namespace Server.Engines.PartySystem
{
	public class Party : IParty
	{
        ...
        [Usage( "ListenToParty" )]
        [Description( "Enables party listening for staff members" )]
        ...
        private delegate void ContextCallback( Mobile player );
        ...
        private class CallbackEntry : ContextMenuEntry
        {
            ...
            public CallbackEntry( int number, ContextCallback callback, Mobile player )
        ...
        private static void OnPartyLeft( Mobile from )
        {
            PartyCommands.Handler.OnRemove( from, from );

            InvalidateMorphPartyPackBonuses( from );
        }

        private static void OnPartyAdd( Mobile added )
        {
            PartyCommands.Handler.OnAdd( added );

            InvalidateMorphPartyPackBonuses( added );
        }

        private static void OnPartyCanLootMe( Mobile from )
        {
            PartyCommands.Handler.OnSetCanLoot( from, true );
        }

        private static void OnPartyCannotLootMe( Mobile from )
        {
            PartyCommands.Handler.OnSetCanLoot( from, false );
        }

        private static void OnPartyRemove( Mobile removed )
        {
            PartyCommands.Handler.OnRemove( removed, null );

            InvalidateMorphPartyPackBonuses( removed );
        }

        private static void OnPartyDisbanded( Mobile from )
        {
            Party p = Party.Get( from );
            if( p != null )
                p.Disband();
        ...
        private static void InvalidateMorphPartyPackBonuses( Mobile from )
        {
            Morph.MorphContext context = Morph.GetContext( from );
            if( context != null && context.PackBonus )
        ...
        public static void GetSelfContextMenus( Mobile from, Mobile player, List<ContextMenuEntry> list )
        {
            ...
            list.Add( new CallbackEntry( 197, new ContextCallback( OnPartyAdd ), player ) ); // Add Party Member

            if( p == null || !p.Contains( player ) )
                return;

            PartyMemberInfo mi = p[ player ];
            if( mi == null )
                return;

            list.Add( new CallbackEntry( 195, new ContextCallback( OnPartyLeft ), player ) ); // Leave the Party

            if( mi.CanLoot )
                list.Add( new CallbackEntry( 199, new ContextCallback( OnPartyCanLootMe ), player ) ); // Party Can Loot Me
            else
                list.Add( new CallbackEntry( 194, new ContextCallback( OnPartyCannotLootMe ), player ) ); // Party CANNOT Loot Me

            if( p.Leader != null && p.Leader == player )
            {
                list.Add( new CallbackEntry( 198, new ContextCallback( OnPartyRemove ), player ) ); // Remove Party Member
                list.Add( new CallbackEntry( 196, new ContextCallback( OnPartyDisbanded ), player ) ); // Disband the Party
            }
        }
        #endregion
