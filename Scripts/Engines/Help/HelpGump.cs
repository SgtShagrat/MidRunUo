		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 2: // Stuck
				{
                    ...
                    else if( from.Region.IsPartOf( typeof( Regions.HouseRegion ) ) && !from.Region.CanUseStuckMenu( from ) )
                    {
                        from.SendMessage( "Thou cannot do that in a house. Try to ask the house owner to let you leave." );
                    }
                    else if ( from.Region.IsPartOf( typeof( Midgard.Engines.MidgardTownSystem.TownJail ) ) )
					{
						from.SendLocalizedMessage( 1041530, "", 0x35 ); // You'll need a better jailbreak plan then that!
					}
                    #endregion
					else if ( from.CanUseStuckMenu() && from.Region.CanUseStuckMenu( from ) && !CheckCombat( from ) && !from.Frozen && !from.Criminal /* && (Core.AOS || from.Kills < 5) */ ) // mod by Dies Irae: abilitato lo stuck ai reds
					{
				...
				case 9: // Young player transport
				{
					if ( IsYoung( from ) )
					{
						...
						else
						{
		                    ...
		                    from.MoveToWorld( new Point3D( 2230, 1224, 0 ), Map.Felucca ); // Cove
		                    // from.MoveToWorld( new Point3D( 3503, 2574, 14 ), Map.Trammel );
							#endregion
						}
					}
