...
namespace Server.Menus.Questions
{
	...
	public class StuckMenu : Gump
	{
        ...
        private static readonly StuckMenuEntry[] m_MidgardEntries = new StuckMenuEntry[]
	    {
	        // Cove
		...
		public StuckMenu( Mobile beholder, Mobile beheld, bool markUse ) : base( 150, 50 )
		{
            ...
            StuckMenuEntry[] entries = IsInSecondAgeArea( beheld ) ? m_T2AEntries : /* m_Entries */ m_MidgardEntries; // mod by Dies Irae

            #region mod by Dies Irae
            /*
            TownSystem t = TownSystem.Find( beheld );
            entries = t != null ? t.GetStuckEntries() : TownSystem.GetDefaultStuckEntries();
            */
            #endregion
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			else
			{
				...
				StuckMenuEntry[] entries = IsInSecondAgeArea( m_Mobile ) ? m_T2AEntries : /* m_Entries */ m_MidgardEntries; // mod by Dies Irae
				//StuckMenuEntry[] entries = IsInSecondAgeArea( m_Mobile ) ? m_T2AEntries : m_Entries;

                #region mod by Dies Irae
               /* TownSystem t = TownSystem.Find( m_Mobile );
                entries = t != null ? t.GetStuckEntries() : TownSystem.GetDefaultStuckEntries();*/
                #endregion
		...
		private class TeleportTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				else
				{
					...
					#region modifica by Dies Irae per non far finire i pg su Trammel
					/*
					Map destMap;
					...
					*/
					...
					#region modifica by Dies Irae per il log del uso degli stuks help
					try
					{
						System.IO.TextWriter tw = System.IO.File.AppendText("Logs/Midgard2StuckMenuUseLog.txt");
						tw.WriteLine( "Il pg {0} (account {1}) ha usato lo stuck menu in locazione {2} su mappa {3}, in data {4} alle ore {5}.",
						              m_Mobile.Name, m_Mobile.Account.Username, m_Mobile.Location.ToString(), m_Mobile.Map.Name,
						              DateTime.Now.ToShortDateString(), DateTime.Now.ToShortTimeString() );
						tw.Close();
					}
					catch( Exception ex )
					{
						Console.Write("Stuck menu Log Failed due to Exception: {0}", ex);
					}					
					#endregion
