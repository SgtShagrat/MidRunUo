...
namespace Server.Spells.First
{
	public class HealSpell : MagerySpell
	{
		...
		public void Target( Mobile m )
		{
			...
			else if ( Core.AOS && ( m.Poisoned || MortalStrike.IsWounded( m ) ) ) // mod by Dies Irae
			{
			...
			else if ( CheckBSequence( m ) )
			{
				...
				{
					/*
					var magery := cint(ret[1]/10)+1;
					var die_string := "1d6";
					var healed := RandomDiceRoll( die_string )+magery ;
					*/

					toHeal = ( (int)(Caster.Skills[SkillName.Magery].Value * 0.1) ) + 1;
					toHeal += DiceRoll.Roll( "1d6" );
				}
				#endregion
				else
				{
				...
				// mod by Dies Irae
				DruidEmpowermentSpell.AddHealBonus( Caster, ref toHeal );
