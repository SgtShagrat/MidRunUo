...
namespace Server.Spells.Fourth
{
	public class GreaterHealSpell : MagerySpell
	{
		...
		public void Target( Mobile m )
		{
			...
			else if ( Core.AOS && ( m.Poisoned || Server.Items.MortalStrike.IsWounded( m ) ) ) // mod by Dies Irae
			{
			...
			else if ( CheckBSequence( m ) )
			{
				...
				{
					/* POL script
					var magery := cint(ret[1]/5);
					var die_string := magery+"d2+20";
					var healed := RandomDiceRoll( die_string );
					*/

					toHeal = DiceRoll.Roll( new DiceRoll( (int) ( Caster.Skills[ SkillName.Magery ].Value / 5.0 ), 2, 20 ) );
				}
				#endregion
