...
namespace Server.Spells.Fourth
{
	public class FireFieldSpell : MagerySpell
	{
		...
		public void Target( IPoint3D p )
		{
			...
			else if ( MidgardSpellHelper.CheckBlockField( p, Caster ) && SpellHelper.CheckTown( p, Caster ) && CheckSequence() )
			{
				...
				if ( Core.AOS )
					...
					duration = TimeSpan.FromSeconds( magery* 0.1 );//TimeSpan.FromSeconds( 4.0 + (Caster.Skills[SkillName.Magery].Value * 0.5) );
		...
		[NoSameNearbyItem] // mod by Dies Irae
		[DispellableField]
		public class FireFieldItem : Item
		{
			...
			private static double DamageScalar = 5.0; // mod by Dies irae

			public override bool OnMoveOver( Mobile m )
			{
				#region mod by Dies Irae
				if( MidgardSpellHelper.CheckResisted( m_Caster, m, SpellRegistry.GetSpellByType( typeof( FireFieldSpell ) ) as MagerySpell, false, false, false ) )
				{
					if( m.Player )
				...
				{
					if( Visible && m_Caster != null )//edit by Arlas
					{
						...
						int damage = m_Damage;//(int)( m_Damage * ( m.Player ? DamageScalar : 1.0 ) );
						...
						*/
						...
						if( Core.AOS )
							...
							MidgardSpellHelper.Damage( m, m_Caster, damage, CustomResType.Fire ); // mod by Dies Irae
					...
					else
					{
						...
						if ( map != null && caster != null )
						{
							...
							while ( m_Queue.Count > 0 )
							{
								...
								int damage = m_Item.m_Damage;//(int)( m_Item.m_Damage * ( m.Player ? DamageScalar : 1.0 ) ); // mod by Dies irae
