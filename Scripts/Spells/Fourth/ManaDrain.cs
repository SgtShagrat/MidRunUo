		...
		public void Target( Mobile m )
		{
			...
			else if ( CheckHSequence( m ) )
			{
				...
				if ( Core.AOS )
				{
                    ...
                    #region mod by Dies Irae
                    if( !Core.AOS )
                        toDrain = MidgardSpellHelper.ScaleByCustomRes( toDrain, m, CustomResType.Mental );
                    #endregion
				...
				{
					//dmg:=randomdiceroll((magery/6)+"d4");
					int manaleech = DiceRoll.Roll( new DiceRoll( (int) ( Caster.Skills[ SkillName.Magery ].Value / 6.0 ), 4, 0 ) );
					if ( manaleech > m.Mana )
						manaleech = m.Mana;
					if ( CheckResisted( m ) )
					...
					//	m.Mana -= Utility.Random( 1, 100 );
						...
						m.Mana -= manaleech;//Utility.Random( 1, m.Mana );
