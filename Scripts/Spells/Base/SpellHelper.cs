...
namespace Server.Spells
{
	...
	public class SpellHelper
	{
		...
		{
			if( sp!= null || !sp.DelayedDamage )// crash fix by magiusche
		...
		public static bool AddStatCurse( Mobile caster, Mobile target, StatType type, int curse, TimeSpan duration )
		{
            ...
            #region mod by Dies Irae [custom resistances]
            if( !Core.AOS && target.Player )
            {
                int scaled = MidgardSpellHelper.ScaleByCustomRes( curse, target, CustomResType.Mental );
                if( target.PlayerDebug )
                {
                    int res = target.CustMentalResistance;
                    target.SendMessage( "Debug: The curse has been reduced to {0} from {1} due to {2}% of mentalresist.", scaled, curse, res );
                }
        ...
        {
            return ValidIndirectTarget( from, to, true, false, false );
        }
        #endregion

        public static bool ValidIndirectTarget( Mobile from, Mobile to, bool ignoreCitizenStatus, bool ignoreGuildStatus, bool ignorePartyStatus )
		{
            if( from == to )
                return true;
            ...
            if( !ignoreCitizenStatus )
            {
                if( Midgard.Engines.MidgardTownSystem.Notoriety.AreCoCitizens( from, to ) && !TownSystem.IsInAnyTownArena( to ) )
                    return false;
            }
            #endregion
            ...
            #region mod by Dies Irae
            if( to is BaseCreature && ( (BaseCreature)to ).IsHumanInTown() )
                return false;
            #endregion
			...
			return ( noto != Notoriety.Innocent || noto == Notoriety.Murderer /*|| from.Kills >= 5*/ ); // modifica by Dies Irae
		}
        ...
        #region mod by Dies Irae
        public static bool CheckUniqueSummon( Type t, Mobile m, bool message )
        {
            PlayerMobile caster = m as PlayerMobile;
            if( caster == null )
                return true;

            foreach( Mobile mob in caster.AllFollowers )
            {
                if( mob != null && !mob.Deleted && mob.GetType() == t )
                {
                    if( message )
                        caster.SendMessage( caster.Language == "ITA"? "Puoi evocare solo una creatura di questo tipo." : "Thou may summon only one of this creature at time." );
		...
		private static TravelValidator[] m_Validators = new TravelValidator[]
			{
				...
				#region modifica by dies irae per impedire il recall\gate da mappe
				new TravelValidator( IsMalas ),
				new TravelValidator( IsTokuno ),
				new TravelValidator( IsTrammel ),
				#endregion
			};
		...
		private static bool[,] m_Rules = new bool[,]
			{
		#region modifica by Dies Irae
						/*T2A(Fel)		Ilshenar		Wind(Tram),	Wind(Fel),	Dungeons(Fel),	Solen(Tram),	Solen(Fel), CrystalCave(Malas),	Gauntlet(Malas),	Gauntlet(Ferry),	Stronghold,		ChampionSpawn, Dungeons(Tokuno[Malas])   LampRoom(Doom),	GuardianRoom(Doom) Malas 			Tokuno		Trammel */
/* Recall From */	{ false,			false,			false,			false,		false,			false,			false,		false,					false,				false,				true,			true,			false,			false,			false,		false,			true,		false	 },
...
/* Tele From */		{  true,			true,			false,			true,		true,			false,			true,		false,					true,				true,				false,			true,			true,			true,			true,		true,			true,		false	 },
/* Tele To */		{  true,			true,			false,			true,		true,			false,			true,		false,					true,				false,				false, 			true,			true,			true,			true,		true,			true,		false	 }
		#endregion	
			};

        public const bool CannotMarkInHouse = true;
        public const bool CannotRecallIntoHouse = true;
		...
		public static bool CheckTravel( Mobile caster, Map map, Point3D loc, TravelCheckType type )
		{
            ...
            #region mod by Dies Irae
            if( type != TravelCheckType.TeleportFrom && type != TravelCheckType.TeleportTo )
            {
                if( CheckIsEnemyTown( caster, map, loc ) ) // sono citta' in guerra...
                {
                    caster.SendMessage( "You cannot cast a travel spell in an opponent town." );
                    isValid = false;
                }

                if( IsTravelRestrictedRegion( caster, map, loc ) )
                    isValid = false;
            }
            #endregion
		...
		#region modifica by dies irae per impedire il recall\gate da mappe
		public static bool IsFelucca( Map map, Point3D loc )
		{
			return ( map == Map.Felucca );
		}	
		
		public static bool IsMalas( Map map, Point3D loc )
		{
			return ( map == Map.Malas );
		}	
        ...
        public static bool IsTravelRestrictedRegion( Mobile caster, Map map, Point3D loc )
        {
            if( caster != null && caster.Region != null )
            {
                if( caster.Region is BaseRegion && ( (BaseRegion)( caster.Region ) ).TravelRestricted )
                    return true;
            }
            ...
            if( r != null )
            {
                if( r is BaseRegion && ( (BaseRegion)( r ) ).TravelRestricted )
                    return true;
            }
		...
		public static bool CheckIsEnemyTown( Mobile m, Map map, Point3D loc )
		{
            return Midgard.Engines.MidgardTownSystem.Notoriety.AreSystemsAtWar( TownSystem.Find( m ), TownSystem.Find( loc, map ) );
		}
		#endregion
        ...
        #region mod by Dies Irae
        public static bool IsTown( Point3D loc, Mobile caster, bool checkEnabled )
        {
            Map map = caster.Map;
            ...
            return !checkEnabled || !reg.IsDisabled();
        }
        #endregion
			...
			if( target.MagicDamageAbsorb > 0 )
			{
                ...
                */
				...
				if( reflect )
				{
                    ...
                    if( target.Player )
                        target.SendMessage( "Your magic reflect spell saved you!" );
		...
		{
		    #region team runuo di merda! mod by dies irae 
		    if( from == null )
		        return;
		    #endregion
