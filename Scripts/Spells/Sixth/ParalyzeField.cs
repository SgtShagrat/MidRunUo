...
namespace Server.Spells.Sixth
{
	public class ParalyzeFieldSpell : MagerySpell
	{
		...
		public void Target( IPoint3D p )
		{
            ...
            else if( !MidgardSpellHelper.CheckNoMoreFieldsNearby( Caster.Map, new Point3D( p ), 2 ) )
            {
                Caster.SendMessage( "You cannot cast more than one of those nearby!" );
            }
		...
		public class InternalItem : Item
		{
			...
			public override bool OnMoveOver( Mobile m )
			{
				if ( Visible && m_Caster != null )
				{
					...
					if( MidgardSpellHelper.CheckResisted( m_Caster, m, SpellRegistry.GetSpellByType( typeof( ParalyzeSpell ) ) as MagerySpell, false, false, false ) )
					{
						if( m.Player )
					...
					{
                        if( DateTime.Now + TimeSpan.FromSeconds( duration ) > m_End )
                            duration = ( m_End - DateTime.Now ).TotalSeconds;
