...
namespace Server.Spells.Sixth
{
	public class MarkSpell : MagerySpell
	{
		...
		public void Target( RecallRune rune )
		{
			...
			else if ( SpellHelper.CheckMulti( Caster.Location, Caster.Map, SpellHelper.CannotMarkInHouse ) )
			{
				Caster.SendLocalizedMessage( 501942 ); // That location is blocked.
			}
            // mod by Dies Irae
            // http://forum.uosecondage.com/viewtopic.php?f=7&t=4743
            // Recall runes no longer need to be in your pack to be marked
			else if ( Core.AOS && !rune.IsChildOf( Caster.Backpack ) )
			{
            ...
            else if( !Core.AOS && ( !rune.IsAccessibleTo( Caster ) || !Caster.InRange( rune.GetWorldLocation(), 3 ) ) )
            {
                Caster.SendLocalizedMessage( 502436 ); // That is not accessible.
            }
