		...
		public void Target( IPoint3D p )
		{
			...
			else if ( CheckSequence() )
			{
				...
				for ( int i = 0; i < targets.Count; ++i )
				{
					...
					{
						dispelChance = 0.3 + (Caster.Skills.Magery.Value - bc.DispelDifficulty)/200.0;
					}

					if (!Caster.Player)
						dispelChance -= 0.2;

					#region Mod by Magius(CHE): Dispeal means HARMFUL
					var master = bc.GetMaster();
					if (master!=null && Caster.CanAreaHarmful( master ) )//&& master!=Caster && Caster.CanBeHarmful( master, false ))
						Caster.DoHarmful( master );
					#endregion
