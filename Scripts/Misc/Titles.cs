...
namespace Server.Misc
{
	public class Titles
	{
		...
		public static string ComputeTitle( Mobile beholder, Mobile beheld )
		{
            ...
            bool showSkillTitle = beheld.ShowFameTitle && ( ( beholder == beheld ) || ( fame >= 5000 || !Core.AOS ) ); // mod by Dies Irae
				...
				for ( int i = 0; i < m_FameEntries.Length; ++i )
				{
					...
					if ( fame <= fe.m_Fame || i == (m_FameEntries.Length - 1) )
					{
						...
						for ( int j = 0; j < karmaEntries.Length; ++j )
						{
							...
							{
                                if( !SketchGump.IsCamuflated( beheld ) ) // mod by Dies Irae
                                {
		...
		private static Skill GetHighestSkill( Mobile m )
		{
            ...
            if( !Core.AOS && m is Midgard2PlayerMobile )
            {
                int index = (int)( (Midgard2PlayerMobile)m ).CustomJobTitle;
                if( index > -1 )
                    return m.Skills[ index ];
                else
        ...
        #region mod by Dies Irae
        private static readonly int[] m_KarmaLevels = new int[] { -10000, -5000, -2500, -1250, -625, 624, 1249, 2499, 4999, 9999, 10000 };

        public static int GetKarmaLevel( Mobile m )
        {
            int karma = m.Karma;

            for( int j = 0; j < m_KarmaLevels.Length - 1; ++j )
            {
                if( karma > m_KarmaLevels[ j ] && karma < m_KarmaLevels[ j + 1 ] )
                    return j + 1;
            }
