	...
	public class Guild : BaseGuild
	{
		...
		private class GuildPropsTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object o )
			{
				...
				{
					from.SendMessage( from.Language == "ITA" ? "Non è in una gilda!" : "That is not in a guild!" );
				}
			}
		}
		#endregion
		...
		public Guild( Mobile leader, string name, string abbreviation )
		{
			...
			GuildsHelper.InitializeMidgardGuild( this );
			GuildsHelper.StringToLog( string.Format( "Guild {0} created in date {1} and time {2}.", Name, DateTime.Now.ToShortDateString(), DateTime.Now.ToShortTimeString() ));
			#endregion
		}
		...
		public void Disband()
		{
			...
			#region modifica by Dies Irae
			GuildsHelper.ClearGuildUniform( this );
			GuildsHelper.StringToLog( string.Format( "Guild {0} has been disbanded in date {1} and time {2}.", Name, DateTime.Now.ToShortDateString(), DateTime.Now.ToShortTimeString() ));
			#endregion
		}
		...
		#region modifica by Dies Irae
	    public ArrayList GuildUniformInstances { get; set; }

	    public List<UniformEntry> UniformList { get; set; }
		...
		private MidgardGuildTypes m_MidgardGuildType;

		[CommandProperty( AccessLevel.Administrator )]
		public MidgardGuildTypes MidgardGuildType
		{
			get { return m_MidgardGuildType; }
			set
			{
		    	MidgardGuildTypes oldValue = m_MidgardGuildType;
		    	if( oldValue != value )
		    	{
					m_MidgardGuildType = value;
					
					OnMidgardGuildTypeChanged( oldValue );
		    	}
			}
		}
		...
		public virtual void OnTownChanged( MidgardTowns oldValue )
		{
			...
			GuildsHelper.StringToLog( string.Format( "Guild {0} changed town in date {1} and time {2}.", Name, DateTime.Now.ToShortDateString(), DateTime.Now.ToShortTimeString() ));

			if( m_Guildstone != null )
			{
				m_Guildstone.PublicOverheadMessage( Network.MessageType.Regular, 15, true, string.Format( "* town for this guild changed to {0} *", m_Town.ToString() ) ) ;
				m_Guildstone.InvalidateProperties();
			}

			foreach ( Mobile m in m_Members )
				m.SendMessage( m.Language == "ITA" ? "La città di gilda è stata modificata in {0}." : "Your guild town changed to {0}.", m_Town.ToString() );
			...
			foreach ( Mobile m in m_Members )
			{
				if( m != null && !m.Deleted )
				{
					if( !GuildsHelper.CanJoinMidgardGuild( m, this ) )
					{
						m.SendMessage( "Because of your guild town change and you actual town signature you have been rejected from that guild.", m_Town.ToString() );
						RemoveMember( m );
					}
				}
			}
		...
		public virtual void OnMidgardGuildTypeChanged( MidgardGuildTypes oldValue )
		{
			if( Disbanded )
				return;

			GuildsHelper.StringToLog( string.Format( "Guild {0} changed midgardGuildType in date {1} and time {2}.", Name, DateTime.Now.ToShortDateString(), DateTime.Now.ToShortTimeString() ));

			if( m_Guildstone != null )
			{
				m_Guildstone.PublicOverheadMessage( Network.MessageType.Regular, 15, true, string.Format( "* type for this guild changed to {0} *", m_MidgardGuildType.ToString() ) ) ;
				m_Guildstone.InvalidateProperties();
			}

			foreach ( Mobile m in m_Members )
				m.SendMessage( m.Language == "ITA" ? "La tipologia della tua gilda è ora {0}." : "Your guild type changed to {0}.", m_MidgardGuildType.ToString() );
			...
			foreach ( Mobile m in m_Members )
			{
				if( m != null && !m.Deleted )
				{
					if( !GuildsHelper.CanJoinMidgardGuild( m, this ) )
					{
						m.SendMessage( "Because of your guild type change and your guild account status you have been rejected from that guild.", m_Town.ToString() );
						RemoveMember( m );
					}
				}
			}
        ...
        private Dictionary<Mobile, ExpireGuildEnemyTimer> m_TempEnemies;
 
        public bool IsTempEnemy( Mobile m )
        {
            if( m_TempEnemies == null )
                return false;

            return m_TempEnemies.ContainsKey( m ) && m_TempEnemies[ m ] != null && m_TempEnemies[ m ].Running;
        }

        public void AddTempEnemy( Mobile m )
        {
            if( m_TempEnemies == null )
                m_TempEnemies = new Dictionary<Mobile, ExpireGuildEnemyTimer>();

            if( m_TempEnemies.ContainsKey( m ) )
            {
                if( m_TempEnemies[ m ] != null )
                    m_TempEnemies[ m ].Stop();
            }

            m_TempEnemies[ m ] = new ExpireGuildEnemyTimer( this, m );
            m_TempEnemies[ m ].Start();

            if( m.PlayerDebug )
                m.SendMessage( 37, "You are enemy of '{0}'.", Name );
        }

        public void RemoveTempEnemy( Mobile m )
        {
            if( m_TempEnemies == null || !m_TempEnemies.ContainsKey( m ) )
                return;

            ExpireGuildEnemyTimer timer = m_TempEnemies[ m ];
            if( timer != null )
                timer.Stop();

            m_TempEnemies.Remove( m );

            if( m.PlayerDebug )
                m.SendMessage( 37, "You are NOT enemy of '{0}' anymore.", Name );
        }

        private static readonly TimeSpan m_ExpireGuildEnemyDelay = TimeSpan.FromMinutes( 2.0 );

        private class ExpireGuildEnemyTimer : Timer
        {
            private readonly Guild m_Guild;
            private readonly Mobile m_Mobile;

            public ExpireGuildEnemyTimer( Guild guild, Mobile m )
                : base( m_ExpireGuildEnemyDelay )
            {
                Priority = TimerPriority.FiveSeconds;
            ...
            protected override void OnTick()
            {
                m_Guild.RemoveTempEnemy( m_Mobile );
            }
        }
		#endregion
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			#region modifica by Dies Irae: Uniform
			writer.Write( (int)m_Town );
			writer.Write( (int)m_MidgardGuildType );

			if( UniformList == null )
				UniformList = new List<UniformEntry>();

			writer.Write( UniformList.Count );

			for( int i = 0; i < UniformList.Count; i++ )
				UniformList[i].Serialize( writer );

			if( GuildUniformInstances == null )
				GuildUniformInstances = new ArrayList();

			writer.WriteItemList( GuildUniformInstances, true );
			#endregion
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 6:
				{
					...
					m_MidgardGuildType = (MidgardGuildTypes)reader.ReadInt();

					if( UniformList == null )
						UniformList = new List<UniformEntry>();

					int uniformListCount = reader.ReadInt();

					for( int i = 0; i < uniformListCount; i++ )
						UniformList.Add( new UniformEntry( reader ) );

					if( GuildUniformInstances == null )
						GuildUniformInstances = new ArrayList();

					GuildUniformInstances = reader.ReadItemList();
		...
		private void VerifyGuild_Callback()
		{
			...
			m_Members.Sort( InternalComparer.Instance ); // mod by Dies Irae
		}
		...
		public void AddMember( Mobile m )
		{
			...
			if ( !m_Members.Contains( m ) )
			{
				...
				#region modifica by Dies Irae
				GuildsHelper.StringToLog( string.Format( "Player {0} joined guild {1} town in date {2} and time {3}.", m.Name, Name, DateTime.Now.ToShortDateString(), DateTime.Now.ToShortTimeString() ));
				#endregion
		...
		public void RemoveMember( Mobile m, int message )
		{
			if ( m_Members.Contains( m ) )
			{
				...
				#region modifica by Dies Irae
				GuildsHelper.StringToLog( string.Format( "Player {0} removed from guild {1} town in date {2} and time {3}.", m.Name, Name, DateTime.Now.ToShortDateString(), DateTime.Now.ToShortTimeString() ));
				GuildsHelper.DeleteUniformForMember( m, this );
				GuildsHelper.DoRejoinPrevention( m );
				#endregion
		...
		#region modifica by Dies Irae
		public static readonly bool AsciiGuildChat = false;
		#endregion
		...
		public void GuildChat( Mobile from, int hue, string text )
		{
			...
			for( int i = 0; i < m_Members.Count; i++ )
			{
				...
				if( state != null )
				{
					...
					{
                        // mod by Dies Irae
                        // When you are under the effects of Incognito or the disguise kit, 
                        // your disguised name will now appear in guild chat. (was real name)
                        // http://forum.uosecondage.com/viewtopic.php?f=7&t=1241
						#region modifica by Dies Irae
						if( AsciiGuildChat )
							p = Packet.Acquire( new AsciiMessage( Serial.MinusOne, -1, MessageType.Regular, 0x0D, 3, "System", string.Format( "[Guild][{0}]: {1}", from.NameMod != null ? from.NameMod : from.Name, text ) ) );
						else						
							p = Packet.Acquire( new UnicodeMessage( from.Serial, from.Body, MessageType.Guild, hue, 3, from.Language, from.NameMod != null ? from.NameMod : from.Name, text ) );
						#endregion
					}
