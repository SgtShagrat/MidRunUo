...
namespace Server
{
	public class AOS
	{
		...
		public static int Damage( Mobile m, Mobile from, int damage, bool ignoreArmor, int phys, int fire, int cold, int pois, int nrgy, int chaos, int direct, bool keepAlive, bool archer )
		{
			...
			if( Core.AOS && from != null && !from.Deleted && from.Alive )
			{
				int reflectPhys = AosAttributes.GetValue( m, AosAttribute.ReflectPhysical );

				#region modifica by Dies Irae
				/* Prima del fix dato 'from' che danneggia 'm'
				 * se 'm' ha un blood oath con 'from' ed in aggiunta ha anche 'reflect physical damage'
				 * si avevano due riflessioni del danno: una per lo spell ed una per reflect physical.
				 * 
				 * GetBloodOath() ritorna l'eventuale necromante al quale 'from' è linkato.
				 * 
				 * Se c'e' il link allora in questa fase va rimosso il reflect della parte fisica del danno
				 * che non sia in esubero al 100% (ovvero a quello che riflette Blood Oath).
				 * In pratica cosi' viene riflesso il massimo tra reflect physical e blood oath.
				 */
				
				if( Spells.Necromancy.BloodOathSpell.GetBloodOath( from ) == m )
				{
					reflectPhys = Math.Max( reflectPhys - 100, 0 );

					if( m.AccessLevel == AccessLevel.Developer )
						m.SendMessage( "Debug BloodOath-RPD: reflectPhys {1}", reflectPhys );
				}
				#endregion
            ...
            if( Core.Debug )
                Console.WriteLine( "Aos.Damage: f:{0} c:{1} p:{2} n:{3} p:{4} dmg: {5} newDmg: {6}", fire, cold, pois, nrgy, phys, damage, totalDamage );
        ...
        public static int Damage( Mobile m, Mobile from, int damage, bool ignoreArmor, int phys, int fire, int cold, int pois, int nrgy, int chaos, int direct, bool keepAlive, bool archer )
        {
            ...
            if( m.Player )
            {
				damage = Midgard.Engines.PVMAbsorbtions.Core.OnDealDamage(from,m,damage /*+ fire + cold + pois + nrgy + chaos*/, (fire + cold + pois + nrgy + chaos > 0)
					                                                          ? Midgard.Engines.PVMAbsorbtions.DamageTypes.Magical
					                                                          : Midgard.Engines.PVMAbsorbtions.DamageTypes.Physical);
                // from.SendMessage( "AosDamage for player: {0}", damage );
                m.Damage( damage, from );
                return damage;
            }
            ...
            int totalDamage = damage * phys * 100;
            totalDamage += damage * fire * ( 100 - m.FireResistance );
            totalDamage += damage * cold * ( 100 - m.ColdResistance );
            totalDamage += damage * pois * ( 100 - m.PoisonResistance );
            totalDamage += damage * nrgy * ( 100 - m.EnergyResistance );
	...
	public sealed class AosAttributes : BaseAttributes
	{
		...
		public static int GetValue( Mobile m, AosAttribute attribute )
		{
			...
			for( int i = 0; i < items.Count; ++i )
			{
				...
				if( obj is BaseWeapon )
				{
                    ...
                    */
					#region modifica By Dies Irae
                    //else
                    //    value += Midgard.AoSHelper.GetResourceBonus( (BaseWeapon)obj, attribute );
					#endregion
				}
				else if( obj is BaseArmor )
				{
                    ...
                    */
                    ...
                    #region modifica By Dies Irae
                    //else
                    //    value += Midgard.AoSHelper.GetResourceBonus( ( (BaseArmor)obj ), attribute );
                    #endregion
				}
			...
			// Dies Irae: RegenHits bonus moved to RegenRates.cs

			// Dies Irae: RegenStam bonus moved to RegenRates.cs

			#region Modifica by Dies Irae per la sfinge
			if( m is Midgard2PlayerMobile )
				value += Midgard.Gumps.FortuneGump.ComputeAoSFortuneBonus( (Midgard2PlayerMobile)m, attribute );
			#endregion

			#region ReflectPhysical
			if( attribute == AosAttribute.ReflectPhysical )
			{
				if( m is PlayerMobile && Midgard.AoSHelper.IsFullMetalArmorSet( (PlayerMobile)m ) )
				{
					// m.SendMessage( "Your armor all made of only one metal gives you a bonus for reflecting physical damage!" );
					value += 20;
				}
			}
			#endregion
	...
	public abstract class BaseAttributes
	{
		...
		public void Serialize( GenericWriter writer )
		{
            ...
            if( m_Values == null )
            {
                Console.WriteLine( "Warning: NULL values in AoS attribute" );
                m_Values = m_Empty;
            }
            #endregion
