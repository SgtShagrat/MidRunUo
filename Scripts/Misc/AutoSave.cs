...
using Midgard.Gumps;
using Midgard.Misc;
using WarSystem = Midgard.Engines.WarSystem;
...
namespace Server.Misc
{
	public class AutoSave : Timer
	{
		...
		protected override void OnTick()
		{
            ...
            if( WarSystem.Core.Instance.WarPending && BaseWar.DisableSavesDuringBattle )
            {
                Config.Pkg.LogInfo( "World saving skipped during a war." );
                return;
            }
            #endregion
		...
		public static void Save()
		{
            ...
            #region Modifica by Dies Irae per il backup giornaliero
            try { DailyBackup.DailySaveBackup( false ); }
            catch( Exception e ) { Console.WriteLine( "WARNING: Daily backup FAILED: {0}", e ); }
            #endregion
            ...
            #region modifica by Dies Irae
			List<NetState> sendList = NetState.Instances;
			foreach ( NetState t in sendList )
			{
			    if( t.Mobile != null )
			        t.Mobile.SendGump( new MidgardSaveGump() );
			}
		    #endregion
            ...
            #region modifica by Dies Irae
			List<NetState> closeList = NetState.Instances;
			foreach ( NetState t in closeList )
			{
			    if( t.Mobile != null )
			        t.Mobile.CloseGump( typeof(MidgardSaveGump) );
			}
		    #endregion
		}
