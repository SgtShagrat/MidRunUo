	...
	public class CharacterCreation
	{
		...
		private static Mobile CreateMobile( Account a )
		{
			...
			for ( int i = 0; i < a.Length; ++i )
			{
				if ( a[i] == null )
				{
					#region Modifica Dies Irae per il Midgard2PlayerMobile
					return (a[i] = new Midgard2PlayerMobile());
					// return (a[i] = new PlayerMobile());
					#endregion fine modifica	
				}
			}
		...
		private static void EventSink_CharacterCreated( CharacterCreatedEventArgs args )
		{
			...
			{
                PreAoSCharacterCreation.Dress( newChar );
			...
			#region Modifica by Dies Irae per la creazione del pg al vilalggio
            //CityInfo city = GetStartLocation( args, young );
			
			//CityInfo cityFinal = new CityInfo( "Cove", "Centro", 2230, 1224, 0, Map.Felucca );
			CityInfo cityFinal = new CityInfo( "Britain", "Centro", 1282, 1340, 15, Map.Felucca );
            /*
			try
			{
				TownSystem system = TownHelper.FindTownSystemFromAccountTag( newChar );
				if( system != null )
				{
					TownPlayerState tps = new TownPlayerState( system, newChar );
					tps.CheckAttach();

                    if( system.Definition.StartCityInfo != null )
                        cityFinal = system.Definition.StartCityInfo;

                    ( (PlayerMobile)newChar ).Young = false;
				}

				((PlayerMobile)newChar).Young = true;
			}
			catch(Exception ex)
			{
				Console.WriteLine( "Errore nello spostamento di un pg alla creazione: {0}", ex);
			}
			*/

			newChar.MoveToWorld( cityFinal.Location, cityFinal.Map );
			#endregion

			Console.WriteLine( "Login: {0}: New character being created (account={1})", args.State, args.Account.Username );
			Console.WriteLine( " - Character: {0} (serial={1})", newChar.Name, newChar.Serial );
			Console.WriteLine( " - Started: {0} {1} in {2}", cityFinal.City, cityFinal.Location, cityFinal.Map.ToString() );

			new WelcomeTimer( newChar ).Start();

            #region mod by Dies Irae
            XmlAttach.AttachTo( newChar, new XmlPointsAttach() );
            XmlAttach.AttachTo( newChar, new XmlData( "LoginData", "JustCreated" ) );
            #endregion
		}
		...
		private static CityInfo GetStartLocation( CharacterCreatedEventArgs args, bool isYoung )
		{
            #region mod by Dies Irae
            if( Core.T2A )
                return new CityInfo( "Cove", "Centro", 2230, 1224, 0, Map.Felucca );
            #endregion
					...
					if ( skill != null )
					{
						...
						if ( addSkillItems )
						{
                            if( Core.AOS )
                                ...
                                PreAoSCharacterCreation.AddSkillItems( snv.Name, m );
						}
					}
				}
			}
