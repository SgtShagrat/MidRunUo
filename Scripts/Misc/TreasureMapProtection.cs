				...
				using ( StreamReader ip = new StreamReader( filePath ) )
				{
					...
					while ( (line = ip.ReadLine()) != null )
					{
						...
						try
						{
							...
							{
								#region mod by Dies Irae
                                TownSystem system = TownSystem.Find( new Point3D( x, y, 0 ), Map.Felucca );
                                if( system != null )
									Console.WriteLine( "Warning: TreasureMap region in {2}. x: {0} - y: {1}", x, y, system.Definition.TownName );
								else
								{
