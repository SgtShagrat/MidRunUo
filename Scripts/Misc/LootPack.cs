...
using SecondAgeLootMagics = Midgard.Engines.SecondAgeLoot.Magics;
...
namespace Server
{
	public class LootPack
	{
		...
		public void Generate( Mobile from, Container cont, bool spawning, int luckChance )
		{
			...
			for ( int i = 0; i < m_Entries.Length; ++i )
			{
				...
				{    
                    ...
                    if( Core.AOS && ( item is BaseJewel || item is BaseArmor ) )
						Midgard.Engines.XmlForceIdentify.DoMask( item );
                    #endregion
                    ...
                    #region mod by Dies Irae
                    if( !spawning && from != null && from is BaseCreature )
                        item.InstanceOwner = from;

                    if( item is EnchantStone )
                        ( (EnchantStone)item ).LogContruct( from, cont );
                    #endregion
                }
			}
		}

		#region LootPackItem definitions
	    /*new LootPackItem for Midgard
         * Wand
         * Talisman
         * RecipeScroll
         * SmeltingRecipeScroll
         * CommonPaganPotion
         * CommonPaganPotion
         * RarePaganPotion
         * PaganPotion
        ...
        public static readonly LootPackItem[] Wand = new LootPackItem[]
			{
				new LootPackItem( typeof( BaseWand ), 1 )
			};

        public static readonly LootPackItem[] Talisman = new LootPackItem[]
			{
				new LootPackItem( typeof( RandomTalisman ), 1 )
			};

        public static readonly LootPackItem[] RecipeScroll = new LootPackItem[]
			{
				new LootPackItem( typeof( RecipeScroll ), 1 )
			};

        public static readonly LootPackItem[] SmeltingRecipeScroll = new LootPackItem[]
			{
				new LootPackItem( typeof( SmeltingRecipeScroll ), 1 )
			};

        public static readonly LootPackItem[] CommonPaganPotion = new LootPackItem[]
			{
				new LootPackItem( typeof( GrandMageRefreshElixirLesser ), 1 )
			};

        public static readonly LootPackItem[] RarePaganPotion = new LootPackItem[]
			{
				new LootPackItem( typeof( GrandMageRefreshElixirGreater ), 1 )
			};

        public static readonly LootPackItem[] PaganPotion = new LootPackItem[]
			{
				new LootPackItem( typeof( TamlaPotion ), 1 )
			};

        public static readonly LootPackItem[] PagansRecipe = new LootPackItem[]
			{
				new LootPackItem( typeof( PagansRecipes ), 1 )
			};

        public static readonly LootPackItem[] PetPotionsRecipe = new LootPackItem[]
			{
				new LootPackItem( typeof( PetPotionsRecipes ), 1 )
			};

        public static readonly LootPackItem[] NarcoticsRecipe = new LootPackItem[]
			{
				new LootPackItem( typeof( NarcoticsRecipes ), 1 )
			};

        public static readonly LootPackItem[] AdvAlchemyRecipe = new LootPackItem[]
			{
				new LootPackItem( typeof( PagansRecipes ), 5 ),
                new LootPackItem( typeof( PetPotionsRecipes ), 1 ),
                new LootPackItem( typeof( NarcoticsRecipes ), 1 )
			};

        public static readonly LootPackItem[] RandomEnchantStone = new LootPackItem[]
            {
                new LootPackItem( typeof( EnchantStone ), 1 )                                                      
            };
		...
		public static readonly LootPack AosUltraRich = new LootPack( new LootPackEntry[]
			{
				...
				new LootPackEntry( false, Instruments,	  2.00, 1 ),
				new LootPackEntry( false, Talisman, 2.00, 1 ),
                new LootPackEntry( false, RecipeScroll, 2.00, 1 )
			} );
		...
		public static readonly LootPack AosSuperBoss = new LootPack( new LootPackEntry[]
			{
				...
				new LootPackEntry( false, AosMagicItemsUltraRich,	100.00, 1, 5, 50, 100 ),
				new LootPackEntry( false, Instruments,	  2.00, 1 ),
				new LootPackEntry( false, Talisman, 5.00, 1 ),
                new LootPackEntry( false, RecipeScroll, 5.00, 3 )
			} );
		#endregion
        ...
        private const double PoorDemultiplier       = 0.40;
        private const double MeagerDemultiplier     = 0.50;
        private const double AverageDemultiplier    = 0.50;
        private const double RichDemultiplier       = 0.50;
        private const double FilthyRichDemultiplier = 0.60;
        private const double UltraRichDemultiplier  = 0.70;
        private const double SuperBossDemultiplier  = 0.80;

        private const double PoorMagicItemDemultiplier       = 0.10;
        private const double MeagerMagicItemDemultiplier     = 0.05;
        private const double AverageMagicItemDemultiplier    = 0.05;
        private const double RichMagicItemDemultiplier       = 0.10;
        private const double FilthyRichMagicItemDemultiplier = 0.15;
        private const double UltraRichMagicItemDemultiplier  = 0.20;
        private const double SuperBossMagicItemDemultiplier  = 0.20;

		public static readonly LootPack OldPoor = new LootPack( new LootPackEntry[]
			{
				new LootPackEntry(  true, Gold,			100.00, "1d25", PoorDemultiplier ),
				new LootPackEntry( false, Instruments,	  0.02, 1 )
			} );

		public static readonly LootPack OldMeager = new LootPack( new LootPackEntry[]
			{
				new LootPackEntry(  true, Gold,			100.00, "5d10+25", MeagerDemultiplier ),
				new LootPackEntry( false, Instruments,	  0.10, 1 ),
				new LootPackEntry( false, OldMagicItems,  1.00 * MeagerMagicItemDemultiplier, 1, 1, 0, 60 ),
				new LootPackEntry( false, OldMagicItems,  0.20 * MeagerMagicItemDemultiplier, 1, 1, 10, 70 ),

                #region mod by Dies Irae
                new LootPackEntry( false, PotionItems,      0.05, 1 )
                #endregion
			} );

		public static readonly LootPack OldAverage = new LootPack( new LootPackEntry[]
			{
				new LootPackEntry(  true, Gold,			100.00, "10d10+50", AverageDemultiplier ),
				new LootPackEntry( false, Instruments,	  0.40, 1 ),
				new LootPackEntry( false, OldMagicItems,  5.00 * AverageMagicItemDemultiplier, 1, 1, 20, 80 ),
				new LootPackEntry( false, OldMagicItems,  2.00 * AverageMagicItemDemultiplier, 1, 1, 30, 90 ),
				new LootPackEntry( false, OldMagicItems,  0.50 * AverageMagicItemDemultiplier, 1, 1, 40, 100 ),
                
                #region mod by Dies Irae : pre-aos stuff
		        new LootPackEntry( false, CommonPaganPotion,    1.00, 1 ),
                new LootPackEntry( false, RandomEnchantStone,   0.05, 1 ),
                new LootPackEntry( false, PotionItems,          0.10, 1 )
	            #endregion
			} );

		public static readonly LootPack OldRich = new LootPack( new LootPackEntry[]
			{
				new LootPackEntry(  true, Gold,			100.00, "10d10+250", RichDemultiplier ),
				new LootPackEntry( false, Instruments,	  1.00, 1 ),
				new LootPackEntry( false, OldMagicItems, 20.00 * RichMagicItemDemultiplier, 1, 1, 60, 100 ),
				new LootPackEntry( false, OldMagicItems, 10.00 * RichMagicItemDemultiplier, 1, 1, 65, 100 ),
				new LootPackEntry( false, OldMagicItems,  1.00 * RichMagicItemDemultiplier, 1, 1, 70, 100 ),
                
                #region mod by Dies Irae : pre-aos stuff
                new LootPackEntry( false, CommonPaganPotion,    1.00, 1 ),
                new LootPackEntry( false, RarePaganPotion,      1.00, 1 ),
                new LootPackEntry( false, RandomEnchantStone,   0.25, 1 ),
                new LootPackEntry( false, PotionItems,          0.25, 1 )
                #endregion
			} );

		public static readonly LootPack OldFilthyRich = new LootPack( new LootPackEntry[]
			{
				new LootPackEntry(  true, Gold,			100.00, "2d125+400", FilthyRichDemultiplier ),
				new LootPackEntry( false, Instruments,	  2.00, 1 ),
				new LootPackEntry( false, OldMagicItems, 33.00 * FilthyRichMagicItemDemultiplier, 1, 1, 50, 100 ),
				new LootPackEntry( false, OldMagicItems, 33.00 * FilthyRichMagicItemDemultiplier, 1, 1, 60, 100 ),
				new LootPackEntry( false, OldMagicItems, 20.00 * FilthyRichMagicItemDemultiplier, 1, 1, 70, 100 ),
				new LootPackEntry( false, OldMagicItems,  5.00 * FilthyRichMagicItemDemultiplier, 1, 1, 80, 100 ),
                
                #region mod by Dies Irae : pre-aos stuff
                new LootPackEntry( false, CommonPaganPotion,    1.00, 1 ),
                new LootPackEntry( false, RarePaganPotion,      1.00, 1 ),
                new LootPackEntry( false, RandomEnchantStone,   1.00, 1 ),
                new LootPackEntry( false, ParrotItem,           0.25, 1 ),
                new LootPackEntry( false, PotionItems,          0.50, 1 )
                #endregion
			} );

		public static readonly LootPack OldUltraRich = new LootPack( new LootPackEntry[]
			{
				new LootPackEntry(  true, Gold,			100.00, "5d100+500", UltraRichDemultiplier ),
				new LootPackEntry( false, Instruments,	  2.00, 1 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * UltraRichMagicItemDemultiplier, 1, 1, 40, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * UltraRichMagicItemDemultiplier, 1, 1, 40, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * UltraRichMagicItemDemultiplier, 1, 1, 50, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * UltraRichMagicItemDemultiplier, 1, 1, 50, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * UltraRichMagicItemDemultiplier, 1, 1, 60, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * UltraRichMagicItemDemultiplier, 1, 1, 60, 100 ),
                
                #region mod by Dies Irae : pre-aos stuff
                new LootPackEntry( false, PaganPotion,          5.00, 3 ),
                new LootPackEntry( false, RandomEnchantStone,   2.00, 1 ),
                new LootPackEntry( false, ParrotItem,           0.50, 1 ),
                new LootPackEntry( false, PotionItems,          2.00, 1 )
                #endregion
			} );

		public static readonly LootPack OldSuperBoss = new LootPack( new LootPackEntry[]
			{
				new LootPackEntry(  true, Gold,			100.00, "5d100+500", SuperBossDemultiplier ),
				new LootPackEntry( false, Instruments,	  2.00, 1 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * SuperBossMagicItemDemultiplier, 1, 1, 40, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * SuperBossMagicItemDemultiplier, 1, 1, 40, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * SuperBossMagicItemDemultiplier, 1, 1, 40, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * SuperBossMagicItemDemultiplier, 1, 1, 50, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * SuperBossMagicItemDemultiplier, 1, 1, 50, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * SuperBossMagicItemDemultiplier, 1, 1, 50, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * SuperBossMagicItemDemultiplier, 1, 1, 60, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * SuperBossMagicItemDemultiplier * SuperBossMagicItemDemultiplier, 1, 1, 60, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * SuperBossMagicItemDemultiplier * SuperBossMagicItemDemultiplier, 1, 1, 60, 100 ),
				new LootPackEntry( false, OldMagicItems,	100.00 * SuperBossMagicItemDemultiplier * SuperBossMagicItemDemultiplier, 1, 1, 70, 100 ),
                
                #region mod by Dies Irae : pre-aos stuff
                new LootPackEntry( false, PaganPotion,          15.00, 10 ),
                new LootPackEntry( false, RandomEnchantStone,   1.00, 1 ),
                new LootPackEntry( false, ParrotItem,           1.00, 1 ),
                new LootPackEntry( false, PotionItems,          2.00, 1 )
                #endregion
			} );
		#endregion
	    ...
	    #region mod by Dies Irae
        public static readonly LootPack Recipe = new LootPack( new LootPackEntry[]
			{
				new LootPackEntry( false, RecipeScroll,			5.00, 1 )
			} );

        public static readonly LootPack EnchantStone = new LootPack( new LootPackEntry[]
            {
                new LootPackEntry( false, RandomEnchantStone, 5.00, 1 )
            } );
        #endregion
    }
	...
	public class LootPackEntry
	{
		...
		{
			#region ARTEGORDONMOD mod allow lootpack construction without a mobile
            if (m == null) 
            	return false;
            #endregion

            #region mod by Dies Irae
            if( !Midgard2Persistance.IsTokunoLootEnabled )
                return false;
            #endregion
        ...
        {
            #region mod by Dies Irae
            if( !Midgard2Persistance.IsMondainLootEnabled )
                return false;
            #endregion
		...
		public Item Mutate( Mobile from, int luckChance, Item item )
		{
			if ( item != null )
			{
				//if ( Core.AOS && item is BaseWeapon && 1 > Utility.Random( 100 ) ) // mod by Dies Irae
				if ( item is BaseWeapon && Utility.Random( 100 )<2 ) // mod by Magius(CHE)
				{
					item.Delete();
					item = new FireHorn();
					return item;
...
*/
                        if( item is BaseWeapon )
                        {
                            SecondAgeLootMagics.ApplyWeaponBonus( (BaseWeapon)item );
                        }
                        else if( item is BaseArmor )
                        {
                            SecondAgeLootMagics.ApplyArmorBonus( (BaseArmor)item );
                        }
                        else if( item is BaseJewel )
                        {
                            SecondAgeLootMagics.ApplyJewelBonus( (BaseJewel)item );
                        }
                        else if( item is BaseClothing )
                        {
                            SecondAgeLootMagics.ApplyClothingBonus( (BaseClothing)item );
                        }
					}
				}
                ...
                if( item is Gold )
                {
                    if( from is BaseCreature && ( (BaseCreature)from ).LootNoGold )
                    {
                        item.Delete();
                        return null;
                    }
                    ...
                    if( m_Scalar > 0.0 )
                        item.Amount = (int)Math.Max( item.Amount * m_Scalar, 1 );
                    else
                        item.Amount = (int)Math.Max( item.Amount * Midgard2Persistance.GoldDemultiplier, 1 );
                }
                #endregion
			}
        ...
        #region mod by Dies Irae
        public LootPackEntry( bool atSpawnTime, LootPackItem[] items, double chance, string quantity, double scalar ) :
            this( atSpawnTime, items, chance, quantity, 0, 0, 0 )
        {
            m_Scalar = scalar;
        }
        #endregion
	...
	public class LootPackItem
	{
        ...
        #region modifica by Dies Irae [Recipes]
        ...
        public static Item RandomRecipe( RecipeType type )
        {
            switch( type )
            {
                case RecipeType.TailorRecipe: return BaseReward.TailorRecipe();
                case RecipeType.SmithRecipe: return BaseReward.SmithRecipe();
                case RecipeType.TinkerRecipe: return BaseReward.TinkerRecipe();
                case RecipeType.CarpRecipe: return BaseReward.CarpRecipe();
            }

            return BaseReward.TailorRecipe();
        }

	    public static Item RandomRecipe()
        {
            return RandomRecipe( (RecipeType)Utility.Random( Enum.GetValues( typeof( RecipeType ) ).Length ) );
        }
		...
		public Item Construct( bool inTokuno, bool isMondain )
		{
			try
			{
                ...
                if( m_Type == typeof( RandomTalisman ) && Midgard2Persistance.TalismansEnabled )
                    item = new RandomTalisman();
                else if( m_Type == typeof( GrandMageRefreshElixirLesser ) && Midgard2Persistance.PaganPotionsEnabled )
                    item = BasePaganPotion.RandomCommonPaganPotion();
                else if( m_Type == typeof( GrandMageRefreshElixirGreater ) && Midgard2Persistance.PaganPotionsEnabled )
                    item = BasePaganPotion.RandomRarePotion();
                else if( m_Type == typeof( TamlaPotion ) && Midgard2Persistance.PaganPotionsEnabled )
                    item = BasePaganPotion.RandomPaganPotion();
                else if( m_Type == typeof( RecipeScroll ) && Midgard2Persistance.RecipeEnabled )
                    item = RandomRecipe();

                else if( m_Type == typeof( PagansRecipes ) && Midgard2Persistance.PaganPotionsEnabled )
                    item = DefAlchemy.RandomPaganRecipe();
                else if( m_Type == typeof( PetPotionsRecipes ) )
                    item = DefAlchemy.RandomPetPotionRecipe();
                else if( m_Type == typeof( NarcoticsRecipes ) )
                    item = DefAlchemy.RandomNarcoticRecipe();
                else if( m_Type == typeof( PagansRecipes ) )
                    item = DefAlchemy.RandomPaganRecipe();
                else if( m_Type == typeof( SmeltingRecipeScroll ) && Midgard2Persistance.AdvSmeltingEnabled )
                    item = SmeltingRecipeScroll.RandomMiningRecipe();
                else if( m_Type == typeof( EnchantStone ) )
                    item = StoneEnchantHelper.RandomEnchantStone();
                ...
                else if( m_Type == typeof( BaseRanged ) )
