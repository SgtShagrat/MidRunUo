...
namespace Server.Commands
{
	public class CommandHandlers
	{
			...
			if ( obj is PlayerMobile )
			{
				...
				if ( pets.Count > 0 )
				{
					...
					for ( int i = 0; i < pets.Count; ++i )
					{
                        ...
                        if( pet is BaseCreature && ( (BaseCreature)pet ).IsHitched )
                            ( (BaseCreature)pet ).IsHitched = false;
                        #endregion
        ...
        [Description( "Replace all bankers in the world." )]
		...
		public static void WriteProfiles_OnCommand( CommandEventArgs e )
		{
			try
			{
				using ( StreamWriter sw = new StreamWriter( "Logs/profiles.log", true ) )
				{
		...
		public static void DumpTimers_OnCommand( CommandEventArgs e )
		{
			try
			{
				using ( StreamWriter sw = new StreamWriter( "Logs/timerdump.log", true ) )
					Timer.DumpInfo( sw );
			}
			catch
			{
		...
		public static void TraceExpanded_OnCommand( CommandEventArgs e )
		{
			...
			try
			{
				using ( StreamWriter op = new StreamWriter( "Logs/expandedItems.log", true ) )
				{
		...
		public static void TraceInternal_OnCommand( CommandEventArgs e )
		{
			...
			using ( StreamWriter op = new StreamWriter( "Logs/internal.log" ) )
			{
		...
		{
			ProfileWorld( "items", "Logs/worldprofile_items.log" );
			ProfileWorld( "mobiles", "Logs/worldprofile_mobiles.log" );
		}
