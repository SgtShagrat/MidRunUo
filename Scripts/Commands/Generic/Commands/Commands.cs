...
namespace Server.Commands.Generic
{
	...
	public class DeleteCommand : BaseCommand
	{
		...
		public override void Execute( CommandEventArgs e, object obj )
		{
			if ( obj is Item )
			{
				#region Modifica by Dies Irae per non deletare XmlSpawner senza permesso
				if ( obj is XmlSpawner && e.Mobile is Midgard2PlayerMobile )
				{
					if( !((Midgard2PlayerMobile)e.Mobile).IsMidgardSpawner || e.Mobile.AccessLevel < AccessLevel.Administrator )
					{
		...
		public override void Execute( CommandEventArgs e, object obj )
		{
			...
			if ( m_Value )
			{
				...
				else
				{
					...
					#region Magius(CHE) - In questo modo si entra per diritto a riceve loot e ad avanziare gli objective nelle quest.
					mob.AggressiveAction( from );
					mob.Damage(1,from);
					#endregion
