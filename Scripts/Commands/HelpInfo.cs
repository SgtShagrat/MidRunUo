...
namespace Server.Commands
{
	public class HelpInfo
	{
		...
		{
			CommandSystem.Register( "Comandi", AccessLevel.Player, new CommandEventHandler( HelpInfo_OnCommand ) );

            Timer.DelayCall( TimeSpan.FromSeconds( 10.0 ), FillTable );

            // CommandSystem.Register( "RebuildCommandList", AccessLevel.Developer, delegate { FillTable(); } );
		}

		[Usage( "AiutoAvanzato [<comando>]" )]
		[Description( "Da informazioni su un comando oppure, se non viene specificato nessun comando, apre un menu informativo su tutti i comandi." )]
		...
		public static void FillTable()
		{
			...
			for( int i = 0; i < commands.Count; ++i )
			{
				...
				{
                    if( Core.Debug )
                        Utility.Log( "HelpInfoFillTable.log", "Warning: null usage for command: {0} - {1}", mi.Name, e.Command );
				    continue;
				}
				...
				{
                    if( Core.Debug )
                        Utility.Log( "HelpInfoFillTable.log", "Warning: null description for command: {0} - {1}", mi.Name, e.Command );
				    continue;
				}
				...
				{
                    if( Core.Debug )
                        Utility.Log( "HelpInfoFillTable.log", "Warning: null usage or description for command: {0} - {1}", mi.Name, e.Command );
				    continue;
				}
			...
			for( int i = 0; i < TargetCommands.AllCommands.Count; ++i )
			{
                ...
                {
                    if( Core.Debug )
                        Utility.Log( "HelpInfoFillTable.log", "Warning: null usage or description for base-command: {0}", command.Commands[ 0 ] );
                    continue;
                }
			...
			for( int i = 0; i < commandImpls.Count; ++i )
			{
				...
				{
                    if( Core.Debug )
                        Utility.Log( "HelpInfoFillTable.log", "Warning: command with null usare or description. -> {0}", command.GetType().Name );
				    continue;
				}
