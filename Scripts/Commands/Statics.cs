...
namespace Server
{
	public class Statics
	{
		...
		{
			CommandSystem.Register( "Freeze", AccessLevel.Seer, new CommandEventHandler( Freeze_OnCommand ) ); // mod by Dies Irae
			CommandSystem.Register( "FreezeMap", AccessLevel.Developer, new CommandEventHandler( FreezeMap_OnCommand ) ); // mod by Dies Irae
			CommandSystem.Register( "FreezeWorld", AccessLevel.Developer, new CommandEventHandler( FreezeWorld_OnCommand ) ); // mod by Dies Irae

			CommandSystem.Register( "Unfreeze", AccessLevel.Seer, new CommandEventHandler( Unfreeze_OnCommand ) ); // mod by Dies Irae
			CommandSystem.Register( "UnfreezeMap", AccessLevel.Developer, new CommandEventHandler( UnfreezeMap_OnCommand ) ); // mod by Dies Irae
			CommandSystem.Register( "UnfreezeWorld", AccessLevel.Developer, new CommandEventHandler( UnfreezeWorld_OnCommand ) ); // mod by Dies Irae

            #region mod by Dies Irae
            CommandSystem.Register( "FreezeRegion", AccessLevel.Administrator, new CommandEventHandler( FreezeRegion_OnCommand ) );
            CommandSystem.Register( "UnfreezeRegion", AccessLevel.Administrator, new CommandEventHandler( UnfreezeRegion_OnCommand ) );
            #endregion
        }
        ...
        #region mod by Dies Irae: freezeregion
        [Usage( "FreezeRegion" )]
        [Description( "Makes every dynamic item in your region static." )]
        public static void FreezeRegion_OnCommand( CommandEventArgs e )
        {
            Map map = e.Mobile.Map;

            if( map != null && map != Map.Internal )
                SendWarning( e.Mobile, "You are about to freeze <u>all items in {0}</u>.", BaseFreezeWarning, map, e.Mobile.Region, new WarningGumpCallback( FreezeRegionWarning_Callback ) );
        }

        private static void FreezeRegionWarning_Callback( Mobile from, bool okay, object state )
        {
            if( !okay )
                return;

            Region region = (Region)state;
            Console.WriteLine( "Freezing {0}.", region.Name );

            for( int i = 0; i < region.Area.Length; i++ )
            {
                Rectangle3D rect = region.Area[i];
                Console.WriteLine( "Freezing area. Start ({0},{1}) , Width {2}, Height {3}.", rect.Start, rect.End,
                                  rect.Width, rect.Height );

                Freeze( from, from.Map, rect.Start, rect.End );
            }
        }

        public static void SendWarning( Mobile m, string header, string baseWarning, Map map, Region region, WarningGumpCallback callback )
        {
            m.SendGump( new WarningGump( 1060635, 30720, String.Format( baseWarning, String.Format( header, map ) ), 0xFFC000,
                                       420, 400, callback, region ) );
        }
        #endregion
        ...
        #region mod by Dies Irae: unfreezeregion
        [Usage( "UnfreezeRegion" )]
        [Description( "Makes every static item in your region dynamic." )]
        public static void UnfreezeRegion_OnCommand( CommandEventArgs e )
        {
            Map map = e.Mobile.Map;

            if( map != null && map != Map.Internal )
                SendWarning( e.Mobile, "You are about to unfreeze <u>all items in {0}</u>.", BaseUnfreezeWarning, map, e.Mobile.Region, new WarningGumpCallback( UnfreezeRegionWarning_Callback ) );
        }

        private static void UnfreezeRegionWarning_Callback( Mobile from, bool okay, object state )
        {
            if( !okay )
                return;

            Unfreeze( from, from.Map, (Region)state );
        }

        public static void Unfreeze( Mobile from, Map map, Region region )
        {
            int totalUnfrozen = 0;
            bool badDataFile = false;

            Console.WriteLine( "Un-freezing {0}.", region.Name );

            for( int i = 0; i < region.Area.Length; i++ )
            {
                Rectangle3D rect = region.Area[i];
                Console.WriteLine( "UnFreezing area. Start ({0},{1}) , Width {2}, Height {3}.", rect.Start, rect.End,
                                  rect.Width, rect.Height );

                CommandLogging.WriteLine( from, "{0} {1} invoking unfreeze for every item in {2}", from.AccessLevel, CommandLogging.Format( from ), region.Name );

                DoUnfreeze( from.Map, new Point2D( rect.Start ), new Point2D( rect.End ), ref badDataFile, ref totalUnfrozen );
            }
