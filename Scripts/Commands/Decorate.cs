...
namespace Server.Commands
{
	public class Decorate
	{
		...
		{
			CommandSystem.Register( "Decorate", AccessLevel.Developer, new CommandEventHandler( Decorate_OnCommand ) ); // mod by Dies Irae
		    CommandSystem.Register( "DecorateRegion", AccessLevel.Developer, new CommandEventHandler( DecorateRegion_OnCommand ) ); // mod by Dies Irae
            CommandSystem.Register( "DecorateMap", AccessLevel.Developer, new CommandEventHandler( DecorateMap_OnCommand ) ); // mod by Dies Irae
            CommandSystem.Register( "DecorateLocal", AccessLevel.Developer, new CommandEventHandler( DecorateLocal_OnCommand ) ); // mod by Dies Irae
        }
        
        [Usage( "DecorateRegion" )]
        [Description( "Generates region decoration." )]
        private static void DecorateRegion_OnCommand( CommandEventArgs e )
        {
			m_Mobile = e.Mobile;
			m_Count = 0;

			m_Mobile.SendMessage( "Generating regional decoration, please wait." );

			Generate( "Data/Decoration/Britannia", m_Mobile.Region, Map.Trammel, Map.Felucca );
			Generate( "Data/Decoration/Trammel", m_Mobile.Region, Map.Trammel );
			Generate( "Data/Decoration/Felucca", m_Mobile.Region, Map.Felucca );
			Generate( "Data/Decoration/Ilshenar", m_Mobile.Region, Map.Ilshenar );
			// Generate( "Data/Decoration/Malas", Map.Malas );
			// Generate( "Data/Decoration/Tokuno", Map.Tokuno );

			m_Mobile.SendMessage( "Regional decoration generating complete. {0} items were generated.", m_Count );
        }
        ...
        [Usage( "DecorateMap" )]
        [Description( "Generates map decoration." )]
        private static void DecorateMap_OnCommand( CommandEventArgs e )
        {
            m_Mobile = e.Mobile;
            ...
            if( map == null )
            {
                m_Mobile.SendMessage( "Invalid map" );
                return;
            }

            m_Mobile.SendMessage( "Generating map decoration, please wait." );

            m_Count = 0;
            Generate( string.Format( "Data/Decoration/{0}", map.Name ), map );

            m_Mobile.SendMessage( "Map generating complete. {0} items were generated.", m_Count );
        }

        [Usage( "DecorateLocal <file> <map>" )]
        [Description( "Generates local decoration." )]
        private static void DecorateLocal_OnCommand( CommandEventArgs e )
        {
            m_Mobile = e.Mobile;

            if( e.Length != 2 )
            {
                m_Mobile.SendMessage( "Command use: [DecorateLocal <file> <map>" );
                return;
            }
            ...
            {
                m_Mobile.SendMessage( "Invalid file" );
                return;
            }

            Map map = Map.Parse( e.GetString( 1 ) );
            if( map == null )
            {
                m_Mobile.SendMessage( "Invalid map" );
                return;
            ...
            m_Mobile.SendMessage( "Generating local map decoration, please wait." );

            m_Count = 0;

            if( File.Exists( file ) )
            {
                ArrayList list = DecorationList.ReadAll( file );

                m_List = list;

                foreach( object t in list )
                    m_Count += ( (DecorationList)t ).Generate( new Map[] { map } );
            }
        ...
        {
            Generate( folder, null, maps );
        } 
        #endregion

		public static void Generate( string folder, Region region, params Map[] maps )
		{
			...
			for ( int i = 0; i < files.Length; ++i )
			{
				...
				for ( int j = 0; j < list.Count; ++j )
					m_Count += ((DecorationList)list[j]).Generate( maps, region );
			}
		}
	...
	public class DecorationList
	{
        ...
        {
            return Generate( maps, null );
        }

		public int Generate( Map[] maps, Region region )
		{
			int count = 0;
			...
			for ( int i = 0; i < m_Entries.Count; ++i )
			{
				...
				for ( int j = 0; j < maps.Length; ++j )
                {
                    ...
                    if( region != null )
                    {
                        Region r = Region.Find( loc, maps[j] );
                        if( r != region )
                        {
                            continue;
                        }
