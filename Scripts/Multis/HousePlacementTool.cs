...
namespace Server.Items
{
	...
	public class NewHousePlacementTarget : MultiTarget
	{
		...
		protected override void OnTarget( Mobile from, object o )
		{
			...
			if ( ip != null )
			{
				...
				#region Modifica by Dies Irae per l'housing di Midgard
				bool HousingEnabled = false;

				if( from.AccessLevel < AccessLevel.GameMaster && !HousingEnabled )
				{
					from.SendMessage( "Questo tool non e' piu' adatto a costruire case." );
					return;
				}
				else if( from.AccessLevel < AccessLevel.Seer && ( from.Map == Map.Tokuno || from.Map == Map.Ilshenar || from.Map == Map.Trammel || from.Map == Map.Malas  ))
				{
					from.SendMessage( "Non ti è permesso costruire una casa sulla mappa di {0}.", from.Map.Name );
					return;
				}
	...
	public class HousePlacementEntry
	{
		...
		*/
		...
		// messo tutto protected perchè è stata ereditata la MidgardHousePlacementEntry
		protected Type m_Type;
		protected int m_Description;
		protected int m_Storage;
		protected int m_Lockdowns;
		protected int m_NewStorage;
		protected int m_NewLockdowns;
		protected int m_Vendors;
		protected int m_Cost;
		protected int m_MultiID;
		protected Point3D m_Offset;
		#endregion
...
#if FaxxHousing
      		PreviewHouse prevHouse = (PreviewHouse)states[0];

      		HouseCommittmentDeed deed = (HouseCommittmentDeed)states[1];
#else
            PreviewHouse prevHouse = (PreviewHouse)states[0];
#endif
            #endregion
			...
			switch ( res )
			{
				case HousePlacementResult.Valid:
				{
					...
					else
                    {
						...
						#region Modifica by Fax
                        // Il codice che c'era è stato messo in CheckPayment per permettere l'override
                        if( !CheckPayment( from, m_Cost ) )
                        {
		...
		#region Modifica by Fax
		// Aggiunto questo metodo per permettere l'override del check per i soldi
		protected virtual bool CheckPayment(Mobile from, int cost)
		{
			if ( from.AccessLevel >= AccessLevel.GameMaster )
			{
		...
		{
            return OnPlacement( from, p, null );
		}
#endif
        #endregion

#if FaxxHousing
        public bool OnPlacement( Mobile from, Point3D p, HouseCommittmentDeed deed ) // mod by Dies Irae
#else
        public bool OnPlacement( Mobile from, Point3D p )
#endif
		{
			...
			switch ( res )
			{
				case HousePlacementResult.Valid:
				{
					...
					else
					{
						 ...
						 */
#if FaxxHousing
						from.SendGump( new WarningGump( 1060635, 30720, 1049583, 32512, 420, 280, new WarningGumpCallback( PlacementWarning_Callback ), new object[]{ prev, deed } ) ); // mod by Dies Irae
#else
                        from.SendGump( new WarningGump( 1060635, 30720, 1049583, 32512, 420, 280, new WarningGumpCallback( PlacementWarning_Callback ), new object[]{ prev } ) );
#endif
						return true;
					}
