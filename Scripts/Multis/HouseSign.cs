...
{
    [Flipable( 0xBD2, 0xBD1 )]
	public class HouseSign : Item
	{
		private BaseHouse m_Owner;
		private Mobile m_OrgOwner;

        private bool m_PreviewRegion;

        [CommandProperty( AccessLevel.GameMaster )]
        public bool PreviewRegion
        {
            get { return m_PreviewRegion; }
            set
            {
                if( value != m_PreviewRegion )
                {
                    m_PreviewRegion = value;

                    if( m_PreviewRegion )
                        m_Owner.Region.ShowAreaPreview();
                    else
                        m_Owner.Region.ClearPreview();
                }
            }
        }
		...
		public void ClaimGump_Callback( Mobile from, bool okay, object state )
		{
			if ( okay && m_Owner != null && m_Owner.Owner == null && m_Owner.DecayLevel != DecayLevel.DemolitionPending )
			{
				...
				if ( canClaim && !BaseHouse.HasAccountHouse( from ) && !BaseHouse.HasAccountHouses( from ) ) // mod by Dies Irae
				{
					m_Owner.Owner = from;
		...
		public override void OnDoubleClick( Mobile m )
		{
			...
			if ( m.AccessLevel < AccessLevel.GameMaster && m_Owner.Owner == null && m_Owner.DecayLevel != DecayLevel.DemolitionPending )
			{
				...
				if ( canClaim && !BaseHouse.HasAccountHouse( m ) && !BaseHouse.HasAccountHouses( m ) ) // mod by Dies Irae )
				{
