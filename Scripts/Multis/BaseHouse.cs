...
namespace Server.Multis
{
	...
	{
        public static readonly bool OldSchoolRules = Core.T2A; // Mod by Dies Irae

		public static bool NewVendorSystem{ get{ return false /* Core.AOS */; } } // Is new player vendor system enabled?
        ...
        private static bool m_DecayEnabled = false;
        ...
        public static bool DecayEnabled
        {
            get { return m_DecayEnabled; }
            set { m_DecayEnabled = value; }
        }

        public static readonly DateTime SummerNoDecayPeriodStart = new DateTime( 2008, 7, 1 );
        public static readonly DateTime SummerNoDecayPeriodEnd = new DateTime( 2008, 8, 31 );

        public const double MinHoursToRefresh = 8.0;

        private TimeSpan m_TotalGameTimeAtLastRefresh;

        [CommandProperty( AccessLevel.GameMaster )]
        public TimeSpan TotalGameTimeAtLastRefresh
        {
            get { return m_TotalGameTimeAtLastRefresh; }
            set { m_TotalGameTimeAtLastRefresh = value; }
        }

        public bool CanRefresh( Mobile m )
        {
            if( m == null || m.Deleted )
                return false;
            ...
            {
                m.SendMessage( "You cannot refresh this house because only its owner can do." );
                return false;
            }
            ...
            TimeSpan hoursOfGameAfterRefresh = acct.TotalGameTime - TotalGameTimeAtLastRefresh;

            // se un gm refresha la casa con più gametime del owner pg poi il pg non puo' piu' rinnovarla
            if( acct.TotalGameTime < TotalGameTimeAtLastRefresh )
                hoursOfGameAfterRefresh = acct.TotalGameTime;

            if( DecayEnabled )
            {
                if( hoursOfGameAfterRefresh < TimeSpan.FromHours( MinHoursToRefresh ) )
                {
                    TimeSpan hoursNeeded = TimeSpan.FromHours( MinHoursToRefresh ) - hoursOfGameAfterRefresh;
                    m.SendMessage( "You have to spend more time on Midgard to be able to refresh this house: approximately {0} hours.", hoursNeeded.TotalHours.ToString( "F0" ) );
                    return false;
                }
                else
                {
                    m.SendMessage( "You were able to refresh this house." );
                    m_TotalGameTimeAtLastRefresh = acct.TotalGameTime;
                    try
                    {
                        using( TextWriter t = new StreamWriter( "Logs/HouseRefreshSuccess.log", true ) )
                        {
                            t.WriteLine( "House refreshed successfully. Serial {0}, Owner {1}, Account {2}, Date {3}, Time {4}, TotalGameTime {5}.",
                                        Serial, m_Owner != null ? m_Owner.Name : "no owner", m_Owner != null ? m_Owner.Account.Username : "no account",
                                        DateTime.Now.ToShortDateString(), DateTime.Now.ToShortTimeString(),
                                        m_TotalGameTimeAtLastRefresh.TotalHours.ToString( "F2" ) );
                        }
                    }
                    catch( Exception ex )
                    {
        ...
        public void RefreshBanList()
        {
            ...
            for( int i = 0; i < m_BanInfo.Count; i++ )
            {
                if( m_BanInfo[ i ].IsExpired() )
                    m_BanInfo.RemoveAt( i );
            }
        }
        #endregion
		...
		public virtual TimeSpan DecayPeriod{ get{ return TimeSpan.FromDays( 31.0 ); } } // TimeSpan.FromDays( 5.0 ) // modifica by Dies Irae
		...
		public virtual DecayType DecayType
		{
			get
			{
				...
				if ( this == newest )
					return DecayType.ManualRefresh; // DecayType.AutoRefresh; // modifica by Dies Irae
		...
		public virtual void Decay_Sandbox()
		{
			...
			#region modifica by Dies Irae
            if( DateTime.Now >= SummerNoDecayPeriodStart && DateTime.Now <= SummerNoDecayPeriodEnd )
                return;

			try
			{
				using( TextWriter t = new StreamWriter( "Logs/HouseDeleteSuccess.log", true ) )
				{
					t.WriteLine( "House deleted FOR DECAY. Serial {0}", Serial );
				}	
			}
			catch( Exception ex )
			{
        ...
        private List<BanInfo> m_BanInfo = new List<BanInfo>(); // mod by Dies Irae
        ...
        #region mod by Dies Irae
		public bool HasHicthes
		{
			get
			{
				foreach ( Item item in Addons )
                {
                    if( item is HitchingPostAddon && ( (HitchingPostAddon)item ).HitchedCreature != null )
                        return true;
                }
		...
		public void MoveAllToCrate()
		{
			...
			foreach ( Item addon in Addons )
			{
				if ( !addon.Deleted )
				{
					#region modifica by Dies Irae per aquario e alveare
					if( addon is Aquarium || addon is ApiBeeHive )
					{
						try
						{
		...
		public static bool CheckAccessible( Mobile m, Item item )
		{
            ...
            // Locked down containers are accessible to anyone who
            // can get close enough to reach into them.
		...
		public bool CheckAccessibility( Item item, Mobile from )
		{
            ...
            */
				...
				return OldSchoolRules ? true : IsCoOwner( from ); // mod by Dies Irae
				...
				return true;
            #region mod by Dies Irae
            else if( item.GetType().IsDefined( typeof( Engines.Craft.ForgeAttribute ), false ) )
                return true;
            else if( item is RedSoulstone )
                return true;
            else if( item is BaseBook )
                return true;
            else if( item is Guildstone )
                return true;
			#endregion
		...
		public SecureAccessResult CheckSecureAccess( Mobile m, Item item )
		{
            ...
            if( OldSchoolRules )
            {
                if( item.IsSecure && !IsFriend( m ) )
                    return SecureAccessResult.Inaccessible;
                else if( item.IsSecure && IsCoOwner( m ) )
        ...
        #region mod by Dies Irae
        public static void DeleteAllHouses()
        {
            for( int i = 0; i < m_AllHouses.Count; ++i )
                m_AllHouses[ i ].Decay_Sandbox();
        }
        #endregion
		...
		public BaseHouse( int multiID, Mobile owner, int MaxLockDown, int MaxSecure ) : base( multiID | 0x4000 )
		{
            ...
            m_BanInfo = new List<BanInfo>(); // mod by Dies Irae
			...
			if( m_Owner != null )
			{
				Account a = m_Owner.Account as Account;
				m_TotalGameTimeAtLastRefresh = ( a != null ) ? a.TotalGameTime : TimeSpan.Zero;
			}
			else
				m_TotalGameTimeAtLastRefresh = TimeSpan.Zero;
			#endregion
		...
		public BaseDoor AddEastDoor( bool wood, int x, int y, int z )
		{
            ...
            if( door is BaseHouseDoor )
                ( (BaseHouseDoor)door ).IsInternal = true;
            #endregion
	    ...
	    {
	        return AddEastDoors( true, x, y, z, k );
	    }

	    public BaseDoor[] AddEastDoors( bool wood, int x, int y, int z, uint k )
	    {
	        BaseDoor northDoor = MakeDoor( wood, DoorFacing.NorthCCW );
	        BaseDoor southDoor = MakeDoor( wood, DoorFacing.SouthCW );
		...
		public uint CreateKeys( Mobile m )
		{
			...
			if ( !IsAosRules )
			{
                ...
                HouseKey packKey = new HouseKey( false, 0 );
                HouseKey bankKey = new HouseKey( false, 0 );
                #endregion
		...
		public bool LockDown( Mobile m, Item item, bool checkIsInside )
		{
			...
			if ( ( item is BaseAddonContainer || item.Movable ) && !IsSecure( item ) )
			{
                ...
                #region mod by Dies Irae
                // mod by Dies Irae : only container items or single items can be locked down
                else if( OldSchoolRules && ( IsLockedDown( rootItem ) || IsLockedDown( parentItem ) ) )
                {
                    m.SendMessage( "You cannot lock down items in a locked down container." );
                }
                else if( OldSchoolRules && LockDownCount >= MaxLockDowns )
                {
                    m.SendMessage( "The maximum number of locked down items has been reached : ", MaxLockDowns.ToString() );
                }
				else if ( !OldSchoolRules && ( parentItem != null && !IsLockedDown( parentItem ) ) )
				{
		...
		private class TransferItem : Item
		{
			...
			public override bool AllowSecureTrade( Mobile from, Mobile to, Mobile newOwner, bool accepted )
			{
			    ...
			    if ( HasAccountHouse( to ) && HasAccountHouses( to ) ) // mod by Dies Irae
				{
		...
		public void BeginConfirmTransfer( Mobile from, Mobile to )
		{
			...
			{
				if ( BaseHouse.HasAccountHouse( to ) && HasAccountHouses( to ) ) // mod by Dies Irae
				{
					// from.SendLocalizedMessage( 501388 ); // You cannot transfer ownership to another house owner or co-owner!
                    from.SendMessage( "That player has already the maximum houses allowed on Sosaria." );
				}
		...
		public void AddSecure( Mobile m, Item item )
		{
			...
			else
			{
			    ...
			    #region mod by Dies Irae
			    // http://wiki.uosecondage.com/index.php?title=House
			    // mod by Dies Irae: under OldSchoolRules no access level on items was permitted
			    // Secures can only be accessed by friends of the house
			    if ( !OldSchoolRules && info != null )
			    {
                ...
                #region mod by Dies Irae
			    // http://wiki.uosecondage.com/index.php?title=House
                // Secures and their contents do not count against the maximum number of lockdowns a house has. 
				else if (!OldSchoolRules && ( IsAosRules ? !CheckAosLockdowns( 1 ) : ((LockDownCount + 125) >= MaxLockDowns) ))
				{
					m.SendLocalizedMessage( 1005379 ); // That would exceed the maximum lock down limit for this house
                }
                else if ( item.TotalWeight > 400 )
                {
                    m.SendMessage( "You cannot secure this, it's weight is over 400 stones." );
                }
                #endregion
				...
				else
				{
			        ...
			        // mod by Dies Irae: under OldSchoolRules no access level on items was permitted
			        // Secures can only be accessed by friends of the house
					info = new SecureInfo( (Container)item, OldSchoolRules ? SecureLevel.Friends : SecureLevel.CoOwners );
                    #endregion
		...
		public void RemoveBan( Mobile from, Mobile targ )
		{
			...
			if ( m_Bans.Contains( targ ) )
			{
				...
				if( !Core.AOS )
                	RemoveBanInfo( from, targ ); // mod by Dies Irae
        ...
        #region mod by Dies Irae
        public void RemoveBanInfo( Mobile from, Mobile targ )
        {
			if ( !IsCoOwner( from ) || m_BanInfo == null )
				return;

            for ( int i = 0; i < m_BanInfo.Count; i++ )
            {
                if( m_BanInfo[ i ].Banned == targ )
                    m_BanInfo.Remove( m_BanInfo[ i ] );
            }
        }
        #endregion
        ...
        public void Ban( Mobile from, Mobile targ )
		{
			...
			else
			{
                ...
                {
                    if( m_BanInfo == null )
                        m_BanInfo = new List<BanInfo>();

                    m_BanInfo.Add( new BanInfo( targ, DateTime.Now ) );
                }
                #endregion
		...
		public void AddCoOwner( Mobile from, Mobile targ )
		{
			...
			else if ( HasAccountHouse( targ ) && HasAccountHouses( targ ) ) // mod by Dies Irae
			{
				// from.SendLocalizedMessage( 501364 ); // That person is already a house owner.
                from.SendMessage( "That person has already the maximum houses allowed on Sosaria." );
			}
			else if ( IsBanned( targ ) )
			{
				from.SendLocalizedMessage( 501367 ); // This person is banned!  Unban them first.
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			#region mod by Dies Irae
            // Version 16
            writer.WriteEncodedInt( m_BanInfo.Count );
            for( int i = 0; i < m_BanInfo.Count; i++ )
            {
                m_BanInfo[ i ].Serialize( writer );
            }

            // Version 15
			writer.Write( m_TotalGameTimeAtLastRefresh );
			#endregion
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
                case 16:
                {
					int banInfoCount = reader.ReadEncodedInt();
					for ( int i = 0; i < banInfoCount; i++ )
					{
						BanInfo info = new BanInfo( reader );
                        if( m_BanInfo == null )
                            m_BanInfo = new List<BanInfo>();
				...
				case 15:
				{
					m_TotalGameTimeAtLastRefresh = reader.ReadTimeSpan();
					goto case 14;
				}
				case 14:
				{
				...
				case 11:
				{
					...
					m_LastRefreshed = DateTime.Now;
					m_TotalGameTimeAtLastRefresh = TimeSpan.Zero;
					Console.WriteLine( "House (serial {0}) refreshed properly.", Serial.ToString() );
#endif
                    #region mod By Dies Irae
                    // Se ci sono lunghi tempi di down aumentare il TimeSpan m_LastRefreshed
                    // della durata del down

                    string filePath = "MidgardDownEventDefinition.def";

                    if( File.Exists( filePath ) )
                    {
                        DateTime down = DateTime.MinValue;
                        DateTime up = DateTime.MinValue;
                        ...
                        using( StreamReader op = new StreamReader( filePath ) )
                        {
                            do
                            {
                                ...
                                try
                                {
                                    if( lineRead.Equals( "[DOWN]" ) )
                                        down = DateTime.Parse( op.ReadLine() );

                                    if( lineRead.Equals( "[UP]" ) )
                                        up = DateTime.Parse( op.ReadLine() );
                                }
                                catch( Exception ex )
                                {
                                    Console.WriteLine( ex.ToString() );
                        ...
                        if( down != DateTime.MinValue && up != DateTime.MinValue && DateTime.Now.Date == up.Date )
                        {
                            TimeSpan t = DateTime.Now - down;
                            m_LastRefreshed += t;

                            try
                            {
                                using( TextWriter tw = new StreamWriter( "Logs/HouseRefreshSuccess.log", true ) )
                                {
                                    tw.WriteLine( "House of (serial {0}) m_LastRefreshed delayed of {1} hours properly.", Serial, t.TotalHours.ToString( "F2" ) );
                                }
                            }
                            catch( Exception ex )
                            {
                    ...
                    if( Midgard.Misc.SaveTimeStamper.LastSaveTime != DateTime.MinValue )
                    {
                        TimeSpan ts = DateTime.Now - Midgard.Misc.SaveTimeStamper.LastSaveTime;
                        // if( ts > TimeSpan.FromHours( 24.0 ) )
                            m_LastRefreshed += ts;
                        ...
                        try
                        {
                            using( TextWriter tw = new StreamWriter( "Logs/HouseRefreshSuccess.log", true ) )
                            {
                                tw.WriteLine( "House of (serial {0}) m_LastRefreshed delayed of {1} hours properly.", Serial, ts.TotalHours.ToString( "F2" ) );
                            }
                        }
                        catch( Exception ex )
                        {
		...
		public HouseRegion /*Region*/ Region{ get{ return m_Region; } } // mod by Dies Irae
		...
		public int LockDownCount
		{
			get
			{
			    ...
			    #region mod by Dies Irae
			    // http://wiki.uosecondage.com/index.php?title=House
                // Locked down containers count as one lockdown against 
                // the maximum number of lockdowns a house is allowed,
                // the contents of the container does not count towards that maximum.
			    // Secures and their contents do not count against the 
			    // maximum number of lockdowns a house has. 
			    if( OldSchoolRules )
			        return count;
			    #endregion
		...
		#region modifica by Dies Irae
		public enum HouseDeleteModes
		{
			Standard,
			ToCrate
		}

		public static readonly HouseDeleteModes HouseDeleteMode = HouseDeleteModes.ToCrate;

		private static List<Item> m_LockDownsAfterDelete;
		private static List<Item> m_VendorRentalContractsAfterDelete;
		private static List<Item> m_SecuresAfterDelete;
		private static List<Item> m_AddonsAfterDelete;
		private static List<Item> m_MovingCrateAfterDelete;

		private static void FillBoxe( string name, List<Item> items, Container chest, int x, int y )
		{
			...
			chest.AddItem( wb );
			wb.Location = new Point3D( x, y, 0 );
			wb.Movable = false;
		}

		private static void OrganizeAfterDeleteChest( Mobile owner, Point3D location, int price, HouseDeed deed )
		{
			MetalBox chest = new MetalBox();
			chest.Hue = Utility.RandomMetalHue();
			chest.Name = string.Format( "Items from house of {0}", ( owner != null ) ? owner.Name : "an unkwown owner" );
			chest.EngravedText = string.Format( "House Location {0}", location );

			FillBoxe( "locked down items", m_LockDownsAfterDelete, chest, 28, 51 );
			FillBoxe( "vendor rental contracts", m_VendorRentalContractsAfterDelete, chest, 43, 51 );
			FillBoxe( "secured items", m_SecuresAfterDelete,chest, 68, 51 );
			FillBoxe( "addons", m_AddonsAfterDelete, chest, 93, 51 );
			FillBoxe( "moving crate items", m_MovingCrateAfterDelete, chest, 118, 53 );

			if( price > 0 )
			{
				BankCheck check = new BankCheck( price );
				chest.AddItem( check );
				check.Location = new Point3D( 64, 137, 0 );
			}
			else if( deed != null )
			{
                chest.AddItem( deed );
                deed.Location = new Point3D( 64, 137, 0 );
			}

			if( owner != null && !owner.Deleted )
				owner.BankBox.DropItem( chest );
			else
			{
				chest.MoveToWorld( new Point3D( 5434, 1104, 0 ), Map.Felucca ); // Green Acres
				chest.Movable = false;
			}
		}
		#endregion
		...
		public override void OnAfterDelete()
		{
            ...
            Point3D tempLoc;
            if( m_Sign != null )
                tempLoc = m_Sign.Location;
            else if( m_Region != null )
                tempLoc = m_Region.Area[ 0 ].Start;
            else
                tempLoc = new Point3D( 0, 0, 0 );

            try
            {
                using( TextWriter t = new StreamWriter( "Logs/HouseDeleteSuccess.log", true ) )
                {
                    t.WriteLine( "House deleted. Serial {0} - Owner {1} - Location {2} - Date {3} - Time {4} - Price {5}",
                                Serial, ( m_Owner != null ) ? m_Owner.Name : "null", tempLoc,
                                DateTime.Now.ToShortDateString(), DateTime.Now.ToShortTimeString(), Price );
                }
            }
            catch
            {
			...
			{
				#region modifica by Dies Irae
				m_LockDownsAfterDelete = new List<Item>();
				#endregion
				...
				for ( int i = 0; i < m_LockDowns.Count; ++i )
				{
					...
					if ( item != null )
					{
						...
						#region modifica by Dies Irae
						if( HouseDeleteMode == HouseDeleteModes.ToCrate )
							m_LockDownsAfterDelete.Add( item );
						#endregion
					}
				}
			...
			{
				#region modifica by Dies Irae
				m_VendorRentalContractsAfterDelete = new List<Item>();
				#endregion
				...
				for ( int i = 0; i < VendorRentalContracts.Count; ++i )
				{
					...
					if ( item != null )
					{
						...
						#region modifica by Dies Irae
						if( HouseDeleteMode == HouseDeleteModes.ToCrate )
							m_VendorRentalContractsAfterDelete.Add( item );
						#endregion
					}
				}
			...
			{
				#region modifica by Dies Irae
				m_SecuresAfterDelete  = new List<Item>();
				#endregion
				...
				for ( int i = 0; i < m_Secures.Count; ++i )
				{
					...
					else
					{
						...
						#region modifica by Dies Irae
						if( HouseDeleteMode == HouseDeleteModes.ToCrate )
							m_SecuresAfterDelete.Add( info.Item );
						#endregion
					}
				}
			...
			{
				#region modifica by Dies Irae
				m_AddonsAfterDelete  = new List<Item>();
				#endregion
			...
			{
				#region modifica by Dies Irae
				if( HouseDeleteMode == HouseDeleteModes.ToCrate && MovingCrate.Items.Count > 0 )
				{
					m_MovingCrateAfterDelete = new List<Item>();
					for( int i = 0; i < MovingCrate.Items.Count; i++ )
						m_MovingCrateAfterDelete.Add( MovingCrate.Items[i] );
				}
				#endregion
				MovingCrate.Delete();
			}
			...
			if( HouseDeleteMode == HouseDeleteModes.ToCrate )
			{
				try
				{
					OrganizeAfterDeleteChest( m_Owner, tempLoc, tempPrice, GetDeed() );
				}
				catch( Exception e )
				{
					using( TextWriter t = new StreamWriter( "Logs/HouseDeleteErrors.log", true ) )
					{
						Console.WriteLine( e.ToString() );
						t.WriteLine( e.ToString() );
					}				
				}				
			}
			#endregion
        ...
        #region mod by Dies Irae
        public static bool HasAccountHouses( Mobile m )
        {
            Account a = m.Account as Account;
		...
		public void RemoveKeys( Mobile m )
		{
			if ( m_Doors != null )
			{
				...
				for ( int i = 0; /* keyValue == 0 && */ i < m_Doors.Count; ++i ) // mod by Dies Irae
				{
					BaseDoor door = m_Doors[i] as BaseDoor;
		...
		public void ChangeLocks( Mobile m )
		{
			...
			if ( m_Doors != null )
			{
				for ( int i = 0; i < m_Doors.Count; ++i )
				{
                    // mod by Dies Irae: solo le front door vanno riloccate
                    if( OldSchoolRules )
                    {
                        BaseHouseDoor houseDoor = m_Doors[ i ] as BaseHouseDoor;
                        if( houseDoor != null )
                        {
                            if( !houseDoor.IsInternal )
                                houseDoor.KeyValue = keyValue;
                        }
                    } // end mod
                    else
                    {
			...
			if ( targeted is Item )
			{
				...
				else
				{
					...
					else
					{
                        ...
                        {
                            if( ( (Item)targeted ).Amount > 2 && !( targeted is Gold ) )
                            {
	...
	public class HouseKickTarget : Target
	{
		...
		protected override void OnTarget( Mobile from, object targeted )
		{
			...
			if ( targeted is Mobile && targeted != from ) // mod by Dies Irae
			{
    ...
    {
        public static readonly TimeSpan DelayOfBan = TimeSpan.FromDays( 1.0 );

        public BanInfo( Mobile banned, DateTime timeOfBan )
        {
            Banned = banned;
            TimeOfBan = timeOfBan;
        }

        public Mobile Banned { get; private set; }

        public DateTime TimeOfBan { get; private set; }
        ...
        public bool IsExpired()
        {
            ...
            return DateTime.Now > TimeOfBan + DelayOfBan;
        }
        ...
        public BanInfo( GenericReader reader )
        {
            int version = reader.ReadInt();
            ...
            switch( version )
            {
                case 0:
                    {
                        Banned = reader.ReadMobile();
                        TimeOfBan = reader.ReadDateTime();
                        break;
                    }
            }
        }
        ...
        public void Serialize( GenericWriter writer )
        {
            ...
            writer.WriteMobile( Banned );

            if( TimeOfBan < DateTime.MaxValue && TimeOfBan > DateTime.MinValue )
                writer.Write( TimeOfBan );
            else
