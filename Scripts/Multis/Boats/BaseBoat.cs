...
namespace Server.Multis
{
	...
	public abstract class BaseBoat : BaseMulti
	{
	    ...
	    #region mod by Dies Irae
	    public virtual int HitsMax { get { return 1000; } }
	    public virtual int ResistFire { get { return 0; } }
	    public virtual int ResistPhysical { get { return 70; } }

        public virtual int BoatPrice { get { return 1000; } }
        public virtual string Description { get { return ""; } }
        public virtual int HoldSize { get { return 400; } }

	    public virtual void AddXmlSiegeAttachment()
	    {
            XmlBoatSiege xmlSiege = new XmlBoatSiege( HitsMax, ResistFire, ResistPhysical );
	        xmlSiege.Name = "xmlSiege";
	        XmlAttach.AttachTo( this, xmlSiege );
	    }
	    #endregion
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            writer.Write( Fixtures != null );
            if( Fixtures != null )
                writer.WriteItemList( Fixtures, false ); // mod by Dies Irae
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
            {
                ...
                case 4:
                {
                    bool hasFixtures = reader.ReadBool();
                    if( hasFixtures )
                        Fixtures = reader.ReadStrongItemList<BoatFixture>();
		...
		public override void OnAfterDelete()
		{
            ...
            if( Fixtures != null )
            {
                ...
                {
                    BoatFixture fixture = Fixtures[ i ];
                    fixture.Delete();
                }
            }
            #endregion
		...
		public bool StartMove( Direction dir, bool fast )
		{
			...
			if ( StartMove( dir, speed, interval, false, true ) )
			{
                ...
                #region mod by Dies Irae
                if( m_TillerMan != null && m_TillerMan.CanSpeech )
			        RestartTimer();
                #endregion
		...
		public void BeginDryDock( Mobile from )
		{
            ...
            else if( result == DryDockResult.Valid )
            {
                ...
                if( Core.AOS )
                    ...
                    from.SendGump( new SmallConfirmGump( "Drydock this Boat?", DrydockResponse ) );
                #endregion
            }
        }

        #region "ancient stories" by Dies Irae
        private string[] m_FemaleStories = new string[]
	    {
	        "turned into an octopus?",
	        "told me she was from another world?",
	        "claimed to be an Avatar of the virtues?",
	        "said she knew Lord British personally?",
	        "had golden hair as lovely as the sun?",
	        "kicked me really hard when I got friendly?",
	        "served the sweetest apple wine I ever tasted?",
	        "made the best apple pies I ever saw?",
	        "made the worst clam chowder I ever tasted?",
	        "thought the world was round? Ha!",
	        "claimed to have visited a magical city in the mountains?",
	        "dared me to race across Brittany Bay?",
	        "claimed to own a fleet of merchant ships?",
	        "threatened to marry me?",
	        "claimed to know thee? I think she was lying, though.",
	        "turned out to be a powerful mage?",
	        "turned out to be a follower of Blackthorn?",
	        "turned out to be married?",
	        "turned out to be a pirate?",
	        "was dead but didn't know it?",
	        "had found sunken treasure but spent it all?",
	        "loved moonlight so much she slept during the day?",
	        "was so scared of the dark she always had a lit lantern?",
	        "thought I was terribly handsome?",
	        "told me I was the ugliest old salt she'd ever seen?",
	        "offered me a kiss and a wish, and like an idiot I took the kiss? She vanished in a puff of smoke right afterwards.",
	        "turned me into a toad? It stuck for two weeks, until another toad kissed me.",
	        "accidentally dropped a tray of ale mugs on my lap?",
	        "used to braid fox tails into her hair?",
	        "played the lute like nothing you ever heard?",
	        "sang with an awful screech?",
	        "wore so much plate mail she rusted solid and starved to death when it rained?"
	    };

        private string[] m_MaleStories = new string[]
	    {
	        "turned into a sea serpent?",
	        "claimed to be an Avatar of the virtues?",
	        "claimed to be Lord British? I beat him up.",
	        "got in a duel and beat three rogues with only a pocketknife?",
	        "tried to make friendly with a tavern wench and got ale dumped on his head?",
	        "got so drunk he claimed he'd tamed a dragon?",
	        "claimed he'd tamed a dragon? They laughed, until he brought it inside the tavern!",
	        "thought this world was just a reflected image of the real one?",
	        "claimed a city named Wind exists somewhere up in the mountains?",
	        "dared me to sail around the world? He tried, and was never seen again.",
	        "claimed to be a pirate who sank whole merchant fleets?",
	        "claimed to have four wives in different cities?",
	        "said he had heard of thee? I figured he must have been lying.",
	        "turned out to be a master mage?",
	        "turned out to be a follower of Blackthorn?",
	        "turned out to be a ghost?",
	        "turned out to be a thief?",
	        "kept muttering about Shadowlords, whatever they are.",
	        "claimed to be some sort of Time Lord?",
	        "said he'd found a piece of Exodus, but accidentally dropped it in a well?",
	        "turned out to be a lich who wanted to steal my soul?",
	        "turned out to be one of Lord British's guards on a secret mission?",
	        "had some get rich quick scheme involving selling tickets to watch people kick a ball around?",
	        "was the best healer I've ever seen... until he caught ill and died.",
	        "liked the wilderness so much he got hives when he was in the city?",
	        "insisted that sailing west would eventually take you to the land where all the dead live?",
	        "said his dog was so smart it could talk?",
	        "danced on the tavern table, but fell off and spitted himself on his sword?",
	        "could tell you all about every sort of animal there ever was?",
	        "built himself a castle out in the woods?",
	        "was having a ship built in the harbor? It was huge, but it sank at the christening.",
	        "used to mutter a spell over his food, just in case it was poisoned.",
	        "said he'd been swallowed by a whirlpool, and it had turned his hair white?"
	    };

        private const string MainStory = "Ar! Did I ever tell thee about the time I was in [randomCity] and I met [randomIncipit] [randomStory]. Anyway, 'tis a dull story.";

        private static string[] m_RandomIncipit = new string[] { "a wench who", "a fellow who" };

        private bool HasValidListeners()
        {
            Map map = Map;
            if( map == null || map == Map.Internal )
                return false;
            ...
            foreach( object o in eable )
            {
                ...
                if( o is Mobile && IsValidListener( (Mobile)o ) )
                {
                    eable.Free();
                    return true;
                }
        ...
        private bool IsValidListener( Mobile m )
        {
            return m != null && Contains( m ) && m.Alive && m_TillerMan != null && m.InLOS( m_TillerMan );
        }

        private static string[] m_Cities = new string[]
	    {
	        "Britain", "Buccaneer's Den", "Jhelom", "Magincia", "Minoc", "Moonglow", "Ocllo", "Serpent's Hold", "Skara Brae", "Trinsic", "Vesper", "Yew"
	    };

        private string GetRandomCity()
        {
            return m_Cities[ Utility.Random( m_Cities.Length ) ];
        }

        private string GetRandomStory( bool female )
        {
            if( female )
                return m_MaleStories[ Utility.Random( m_MaleStories.Length ) ];
            else
                return m_FemaleStories[ Utility.Random( m_FemaleStories.Length ) ];
        }

        private void TellStory()
        {
            String story = MainStory;
            story = story.Replace( "[randomCity]", GetRandomCity() );

            bool female = Utility.RandomBool();
            story = story.Replace( "[randomIncipit]", m_RandomIncipit[ female ? 0 : 1 ] );
            story = story.Replace( "[randomStory]", GetRandomStory( female ) );

            TillerSay( story );
        }

        private void TillerSay( string message )
        {
            if( m_TillerMan != null )
                m_TillerMan.PublicOverheadMessage( MessageType.Regular, 0x3B2, true, message );
        }

        private StoryTimer m_StoryTimer;

        private void RestartTimer()
        {
            if( m_StoryTimer != null && m_StoryTimer.Running )
            {
                m_StoryTimer.Stop();
                m_StoryTimer = null;
            }

            m_StoryTimer = new StoryTimer( this );
            m_StoryTimer.Start();
        }

        private class StoryTimer : Timer
        {
            private BaseBoat m_Owner;

            public StoryTimer( BaseBoat owner )
                : base( TimeSpan.FromSeconds( Core.Debug ? 5.0 : Utility.Dice( 1, 100, 100 ) ), TimeSpan.FromSeconds( 100.0 ) )
            {
                m_Owner = owner;
            ...
            protected override void OnTick()
            {
                if( m_Owner.Deleted || m_Owner.Map == Map.Internal || !m_Owner.HasValidListeners() )
                {
                    m_Owner.TillerSay( "Nobody want to listen a story of mines..." );
                    Stop();
                }

                m_Owner.TellStory();

                Interval = TimeSpan.FromSeconds( Core.Debug ? 5.0 : Utility.Dice( 1, 100, 100 ) );

                if( Core.Debug )
                    m_Owner.TillerSay( string.Format( "My next story will be in {0} seconds...", Interval.TotalSeconds ) );
            }
        }
        #endregion

        #region mod by Dies Irae
	    public List<BoatFixture> Fixtures { get; set; }

	    public virtual void AddFixture( int itemid, int x, int y, int z, int hue )
        {
            if( Fixtures == null )
                Fixtures = new List<BoatFixture>();

            BoatFixture fixture = new BoatFixture( this, itemid, hue );
            Fixtures.Add( fixture );
            fixture.MoveToWorld( new Point3D( X + x, Y + y, Z + z ), Map );
        }

        public virtual void UpdateFixtures( int xOffset, int yOffset )
        {
            if( Fixtures == null )
                return;

            for( int i = 0; i < Fixtures.Count; i++ )
                Fixtures[ i ].OffsetFixture( xOffset, yOffset );
        }

	    private void DrydockResponse( Mobile from, bool okay, object state )
	    {
            if( okay )
	            EndDryDock( from );
	    }

        public List<Mobile> GetMobilesOnBoat()
        {
            Map map = Map;
            if( map == null || map == Map.Internal )
                return null;
        ...
        public bool IsOnDeck( Point3D loc )
        {
            ...
            List< Tile > list = Map.GetTilesAt( new Point2D( loc.X, loc.Y ), false, false, true );
        ...
        public static BaseBoat FindBoatAt( Mobile m )
        {
            ...
            return FindBoatAt( m.Location, m.Map );
        }
        ...
        public static BaseBoat FindBoatAt( Item item )
        {
            ...
            return FindBoatAt( item.GetWorldLocation(), item.Map );
        }
        ...
        /// </summary>
        /// <returns></returns>
        public List<Mobile> GetMobilesOnDeck()
        {
        ...
        public Point3D FindSpawnLocationOnDeck()
        {
            ...
            for( int x = 0; x < mcl.Width; x++ )
                for( int y = 0; y < mcl.Height; y++ )
                    locations.Add( new Point3D( X + mcl.Min.X + x, Y + mcl.Min.Y + y, Z + 3 ) );

            while( locations.Count > 0 )
            {
                int rnd = Utility.Random( locations.Count - 1 );

                if( Map.CanSpawnMobile( locations[ rnd ] ) )
        ...
        public bool CorpseHasKey( Mobile m )
        {
            ...
            foreach( Object o in eable )
            {
                ...
                if( m_PPlank == null || m_SPlank == null )
                    continue;
        ...
        public virtual void BeginCommand( Mobile from )
        {
            if( CanCommand( from ) && Contains( from ) /*&& from.InRange( m_TillerMan, 5 )*/ )
            {
                #region mod by Dies Irae
                List<Mobile> list = GetMobilesOnBoat();
                if( from.AccessLevel == AccessLevel.Player && ( list == null || list.Count < MinSailors ) )
                {
                    from.SendMessage("There are not enough sailors on the boat to do that.");
                    return;
                }
                #endregion

                if( !from.HasGump( typeof( BoatGump ) ) )
                {
                    from.SendGump( new BoatGump( this ) );
                }
            }
            else
            {
                from.SendMessage( "Only the owner of the ship may command on the deck!" );
            }
        }
	    #endregion
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
            ...
            #region mod by Dies Irae
            if( !e.Handled && from.InRange( this, 10 ) && from.CanSee( this ) )
            {
                if( CanCommand( from ) && Contains( from ) )
                {
                    if( Insensitive.Equals( e.Speech, "Status" ) )
                    {
                        int hits = XmlSiege.GetHits( this );
                        int hitsMax = XmlSiege.GetHitsMax( this );
                        e.Mobile.SendMessage( "This boat has {0}/{1} structural points.", hits, hitsMax );
                        return;
                    }
                }

                foreach( int kw in e.Keywords )
                {
                    if( kw == 0x2F ) //shazam
                        Shazam( from );
                }
		...
		public void Shazam( Mobile from )
		{
			if( this.m_TillerMan == null )
				return;

			if( from.Alive )
				m_TillerMan.PublicOverheadMessage( MessageType.Regular, 30, true, "Arrrr! You are not a dead fish, guy!" );
			else
				from.SendGump( new ResurrectGump( from, ResurrectMessage.Healer ) );
		}

        public virtual bool CanTurn { get { return true; } }
		#endregion
		...
		public bool CanFit( Point3D p, Map map, int itemID )
		{
			...
			foreach ( Item item in eable )
			{
                ...
                if( item is BoatFixture )
                    continue;

				eable.Free();
                Console.WriteLine( "Item {0} blocked a galleon placement ", item.GetType().Name );
				return false;
			}
		...
		public void Teleport( int xOffset, int yOffset, int zOffset )
		{
			...
			foreach ( object o in eable )
			{
				if ( o != this && !(o is TillerMan || o is Hold || o is Plank || o is BoatFixture || o is AddonComponent ) )
					toMove.Add( o );
			}
			...
			for ( int i = 0; i < toMove.Count; ++i )
			{
                ...
                #region mod by Dies Irae
                if( o is ISiegeWeapon && o is Item )
                {
                    Item item = (Item)o;

                    Console.WriteLine( "Item is Addon. Offset is " + xOffset );
                    Console.WriteLine( "OLD" + item.Location );
                    item.Location = new Point3D( item.X + xOffset, item.Y + yOffset, item.Z + zOffset );
                    Console.WriteLine( "NEW" + item.Location );
                }
                else if( o is BaseAddon )
                {
                    Item item = (Item)o;

                    Console.WriteLine( "Item is Addon, not ISiegeWeapon. Offset is " + xOffset );
                    Console.WriteLine( "OLD" + item.Location );
                    item.Location = new Point3D( item.X + xOffset, item.Y + yOffset, item.Z + zOffset );
                    Console.WriteLine( "NEW" + item.Location );
                }
                #endregion
                else if( o is Item && !(o is ISiegeWeapon) )
                {
                    Item item = (Item)o;
