...
namespace Server.Targets
{
	public class AIControlMobileTarget : Target
	{
		...
		protected override void OnTarget( Mobile from, object o )
		{
            ...
            if( m_Order == OrderType.Transfer && o is IPoint3D && o is AnimalTrainer && m_List.Count > 0 )
            {
                AnimalTrainer trainer = o as AnimalTrainer;

                if( trainer.GetSellPriceFor( m_List[ 0 ].m_Mobile ) > 0 )
                    from.SendGump( new Midgard.Gumps.ConfirmSellPetGump( (AnimalTrainer)o, new Point3D( (IPoint3D)o ), m_List[ 0 ].m_Mobile ) );
                else
                    trainer.Say( "I'm not interested in." );
            }
            #endregion
