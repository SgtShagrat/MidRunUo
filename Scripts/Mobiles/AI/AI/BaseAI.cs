...
using EtherealVoyageSpell = Server.Spells.Spellweaving.EtherealVoyageSpell;
...
namespace Server.Mobiles
{
	...
	public abstract class BaseAI
	{
		...
		#region modifica by Dies Irae per la disattivazione della AI
        private DateTime m_DeactivationTime;
        public virtual double DeactivationDelay { get { return SetDeactivation.DefaultDeactivationDelay; } }  // delay in minuti
		#endregion
		...
		public virtual void OnAggressiveAction( Mobile aggressor )
		{
            ...
            if( currentCombat != null && !aggressor.Hidden && currentCombat != aggressor && aggressor.Player && Utility.Random( 10 ) < 3 )
                m_Mobile.Combatant = aggressor;
            #endregion
		}
		...
		public virtual void EndPickTarget( Mobile from, Mobile target, OrderType order )
		{
			...
			if( order == OrderType.Attack )
			{
				...
				if( target is Factions.BaseFactionGuard || target is BaseTownGuard || target is BaseGuard ) // mod by Dies Irae
				{
					m_Mobile.SayTo( from, "Your pet refuses to attack the guard." );
					return;
				}
				//fix cavalli impazziti
				if ( (m_Mobile.ControlMaster).CanBeHarmful( target ) )
				{
					(m_Mobile.ControlMaster).DoHarmful( target );
				}
			}
		...
		public virtual bool HandlesOnSpeech( Mobile from )
		{
            ...
            #region modifica by Dies Irae : talking vendors and fragment speech system
            if( m_Mobile.IsSpeechEnabled && m_Mobile.Speech != null && TalkingVendors.Core.HandlesOnSpeech( m_Mobile, from ) )
                return true;

            if( m_Mobile.IsSpeechEnabled && from.Alive && from.Player && from.InRange( m_Mobile.Location, 3 ) && m_Mobile.Body.IsHuman )
                return true;
            #endregion
		...
		private static SkillName[] m_KeywordTable = new SkillName[]
			{
				...
				SkillName.Chivalry, // mod by Dies Irae
				SkillName.Necromancy  // mod by Dies Irae
			};

        #region taming system : taming system
        public enum CommandAbilityTypes
        {
            WordOfHealing,
            WordOfThunder,
            WordOfGreetings,
	    ...
	    private string[] m_CommandkeyWords = new string[]
        {
            "Heal Me",
            "Thunder",
            "Greet",
        ...
        private DateTime m_NextCommandAbility;

        public virtual TimeSpan CommandAbilityDelay { get { return TimeSpan.FromMinutes( 0.1 ); } }

        [CommandProperty( AccessLevel.GameMaster )]
        public DateTime NextCommandAbility
        {
            get { return m_NextCommandAbility; }
            set { m_NextCommandAbility = value; }
        }

        public virtual bool DoCommandAbility( Mobile from, CommandAbilityTypes command )
        {
			if( m_Mobile.Deleted )
				return false;
            ...
            switch( command )
            {
                case CommandAbilityTypes.WordOfHealing:
                    return DoWordOfHealing( from );
                case CommandAbilityTypes.WordOfThunder:
                    return DoWordOfThunder( from );
                case CommandAbilityTypes.WordOfGreetings:
                    return DoWordOfGreetings( from );
                case CommandAbilityTypes.WordOfPoison:
                    return DoPoisonAttack( from );
                default:
                    return false;
            }
        }

        public virtual bool DoWordOfHealing( Mobile from )
        {
            if( from.Poisoned && ( from.Hits < 40 ) )
            {
                Poison p = from.Poison;

                if( p != null )
                {
                    int chanceToCure = 10000 + (int)( m_Mobile.Skills[ SkillName.Magery ].Value * 75 ) - ( ( p.Level + 1 ) * ( Core.AOS ? ( p.Level < 4 ? 3300 : 3100 ) : 1750 ) );
                    chanceToCure /= 100;

                    if( chanceToCure > Utility.Random( 100 ) )
                    {
                        if( from.CurePoison( m_Mobile ) )
                        {
                            from.LocalOverheadMessage( MessageType.Regular, 0x3B2, true, 
                                "Your friend senses you are in danger and aids you with magic." );
        ...
        public virtual bool DoWordOfThunder( Mobile from )
        {
            if( m_Mobile.Combatant != null && m_Mobile.ControlOrder == OrderType.Attack )
            {
                Mobile target = m_Mobile.Combatant;

                if( target != null && m_Mobile.CanBeHarmful( target ) && m_Mobile.Map == target.Map && m_Mobile.InRange( target, 18 ) )
                {
                    target.BoltEffect( 0 );
                    target.Damage( Utility.RandomMinMax( 35, 100 ), m_Mobile, false );

                    from.LocalOverheadMessage( MessageType.Regular, 0x3B2, true,
                                              "Your friend calls down the forces of nature on your opponent" );
                    m_Mobile.FixedParticles( 0, 0, 0, 0x13A7, EffectLayer.Waist );
                    m_Mobile.PlaySound( 0xA9 );
                    return true;
                }
        ...
        public virtual bool DoWordOfGreetings( Mobile from )
        {
            m_Mobile.PublicOverheadMessage( MessageType.Regular, 0x3B2, true, String.Format( "I am honored to serve {0}, my friend.", from.Name ) );
            return true;
        }

        public virtual bool DoPoisonAttack( Mobile from )
        {
            if( m_Mobile.Combatant != null && m_Mobile.ControlOrder == OrderType.Attack )
            {
                Mobile target = m_Mobile.Combatant;
                ...
                if( target != null && m_Mobile.CanBeHarmful( target ) && m_Mobile.Map == target.Map && m_Mobile.InRange( target, 18 ) )
                {
                    Effects.SendLocationParticles(
                        EffectItem.Create( target.Location, target.Map, EffectItem.DefaultDuration ), 0x36B0, 1, 14, 63, 7,
                        9915, 0 );
                    ...
                    if( m_Mobile.PetPoisonAttack > 3 )
                    {
                        mindam = m_Mobile.PetPoisonAttack / 3;
                        maxdam = m_Mobile.PetPoisonAttack / 2;
                    }
                    else
                    {
                        mindam = 1;
                    ...
                    int level = m_Mobile.PetPoisonAttack / 20;

                    if( level > 5 )
                        level = 5;

                    target.ApplyPoison( from, Poison.GetPoison( level ) );
        ...
        public bool IsFocusOnSpeaker( Mobile speaker )
        {
            return speaker != null && m_Mobile.FocusMob == speaker;
        }

        public bool SetFocunOn( SpeechEventArgs e, bool greet, bool checkNeighbours )
        {
            m_Mobile.DebugSay( "I'm checking the focus..." );
            if( checkNeighbours )
                m_Mobile.DebugSay( "And I will check for neighbours..." );

            if( greet )
                HandleGreetings( e );

            if( IsFocusOnSpeaker( e.Mobile ) )
            {
                m_Mobile.DebugSay( "I'm already focusing on {0}.", e.Mobile == null ? "no-one" : e.Mobile.Name );
                return true;
            }
            ...
            if( WasNamed( e.Speech ) )
            {
                ...
                m_Mobile.DebugSay( "I was named by {0}.", e.Mobile == null ? "no-one" : e.Mobile.Name );
                return true;
            }

            if( checkNeighbours && !WasNamed( e.Speech ) && SomeoneElseIsTalkingTo( e.Mobile ) )
            {
                m_Mobile.DebugSay( string.Format( "Someone else is talking to {0}.", e.Mobile.Name ?? "him" ) );
                return false;
            }
        ...
        public void HandleGreetings( SpeechEventArgs e )
        {
            if( m_LastGreeter == e.Mobile && DateTime.Now - m_LastGreet < TimeSpan.FromSeconds( 3.0 ) )
            {
                m_Mobile.DebugSay( "My friend greets to much times..." );

                string resp = m_Mobile.GetReHello();
                if( resp != null && m_Mobile.IsSpeechEnabled )
                    m_Mobile.Say( resp );

                m_Mobile.AdjustAttitude( DiceRoll.Roll( "1d50" ) );

                m_LastGreet = DateTime.Now;
                m_LastGreeter = e.Mobile;
            }
            else
                DoGreet( e.Mobile );
        }

        public void DoGreet( Mobile to )
        {
            string greetings = m_Mobile.GetGreetings();
            if( greetings != null && m_Mobile.IsSpeechEnabled )
                m_Mobile.Say( greetings );
            else if( m_Mobile.IsSpeechEnabled )
                m_Mobile.Say( true, "Thou hast mine attention." );

            m_Mobile.AdjustAttitude( GetAttitudeOffsetForKarma( to.Karma, true ) );

            m_LastGreet = DateTime.Now;
            m_LastGreeter = to;
        }

        public static int GetAttitudeOffsetForKarma( int karma, bool isGreet )
        {
            int offset = 0;

            if( karma > 10000 )
                offset = -1 * DiceRoll.Roll( "1d500+100" );
            else if( karma > 5000 )
                offset = -1 * DiceRoll.Roll( "1d100+100" );
            else if( karma > 1000 )
                offset = -1 * DiceRoll.Roll( "1d50+50" );
            else if( karma > 0 )
                offset = -1 * DiceRoll.Roll( "1d50" );
            else if( karma > -1000 )
                offset = DiceRoll.Roll( "1d50" );
            else if( karma > -5000 )
                offset = DiceRoll.Roll( "1d50+50" );
            else if( karma > -10000 )
                offset = DiceRoll.Roll( "1d100+100" );
            else
                offset = DiceRoll.Roll( "1d500+100" );
        ...
        public bool SomeoneElseIsTalkingTo( Mobile to )
        {
            m_Mobile.DebugSay( "I'm checking if another being nearby has focused our friend." );

            // Check if another talkingMobile nearby has focused our talker
            foreach( Mobile m in m_Mobile.GetMobilesInRange( m_Mobile.RangePerception ) )
            {
                if( m is BaseCreature )
                {
                    ...
                    // like escortables
                    /*
                    if( bc.Controlled && bc.ControlMaster == to )
                    {
                        m_Mobile.DebugSay( "{0} is a friend of {1}. I will ignore that being.", bc.Name ?? "", to.Name ?? "" );
                        continue;
                    }
                    */

                    // If it is so, don't process anything
                    if( bc != m_Mobile && bc.SupportSpeech && bc.FocusMob == to )
                    {
                        m_Mobile.DebugSay( string.Format( "{0} is dealing with him.", bc.Name ) );
                        return true;
                    }
                }
            }
        ...
        public virtual void OnSpeech( SpeechEventArgs e )
        {
            ...
            {
                    if( DateTime.Now > m_NextCommandAbility )
                    {
                        m_Mobile.DebugSay( "Listening for special commands..." );
                        ...
                        if( e.Mobile.Alive && ( isOwner || isFriend ) )
                        {
                            ...
                            if( !m_Mobile.IsDeadPet && WasNamed( e.Speech ) && m_Mobile.CheckControlChance( e.Mobile ) )
                            {
                                for( int i = 0; i < m_CommandkeyWords.Length; i++ )
                                {
                                    if( e.Speech.ToLower().Contains( m_CommandkeyWords[ i ] ) )
                                    {
                                        if( DoCommandAbility( e.Mobile, (CommandAbilityTypes)i ) )
                                            m_NextCommandAbility = DateTime.Now + CommandAbilityDelay;
                                    }
                                }
                            }
                        }
            ...
            bool isSpecialActiveTalker = ( m_Mobile.SupportSpeech && m_Mobile.IsSpeechEnabled && m_Mobile.Speech != null ) || 
                ( m_Mobile.IsSpeechEnabled && m_Mobile.HasSpeechFragment );

            m_Mobile.DebugSay( "I'm dealing on speech..." );

            if( e.Mobile.Alive && e.Mobile.InRange( m_Mobile.Location, 3 ) && e.Mobile.InLOS( m_Mobile ) && ( m_Mobile.IsHumanInTown() || isSpecialActiveTalker ) )
			{
                #region mod by Dies Irae
                m_Mobile.DebugSay( "It's all ok for speech..." );

                if( !SetFocunOn( e, e.HasKeyword( 0x003B ), true ) ) // greetings hail hello hey yo
                {
                    m_Mobile.DebugSay( "I cant set focus on." );
                    return;
                }

                m_Mobile.AddToMemoryRecent( e.Mobile );
                #endregion
			    ...
			    #region [speech system by Derrick]
                if( !e.Handled && IsFocusOnSpeaker( e.Mobile ) && m_Mobile.IsSpeechEnabled )
                {
                    if( Insensitive.Contains( e.Speech, "recent memory" ) )
                        m_Mobile.SpamRecentMemory();

                    if( Insensitive.Contains( e.Speech, "noto memory" ) )
                        m_Mobile.SpamRecentMemory();

                    if( Insensitive.Contains( e.Speech, "attitude" ) )
                        m_Mobile.Say( "My attitude now is {0}.", m_Mobile.Attitude );

                    List<int> words = e.Keywords == null ? new List<int>() : new List<int>( e.Keywords );

                    string keyFound = null;

                    string resp = m_Mobile.GetSpeechResponce( e.Speech, words, out keyFound );
                    if( !string.IsNullOrEmpty( resp ) )
                    {
                        m_Mobile.DebugSay( "I'm responding to: {0}", keyFound ?? "no key" );

                        if( !string.IsNullOrEmpty( keyFound ) || Utility.Random( 5 ) == 0 )
                            m_Mobile.Say( resp );

                        if( !string.IsNullOrEmpty( keyFound ) )
                            m_Mobile.AdjustAttitude( GetAttitudeOffsetForKarma( e.Mobile.Karma, false ) );

                        // NB: if another creature is focusing on our mob the speech is not entirely handled.
                        // This is the case when we say 'Allan bank' while being escorted.
                        e.Handled = !SomeoneElseIsTalkingTo( e.Mobile );

                        m_Mobile.DebugSay( "GetSpeechResponce: handled: {0}", e.Handled );
                        return;
                    }

                    if( isSpecialActiveTalker && TalkingVendors.Core.OnSpeech( m_Mobile, e, false, false ) )
                    {
                        e.Handled = !SomeoneElseIsTalkingTo( e.Mobile );

                        m_Mobile.DebugSay( "TalkingVendors.Core.OnSpeech: handled: {0}", e.Handled );
                        return;
                    }
                }
			    #endregion
			...
			if( m_Mobile.Controlled && m_Mobile.Commandable )
			{
				...
				if( e.Mobile.Alive && (isOwner || isFriend) )
				{
                    ...
                    #region mod by Dies Irae
                    if( m_Mobile.Summoned && isOwner && Insensitive.Equals( e.Speech, "all release" ) )
                    {
					...
					for( int i = 0; i < keywords.Length; ++i )
					{
						...
						switch( keyword )
						{
							...
							case 0x16E: // *transfer
							{
								...
								if( !m_Mobile.IsDeadPet && WasNamed( speech ) && m_Mobile.CheckControlChance( e.Mobile ) )
								{
								    //Summoned Creature can be tranfer now. - MagiusCHE
									/*if( m_Mobile.Summoned )
		...
		public virtual bool DoOrderCome()
		{
			if( m_Mobile.ControlMaster != null && !m_Mobile.ControlMaster.Deleted )
			{
				...
				else
				{
					...
					bool WMR = WalkMobileRange( m_Mobile.ControlMaster, 1, bRun, 0, 1 );
					if ( m_Mobile.ControlMaster != null && m_Mobile.ControlMaster.Skills.Necromancy.Value > 0 )
					{
					...
					if ( WMR )
					{
						m_Mobile.DebugSay( "I made a step successfully" );
						...
						m_Mobile.DebugSay( "I cant walk upon my Master" );
				}
			}
		...
		public virtual bool DoOrderFollow()
		{
			...
			else if( m_Mobile.ControlTarget != null && !m_Mobile.ControlTarget.Deleted && m_Mobile.ControlTarget != m_Mobile )
			{
				...
				else
				{
					...
					// Not exactly OSI style, but better than nothing.
					bool bRun = (iCurrDist > 5);
					bool WMR = WalkMobileRange( m_Mobile.ControlTarget, 1, bRun, 0, 1 );
					if ( m_Mobile.ControlMaster != null && m_Mobile.ControlMaster.Skills.Necromancy.Value > 0 )
					{
			...
			if( m_Mobile.ControlTarget == null || m_Mobile.ControlTarget.Deleted || m_Mobile.ControlTarget.Map != m_Mobile.Map || !m_Mobile.ControlTarget.Alive || m_Mobile.ControlTarget.IsDeadBondedPet )
			{
				...
				if( m_Mobile.FightMode == FightMode.Closest || m_Mobile.FightMode == FightMode.Aggressor )
				{
					...
					foreach( Mobile aggr in m_Mobile.GetMobilesInRange( m_Mobile.RangePerception ) )
					{
						...
						#region Md by Magius(CHE)
						var bc = aggr as BaseCreature;
						if (bc != null && (bc.GetMaster() is BaseCreature))
						   continue; //does not attack summoned NON plyer creatures.
						#endregion
		...
		public virtual bool DoOrderRelease()
		{
            ...
            #region mod by Dies Irae
            if( m_Mobile is HolyMount )
                return ( (HolyMount)m_Mobile ).DoOrderRelease();
            #endregion
        ...
        #region mod by Dies Irae
        public double ChanceToCheckMasters = 0.30;
         ...
         *  bMasters : Check for creature masters instead of simple pets
         */
        public virtual bool AcquireFocusMob( int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend, bool bFacFoe, bool bMasters )
		{
			if( m_Mobile.Deleted )
				return false;
            ...
            #region mod by Dies Irae
            if( m_Mobile.FocusMob != null && m_Mobile.FocusMob is BaseCreature && Utility.RandomDouble() < ChanceToCheckMasters )
            {
                Mobile master = ( (BaseCreature)m_Mobile.FocusMob ).GetMaster();
                // Mobile myMaster = m_Mobile.GetMaster();

                if( master != null && master.Player /* && myMaster != null && master != myMaster*/ )
                {
                    m_Mobile.DebugSay( "My combatant is only a stupid foe. I want blood from its master!" );
			...
			if( map != null )
			{
				...
				{
					if ( m == null || m.Deleted || m.Blessed ) // mod by Dies Irae
						continue;
					...
					if ( m_Mobile.Summoned && m_Mobile.SummonMaster != null )
					{
						...
						if ( !Server.Spells.SpellHelper.ValidIndirectTarget( m_Mobile.SummonMaster, m, false, false, false ) )
							continue;
					...
					if ( (m is BaseCreature) && ((BaseCreature)m).GetMaster() == m_Mobile)
						continue; //Mod by Magius(CHE): cannot attack your minions
                    ...
                    // mod by Dies Irae
                    if( TransformationSpellHelper.UnderTransformation( m, typeof( Midgard.Engines.SpellSystem.EtherealVoyageSpell ) ) )
                        continue;
		...
		private class AITimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				{
					Stop(); // modifica by Dies Irae per la disattivazione della AI
					return;
				}
