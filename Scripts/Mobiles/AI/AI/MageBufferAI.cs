...
{
	public class MageBufferAI : BaseAI
	{
		protected DateTime m_NextCastTime;
		protected DateTime m_NextBuffTime;
		protected Mobile m_found;

		public MageBufferAI( BaseCreature m ) : base( m )
		{
		}
		...
		public override bool Obey()
		{
			...
			{
				Spell spell = CheckCastBuffSpell();
		...
		private Spell CheckCastBuffSpell()
		{
			if ( DateTime.Now < m_NextBuffTime )
				return null;

			//check prima NightSight al master
			m_found = m_Mobile.GetMaster();

			if ( m_found != null && m_found.LightLevel < 20 )
			{
				//m_Mobile.PublicOverheadMessage( MessageType.Regular, 0x3B2, true ,"*nightsigth*" );
				return new NightSightSpell( m_Mobile, null );
			}

			//check poi ReactiveArmorSpell o ArchProtection-
			//if ( m_found != null && ( m_found.GetStatOffset(StatType.AR) == 0 ) )
			//{
			//	m_Mobile.PublicOverheadMessage( MessageType.Regular, 0x3B2, true ,"*arch protection*" );
			//	return new ArchProtectionSpell( m_Mobile, null );
			//}

			//check agility cunning strength ecc?
			//check bless
			//if ( toTarget.StatMods.count == 0 )
			//	return new BlessSpell( m_Mobile, null );
			//else
			m_found = FindBlessTarget();
			if ( m_found != null )
				return new BlessSpell( m_Mobile, null );
			
			//m_Mobile.PublicOverheadMessage( MessageType.Regular, 0x3B2, true ,"*no bless found*" );
			//check magic reflect
			m_found = FindMagicReflectTarget();
			if ( m_found != null )
		...
		public Mobile FindBlessTarget()
		{
			m_Mobile.DebugSay( "*Searching Bless target*" );
			...
			if ( master != null && ( master.GetStatOffset(StatType.Str) == 0 ) )
			{
				m_Mobile.DebugSay( "*master found*" );
				return master;
			}
			else if ( m_Mobile.GetStatOffset(StatType.Str) == 0 && bc.Experience > 0 )
			{
				m_Mobile.DebugSay( "*myself*" );
				return m_Mobile;
			}
			else if ( bc.Experience > 1 )
			{
				...
				foreach( Mobile mobile in m_Mobile.GetMobilesInRange( 6 ) )
				{
					...
					if( mobile is BaseNecroFamiliar )
					{
						...
						if( masterTwo != null && masterTwo == master && ( bctwo.GetStatOffset(StatType.Str) == 0 ))//altro summonato
							return mobile;
		...
		public Mobile FindMagicReflectTarget()
		{
			if ( m_Mobile.Deleted || !(m_Mobile is BaseCreature) )
				return null;
			...
			if ( master != null && ( master.MagicDamageAbsorb <= 0 ) )
				return master;
			else if ( m_Mobile.MagicDamageAbsorb <= 0 && bc.Experience > 0 )
				return m_Mobile;
			else if ( bc.Experience > 1 )
			{
				foreach( Mobile mobile in m_Mobile.GetMobilesInRange( 6 ) )
				{
					...
					if( mobile is BaseNecroFamiliar )
					{
						...
						Mobile masterTwo = bctwo.GetMaster();
						if( masterTwo != null && masterTwo == master && (mobile.MagicDamageAbsorb <= 0))//altro summonato
							return mobile;
		...
		private bool ProcessTarget()
		{
			...
			m_Mobile.DebugSay( "*processing target*" );
