...
namespace Server.Mobiles
{
	...
	{
        public override SpeechFragment PersonalFragmentObj { get { return PersonalFragment.Tailor; } } // mod by Dies Irae
        public override Type[] Quests
        {
			get{ return new Type[] 
			{ 
				typeof( TailorQuestKilts ),
                typeof( TailorQuestShirts ),
                typeof( TailorQuestFemaleStuddedChest ),
                typeof( TailorQuestFlowerGarland )
			};} 
        }
		...
		public override Item CreateBulkOrder( Mobile from, bool fromContextMenu )
		{
			...
			if ( pm != null && pm.NextTailorBulkOrder == TimeSpan.Zero && (fromContextMenu || 0.2 > Utility.RandomDouble()) )
			{
				...
				// if ( theirSkill >= 70.1 && ((theirSkill - 40.0) / 300.0) > Utility.RandomDouble() ) // mod by Dies Irae
				if ( theirSkill >= 70.1 && ((theirSkill - 20.0) / 300.0) > Utility.RandomDouble() )
					return BodHelper.CreateRandomLargeTailorBodFor( from ); // LargeTailorBOD(); // mod by Dies Irae
