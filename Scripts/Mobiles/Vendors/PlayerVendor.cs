...
namespace Server.Mobiles
{
	...
	public class VendorBackpack : Backpack
	{
        ...
        public override int DefaultMaxItems{ get{ return 250; } } // mod by Dies Irae
			...
			if ( RootParent is PlayerVendor )
			{
				...
				if ( vi != null )
				{
					...
					if ( !String.IsNullOrEmpty( vi.Description ) )
					{
						...
						item.LabelTo( from, from.Language == "ITA" ? "Descrizione: {0}" : "Description: {0}", vi.Description );
					}
				}
			}
	...
	{
        private static List<PlayerVendor> m_AllVendors = new List<PlayerVendor>(); // mod by Dies Irae
		...
		private int m_Visits; // mod by Dies Irae
		...
		{
			AllVendors.Add( this ); // mod by Dies Irae
			...
			ShopName = owner.Language == "ITA" ? "Negozio Senza Nome" : "Shop Not Yet Named";
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			if( m_ShoutEntries == null )
				m_ShoutEntries = new List<string>();

			writer.Write( (int)m_ShoutEntries.Count );

			for( int i = 0; i < m_ShoutEntries.Count; i++ )
				writer.Write( m_ShoutEntries[ i ] );
			#endregion
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 2:
				{
					...
					int numShouts = reader.ReadInt();
					for( int i = 0; i < numShouts; i++ )
						m_ShoutEntries.Add( reader.ReadString() );
		...
		#region modifica by Dies Irae per i messaggi dei vendor e per le visite
		...
		private List<string> m_ShoutEntries;
	
		public List<string> ShoutEntries
		{
			get 
			{ 
				if( m_ShoutEntries == null )
				...
				return m_ShoutEntries; 
			}
			set 
			{
				m_ShoutEntries = value; 
			}
		}
	    ...
	    public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			if( m_SellItems.Count > 0 )
				Midgard.Engines.TalkingPlayerVendorSystem.Core.HandleShout( this, m, oldLocation );
		}
		#endregion
		...
		public virtual void Destroy( bool toBackpack )
		{
			...
			if ( list.Count > 0 || HoldGold > 0 ) // No case 1
			{
				...
				else if ( ( toBackpack || House == null || !House.IsAosRules ) && this.Map != Map.Internal ) // Case 3 - Move to backpack
				{
                    ...
                    {
                        backpack.MoveToWorld( new Point3D( 5434, 1104, 0 ), Map.Felucca ); // Green Acres
		...
		{
			AllVendors.Remove( this ); // mod by Dies Irae
...
//				SetVendorItem( item, 999, "" );
		...
		public void SendOwnerGump( Mobile to )
		{
			if ( BaseHouse.NewVendorSystem )
			{
				...
				to.SendGump( new NewPlayerVendorOwnerGump( this, to ) );
			}
			else
			{
				...
				to.SendGump( new PlayerVendorOwnerGump( this, to ) );
			}
		}
		...
		public static void TryToBuy( Item item, Mobile from )
		{
			...
			{
				from.SendMessage( from.Language == "ITA" ? "Non puoi comprarlo adesso. Aspetta un minuto e riprova." : "You cannot buy this item right now.  Please wait one minute and try again." );
			}
			else
			{
		...
		public void CollectGold( Mobile to )
		{
			if ( HoldGold > 0 )
			{
				SayTo( to, to.Language == "ITA" ? "Quanto vorresti dei {0} che possiedo attualmente?" : "How much of the {0} that I'm holding would you like?", HoldGold.ToString() );
				to.SendMessage( to.Language =="ITA" ? "Inserisci la quantità di oro che vuoi ritirare (ESC per CANCELLARE):" : "Enter the amount of gold you wish to withdraw (ESC = CANCEL):" );
		...
		public int GiveGold( Mobile to, int amount )
		{
			...
			if ( amount > HoldGold )
			{
				SayTo( to, to.Language == "ITA" ? "Spiacente, ho solo {0} oro per te." : "I'm sorry, but I'm only holding {0} gold for you.", HoldGold.ToString() );
				return 0;
			}
			...
			if( m_Owner != null )
			{
				TownSystem system = TownSystem.Find( m_Owner );

				if( system != null )
					system.RegisterTransaction( amountGiven );
			}
			#endregion
		...
		#region mod by Dies Irae
		public void EditCommercialMessage( Mobile from )
		{
			from.SendMessage( from.Language == "ITA" ? "Inserisci un nuovo messaggio commerciale (50 caratteri max):" : "Enter a new commercial mesage for your shop (50 chars max):" );

			from.Prompt = new ShopEditCommercialPrompt( this );
		}
		#endregion
		...
		private class VendorPricePrompt : Prompt
		{
			...
			private void SetInfo( Mobile from, int price, string description )
			{
				...
				else
				{
					...
					{
						price = 100000000;
						from.SendMessage( from.Language == "ITA" ? "C'é un limite di 100,000,000 oro. Il prezzo è stato ridotto." : "You cannot price items above 100,000,000 gold.  The price has been adjusted." );
					}
		...
		private class CollectGoldPrompt : Prompt
		{
			...
			private void GiveGold( Mobile to, int amount )
			{
				if ( amount <= 0 )
				{
					m_Vendor.SayTo( to, to.Language == "ITA" ? "Molto bene. Mi terrò i soldi, per ora." : "Very well. I will hold on to the money for now then." );
				}
				else
				{
		...
		private class VendorNamePrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				{
					m_Vendor.SayTo( from, from.Language == "ITA" ? "Il nome non è ammissibile." : "That name is unacceptable." );
					return;
				}
				...
				#region mod by Dies Irae
				if( Core.AOS )
					from.SendGump( new NewPlayerVendorOwnerGump( m_Vendor, from ) );
				else
					from.SendGump( new PlayerVendorOwnerGump( m_Vendor, from ) );
				#endregion
			}
		}
		...
		#region mod by Dies Irae
		private class ShopEditCommercialPrompt : Prompt
		{
			private PlayerVendor m_Vendor;

			public ShopEditCommercialPrompt( PlayerVendor vendor )
			{
				m_Vendor = vendor;
			}
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				if( text.Length > 0 )
					m_Vendor.CommercialMessage = Utility.FixHtml( text );

				from.SendGump( new PlayerVendorOwnerGump( m_Vendor, from ) );
			}
		}

		public string CommercialMessage { get; private set; }

		public static List<PlayerVendor> AllVendors
		{
			get { return m_AllVendors; }
		}
		#endregion
