...
namespace Server.Mobiles
{
	public abstract class BaseMount : BaseCreature, IMount
	{
		...
		public virtual bool CheckValidRace( Mobile from )
		{
			if( RequiredRace != null && from.Race != RequiredRace )
			{
				from.SendMessage( "Only {0} can ride this mount!", RequiredRace.PluralName );
				return false;
			}
        ...
        public virtual bool CheckValidVirtue( Mobile from )
        {
			return true;         
        }
		#endregion
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			#region modifica by Dies Irae
			if( !CheckValidRace( from ) )
				return;

            if( !CheckValidVirtue( from ) )
                return;

            if( from.AccessLevel == AccessLevel.Player && Blessed )
            {
		...
		public Mobile Rider
		{
			...
			set
			{
				if ( m_Rider != value )
				{
                    ...
                    Mobile oldValue = m_Rider; // mod by Dies Irae
                   
					m_Rider = value;

                    OnRiderChanged( oldValue, value ); // mod by Dies Irae
				}
			}
		}
        ...
        /// <summary>
        /// Evento invocato quando il mount cambia il suo cavaliere
        /// </summary>
        public virtual void OnRiderChanged( Mobile oldValue, Mobile newValue )
	    {
	    }
	    #endregion
