...
namespace Server.Mobiles
{
	...
	{
        public override SpeechFragment PersonalFragmentObj { get { return PersonalFragment.Healer; } } // mod by Dies Irae
		...
		public bool CheckTownResurrect( Mobile from )
		{
            TownSystem town = TownSystem.Find( this );

            return town == null || town.CheckResurrect( this, from );
		}
		#endregion
		...
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			if ( !m.Frozen && DateTime.Now >= m_NextResurrect && InRange( m, 4 ) && !InRange( oldLocation, 4 ) && InLOS( m ) )
			{
                ...
                if( AI != AIType.AI_Healer && Region.IsPartOf( typeof( GuardedRegion ) ) )
                {
                    DebugSay( "I'll try to heal my nearbies..." );
                    ChangeAIType( AIType.AI_Healer );
                    ActiveSpeed = 0.15;
                    PassiveSpeed = 0.5;
                    RangePerception = 10;
                    FightMode = FightMode.None;
                    Timer.DelayCall( TimeSpan.FromSeconds( 10.0 ), ResetAI );
                }
			}
		}
