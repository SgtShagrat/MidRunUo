...
namespace Server.Mobiles
{
	...
	public class PlagueBeast : BaseCreature, IDevourer
	{
		...
		public override void BreathDamage_Callback( object state )
		{
			...
			if( target == null || target.Deleted || target.Map == Map.Internal )
				return;
...
#if HealthGlandInPack
		public override void OnDeath( Container c )
		{
			if ( Utility.RandomDouble() < 0.1 )
				c.DropItem( new HealtyGland() );
		...
		public override void OnCarve( Mobile from, Corpse corpse )
		{
			if ( Utility.RandomDouble() < 0.1 )
			{
				corpse.PublicOverheadMessage( Network.MessageType.Regular, 0x66B, 1066130 );
				corpse.DropItem( new HealtyGland() );
			}
