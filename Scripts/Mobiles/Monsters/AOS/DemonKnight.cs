	...
	public class DemonKnight : BaseCreature
	{
		...
		public static void DistributeArtifact( Mobile to, Item artifact )
		{
			...
			Loot.StringToLog( string.Format( "Artefatto {0} (seriale {1}) droppato in data {2} al pg {3} (seriale {4} locazione {5}).",
				              						...
				              						to.Name, to.Serial, to.Location ), "Logs/Midgard2ArtifactsLog.txt" );
			#endregion
		...
		public static int GetArtifactChance( Mobile boss )
		{
            ...
            Loot.StringToLog( string.Format( "BossType {0} - Luck {1} - Chance {2} - DateTime {3} - Location {4}",
                boss.GetType().Name, luck, chance, DateTime.Now, boss.Location ), "Logs/DoomStatistics.log" );
            #endregion
		...
		{
            #region modifica by Dies Irae
            int chance = Utility.Random( 100000 );

            Loot.StringToLog( string.Format( "BossType {0} - Chance {1} - DateTime {2} - Location {3}",
                boss.GetType().Name, chance, DateTime.Now, boss.Location ), "Logs/DoomStatistics.log" );
            #endregion

			return GetArtifactChance( boss ) > chance;
		}
		...
		[Constructable]
		public DemonKnight() : base( AIType.AI_DarkFather, FightMode.Closest, 10, 1, 0.2, 0.4 ) // AI moddata by Dies Irae
		{
		    ...
		    SetSkill( SkillName.Magery, 150 ); // 100 ); // mod by Dies Irae
