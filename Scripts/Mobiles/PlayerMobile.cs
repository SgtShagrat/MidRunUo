...
using AnimalForm=Midgard.Engines.SpellSystem.AnimalFormSpell;
using AnimalFormContext = Midgard.Engines.SpellSystem.AnimalFormSpell.AnimalFormContext;
namespace Server.Mobiles
{
	...
	public class PlayerMobile : Mobile, IHonorTarget
	{
		...
		*/
		...
		//private List<Mobile> m_AllFollowers; // Magius(CHE): Moved to Mobile.

		#region Getters & Setters
		public List<Mobile> AutoStabled { get { return m_AutoStabled; } }

		/*public List<Mobile> AllFollowers  // Magius(CHE): Moved to Mobile.
		{ 
			get
			{
		...
		protected override void OnRaceChange( Race oldRace )
		{
            ...
            base.OnRaceChange( oldRace ); // mod by Dies Irae
		}
		...
		private void ValidateEquipment_Sandbox()
		{
			try
			{
                ...
                {
                    if( obj is BaseArmor && ( (BaseArmor)obj ).MaterialType == ArmorMaterialType.Plate )
                        hasPlate = true;
                }
                #endregion
				...
				for ( int i = items.Count - 1; i >= 0; --i )
				{
					...
					if ( item is BaseWeapon )
					{
						...
						*/
                        ...
                        #region mod by Dies Irae
                        if( item is BaseRanged && hasPlate )
                            drop = true;
                        #endregion
					...
					else if ( item is BaseArmor )
					{
						...
						else if( armor.RequiredRace != null )
						{
							if( armor.RequiredRace == Race.Elf )
							{
                                if( !Midgard.Engines.Races.Core.IsElfRace( from.Race ) )
								{
									drop = true;
								}
							}
							else if( armor.RequiredRace != Race )
							{
								drop = true;
							}
						}
                        else if( !Midgard.Engines.Races.Morph.CheckArmorAllowed( this ))
                        {
                            drop = true;
                        }
						/*
					...
					else if ( item is BaseClothing )
					{
						...
						#region Modifica By Dies Irae
                        else if( clothing.RequiredRace != null )
                        {
                            ...
                            else if( clothing.RequiredRace != Race )
                            {
                                drop = true;
                            }
                        }
		...
		public override void RevealingAction()
		{
			...
			IsStealthing = false; // IsStealthing should be moved to Server.Mobiles // note by magiusche: use Property to cast chain events :-)
		}
		...
		#region Edit By Arlas [AREA SPELLS]
		public override bool CanAreaHarmful( Mobile target )
		{
			...
			else if (target is BaseCreature)
			{
				...
				Mobile master = bc.GetMaster();//se master = null non ha master.
				...
				else//è un tamato/summonato
				{
					if (master.Player)
					{
						if (master == this)//mio tamato/summonato
							return false;
						else
						{
							Server.Engines.PartySystem.Party p = Server.Engines.PartySystem.Party.Get( master );
							if( p != null && p.Contains( this ) )
								return false;

							if( master is PlayerMobile && master.Guild != null && Guild != null && master.Guild == Guild )
								return false; // per permettere ai co-gildati di allenarsi
		...
		public override bool Move( Direction d )
		{
			...
			if ( !Alive || AccessLevel >= AccessLevel.GameMaster ) // mod by Dies Irae
		...
		private bool CheckInsuranceOnDeath( Item item )
		{
			...
			{
				#region modifica by Dies Irae (non puo' essere messa in m2pm perchè m_InsuranceAward è privato
                if( XmlPointsAttach.InsuranceIsFree( this, m_InsuranceAward ) )
                {
                    item.PayedInsurance = true;
                    return true;
                }
				...
				if ( m_InsuranceAward != null )
				{
                    ...
                    if( m_InsuranceAward is BaseCreature )
                    {
                        BaseCreature bc = (BaseCreature)m_InsuranceAward;
                        if( !bc.Summoned && !bc.Tamable && !bc.IsBonded && !bc.Controlled )
                        {
                            bc.AddItem( new Gold( 300 ) );
                        }
                    }
                    #endregion
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 27:
				{
					Engines.BulkOrders.TamingBOBFilter t = new Engines.BulkOrders.TamingBOBFilter( reader );
					goto case 26;
				}
				case 26: 
				{
					bool tmp1 = reader.ReadBool();
				...
				case 25:
				{
					...
					if( recipeCount > 0 )
					{
						...
						for( int i = 0; i < recipeCount; i++ )
						{
							...
							if( reader.ReadBool() && !m_AcquiredRecipes.ContainsKey( r ) )	//Don't add in recipies which we haven't gotten or have been removed
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			#region Mondain's Legacy version 29
		...
		public override TimeSpan ComputeMovementSpeed( Direction dir, bool checkTurning )
		{
            ...
            #region mod by Dies Irae
            Morph.MorphContext morphContext = Morph.GetContext( this );
            bool isMorphBoosted = ( morphContext != null && morphContext.SpeedBoost );
            #endregion

			if( onHorse || isMorphBoosted || (animalContext != null && animalContext.SpeedBoost) )
		...
		public virtual void AutoStablePets()
		{
		...
		public virtual void ClaimAutoStabledPets()
		{
