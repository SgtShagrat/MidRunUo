...
{
    public class ChristmasMistletoeAddon : Item, IDyable, IAddon, IEventItem
    {
        #region IEventItem
        ...
        [Constructable]
        public ChristmasMistletoeAddon()
            : this( Utility.RandomDyedHue() )
        {
        }

        [Constructable]
        public ChristmasMistletoeAddon( int hue )
            : base( 0x2375 )
        {
            Hue = hue;
            Movable = false;
        ...
        public ChristmasMistletoeAddon( Serial serial )
            : base( serial )
        {
        }
        ...
        public Item Deed
        {
            get { return new ChristmasMistletoeDeed( Hue ); }
        }
        ...
        public override void OnDoubleClick( Mobile from )
        {
            ...
            if( house != null && house.IsCoOwner( from ) )
            {
                ...
                {
                    from.CloseGump( typeof( MistletoeAddonGump2010 ) );
                    from.SendGump( new MistletoeAddonGump2010( from, this ) );
                }
                else
                {
        ...
        private class MistletoeAddonGump2010 : Gump
        {
            private readonly Mobile m_From;
            private readonly ChristmasMistletoeAddon m_Addon;

            public MistletoeAddonGump2010( Mobile from, ChristmasMistletoeAddon addon )
                : base( 150, 50 )
            {
                m_From = from;
                m_Addon = addon;
    ...
    public class ChristmasMistletoeDeed : Item
    {
        public override int LabelNumber { get { return 1070882; } } // Mistletoe Deed

        [Constructable]
        public ChristmasMistletoeDeed()
            : this( 0 )
        {
        }

        [Constructable]
        public ChristmasMistletoeDeed( int hue )
            : base( 0x14F0 )
        {
            Hue = hue;
            Weight = 1.0;
        ...
        public ChristmasMistletoeDeed( Serial serial )
            : base( serial )
        {
        }
        ...
        private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
        {
            ...
            if( itemID > 0 )
            {
                Item addon = new ChristmasMistletoeAddon( Hue );
        ...
        private class MistletoeDeedGump : Gump
        {
            ...
            private readonly ChristmasMistletoeDeed m_Deed;

            public MistletoeDeedGump( Mobile from, Point3D loc, ChristmasMistletoeDeed deed )
                : base( 150, 50 )
            {
                m_From = from;
                m_Loc = loc;
