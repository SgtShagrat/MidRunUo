...
namespace Midgard.Commands
{
    ...
    {
        private static string m_CustomOutputDirectory = Path.Combine( Core.BaseDirectory, "GenetaredAddons" );
...
{components}
	...
	public class {name}AddonDeed : BaseAddonDeed
	{
        ...
        {
            CommandSystem.Register( "AddonGen", AccessLevel.Developer, new CommandEventHandler( OnAddonGen ) );
        }
        ...
        private static void PickerCallback( Mobile from, Map map, Point3D start, Point3D end, object state )
        {
            ...
            Dictionary<Point2D, List<Tile>> tiles = new Dictionary<Point2D, List<Tile>>();
            ...
            if( statics )
            {
                for( int x = start.X; x <= end.X; x++ )
                {
                    for( int y = start.Y; y <= end.Y; y++ )
                    {
                        ...
                        list.AddRange( map.GetTilesAt( new Point2D( x, y ), items, false, true ) );
            ...
            foreach( Point2D p in tiles.Keys )
            {
                ...
                if( list != null )
                    foreach( Tile t in list )
                    {
                        ...
                        sb.AppendFormat( "\t\t\tAddComponent( new AddonComponent( {0:0xX2} ), {1}, {2}, {3} );\n", id, xOffset, yOffset, zOffset );
                    }
            }
            ...
            foreach( Static item in target )
            {
                ...
                sb.AppendFormat( "\t\t\tAddComponent( ac, {0:0xX2}, {1}, {2} );\n", xOffset, yOffset, zOffset );
            }
        ...
        private class InternalGump : Gump
        {
            ...
            private void MakeGump()
            {
                ...
                // Name: 0
                AddTextEntry( 95, 35, 165, 20, LabelHue, 0, m_State[ 0 ] as string );

                AddLabel( 20, 60, LabelHue, @"Namespace" );
                AddImageTiled( 95, 75, 165, 1, 9304 );

                // Namespace: 1
                AddTextEntry( 95, 55, 165, 20, LabelHue, 1, m_State[ 1 ] as string );

                // Items: Check 0
                AddCheck( 20, 85, 2510, 2511, ( (bool)m_State[ 2 ] ), 0 );
                AddLabel( 40, 85, LabelHue, @"Export Items" );

                // Statics: Check 1
                AddCheck( 20, 110, 2510, 2511, ( (bool)m_State[ 3 ] ), 1 );
                AddLabel( 40, 110, LabelHue, @"Export Statics" );

                // Range: Check 2
                AddCheck( 20, 135, 2510, 2511, ( (bool)m_State[ 4 ] ), 2 );
                AddLabel( 40, 135, LabelHue, @"Specify Z Range" );

                // Min Z: Text 2
                AddLabel( 50, 160, LabelHue, @"min." );
                AddImageTiled( 85, 175, 60, 1, 9304 );
                AddTextEntry( 85, 155, 60, 20, LabelHue, 2, m_State[ 5 ].ToString() );

                // Max Z: Text 3
                AddLabel( 160, 160, LabelHue, @"max." );
                AddImageTiled( 200, 175, 60, 1, 9304 );
                AddTextEntry( 200, 155, 60, 20, LabelHue, 3, m_State[ 6 ].ToString() );

                // Cancel: B0
                AddButton( 20, 185, 4020, 4021, 0, GumpButtonType.Reply, 0 );
                AddLabel( 55, 185, LabelHue, @"Cancel" );

                // Generate: B1
                AddButton( 155, 185, 4005, 4006, 1, GumpButtonType.Reply, 0 );
            ...
            public override void OnResponse( Server.Network.NetState sender, RelayInfo info )
            {
                ...
                if( Verify( m_State ) )
                {
                    BoundingBoxPicker.Begin( sender.Mobile, new BoundingBoxCallback( PickerCallback ), m_State );
                }
                else
                {
            ...
            {
                if( state[ 0 ] == null || ( (string)state[ 0 ] ).Length == 0 )
                    return false;

                if( state[ 1 ] == null || ( (string)state[ 1 ] ).Length == 0 )
                    return false;
