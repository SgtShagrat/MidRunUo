...
namespace Midgard.Engines
{
    public class AddonGenerator
    {
        ...
        /// </summary>
        private static string m_CustomOutputDirectory = Path.Combine( Core.BaseDirectory, @"Scripts\working" );
...
{complexnamecomponentscode}
	...
	public class {name}AddonDeed : BaseAddonDeed
	{
        ...
        {
            CommandSystem.Register( "CeoAddonGen", AccessLevel.Developer, new CommandEventHandler( OnAddonGen ) );
        }
        ...
        private static void PickerCallback( Mobile from, Map map, Point3D start, Point3D end, object state )
        {
            ...
            if( getTiles )
            {
                for( int x = start.X; x <= end.X; x++ )
                {
                    ...
                    {
                        List<Tile> list = map.GetTilesAt( new Point2D( x, y ), false, false, true );
                ...
                foreach( object o in en )
                {
                    if( getStatics )
                    {
                        ...
                        else
                        {
                            ...
                            if( Core.Debug )
                                Console.WriteLine( "Static={0}:{1}", s.GetType(), s.ItemID );
                    ...
                    if( getItems )
                    {
                        ...
                        if( Core.Debug )
                            Console.WriteLine( "item={0}:{1}", i.GetType(), i.ItemID );
            ...
            {
                List<Tile> list = tiles[ p ] as List<Tile>;
