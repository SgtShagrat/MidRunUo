...
namespace Server.Poker
{
    public class PokerBetGump : Gump
    {
        ...
        private const int ColorBlack = 0x000001;
            ...
            : base( 460, 400 )
        {
            ...
            if( canCall )
                AddHtml( 75, 14, 60, 22, Color( Center( ( m_Game.CurrentBet - player.RoundBet >= player.Gold ? "all-in" : String.Format( "{0}", ( m_Game.CurrentBet - m_Player.RoundBet ).ToString( "#,###" ) ) ) ), ColorGreen ), false, false );
        ...
        public override void OnResponse( NetState state, RelayInfo info )
        {
            ...
            if( info.ButtonID == 8 ) //Okay
            {
                ...
                else if( info.IsSwitched( (int)Buttons.Bet ) )
                {
                    ...
                    try
                    {
                        bet = Convert.ToInt32( info.GetTextEntry( (int)Buttons.TxtBet ).Text );
                    }
                    catch
                    {
                    }
