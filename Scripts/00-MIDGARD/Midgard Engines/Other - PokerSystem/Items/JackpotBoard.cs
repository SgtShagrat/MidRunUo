            ...
            if( from.InRange( Location, 4 ) && from.NetState != null )
            {
                if( PokerDealer.Jackpot > 0 && PokerDealer.JackpotWinners != null && PokerDealer.JackpotWinners.Winners.Count > 0 )
                {
                    if( PokerDealer.JackpotWinners.Winners.Count > 1 )
                    {
                        ...
                        {
                            if( PokerDealer.JackpotWinners.Winners[ i ].Owner != null )
                                sb.Append( PokerDealer.JackpotWinners.Winners[ i ].Owner.Name );
                            else
                                sb.Append( "(-null-)" );
                    ...
                    else if( PokerDealer.JackpotWinners.Winners[ 0 ] != null && PokerDealer.JackpotWinners.Winners[ 0 ].Owner != null )
                    {
                        DisplayMessage( from, String.Format( "The jackpot is {0} gold. {1} leads with {2}", PokerDealer.Jackpot.ToString( "#,###" ), PokerDealer.JackpotWinners.Winners[ 0 ].Owner.Name, HandRanker.RankString( PokerDealer.JackpotWinners.Hand ) ) );
                        return;
                    }
                }
