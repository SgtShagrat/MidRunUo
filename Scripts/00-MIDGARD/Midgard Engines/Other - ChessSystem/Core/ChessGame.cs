...
namespace Midgard.Engines.ChessSystem
{
    ...
    public class ChessGame
    {
        ...
        /// <summary>
        /// The <see cref="BChessboard"/> object providing game logic
        /// </summary>
        ...
        /// <summary>
        /// The bounds of the <see cref="BChessboard"/>
        /// </summary>
        private Rectangle2D m_Bounds;

        /// <summary>
        /// The height of the <see cref="BChessboard"/>
        /// </summary>
        private int m_Z;
        ...
        /// <summary>
        /// The <see cref="ChessControl"/> object owner of this game
        /// </summary>
        ...
        public Mobile Owner
        {
            get { return m_BlackOwner ? m_Black : m_White; }
        }
        ...
        public Mobile Guest
        {
            get { return m_BlackOwner ? m_White : m_Black; }
            set
            {
                if( m_BlackOwner )
                    m_White = value;
        ...
        /// <summary>
        /// Sets the attack effect on the <see cref="BChessboard"/>
        /// </summary>
        ...
        /// <summary>
        /// Sets the capture effect on the <see cref="BChessboard"/>
        /// </summary>
        ...
        /// <summary>
        /// Sets the BoltOnDeath property on the <see cref="BChessboard"/>
        /// </summary>
        ...
        /// <summary>
        /// Gets the orientation of the <see cref="BChessboard"/>
        /// </summary>
        ...
        /// <summary>
        /// Gets the <see cref="BChessboard"/> region
        /// </summary>
        public ChessRegion Region { get; private set; }
        ...
        public void AcceptGame( Mobile guest )
        {
            ...
            Owner.CloseGump( typeof( StartGameGump ) );

            m_Board = new BChessboard( m_Black, m_White, m_Z, m_Bounds, this, m_Parent.ChessSet, m_Parent.WhiteHue, m_Parent.BlackHue,
                                      m_Parent.WhiteMinorHue, m_Parent.BlackMinorHue, m_Parent.OverrideMinorHue );

            m_MoveTime = DateTime.Now;

            // Create the region
            Region = new ChessRegion( m_Parent.Map, this, m_AllowSpectators, m_Bounds, m_Z );
            Region.Register();
        ...
        private void OnPlayerDisconnected( DisconnectedEventArgs e )
        {
            ...
            if( m_Black != null && m_Black.NetState != null )
            {
                ...
                m_Black.SendGump( new EndGameGump( m_Black, this, false, "Your partner has been disconnected", Config.DisconnectTimeOut.Minutes ) );
            }

            if( m_White != null && m_White.NetState != null )
            {
                m_White.CloseGump( typeof( GameGump ) );
                m_White.SendGump( new EndGameGump( m_White, this, false, "Your partner has been disconnected", Config.DisconnectTimeOut.Minutes ) );
            }
        ...
        public void SendMoveTarget( Mobile m )
        {
            ...
            m.Target = new ChessTarget( this, m, "Select the piece you wish to move...", new ChessTargetCallback( OnPickPieceTarget ) );
        }
        ...
        public void OnPickPieceTarget( Mobile from, object targeted )
        {
            ...
            m_MovingPiece = piece;
            from.Target = new ChessTarget( this, from, "Where do you wish to move?", new ChessTargetCallback( OnPieceMove ) );
        }
        ...
        public void OnMoveTimeout()
        {
            ...
            {
                m_Black.SendGump( new EndGameGump( m_Black, this, false, string.Format( "No move made in {0} minutes", Config.MoveTimeOut.Minutes ),
                                                 Config.EndGameTimerOut.Minutes ) );
            }

            if( m_White.NetState != null )
            {
                m_White.SendGump( new EndGameGump( m_White, this, false, string.Format( "No move made in {0} minutes", Config.MoveTimeOut.Minutes ),
                                                 Config.EndGameTimerOut.Minutes ) );
            }
        }
        ...
        public void SendAllGumps( string whiteMsg, string blackMsg )
        {
            ...
            m_Black.SendGump( new GameGump( m_Black, this, ChessColor.Black, blackMsg, !m_Pending && m_Status == GameStatus.BlackToMove,
                                          m_Status == GameStatus.BlackMoving ) );

            m_White.SendGump( new GameGump( m_White, this, ChessColor.White, whiteMsg, !m_Pending && m_Status == GameStatus.WhiteToMove,
                                          m_Status == GameStatus.WhiteMoving ) );
        ...
        /// <param name="whiteMinor"></param>
        /// <param name="blackMinor"></param>
