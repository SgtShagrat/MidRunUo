    ...
    public class ChessTimer
    {
        ...
        public void OnTick()
        {
            ...
            if( m_GameStart != DateTime.MaxValue )
            {
                ...
                if( ( DateTime.Now - m_GameStart ) >= Config.GameStartTimeOut )
                {
                    m_Game.Cleanup();
                }
            }
            ...
            {
                if( ( DateTime.Now - m_EndGame ) >= Config.EndGameTimerOut )
                {
                    m_Game.Cleanup();
                }
            ...
            {
                // Now playing
                if( ( DateTime.Now - m_LastMove ) >= Config.MoveTimeOut )
                {
            ...
            if( m_Disconnect != DateTime.MaxValue )
            {
                ...
                if( ( DateTime.Now - m_Disconnect ) >= Config.DisconnectTimeOut )
                {
                    m_Game.Cleanup();
                }
            }
        ...
        private class InternalTimer : Timer
        {
            ...
            public InternalTimer( ChessTimer parent )
