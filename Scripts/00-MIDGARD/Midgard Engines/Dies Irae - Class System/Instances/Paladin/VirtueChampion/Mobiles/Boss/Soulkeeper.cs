...
{
    public class Soulkeeper : BaseVirtueChampionBoss
    {
        public Virtues Virtue { get; set; }

        [Constructable]
        public Soulkeeper( Virtues virtue )
            : base( AIType.AI_Mage, FightMode.Closest, 10, 1, 0.2, 0.4 )
        {
            Virtue = virtue;
            Body = 785;
            Name = string.Format( "Soulkeeper of {0}", Core.GetVirtueName( virtue ) );
            BaseSoundID = 357;
        ...
        public Soulkeeper( Serial serial )
            : base( serial )
        {
        }
