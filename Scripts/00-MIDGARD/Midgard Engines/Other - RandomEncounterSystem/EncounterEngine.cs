...
namespace Midgard.Engines.RandomEncounterSystem
{
    ...
    public class RandomEncounterEngine
    {
        ...
        private static Dictionary<string, RegionRecord> m_RegionHash;
        ...
        private static string m_EncountersFile = "Data/RandomEncounters.xml";
        ...
        public static void Initialize()
        {
            ...
            m_RegionHash = new Dictionary<string, RegionRecord>();
            ...
            if( MaybeLoadXml() )
            {
                ...
                Logger.Log( "RandomEncounters System Initializing..." );
                Logger.Log(
                ...
                if( m_Intervals.Length >= 4 )
                {
                    ...
                    Logger.Log( "RandomEncounters: WARNING, House Spawning not implemented yet." );
                }
                if( m_Intervals.Length == 5 )
                {
                    //m_EncounterTimers[4] = new EncounterTimer( "Jail", m_Intervals[4] );
                    Logger.Log( "RandomEncounters: WARNING, Jail Spawning not implemented yet." );
                }
            ...
            {
                Logger.Log( "##### RandomEncounters: ***FAILED INITIALIZATION***!" );
            }
        }
        ...
        {
            if( !File.Exists( m_EncountersFile ) )
            {
                Logger.Log(
        ...
        {
            Console.Write( "RandomEncounters System Loading..." );
            ...
            try
            {
                ...
                foreach( XmlNode facetNode in facetNodes )
                {
                    ...
                    if( regionNodes.Count == 0 )
                        Logger.Log( "RandomEncounters: Facet \"{0}\" at {1} had no elements. IGNORED.",
                                          facetName,
                                          facetNode.Attributes[ "lineNumber" ].Value );
                    //--------------------------------------------------------------
                    // Now over regions
                    ...
                    foreach( XmlNode regionNode in regionNodes )
                    {
                        ...
                        if( failed != "" )
                        {
                            Logger.Log( "Attempted to add an element without a {0} at {1}. IGNORING.",
                                              failed,
                        ...
                        if( encounterNodes.Count == 0 )
                            Logger.Log( "RandomEncounters: {0} Region \"{1}\" at {2} had no elements. IGNORED.",
                                              regionType,
                                              regionName,
                                              regionNode.Attributes[ "lineNumber" ].Value );
                        ...
                        foreach( XmlNode encounterNode in encounterNodes )
                        {
                            ...
                            string[] distance_tok = encounterDistance.Split( new Char[] { ':' } );
                            // splits to shortest:farthest, or just distance if no ':'

                            string[] level_tok = encounterLevel.Split( new Char[] { ':' } );

                            RandomEncounter randomEncounter = new RandomEncounter( encounterNode,
                                                                                  facetName,
                                                                                  regionType,
                                                                                  regionName,
                                                                                  encounterProbability,
                                                                                  distance_tok[ 0 ],
                                                                                  ( distance_tok.Length > 1
                                                                                       ? distance_tok[ 1 ]
                                                                                       : distance_tok[ 0 ] ),
                                                                                  encounterLand,
                                                                                  encounterTime,
                            ...
                            if( elementNodes.Count == 0 )
                                Logger.Log( "RandomEncounters: Encounter on line {0} had no children. IGNORED.",
                            ...
                            foreach( XmlNode elementNode in elementNodes )
                                AddRecursiveMobilesAndItems( encounterNode, elementNode, randomEncounter );
            ...
            {
                Logger.Log( "RandomEncounters: Exception caught attempting to load file: " + m_EncountersFile );
                Logger.Log( "{0}", e );
                xmlDoc.Close();
                return false;
            }
        ...
        //----------------------------------------------------------------------
        private static void AddRecursiveMobilesAndItems( XmlNode parentNode, XmlNode childNode,
                                                        IElementContainer elementContainer )
        {
            ...
            {
                Logger.Log( "Attempted to add an element without a pick at {0}. IGNORING.",
            ...
            EncounterElement newElement = new EncounterElement( childNode,
                                                               probability,
                                                               pick,
                                                               id,
                                                               tokens[ 0 ],
        ...
        {
            Logger.Log( "Adding property to " + parentNode.Name );
        }
        ...
        private static void ProcessNewEncounter( RandomEncounter encounter )
        {
            ...
            //  and region name...
            //------------------------------------------------------------------
            if( m_RegionHash.ContainsKey( encounter.Key ) )
            {
                RegionRecord region = m_RegionHash[ encounter.Key ];
        ...
        {
            Logger.Log( "RandomEncounters: dumping tables:" );
            foreach( string key in m_RegionHash.Keys )
            {
                RegionRecord region = m_RegionHash[ key ];
                region.DumpAll();
            }
        }
        ...
        {
            List<Mobile> playerMobiles = GenerateLivePlayerMobiles();
            List<Mobile> candidateMobiles = new List<Mobile>();
            List<Mobile> chosenMobiles = new List<Mobile>();
            List<IEntity> cleanupList = new List<IEntity>();
            ...
            foreach( Mobile playerMobile in playerMobiles )
            {
                ...
                string str = Helpers.RegionCategory( currentRegion );

                if(!Equals(str, timerType))
                    continue;
            ...
            {
                if( m_Debug )
                    Logger.Log( "RandomEncounters: No one in region for timerType:" + timerType );
                return;
            }
            ...
            if( m_Picker == "sqrt" )
            {
                ...
                for( int i = 0; i < candidateMobiles.Count; i++ )
                {
                    ...
                    Mobile chosenMobile = candidateMobiles[ chosen ];
            ...
            if( m_Debug )
                Logger.Log( "RandomEncounters: Generating {0} encounter checks for {1} players in {2} region...",
                                  chosenMobiles.Count,
                                  nPlayersInRegion,
                                  timerType );
            ...
            foreach( PlayerMobile chosenMobile in chosenMobiles )
            {
                ...
                {
                    Logger.Log( "Searching for region key: " + key );
                    if( m_RegionHash.ContainsKey( key ) ) //  Search to for match against named region;
                    {
                        possibleEncounters = m_RegionHash[ key ].PossibleEncounters;
                        Logger.Log( "Found match for key: {0}, with value: {1}", key, possibleEncounters );
                        break;
                    }
                }
                ...
                //{
                    ...
                    //                     {
                    //                         chosenMobile.Map.Name + ":" + timerType + ":" + "default" + ":" + landType +
                    //                         ":" + encounterTime,
                    //                         chosenMobile.Map.Name + ":" + timerType + ":" + "default" + ":" + landType +
                    //                         ":" + EncounterTime.AnyTime,
                    //                         chosenMobile.Map.Name + ":" + timerType + ":" + "default" + ":" +
                    //                         LandType.AnyLand + ":" + EncounterTime.AnyTime,
                    //                     };
                    ...
                    //    if( m_RegionHash.ContainsKey( key ) ) //  Search to for match against named region;
                    //    {
                    //        Logger.Log( "Found match for key: " + key );
                    //        possibleEncounters = m_RegionHash[ key ].PossibleEncounters;
                ...
                foreach( EncounterSet encounterSet in possibleEncounters )
                {
                    ...
                    if( draw <= encounterSet.Probability )
                    {
                        ...
                        {
                            Logger.Log( "Encounter Level " + encounter.LevelType + " <= Player Level " +
                                              playerLevel );
                ...
                if( inclusiveSet != null )
                    foreach( RandomEncounter encounter in inclusiveSet )
                    {
                        ...
                        Logger.Log( "Encounter Level " + encounter.LevelType + " > Player Level " + playerLevel );
                    }
            }
        ...
        //----------------------------------------------------------------------
        private static void GenerateEncounter( PlayerMobile playerMobile, RandomEncounter encounterTemplate,
                                              ICollection<IEntity> cleanupList )
        {
            ...
            if( m_Debug )
            {
                Logger.Log( "RandomEncounters: generating instances: {0} ...", encounter );
            ...
            //
            //            Logger.Log("Player Location: {0}", location );
            //            Logger.Log("    Land Tile id={0} z={1} height={2}: ", playerTile.ID, playerTile.Z, playerTile.Height );
            //
            //            uint flags = (uint) TileData.LandTable[playerTile.ID & 0x3FFF].Flags;
            //            Logger.Log("    Land Flags: {0:x}", flags );
            //
            //            foreach( Tile staticTile in staticTiles )
            //            {
            //                Logger.Log("    Static Tile id={0} z={1} height={2}", staticTile.ID, staticTile.Z, staticTile.Height );
            //                flags = (uint) TileData.LandTable[staticTile.ID & 0x3FFF].Flags;
            //                Logger.Log("    Static Flags: {0:x}", flags );
            //            }
            ...
            foreach( EncounterElement element in encounter.Elements )
            {
                ...
                if( running )
                {
                    foundQuadrant = SpawnFinder.FindAhead( playerMobile,
                                                          location,
                                                          ref spawnPoint,
                                                          encounter.LandType,
                                                          encounter.Distance,
                ...
                if( !foundQuadrant )
                    if(
                        !SpawnFinder.FindInwards( playerMobile,
                                                 location,
                                                 ref spawnPoint,
                                                 encounter.LandType,
                                                 encounter.Distance,
                                                 ...
                                                 element.EffectHue ) )
                    {
                        ...
                        //----------------------------------------------------------
                        // Logger.Log("NO SPAWN LOCATION AVAILABLE");
                        return;
                    }

                GenerateEncounterRecursive( map, playerMobile, spawnPoint, cleanupList, encounterTemplate, element, null );
            }
        }

        private static void GenerateEncounterRecursive( Map map, PlayerMobile playerMobile, Point3D parentLocation,
                                                       ICollection<IEntity> cleanupList,
                                                       RandomEncounter encounterTemplate, EncounterElement parentElement,
                                                       object parentObject )
        {
            ...
            if( typeObject == null )
            {
                Logger.Log( "RandomEncounters: Bad attempt to generate an element of type \"{0}\".",
            ...
            else
            {
                ...
                if(
                    !SpawnFinder.FindOutwards( playerMobile,
                                              parentLocation,
                                              ref spawnPoint,
            ...
            //--------------------------------------------------------------
            {
                ...
                Logger.Log( "RandomEncounters: Bad attempt to contruct an element of type \"{0}\", on line {1}",
                                  parentElement.PickFrom,
                                  line );
                Logger.Log( "{0}", e );
            ...
            else if( created is Item )
            {
                ...
                if( !item.Movable )
                {
                    cleanupList.Add( (IEntity)created );
                }
                ...
                if( parentObject != null )
                {
                    if( parentObject is Mobile )
                    {
                        ...
                        //Logger.Log( "RandomEncounters: added item to mobile: " + item );
                    ...
                    else if( parentObject is Container )
                    {
                        ...
                        //Logger.Log( "RandomEncounters: added item to container: " + item );
                        itemParent.AddItem( item );
                    }
                    else
                    {
                        item.MoveToWorld( spawnPoint, map );
                        //Logger.Log( "RandomEncounters: inserted item into world: " + item );
                    }
                }
                else
                {
            ...
            {
                GenerateEncounterRecursive( map,
                                           playerMobile,
                                           spawnPoint,
                                           cleanupList,
                                           encounterTemplate,
        ...
        internal static void DumpEncounter( int depth, RandomEncounter encounter )
        {
            ...
            Logger.Log( "{0}{1}", prepend, encounter );
        ...
        internal static void DumpElementRecursive( int depth, EncounterElement element )
        {
            ...
            Logger.Log( "{0}{1}", prepend, element );
        ...
        //----------------------------------------------------------------------
        public static List<Mobile> GenerateLivePlayerMobiles()
        {
            ...
            foreach( NetState state in NetState.Instances )
            {
                if( state.Mobile is PlayerMobile )
                {
                    ...
                    if( SkipStaff && mobile.AccessLevel != AccessLevel.Player )
                        continue; // staff doesn't qualify
                    if( SkipDead && !mobile.Alive )
                        continue; // no randoms for dead players
                    if( SkipHidden && mobile.Hidden )
        ...
        private static int[] m_RoadTileIds = new int[]
                                                 {
                                                     0x071, 0x08C, 0x0E8, 0x0EB, 0x14C, 0x14F, 0x161, 0x174, 0x1F0, 0x1F3,
                                                     0x26E, 0x279, 0x27E, 0x281, 0x324, 0x3AC, 0x547, 0x556, 0x597, 0x5A6,
                                                     0x637, 0x63A, 0x7AE, 0x7B1, 0x442, 0x479, // Sand stones
