...
namespace Midgard.Engines.RandomEncounterSystem
{
        ...
        protected override void OnTick()
        {
            ...
            if( checkTime > m_LastWriteTime )
            {
                if( RandomEncounterEngine.Debug )
                    Logger.Log( "RandomEncounters: rereading encounters file" );
    ...
    public class DeleteTimer : Timer
    {
        ...
        {
            if( RandomEncounterEngine.Debug )
                Console.WriteLine( "RandomEncounters: Running Cleaner..." );
            ...
            foreach( XmlAttachment attachment in XmlAttach.AllAttachments.Values )
            {
                if( attachment is XmlDateCount )
                {
                    ...
                    {
                        //Logger.Log("Object too new {0} for cleanup; skipping", elapsed );
                        continue;
                    }
                    ...
                    //Logger.Log("Object {0} has been around for {1} ", o, elapsed);
                    ...
                    if( o is Item )
                    {
                        ...
                        if( !i.Movable )
                        {
                            if( MaybeRemove( xmlDate, i ) )
                                removeList.Add( new object[] { xmlDate, i } );
                        }
                        ...
                        while( true )
                        {
                            ...
                            else if( parent is Mobile )
                            {
                                ...
                                removeList.Add( new object[] { xmlDate, null } );
                    ...
                    else if( o is Mobile )
                    {
                        if( o is BaseCreature )
                        {
                            ...
                            if( c.Owners.Count > 0 )
                            {
                                ...
                                //Logger.Log("Detaching tamed creature: "+o);
                        ...
                        if( m.Combatant == null )
                        {
                            ...
                            {
                                removeList.Add( new object[] { xmlDate, m } );
                            }
                        }
                    }
                }
            ...
            if( RandomEncounterEngine.Debug )
                Logger.Log( " detached = {0}; deleted = {1}", nDetached, nDeleted );

            if( Interval.Ticks == 0 )
                Stop();
        }

        private static bool MaybeRemove( XmlDateCount xmlDate, object o )
        {
    ...
    //                    if (c.Owners.Count > 0) 
    //                    {
    //                        //Logger.Log("Skipping deletion of mobile {0} because it is tame.", m);
    //                        continue;
    //                    }
    //                }
    //
