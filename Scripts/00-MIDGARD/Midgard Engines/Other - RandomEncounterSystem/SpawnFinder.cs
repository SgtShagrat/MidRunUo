...
using Midgard.Engines.Searches;
using Server;
using Server.Items;
...
namespace Midgard.Engines.RandomEncounterSystem
{
    public class SpawnFinder
    {
        ...
        //----------------------------------------------------------------------
        public static bool FindInwards( PlayerMobile pm, Point3D centerPoint, ref Point3D spawnPoint, LandType landType,
                                       int distance, EffectType effectType, int effectHue )
        {
        ...
        //----------------------------------------------------------------------
        public static bool FindOutwards( PlayerMobile pm, Point3D centerPoint, ref Point3D spawnPoint, LandType landType,
                                        int distance, EffectType effectType, int effectHue )
        {
        ...
        //  he is running
        //----------------------------------------------------------------------
        public static bool FindAhead( PlayerMobile pm, Point3D centerPoint, ref Point3D spawnPoint, LandType landType,
                                     int distance, EffectType effectType, int effectHue )
        {
        ...
        //----------------------------------------------------------------------
        private static bool FindSpiral( Mobile pm, IPoint3D centerPoint, ref Point3D spawnPoint, LandType landType,
                                       int distance, bool inwards, EffectType effectType, int effectHue )
        {
            Point3D foundPoint = new Point3D();
            ...
            bool found = Search.Spiral( pm.Map,
                                       distance,
        ...
        //  here
        //----------------------------------------------------------------------
        private static bool FindSpawnTileInternal( Mobile pm, IPoint3D centerPoint, IPoint2D currentPoint,
                                                  ref Point3D spawnPoint, LandType landType, EffectType effectType,
                                                  int effectHue )
        {
            Map map = pm.Map;
            ...
            {
                //Logger.Log( "player.Location="+pm.Location );
                //Logger.Log( "rquery={0},{1}", currentPoint.X, currentPoint.Y );
                //Logger.Log( "sourceRegion="+sourceRegion.GetType() );
                //Logger.Log( "r="+r.GetType() );
                return false;
            }
            ...
            else
            {
                ...
                List<Tile> staticTileArray = new List<Tile>( staticTiles );
                ...
                foreach( Tile staticTile in staticTileArray )
                {
                    if( map.CanSpawnMobile( currentPoint.X, currentPoint.Y, staticTile.Z ) )
                    {
                        ...
                        if( RandomEncounterEngine.DebugEffect )
                            Effects.SendLocationParticles(
                                EffectItem.Create( new Point3D( spawnPoint.X, spawnPoint.Y, spawnPoint.Z ),
                                                  map,
                        ...
                        //Logger.Log("Found good point at static.Z");
                        return true;
                    }
                ...
                if( map.CanSpawnMobile( currentPoint.X, currentPoint.Y, spawnTile.Z ) )
                {
                    ...
                    //Logger.Log("Found good point at search point.Z");
                    return true;
                }
                ...
                if( map.CanSpawnMobile( currentPoint.X, currentPoint.Y, centerPoint.Z ) )
                {
                    ...
                    //Logger.Log("Found good point at center point.Z");
                    return true;
                }
