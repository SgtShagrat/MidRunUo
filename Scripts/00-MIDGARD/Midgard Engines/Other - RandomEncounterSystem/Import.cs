...
namespace Midgard.Engines.RandomEncounterSystem
{
    ...
    public class ImportRecord
    {
        ...
        private string m_Spawn;
        private Point3D m_Location;
        private int m_HRange;
    ...
    {
        private static Dictionary<string, Dictionary<string, List<ImportRecord>>> facetEncounters;
        ...
        public static void Import( Mobile mobile )
        {
            ...
            {
                Logger.Log( "#### RandomEncounters: Error; no such directory \"{0}\" ... ", topDir );
                return;
            }
            ...
            foreach( string dir in dirs )
            {
                ...
                Logger.Log( "RandomEncounters: importing from \"{0}\"", dir );

                if( !Directory.Exists( dir ) )
                {
                    Logger.Log( "#### RandomEncounters: Error; no such directory \"{0}\" ... ", dir );
                    return;
                }
            ...
            )
        {
            ...
            if( mapInstance == null )
            {
                Logger.Log( "#### RandomEncounters: no map for map nammed \"{0}\".", mapName );
            }
                ...
                foreach( string facet in facetEncounters.Keys )
                {
                    ...
                    foreach( string regionName in regionEncounters.Keys )
                    {
                        ...
                        foreach( ImportRecord ir in encounterSet )
                        {
                            ...
                            sw.WriteLine(
                                "            <Encounter p=\"{0:0.000}\" level=\"{1:0.000}\" distance=\"7:11\">", ir.P,
                                level );
                            sw.WriteLine(
                                "                <Mobile pick=\"{0}\" n=\"1:{1}\"{2}/> <!-- x={3},y={4},z={5} -->",
                                ir.Spawn, n, forceFlag, ir.Location.X, ir.Location.Y, ir.Location.Z );
