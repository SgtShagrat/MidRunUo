...
{
	public class EditPermaBanListGump : TownGump
	{
		public enum Buttons
		{
			Close,
		...
		public EditPermaBanListGump( TownSystem system, Mobile owner ) : this( system, owner, null )
		{
		}

		public EditPermaBanListGump( TownSystem system, Mobile owner, IEnumerable<Mobile> list ) : base( system, owner, 50, 50 )
		{
			owner.CloseGump( typeof( EditPermaBanListGump ) );

			m_Mobiles = list == null ? BuildList( system ) : new List<Mobile>( list );

			Design( owner.Language );

			base.RegisterUse( typeof( EditPermaBanListGump ) );
		}
		...
		private class InternalComparer : IComparer<Mobile>
		{
			...
			public int Compare( Mobile x, Mobile y )
			{
				...
				{
					if( m_Town.PermaExiledPlayers.Contains( x ) && !m_Town.PermaExiledPlayers.Contains( y ) )
						return -1;
					else if( m_Town.PermaExiledPlayers.Contains( y ) && !m_Town.PermaExiledPlayers.Contains( x ) )
						return 1;
				}
		...
		private void Design( string lang )
		{
			...
			AddMainBackground();
			AddMainTitle( 315, String.Format( lang == "ITA" ? "Lista PermaEsiliati da {0}" : "{0} Citizens PermaBan List", Town.Definition.TownName ) );
			AddMainWindow();
			...
			for( int i = 0; i < m_Mobiles.Count; ++i )
			{
				...
				AddLabel( labelOffsetX + 150, y, hue, HasAccess( (int) TownAccessFlags.PermaBanCitizen ) ? FormatPrivateInfo( m.Account.Username ) : "--private info--" );

				if( tps != null )
					AddLabel( labelOffsetX + 280, y, hue, tps.TownSystem.Definition.TownName );

				AddMainWindowButton( labelOffsetX + 410, y + 3, i + 1, (int) TownAccessFlags.PermaBanCitizen );
			}
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			bool shouldBan = !TownHelper.IsTownPermaBanned( (Midgard2PlayerMobile)m, Town );

			TownHelper.DoTownPermaBan( m, Town, shouldBan );

			from.SendMessage( from.Language == "ITA" ? "Il giocatore {0} ora{1} è permaesiliato da {2}." : "Player {0} is now {1} permaexiled from {2}.", m.Name,
				shouldBan ? "" : (from.Language == "ITA" ? " NON" : "NOT"), Town.Definition.TownName );
		}

		public override int GetHueFor( Mobile m, int hue )
		{
			if( Town.PermaExiledPlayers.Contains( m ) )
				return 37;
