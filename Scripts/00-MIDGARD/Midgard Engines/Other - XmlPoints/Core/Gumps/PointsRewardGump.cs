...
namespace Server.Engines.XmlPoints
{
    ...
    {
        private readonly List<XmlPointsRewards> m_Rewards;
            ...
            : base( 20, 30 )
        {
            ...
            m_Rewards = XmlPointsRewards.RewardsList;
            ...
            */
            ...
            AddLabel( 400, 20, 0, String.Format( "Available Credits: {0}", XmlPointsAttach.GetCredits( from ) ) );
            ...
            if( m_Rewards != null && m_Rewards.Count > 0 )
            {
                AddLabel( width - 165,
                          height - 35,
                          0,
                          String.Format( "Page: {0}/{1}", m_Viewpage + 1, ( m_Rewards.Count / MaxItemsPerPage ) + 1 ) );
            ...
            if( m_Rewards != null )
            {
                ...
                for( int i = 0; i < m_Rewards.Count; i++ )
                {
                    ...
                    XmlPointsRewards r = m_Rewards[ i ];
                    if( r == null )
                        continue;
                    ...
                    // display the item
                    if( r.MinPoints > XmlPointsAttach.GetPoints( from ) )
                    {
                        texthue = 33;
                    }
                    else
        ...
        {
            if( info == null || state == null || state.Mobile == null || m_Rewards == null )
                return;
                ...
                default:
                    {
                        if( info.ButtonID >= 1000 )
                        {
                            ...
                            if( selection < m_Rewards.Count )
                            {
                                XmlPointsRewards r = m_Rewards[ selection ];

                                // check the price
                                if( XmlPointsAttach.HasCredits( from, r.Cost ) )
                                {
                                    ...
                                    {
                                        // charge them
                                        XmlPointsAttach.TakeCredits( from, r.Cost );
                                ...
                                from.SendGump( new PointsRewardGump( from, m_Viewpage ) );
                            }
                        }
                        break;
                    }
