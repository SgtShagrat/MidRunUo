...
namespace Midgard.Engines.RazorNegotiator
{
    public class RazorFeatureControl
    {
        // Is the "Feature Enforced" turned on?
        public static readonly bool Enabled = false;

        // When true, this will cause anyone who does not negotiate (include those not running Razor at all) to be disconnected from the server.
        public static readonly bool KickOnFailure = false;
        ...
        public const string WarningMessage = "The server was unable to negotiate features with Razor on your system." +
            "<BR>You must be running the <A HREF=\"http://razor.runuo.com/download.php\">latest version of Razor</A>" +
            "to play on this server.<BR>Once you have Razor installed and running, go to the <B>Options</B>" +
            " tab and check the box in the lower right-hand corner marked <B>Negotiate features with server</B>." +
            "Once you have this box checked, you may log in and play normally.<BR>You will be disconnected shortly.";
        ...
        public static void Configure()
        {
            ...
            else
            {
                DisallowFeature( RazorFeatures.FilterWeather );
                DisallowFeature( RazorFeatures.FilterLight );
                //DisallowFeature( RazorFeatures.SmartTarget );
                //DisallowFeature( RazorFeatures.RangedTarget );
                ...
                //DisallowFeature( RazorFeatures.PotionHotkeys );
                //DisallowFeature( RazorFeatures.RandomTargets );
    ...
    public class RazorFeatureEnforcer
    {
        ...
        private static TimerStateCallback m_OnHandshakeTimeoutCallback = new TimerStateCallback( OnHandshakeTimeout );
        private static TimerStateCallback m_OnForceDisconnectCallback = new TimerStateCallback( OnForceDisconnect );
        ...
        private static void EventSink_Login( LoginEventArgs e )
        {
            ...
            if( m != null && m.NetState != null && m.NetState.Running )
            {
                ...
                if( acct != null && acct.GetTag( "NORAZOR" ) == "1" )
                {
                    Console.WriteLine( "User {0} is logged-in properly without razor.", acct.Username );
                    return;
                }
                #endregion
                ...
                m_Table[ m ] = t = Timer.DelayCall( RazorFeatureControl.HandshakeTimeout, m_OnHandshakeTimeoutCallback, m );
                t.Start();
            }
        }
        ...
        {
            // pvSrc.Trace( state ); // mod by Dies Irae
        ...
        private static void OnHandshakeTimeout( object state )
        {
            ...
            {
                m.SendGump( new WarningGump( 1060635, 30720, RazorFeatureControl.WarningMessage, 0xFFC000, 420, 250, null, null ) );

                if( m.AccessLevel <= AccessLevel.Player )
                {
                    m_Table[ m ] = t = Timer.DelayCall( RazorFeatureControl.DisconnectDelay, m_OnForceDisconnectCallback, m );
                    t.Start();
                }
            }
        }
