...
namespace Midgard.Engines.AuctionSystem
{
    public class AuctionLog
    {
        ...
        {
            if( AuctionSystem.Running && Config.EnableLogging )
            {
        ...
        public static void WriteNewAuction( AuctionItem auction )
        {
            ...
            try
            {
                ...
                m_Writer.WriteLine( "# Owner : {0} [{1}] Account: {2}", auction.Owner.Name, auction.Owner.Serial, auction.Account.Username );
                m_Writer.WriteLine( "# Expires on {0} at {1}", auction.EndTime.ToShortDateString(), auction.EndTime.ToShortTimeString() );
                m_Writer.WriteLine( "# Starting Bid: {0}. Reserve: {1}. Buy Now: {2}", auction.MinBid, auction.Reserve,
                                   auction.AllowBuyNow ? auction.BuyNow.ToString() : "Disabled" );
                ...
                if( auction.Creature )
                {
                    ...
                    m_Writer.WriteLine( "# Type : {0}. Serial : {1}. Name : {2} Hue : {3}", auction.Pet.GetType().Name, auction.Pet.Serial,
                                       auction.Pet.Name ?? "Unnamed", auction.Pet.Hue );
                    m_Writer.WriteLine( "# Statuette Serial : {0}", auction.SoldAuctionItem.Serial );
                ...
                else
                {
                    ...
                    for( int i = 0; i < auction.ItemCount; i++ )
                    {
                        ...
                        m_Writer.WriteLine( "# {0}. {1} [{2}] Type {3} Hue {4}", i, info.Name, info.AItem.Serial, info.AItem.GetType().Name,
                                           info.AItem.Hue );
        ...
        public static void WriteBid( AuctionItem auction )
        {
            ...
            try
            {
                m_Writer.WriteLine( "> [{0}] Bid Amount : {1}, Mobile : {2} [{3}] Account : {4}", auction.ID, auction.HighestBidValue.ToString( "#,0" ),
                                   auction.HighestBidder.Name, auction.HighestBidder.Serial, auction.HighestBidder.Account.Username );
            }
            catch( Exception ex )
            {
                Console.WriteLine( ex.ToString() );
        ...
        public static void WritePending( AuctionItem auction, string reason )
        {
            ...
            try
            {
                m_Writer.WriteLine( "> [{0}] Becoming Pending on {1} at {2}. Reason : {3}", auction.ID, DateTime.Now.ToShortDateString(),
                                   DateTime.Now.ToShortTimeString(), reason );
            }
            catch( Exception ex )
            {
                Console.WriteLine( ex.ToString() );
        ...
        /// <param name = "reason">The <see cref = "AuctionResult" /> stating why the auction is ending</param>
        ...
        public static void WriteEnd( AuctionItem auction, AuctionResult reason, Mobile m, string comments )
        {
            ...
            try
            {
                ...
                if( m != null )
                    m_Writer.WriteLine( "# Ended by {0} [{1}], {2}, Account : {3}", m.Name, m.Serial, m.AccessLevel, m.Account.Username );
        ...
        public static void WriteViewItem( AuctionItem auction, Mobile m )
        {
            ...
            try
            {
                m_Writer.WriteLine( "> Vieweing item [{0}] Mobile: {1} {2}, {3}, Account : {4}", auction.ID, m.Name, m.Serial, m.AccessLevel,
                                   m.Account.Username );
            }
            catch( Exception ex )
            {
        ...
        public static void WriteReturnItem( AuctionItem auction, Mobile m )
        {
            ...
            try
            {
                m_Writer.WriteLine( "> Returning item [{0}] Mobile: {1} {2}, {3}, Account : {4}", auction.ID, m.Name, m.Serial, m.AccessLevel,
                                   m.Account.Username );
            }
            catch( Exception ex )
            {
