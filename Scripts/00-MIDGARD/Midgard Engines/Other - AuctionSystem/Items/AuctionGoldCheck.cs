...
namespace Midgard.Engines.AuctionSystem
{
    ...
    public class AuctionGoldCheck : AuctionCheck
    {
        ...
        public AuctionGoldCheck( AuctionItem auction, AuctionResult result )
        {
            ...
            switch( result )
            {
                ...
                case AuctionResult.ItemDeleted:
                    ...
                    {
                        case AuctionResult.Outbid:
                            Message = string.Format( AuctionSystem.ST[ 123 ], m_ItemName, m_GoldAmount.ToString( "#,0" ) );
                            break;

                        case AuctionResult.SystemStopped:
                            Message = string.Format( AuctionSystem.ST[ 124 ], m_ItemName, m_GoldAmount.ToString( "#,0" ) );
                            break;

                        case AuctionResult.PendingRefused:
                            Message = string.Format( AuctionSystem.ST[ 125 ], m_ItemName );
                            break;

                        case AuctionResult.ReserveNotMet:
                            Message = string.Format( AuctionSystem.ST[ 126 ], m_GoldAmount.ToString( "#,0" ), m_ItemName );
                            break;
                        ...
                        case AuctionResult.ItemDeleted:
                            Message = AuctionSystem.ST[ 128 ];
                            break;

                        case AuctionResult.StaffRemoved:
                            Message = AuctionSystem.ST[ 202 ];
                            break;
                    }
                    break;
                ...
                case AuctionResult.BuyNow:
                    ...
                    Hue = SoldHue;
                    Message = string.Format( AuctionSystem.ST[ 129 ], m_ItemName, m_GoldAmount.ToString( "#,0" ) );
                    break;
        ...
        {
            get { return string.Format( "{0} Gold Coins", m_GoldAmount.ToString( "#,0" ) ); }
        }
        ...
        public override void GetProperties( ObjectPropertyList list )
        {
            ...
            list.Add( 1060659, AuctionSystem.ST[ 235 ], m_GoldAmount.ToString( "#,0" ) ); // Gold\t{0}
        }
        ...
        public override bool Deliver( Mobile to )
        {
            ...
            {
                m_Owner.SendMessage( Config.MessageHue, AuctionSystem.ST[ 212 ] );
                return false;
            }
            else // Success
            {
                DeliveryComplete();
                Delete();
                m_Owner.SendMessage( Config.MessageHue, AuctionSystem.ST[ 117 ] );
                return true;
            }
        }
