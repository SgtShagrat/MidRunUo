...
namespace Midgard.Engines.AuctionSystem.Savings
{
    ...
    public class SavingsAccount : Item
    {
        ...
        private static DateTime m_DateLastPaid;
        ...
        {
            CommandHandlers.Register( "InstallSavings", AccessLevel.Developer, new CommandEventHandler( InstallSavings_OnCommand ) );
            CommandHandlers.Register( "ScaleSavings", AccessLevel.Developer, new CommandEventHandler( ScaleSavings_OnCommand ) );
        }
        ...
        {
            WorldSaveProfiler.Instance.StartHandlerProfile( World_Save );

            DateTime now = DateTime.Now;

            if( Config.InterestHour <= now.Hour && m_DateLastPaid != now.Date )
            {
                ...
                foreach( Item item in accounts )
                {
                    ...
                    if( account != null )
                    {
                        account.Gold += (long)( Config.GoldInterestRate * account.Gold );
                        if( EnableTokens )
                            account.Tokens += (long)( Config.TokensInterestRate * account.Tokens );

                        BankBox box = account.Parent as BankBox;

                        if( null != box && null != box.Owner && box.Owner.Map != Map.Internal )
                            box.Owner.SendMessage( Config.AuctionRedHue, "Interest has been paid on the {0} in your savings account!",
                                                  Config.EnableTokens ? "gold and tokens" : "gold" );
                    }
                }
                m_DateLastPaid = now.Date;
            }
        ...
        {
            get { return Config.EnableTokens && Config.TokenType != null; }
        }
        ...
        public SavingsAccount( Serial serial )
            : base( serial )
        {
        }
        ...
        private bool DepositItem( IEntity forDeposit, out bool isContainer )
        {
            bool result = true;
            isContainer = false;
            ...
            /*
        else if( EnableTokens && forDeposit.GetType() == Config.TokenType )
        {
            int deposit = 0;
        ...
        else if( EnableTokens && forDeposit.GetType() == Config.TokenCheckType )
        {
            int deposit = 0;
        ...
        */
            else if( forDeposit is Container )
            {
                ...
                if( null != container.Items )
                {
                    ...
                    for( int i = list.Count - 1; i >= 0; i-- )
                    {
                        if( list[ i ] != null )
                        {
                            if( !DepositItem( list[ i ], out isContainer ) )
                                result = false;
                        }
                    }
                }
        ...
        private static void ScaleSavings_OnCommand( CommandEventArgs e )
        {
            ...
            if( percent >= 0 )
                ScaleSavingsAccounts( option, percent / 100.00 );
        }

        private static void ScaleSavingsAccounts( SavingsOption option, double percent )
        {
        ...
        public override void Deserialize( GenericReader reader )
        {
            ...
            switch( version )
            {
                case 0:
                    ...
                    break;
                case 1:
                    m_DateLastPaid = reader.ReadDateTime();
                    goto case 0;
            }
        }
    }
    ...
    public class SavingsGump : Gump
    {
        ...
        private static Type[] s_ArgTypes = new Type[] { typeof( Int32 ), };

        public SavingsGump( Mobile from )
            : base( 400, 300 )
        {
            ...
            m_MaximumCheckValue = Config.MaximumCheckValue.ToString( "#,0" );
        ...
        private int GetCheckAmount( Mobile from, CurrencyType currency, string amountText )
        {
            if( string.IsNullOrEmpty( amountText ) )
                from.SendMessage( string.Format( "Enter an amount between 1 and {0} for a bank check.", m_MaximumCheckValue ) );
            else
            {
                try
                {
                    ...
                    if( amount >= 1 && amount <= Config.MaximumCheckValue )
                        return amount;
                    else
                        from.SendMessage( string.Format( "Enter an amount between 1 and {0} for a bank check.", m_MaximumCheckValue ) );
                }
        ...
        private void IssueCheck( Mobile from, CurrencyType currency, int amount )
        {
            ...
            else
            {
                ...
                try
                {
                    ConstructorInfo ci = Config.TokenCheckType.GetConstructor( s_ArgTypes );
            ...
            if( null != check )
            {
                if( m_Account.Withdraw( currency, amount ) )
                {
                    if( m_Pack == null || !m_Pack.TryDropItem( from, check, false ) )
                    {
                        ...
                        {
                            from.SendMessage( string.Format( "You have withdrawn {0} {{0}} from savings.", amount.ToString( "#,0" ) ), currencyString );
                        }
                    }
                    else
                    {
                    ...
                    from.SendMessage( string.Format( "You have insufficient funds to withdraw {0} {{0}} from savings!", amount.ToString( "#,0" ) ),
                                     currencyString );
            }
        }
    }
