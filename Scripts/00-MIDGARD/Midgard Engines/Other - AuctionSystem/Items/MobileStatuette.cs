...
namespace Midgard.Engines.AuctionSystem
{
    ...
    /// <summary>
    /// This item holds information about a <see cref="BaseCreature"/> sold through an auction system
    /// </summary>
    public class MobileStatuette : Item, IShrinkItem
    {
        ...
        private MobileStatuette( BaseCreature creature )
        {
            ...
            Name = MidgardUtility.GetFriendlyClassName( creature.GetType().Name );
        }
        ...
        /// <summary>
        /// Converta a given <see cref="BaseCreature"/> object into a statuette object which can be sold through the auction
        ...
        /// <returns>A <c>MobileStatuette</c> if the creature can be auctioned, null if the process fails</returns>
        public static MobileStatuette Create( Mobile from, BaseCreature creature )
        {
            if( !creature.Controlled || creature.ControlMaster != from )
            {
                from.SendMessage( Config.MessageHue, AuctionSystem.ST[ 139 ] );
                return null;
            }

            if( creature.IsAnimatedDead || creature.IsDeadPet )
            {
                from.SendMessage( Config.MessageHue, AuctionSystem.ST[ 140 ] );
                return null;
            }

            if( creature.Summoned )
            {
                from.SendMessage( Config.MessageHue, AuctionSystem.ST[ 141 ] );
                return null;
            }

            if( creature is BaseFamiliar )
            {
                from.SendMessage( Config.MessageHue, AuctionSystem.ST[ 142 ] );
                return null;
            }
            ...
            {
                from.SendMessage( Config.MessageHue, AuctionSystem.ST[ 143 ] );
                return null;
            }
        ...
        public bool Stable( Mobile m )
        {
            if( m_Creature == null )
            {
                m.SendMessage( Config.MessageHue, AuctionSystem.ST[ 144 ] );
                Delete();
                return true;
            }
            ...
            {
                m.SendMessage( Config.MessageHue, AuctionSystem.ST[ 178 ] );
                return false;
            }
