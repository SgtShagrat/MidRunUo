...
namespace Midgard.Engines.AuctionSystem
{
    ...
    public class AuctionViewGump : Gump
    {
        ...
        private static int GetItemHue( Item item )
        {
            if( null == item )
                return 0;

            int hue = item.Hue == 1 ? Config.BlackHue : item.Hue;
        ...
        private void MakeGump()
        {
            ...
            int itemHue = GetItemHue( item.AItem );
            ...
            // Reserve and bids
            AddLabel( 10, 175, Config.AuctionLabelHue, AuctionSystem.ST[ 68 ] );
            AddLabel( 45, 190, Config.AuctionGreenHue, m_Auction.MinBid.ToString( "#,0" ) );

            AddLabel( 10, 280, Config.AuctionLabelHue, AuctionSystem.ST[ 69 ] );
            AddLabel( 45, 295, m_Auction.ReserveMet ? Config.AuctionGreenHue : Config.AuctionRedHue,
                     m_Auction.ReserveMet ? "Met" : "Not Met" );

            AddLabel( 10, 210, Config.AuctionLabelHue, AuctionSystem.ST[ 70 ] );

            if( m_Auction.HasBids )
                AddLabel( 45, 225, m_Auction.ReserveMet ? Config.AuctionGreenHue : Config.AuctionRedHue,
                         m_Auction.HighestBid.Amount.ToString( "#,0" ) );
            else
                AddLabel( 45, 225, Config.AuctionRedHue, AuctionSystem.ST[ 71 ] );

            // Time remaining
            string timeleft;

            AddLabel( 10, 245, Config.AuctionLabelHue, AuctionSystem.ST[ 56 ] );
            ...
            AddLabel( 45, 260, Config.AuctionGreenHue, timeleft );

            bool highest = ( m_Auction != null && m_Auction.HighestBid != null && m_Auction.HighestBid.Mobile != null && m_User != null &&
                            m_User == m_Auction.HighestBid.Mobile );

            if( highest )
                AddLabel( 10, 338, 88, AuctionSystem.ST[ 232 ] ); // "You're highest bidder"
            ...
            if( m_Auction != null )
            {
                ...
                {
                    // View bids: button 5
                    AddLabel( 10, 338, Config.AuctionLabelHue, AuctionSystem.ST[ 80 ] );
            ...
            if( m_Auction != null )
            {
                ...
                {
                    AddLabel( 170, 10, Config.AuctionGreenHue, string.Format( AuctionSystem.ST[ 231 ], m_Auction.ItemName ) );

                    AddImageTiled( 169, 29, 317, 27, kBeigeBorderOuter );
                    AddImageTiled( 170, 30, 315, 25, kBeigeBorderInner );
                    AddAlphaRegion( 170, 30, 315, 25 );
                    AddLabel( 185, 35, Config.AuctionGreenHue, string.Format( AuctionSystem.ST[ 67 ], m_Page + 1, m_Auction.ItemCount ) );
                    ...
                    AddHtml( 173, 56, 312, 114, m_Auction[ m_Page ].Properties, false, true );
                }
                else
                {
                    AddLabel( 170, 10, Config.AuctionGreenHue, m_Auction.ItemName );
                    AddHtml( 173, 30, 312, 140, m_Auction[ m_Page ].Properties, false, true );
                }
            }
            ...
            AddLabel( 170, 175, Config.AuctionLabelHue, AuctionSystem.ST[ 81 ] );
            AddImageTiled( 170, 195, 315, 110, kBeigeBorderInner );
            AddAlphaRegion( 170, 195, 315, 110 );

            if( m_Auction != null )
            {
                AddHtml( 173, 195, 312, 110, string.Format( "<basefont color=#FFFFFF>{0}", m_Auction.Description ), false, true );

                // Web link button: 3
                if( !string.IsNullOrEmpty( m_Auction.WebLink ) )
                {
                    AddLabel( 350, 175, Config.AuctionLabelHue, AuctionSystem.ST[ 72 ] );
            ...
            // Buy now : Button 8
            if( m_Auction != null && m_Auction.AllowBuyNow && m_Auction.CanBid( m_User ) && !m_Auction.Expired )
            {
                AddButton( 170, 310, 4029, 4030, 8, GumpButtonType.Reply, 0 );
                AddLabel( 205, 312, Config.AuctionGreenHue, string.Format( AuctionSystem.ST[ 210 ], m_Auction.BuyNow.ToString( "#,0" ) ) );
            }

            // Button 6 : Admin Auction Panel
            if( m_User != null && m_User.AccessLevel >= Config.AuctionAdminAcessLevel )
            {
                AddButton( 170, 338, 4011, 4012, 6, GumpButtonType.Reply, 0 );
                AddLabel( 205, 340, Config.AuctionLabelHue, AuctionSystem.ST[ 227 ] );
            }

            // Close button: 0
            AddButton( 455, 338, 4017, 4018, 0, GumpButtonType.Reply, 0 );
            AddLabel( 415, 340, Config.AuctionLabelHue, AuctionSystem.ST[ 7 ] );
        }
        ...
        public override void OnResponse( NetState sender, RelayInfo info )
        {
            ...
            {
                sender.Mobile.SendMessage( Config.MessageHue, AuctionSystem.ST[ 207 ] );
            ...
            switch( buttonid )
            {
                ...
                case 4: // Bid
                    ...
                    if( !String.IsNullOrEmpty( info.TextEntries[ 0 ].Text ) )
                    {
                        try
                        {
                            bid = uint.Parse( info.TextEntries[ 0 ].Text, NumberStyles.AllowThousands );
                        }
                        catch
                        {
                        }
                    ...
                    if( m_Auction.Expired )
                    {
                        m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 84 ] );
                    }
                    else if( bid == 0 )
                    {
                        m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 85 ] );
                    }
                    else
                    {
