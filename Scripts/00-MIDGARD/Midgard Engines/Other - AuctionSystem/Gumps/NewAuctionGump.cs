...
namespace Midgard.Engines.AuctionSystem
{
    ...
    public class NewAuctionGump : Gump
    {
        ...
        private void MakeGump()
        {
            ...
            AddItemCentered( 5, 5, 155, 155, m_Auction.SoldAuctionItem.ItemID, this );
            ...
            AddLabel( 250, 10, Config.AuctionRedHue, AuctionSystem.ST[ 100 ] );

            // Starting bid: text 0
            AddLabel( 170, 35, Config.AuctionLabelHue, AuctionSystem.ST[ 68 ] );
            AddImageTiled( 254, 34, 72, 22, 2524 );
            AddImageTiled( 255, 35, 70, 20, 2624 );
            AddAlphaRegion( 255, 35, 70, 20 );
            AddTextEntry( 255, 35, 70, 20, Config.AuctionGreenHue, 0, m_Auction.MinBid.ToString( "#,0" ) );

            // Reserve: text 1
            AddLabel( 345, 35, Config.AuctionLabelHue, AuctionSystem.ST[ 69 ] );
            AddImageTiled( 414, 34, 72, 22, 2524 );
            AddImageTiled( 415, 35, 70, 20, 2624 );
            AddAlphaRegion( 415, 35, 70, 20 );
            AddTextEntry( 415, 35, 70, 20, Config.AuctionGreenHue, 1, m_Auction.Reserve.ToString( "#,0" ) );

            // Days duration: text 2
            AddLabel( 170, 60, Config.AuctionLabelHue, AuctionSystem.ST[ 101 ] );
            AddImageTiled( 254, 59, 32, 22, 2524 );
            AddImageTiled( 255, 60, 30, 20, 2624 );
            AddAlphaRegion( 255, 60, 30, 20 );
            AddTextEntry( 255, 60, 30, 20, Config.AuctionGreenHue, 2, m_Auction.Duration.TotalDays.ToString() );
            AddLabel( 290, 60, Config.AuctionLabelHue, AuctionSystem.ST[ 102 ] );

            // Item name: text 3
            AddLabel( 170, 85, Config.AuctionLabelHue, AuctionSystem.ST[ 50 ] );
            AddImageTiled( 254, 84, 232, 22, 2524 );
            AddImageTiled( 255, 85, 230, 20, 2624 );
            AddAlphaRegion( 255, 85, 230, 20 );
            AddTextEntry( 255, 85, 230, 20, Config.AuctionGreenHue, 3, m_Auction.ItemName );

            // Buy now: Check 0, Text 6
            AddCheck( 165, 110, 2152, 2153, false, 0 );
            AddLabel( 200, 115, Config.AuctionLabelHue, AuctionSystem.ST[ 208 ] );
            AddImageTiled( 329, 114, 157, 22, 2524 );
            AddImageTiled( 330, 115, 155, 20, 2624 );
            AddAlphaRegion( 330, 115, 155, 20 );
            AddTextEntry( 330, 115, 155, 20, Config.AuctionGreenHue, 6, "" );

            // Description: text 4
            AddLabel( 170, 140, Config.AuctionLabelHue, AuctionSystem.ST[ 103 ] );
            AddImageTiled( 169, 159, 317, 92, 2524 );
            AddImageTiled( 170, 160, 315, 90, 2624 );
            AddAlphaRegion( 170, 160, 315, 90 );
            AddTextEntry( 170, 160, 315, 90, Config.AuctionGreenHue, 4, m_Auction.Description );

            // Web link: text 5
            AddLabel( 170, 255, Config.AuctionLabelHue, AuctionSystem.ST[ 104 ] );
            AddImageTiled( 224, 274, 262, 22, 2524 );
            AddLabel( 170, 275, Config.AuctionLabelHue, @"http://" );
            AddImageTiled( 225, 275, 260, 20, 2624 );
            AddAlphaRegion( 225, 275, 260, 20 );
            AddTextEntry( 225, 275, 260, 20, Config.AuctionGreenHue, 5, m_Auction.WebLink );
            ...
            AddLabel( 210, 300, Config.AuctionRedHue, AuctionSystem.ST[ 106 ] );
            AddLabel( 210, 315, Config.AuctionRedHue, AuctionSystem.ST[ 107 ] );

            // Cancel button: button 0
            AddButton( 170, 335, 4020, 4020, 0, GumpButtonType.Reply, 0 );
            AddLabel( 210, 335, Config.AuctionLabelHue, AuctionSystem.ST[ 108 ] );
        }
        ...
        public override void OnResponse( NetState sender, RelayInfo info )
        {
            ...
            switch( info.ButtonID )
            {
                ...
                case 1: // Commit the auction
                    ...
                    // Collect information
                    int minbid = 1; // text 0
                    int reserve = 1; // text 1
                    int days = 7; // text 2
                    string name = "null name"; // text 3
                    ...
                    try
                    {
                        minbid = (int)uint.Parse( tr[ 0 ], NumberStyles.AllowThousands );
                    }
                    catch
                    {
                    }
                    ...
                    try
                    {
                        reserve = (int)uint.Parse( tr[ 1 ], NumberStyles.AllowThousands );
                    }
                    catch
                    {
                    }
                    ...
                    try
                    {
                        buynow = (int)uint.Parse( tr[ 6 ], NumberStyles.AllowThousands );
                    }
                    catch
                    {
                    }
                    ...
                    if( minbid < 1 )
                    {
                        m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 109 ] );
                        ok = false;
                    }
                    ...
                    {
                        reserve = minbid;
                        m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 110 ] );
                        ok = false;
                    }

                    if( days < AuctionSystem.MinAuctionDays && m_User.AccessLevel < AccessLevel.GameMaster || days < 1 )
                    {
                        m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 111 ], AuctionSystem.MinAuctionDays );
                        ok = false;
                    }

                    if( days > AuctionSystem.MaxAuctionDays && m_User.AccessLevel < AccessLevel.GameMaster )
                    {
                        m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 112 ], AuctionSystem.MaxAuctionDays );
                        ok = false;
                    }

                    if( name.Length == 0 )
                    {
                        m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 113 ] );
                        ok = false;
                    }

                    if( minbid * Config.MaxReserveMultiplier < reserve && m_User.AccessLevel < AccessLevel.GameMaster )
                    {
                        m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 114 ] );
                        ok = false;
                    }

                    if( allowBuyNow && buynow < reserve )
                    {
                        m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 209 ] );
                        ok = false;
                    }

                    if( ok && Config.CostOfAuction > 0.0 )
                    {
                        int toPay;

                        if( Config.CostOfAuction <= 1.0 )
                            toPay = (int)( Math.Max( minbid, reserve ) * Config.CostOfAuction );
                        else
                            toPay = (int)Config.CostOfAuction;

                        if( toPay > 0 )
                        {
                            if( Banker.Withdraw( m_User, toPay ) )
                                m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 228 ], toPay );
                            else
                            {
                                m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 229 ], toPay );
                    ...
                    {
                        m_User.SendMessage( Config.MessageHue, AuctionSystem.ST[ 115 ] );
                    }
