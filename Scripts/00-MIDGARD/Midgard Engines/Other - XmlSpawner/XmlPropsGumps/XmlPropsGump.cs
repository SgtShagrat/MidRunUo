...
namespace Server.Gumps
{
    public class XmlPropertiesGump : Gump
    {
        ...
        private static readonly int TotalWidth = OffsetSize + NameWidth + OffsetSize + ValueWidth + OffsetSize +
                                                 SetWidth + OffsetSize;
        ...
        private void Initialize( int page )
        {
            ...
            AddImageTiled( BorderSize, BorderSize + EntryHeight, ( TotalWidth - ( OldStyle ? SetWidth + OffsetSize : 0 ) ) * 3,
                          totalHeight - EntryHeight, OffsetGumpID );
            ...
            for( int i = 0, index = page * EntryCount; i < count && index < m_List.Count; ++i, ++index )
            {
                ...
                */
                    if( o is PropertyInfo )
                    {
                        ...
                        var de = new XmlSpawnerDefaults.DefaultEntry();
                ...
                default:
                    {
                        ...
                        if( index >= 0 && index < m_List.Count )
                        {
                            ...
                            if( IsType( type, m_TypeofMobile ) || IsType( type, m_TypeofItem ) )
                            ...
                            else if( IsType( type, m_TypeofString ) || IsType( type, m_TypeofReal ) ||
                                     IsType( type, m_TypeofNumeric ) )
                            ...
                            else if( IsType( type, m_TypeofSkills ) && m_Object is Mobile )
                            {
                                ...
                                from.SendGump( new XmlPropertiesGump( from, prop.GetValue( m_Object, null ), m_Stack,
                                                                    m_Object ) );
                        }
        ...
        {
            object[] attrs = type.GetCustomAttributes( m_TypeofCustomEnum, false );
        ...
        private static Type m_TypeofMobile = typeof( Mobile );
        private static Type m_TypeofItem = typeof( Item );
        private static Type m_TypeofType = typeof( Type );
        private static Type m_TypeofPoint3D = typeof( Point3D );
        private static Type m_TypeofPoint2D = typeof( Point2D );
        private static Type m_TypeofTimeSpan = typeof( TimeSpan );
        private static Type m_TypeofCustomEnum = typeof( CustomEnumAttribute );
        private static Type m_TypeofEnum = typeof( Enum );
        private static Type m_TypeofBool = typeof( Boolean );
        private static Type m_TypeofString = typeof( String );
        private static Type m_TypeofPoison = typeof( Poison );
        private static Type m_TypeofMap = typeof( Map );
        private static Type m_TypeofSkills = typeof( Skills );
        private static Type m_TypeofPropertyObject = typeof( PropertyObjectAttribute );
        private static Type m_TypeofNoSort = typeof( NoSortAttribute );

        private static Type[] m_TypeofReal = new Type[]
                                               {
                                                   typeof (Single),
                                                   typeof (Double)
                                               };

        private static Type[] m_TypeofNumeric = new Type[]
                                                  {
                                                      typeof (Byte),
                                                      typeof (Int16),
                                                      typeof (Int32),
        ...
        public static string ValueToString( object o )
        {
            ...
            else if( o is byte || o is sbyte || o is short || o is ushort || o is int || o is uint || o is long ||
                     o is ulong )
            {
        ...
        private static Type m_TypeofCpa = typeof( CommandPropertyAttribute );
        private static Type m_TypeofObject = typeof( object );

        private static CommandPropertyAttribute GetCPA( PropertyInfo prop )
        {
            object[] attrs = prop.GetCustomAttributes( m_TypeofCpa, false );
