...
namespace Server.Items
{
    ...
    public abstract class XmlQuest
    {
        ...
        public static object CreateItem( IEntity from, string action, out string statusStr, Type typerestrict )
        {
            statusStr = null;

            if( string.IsNullOrEmpty( action ) || from == null )
                return null;

            var theSpawn = new SpawnObject( null, 0 );

            //BaseXmlSpawner.ApplyObjectStringProperties(null, action, m_TargetItem, m, m_TargetItem, out statusStr);
            ...
            {
                BaseXmlSpawner.SpawnTypeKeyword( null, theSpawn, typeName, substitutedtypeName, true, null, from.Location,
                                                Map.Internal, out statusStr );
            }
            else
            {
                ...
                try
                {
                    ...
                    if( o == null )
                    {
                        statusStr = "invalid type specification: " + arglist[ 0 ];
                    }
                    else if( o is Mobile )
                    {
                        var m = (Mobile)o;
                    ...
                    {
                        var item = (Item)o;
                        BaseXmlSpawner.AddSpawnItem( null, from, theSpawn, item, from.Location, Map.Internal, null, false,
                                                    substitutedtypeName, out statusStr );
                    }
        ...
        public static void CheckArgList( string[] arglist, int argstart, object propobj, out string typestr,
                                        out int targetcount, out bool checkprop, out string statusStr )
        {
            targetcount = 1;
            checkprop = true;
            statusStr = null;
            ...
            if( arglist.Length > argstart )
            {
                ...
                for( int i = argstart; i < arglist.Length; i++ )
                {
                    ...
                    {
                        // its a prop arg
                        checkprop = BaseXmlSpawner.CheckPropertyString( null, propobj, arglist[ i ], null, out statusStr );
                    }
                    else if( !string.IsNullOrEmpty( arglist[ i ] ) && arglist[ i ][ 0 ] >= '0' && arglist[ i ][ 0 ] <= '9' )
                    {
                        // its a count arg
                        try
                        {
        ...
        public static void ApplyCollected( Item target, IXmlQuest quest )
        {
            ...
            if( !quest.Completed1 &&
                CheckCollectObjective( quest, target, quest.Objective1, quest.State1, out newstatestr, out collectstatus ) )
            {
                quest.State1 = newstatestr;
                quest.Completed1 = collectstatus;
            }
            else if( !quest.Completed2 &&
                     CheckCollectObjective( quest, target, quest.Objective2, quest.State2, out newstatestr,
                                           out collectstatus ) )
            {
                quest.State2 = newstatestr;
                quest.Completed2 = collectstatus;
            }
            else if( !quest.Completed3 &&
                     CheckCollectObjective( quest, target, quest.Objective3, quest.State3, out newstatestr,
                                           out collectstatus ) )
            {
                quest.State3 = newstatestr;
                quest.Completed3 = collectstatus;
            }
            else if( !quest.Completed4 &&
                     CheckCollectObjective( quest, target, quest.Objective4, quest.State4, out newstatestr,
                                           out collectstatus ) )
            {
                quest.State4 = newstatestr;
                quest.Completed4 = collectstatus;
            }
            else if( !quest.Completed5 &&
                     CheckCollectObjective( quest, target, quest.Objective5, quest.State5, out newstatestr,
                                           out collectstatus ) )
            {
                                                 ...
                                                 out string newstatestr, out bool collectstatus )
        {
            ...
            CheckArgList( arglist, 2, item, out typestr, out targetcount, out checkprop, out statusStr );

            if( statusStr != null )
                quest.Status = statusStr;
            ...
            if( arglist.Length > 1 )
            {
                ...
                if( arglist[ 0 ] == "COLLECT" )
                {
                    ...
                    if( item != null && !item.Deleted && BaseXmlSpawner.CheckType( item, arglist[ 1 ] )
                        /*(item.GetType() == targettype)*/&& checkprop )
                    {
                        // found a match
                        found = true;
                    }
        ...
        public static bool ApplyGiven( Mobile mob, Item target, IXmlQuest quest )
        {
            ...
            if( !quest.Completed1 &&
                CheckGiveObjective( quest, mob, target, quest.Objective1, quest.State1, out newstatestr, out givestatus ) )
            {
                quest.State1 = newstatestr;
                ...
                found = true;
                     ...
                     CheckGiveObjective( quest, mob, target, quest.Objective2, quest.State2, out newstatestr,
                                        out givestatus ) )
            {
                quest.State2 = newstatestr;
                ...
                found = true;
                     ...
                     CheckGiveObjective( quest, mob, target, quest.Objective3, quest.State3, out newstatestr,
                                        out givestatus ) )
            {
                quest.State3 = newstatestr;
                ...
                found = true;
                     ...
                     CheckGiveObjective( quest, mob, target, quest.Objective4, quest.State4, out newstatestr,
                                        out givestatus ) )
            {
                quest.State4 = newstatestr;
                ...
                found = true;
                     ...
                     CheckGiveObjective( quest, mob, target, quest.Objective5, quest.State5, out newstatestr,
                                        out givestatus ) )
            {
                quest.State5 = newstatestr;
        ...
        public static bool RegisterGive( Mobile from, Mobile to, Item item )
        {
            ...
            {
                List<Item> questlist = FindXmlQuest( from as PlayerMobile );
                if( questlist != null )
                {
                                              ...
                                              string statestr, out string newstatestr, out bool givestatus )
        {
            ...
            CheckArgList( arglist, 3, item, out typestr, out targetcount, out checkprop, out statusStr );
            ...
            if( arglist.Length > 2 )
            {
                ...
                if( arglist[ 0 ] == "GIVE" )
                {
                    ...
                    if( item != null && !item.Deleted && BaseXmlSpawner.CheckType( item, arglist[ 2 ] )
                        /*(item.GetType() == targettype)*/&& checkprop )
                    {
                        // found a match
                        found = true;
        ...
        public static bool CheckKillObjective( IXmlQuest quest, Mobile mKilled, Mobile mKiller, string objectivestr,
                                              string statestr, out string newstatestr, out bool killstatus )
        {
            ...
            CheckArgList( arglist, 2, mKilled, out typestr, out targetcount, out checkprop, out statusStr );
            ...
            if( arglist.Length > 1 )
            {
                ...
                if( arglist[ 0 ] == "KILL" )
                {
                    ...
                    if( mKilled != null && !mKilled.Deleted && BaseXmlSpawner.CheckType( mKilled, arglist[ 1 ] )
                        /*(mKilled.GetType() == targettype)*/&& checkprop )
                    {
                        // found a match
                        found = true;
                    }
                ...
                {
                    if( mKilled != null && !mKilled.Deleted && ( arglist[ 1 ] == mKilled.Name ) && checkprop &&
                        ( typestr == null || BaseXmlSpawner.CheckType( mKilled, typestr ) )
                        )
                    {
                        // found a match
                        found = true;
        ...
        public static void ApplyKilled( Mobile mKilled, Mobile mKiller, IXmlQuest quest )
        {
            if( quest == null || !quest.IsValid )
                return;
            ...
            if( !quest.Completed1 &&
                CheckKillObjective( quest, mKilled, mKiller, quest.Objective1, quest.State1, out newstatestr,
                                   out killstatus ) )
            {
                quest.State1 = newstatestr;
                quest.Completed1 = killstatus;
                     ...
                     CheckKillObjective( quest, mKilled, mKiller, quest.Objective2, quest.State2, out newstatestr,
                                        out killstatus ) )
            {
                quest.State2 = newstatestr;
                quest.Completed2 = killstatus;
                     ...
                     CheckKillObjective( quest, mKilled, mKiller, quest.Objective3, quest.State3, out newstatestr,
                                        out killstatus ) )
            {
                quest.State3 = newstatestr;
                quest.Completed3 = killstatus;
                     ...
                     CheckKillObjective( quest, mKilled, mKiller, quest.Objective4, quest.State4,
                                        out newstatestr, out killstatus ) )
            {
                ...
                quest.Completed4 = killstatus;
                     ...
                     CheckKillObjective( quest, mKilled, mKiller, quest.Objective5, quest.State5,
                                        out newstatestr, out killstatus ) )
            {
                quest.State5 = newstatestr;
        ...
        public static void CheckKilled( Mobile mKilled, Mobile mKiller, Mobile member )
        {
            if( !( member is PlayerMobile ) )
                return;

            // search the player for IXmlQuest objects
            List<Item> mobitems = FindXmlQuest( member as PlayerMobile );
                    ...
                    if( quest != null && !quest.Deleted && quest.PartyEnabled )
                    {
                        if( !member.Deleted )
                        {
                            if( quest.PartyRange < 0 ||
                                Utility.InRange( mKiller.Location, member.Location, quest.PartyRange ) )
                                ApplyKilled( mKilled, member, quest );
                        }
                    }
                    else if( !member.Deleted && member == mKiller && quest != null && !quest.Deleted )
                    {
                        ApplyKilled( mKilled, mKiller, quest );
                    }
                }
            }
        }
        ...
        public static void RegisterKill( Mobile mKilled, Mobile mKiller )
        {
            ...
            if( p != null )
            {
                foreach( PartyMemberInfo mi in p.Members )
                {
                    ...
                    {
                        CheckKilled( mKilled, mKiller, member );
                    }
                }
            }
            else
            {
                if( mKiller != null && mKiller is PlayerMobile &&
                    ( (PlayerMobile)mKiller ).GetFlag( CarriedXmlQuestFlag ) )
                {
                    CheckKilled( mKilled, mKiller, mKiller );
                }
            }
        }

        public static bool CheckEscortObjective( IXmlQuest quest, Mobile mEscorted, Mobile mEscorter,
                                                string objectivestr, string statestr, out string newstatestr,
                                                out bool escortstatus )
        {
            ...
            if( arglist.Length > 2 )
            {
                checkprop = BaseXmlSpawner.CheckPropertyString( null, mEscorted, arglist[ 2 ], null, out statusStr );
            }
            ...
            if( arglist.Length > 1 )
            {
                ...
                if( mEscorted != null && !mEscorted.Deleted &&
                    ( arglist[ 1 ] == mEscorted.Name || ( string.IsNullOrEmpty( arglist[ 1 ] ) ) ) && checkprop )
                {
                    // found a match
                    found = true;
                }
            ...
            else
            {
                ...
                if( mEscorted != null && !mEscorted.Deleted && checkprop )
                {
                    // found a match
                    found = true;
                }
        ...
        public static void ApplyEscorted( Mobile mEscorted, Mobile mEscorter, IXmlQuest quest )
        {
            if( quest == null || !quest.IsValid )
                return;
            ...
            if( !quest.Completed1 &&
                CheckEscortObjective( quest, mEscorted, mEscorter, quest.Objective1, quest.State1, out newstatestr,
                                     out escortstatus ) )
            {
                quest.State1 = newstatestr;
                quest.Completed1 = escortstatus;
                     ...
                     CheckEscortObjective( quest, mEscorted, mEscorter, quest.Objective2, quest.State2, out newstatestr,
                                          out escortstatus ) )
            {
                quest.State2 = newstatestr;
                quest.Completed2 = escortstatus;
                     ...
                     CheckEscortObjective( quest, mEscorted, mEscorter, quest.Objective3, quest.State3,
                                          out newstatestr, out escortstatus ) )
            {
                ...
                quest.Completed3 = escortstatus;
                     ...
                     CheckEscortObjective( quest, mEscorted, mEscorter, quest.Objective4, quest.State4,
                                          out newstatestr, out escortstatus ) )
            {
                quest.State4 = newstatestr;
                quest.Completed4 = escortstatus;
                     ...
                     CheckEscortObjective( quest, mEscorted, mEscorter, quest.Objective5, quest.State5,
                                          out newstatestr, out escortstatus ) )
            {
                quest.State5 = newstatestr;
        ...
        public static void CheckEscorted( Mobile mEscorted, Mobile mEscorter, Mobile member )
        {
            if( !( member is PlayerMobile ) )
                return;
            ...
            for( int i = 0; i < mobitems.Count; i++ )
            {
                if( mobitems[ i ] is IXmlQuest )
                {
                    ...
                    if( quest != null && !quest.Deleted && quest.PartyEnabled )
                    {
                        ...
                        {
                            if( quest.PartyRange < 0 ||
                                Utility.InRange( mEscorter.Location, member.Location, quest.PartyRange ) )
                            {
                                ApplyEscorted( mEscorted, member, quest );
                            }
                        }
                    }
                    else if( !member.Deleted && member == mEscorter && quest != null && !quest.Deleted )
                    {
                        ApplyEscorted( mEscorted, mEscorter, quest );
                    }
                }
            }
        }

        public static void RegisterEscort( Mobile mEscorted, Mobile mEscorter )
        {
            // go through all of the party members to to try to fill escort objectives
            Party p = Party.Get( mEscorter );
            if( p != null )
            {
                foreach( PartyMemberInfo mi in p.Members )
                {
                    ...
                    if( member != null && member is PlayerMobile && ( (PlayerMobile)member ).GetFlag( CarriedXmlQuestFlag ) )
                    {
                        CheckEscorted( mEscorted, mEscorter, member );
                    }
                }
            }
            else
            {
                if( mEscorter != null && mEscorter is PlayerMobile &&
                    ( (PlayerMobile)mEscorter ).GetFlag( CarriedXmlQuestFlag ) )
                {
                    CheckEscorted( mEscorted, mEscorter, mEscorter );
                }
            }
        }

        public static List<Sector> VisitSectorList = new List<Sector>();

        public static void RegisterMove( PlayerMobile mPlayer )
        {
            if( mPlayer == null || mPlayer.Map == null )
                return;

            // check for any attachments that might support the OnMove method
            //XmlAttach.CheckOnMove(mPlayer);

            // check to see if the current sector that the player is in, is registered in the VISIT sector list
            Sector newSector = mPlayer.Map.GetSector( mPlayer.Location );
            ...
            if( VisitSectorList != null && VisitSectorList.Contains( newSector ) )
            {
                ...
                if( mPlayer.GetFlag( CarriedXmlQuestFlag ) )
                {
                    CheckVisited( mPlayer );
                }
            }
        }

        public static bool CheckVisitObjective( IXmlQuest quest, PlayerMobile mPlayer, string objectivestr,
            ...
            if( arglist.Length > 3 )
            {
                ...
                if( arglist[ 0 ] == "VISIT" )
                {
                    ...
                    if( arglist.Length > 4 )
                    {
                        ...
                        {
                            statusStr = "invalid VISIT duration";
                        }
                    }

                    // check them against the players current location

                    if( mPlayer != null && mPlayer.InRange( new Point2D( x, y ), range ) )
                    {
                        if( duration > 0 )
                        {
        ...
        public static void ApplyVisited( PlayerMobile mPlayer, IXmlQuest quest )
        {
            if( quest == null || !quest.IsValid )
                return;
            ...
            if( !quest.Completed1 &&
                CheckVisitObjective( quest, mPlayer, quest.Objective1, quest.State1, out newstatestr, out visitstatus ) )
            {
                quest.State1 = newstatestr;
                quest.Completed1 = visitstatus;
            }
            else if( !quest.Completed2 &&
                     CheckVisitObjective( quest, mPlayer, quest.Objective2, quest.State2, out newstatestr,
                                         out visitstatus ) )
            {
                quest.State2 = newstatestr;
                                         ...
                                         out visitstatus ) )
            {
                ...
                quest.Completed3 = visitstatus;
                     ...
                     CheckVisitObjective( quest, mPlayer, quest.Objective4, quest.State4, out newstatestr,
                                         out visitstatus ) )
            {
                quest.State4 = newstatestr;
                quest.Completed4 = visitstatus;
                     ...
                     CheckVisitObjective( quest, mPlayer, quest.Objective5, quest.State5, out newstatestr,
                                         out visitstatus ) )
            {
                quest.State5 = newstatestr;
        ...
        public static void CheckVisited( PlayerMobile mPlayer )
        {
            // search the player for IXmlQuest objects
            List<Item> mobitems = FindXmlQuest( mPlayer );
        ...
        public static bool VerifyObjective( string[] arglist, out string statusStr )
        {
            statusStr = null;
            ...
            switch( arglist[ 0 ] )
            {
                case "COLLECT":
                case "KILL":
                    CheckArgList( arglist, 2, null, out typestr, out targetcount, out checkprop, out statusStr );
                    if( arglist.Length > 1 )
                    {
                        if( SpawnerType.GetType( arglist[ 1 ] ) == null )
                        {
                            statusStr = "Invalid type: " + arglist[ 1 ];
                            return false;
                        }
                    }
                    else
                    {
                        statusStr = arglist[ 0 ] + "missing args";
                        return false;
                    }
                    break;
                case "COLLECTNAMED":
                case "KILLNAMED":
                    ...
                    break;
                case "GIVENAMED":
                    CheckArgList( arglist, 3, null, out typestr, out targetcount, out checkprop, out statusStr );
                    if( arglist.Length < 1 )
                    {
                        statusStr = arglist[ 0 ] + "missing args";
                        return false;
                ...
                case "GIVE":
                    ...
                    if( arglist.Length > 2 )
                    {
                        ...
                        {
                            statusStr = "Invalid type: " + arglist[ 2 ];
                            return false;
                        }
                    }
                    else
            ...
            if( typestr != null )
            {
                ...
                {
                    statusStr = "Invalid type: " + typestr;
                    return false;
                }
            }
        ...
        public static void VerifyObjectives( IXmlQuest quest )
        {
            ...
            VerifyObjective( BaseXmlSpawner.ParseString( quest.Objective1, 6, "," ), out statusStr );
            if( statusStr != null )
                quest.Status = statusStr;
            VerifyObjective( BaseXmlSpawner.ParseString( quest.Objective2, 6, "," ), out statusStr );
            if( statusStr != null )
                quest.Status = statusStr;
            VerifyObjective( BaseXmlSpawner.ParseString( quest.Objective3, 6, "," ), out statusStr );
            if( statusStr != null )
                quest.Status = statusStr;
            VerifyObjective( BaseXmlSpawner.ParseString( quest.Objective4, 6, "," ), out statusStr );
            if( statusStr != null )
                quest.Status = statusStr;
            VerifyObjective( BaseXmlSpawner.ParseString( quest.Objective5, 6, "," ), out statusStr );
            if( statusStr != null )
                quest.Status = statusStr;
        }
    }
