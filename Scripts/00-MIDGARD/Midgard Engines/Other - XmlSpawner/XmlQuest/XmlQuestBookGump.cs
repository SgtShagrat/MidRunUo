...
namespace Server.Gumps
{
    public class XmlQuestBookGump : Gump
    {
        ...
        public XmlQuestBookGump( PlayerMobile from, XmlQuestBook book, int page, List<IXmlQuest> list )
            : base( 12, 24 )
        {
            ...
            if( list == null )
            {
                ...
                list = new List<IXmlQuest>( nquests );

                for( int i = 0; i < nquests; ++i )
                {
                    if( questitems != null )
                        list.Add( (IXmlQuest)questitems[ i ] );
                }
            }
