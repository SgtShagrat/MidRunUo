...
namespace Server.Engines.XmlSpawner2
{
    public class XmlQuestLeaders
    {
        ...
        private static bool m_Needsupdate = true;
        ...
        {
            if( m_Needsupdate && QuestRankList != null )
            {
                QuestRankList.Sort();
        ...
        {
            CommandSystem.Register( "QuestLeaderboardSave", AccessLevel.Developer,
        ...
        public static void QuestLeaderboardSave_OnCommand( CommandEventArgs e )
        {
            ...
            if( m_QuestLeaderboardTimer != null && m_QuestLeaderboardTimer.Running )
            {
                e.Mobile.SendMessage( "Quest Leaderboard is saving to {0} every {1} minutes. Nranks = {2}",
                                     m_QuestLeaderboardFile, m_QuestLeaderboardSaveInterval.TotalMinutes,
                                     m_QuestLeaderboardSaveRanks );
            }
            else
            {
        ...
        public class TopQuestPlayersGump : Gump
        {
                ...
                : base( 0, 0 )
            {
                ...
                for( int i = 0; i < QuestRankList.Count; i++ )
                {
                    ...
                    if( r.Quester != null && !r.Quester.Deleted && r.Rank > 0 && !a.Deleted )
                    {
                        string guildname = null;
                        ...
                        if( !string.IsNullOrEmpty( m_Attachment.GuildFilter ) )
                        {
                        ...
                        if( !string.IsNullOrEmpty( m_Attachment.NameFilter ) )
                        {
                            // parse the comma separated list
