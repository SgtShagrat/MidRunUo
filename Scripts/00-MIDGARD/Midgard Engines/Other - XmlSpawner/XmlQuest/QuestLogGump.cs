...
namespace Server.Gumps
{
    public class QuestLogGump : Gump
    {
        ...
        public QuestLogGump( Mobile from, int page, List<IXmlQuest> list )
            : base( 12, 24 )
        {
            if( from == null )
                return;
            ...
            if( list == null )
            {
                ...
                if( from.Backpack != null )
                {
                    ...
                    if( packquestitems != null )
                    {
                        ...
                        for( int i = 0; i < packquestitems.Length; ++i )
                        {
                            if( packquestitems[ i ] != null && !packquestitems[ i ].Deleted &&
                                ...
                                list.Add( packquestitems[ i ] as IXmlQuest );
                        }
                    }
                    ...
                    if( questbookitems != null )
                    {
                        for( int j = 0; j < questbookitems.Length; ++j )
                        {
                            ...
                            if( questitems != null )
                            {
                                ...
                                {
                                    list.Add( questitems[ i ] as IXmlQuest );
                                }
                            }
                        }
                    }
                    ...
                    if( p != null && p.QuestList != null )
                    {
                        ...
                        foreach( QuestEntry q in p.QuestList )
                        {
                            list.Add( q as IXmlQuest );
                        }
                    }
                }
            }
            ...
            AddHtml( 610 - xoffset, 64, 200, 32, XmlSimpleGump.Color( "Next Available", "FFFFFF" ), false, false );
            // Next Available
            ...
            AddHtml( 150 - xoffset, 416, 200, 32, XmlSimpleGump.Color( "Top Players", "FFFFFF" ), false, false );
            // Top players gump
            ...
            for( int i = index; i < ( index + count ) && i >= 0 && i < list.Count; ++i )
            {
                ...
                else if( obj is QuestEntry )
                {
                    var e = (QuestEntry)obj;
