...
namespace Server.Gumps
{
    ...
    public class XmlSimpleGump : Gump
    {
        ...
        private List<GumpSelection> m_GumpSelections = new List<GumpSelection>();
        ...
        private void LocalAddHtml( string text, int x, int y, int width, int height, int color, bool background,
                                  bool scrollbar )
        {
            if( text == null )
                return;
        ...
        public XmlSimpleGump( object invoker, string gumptext, string gumptitle, int gumptype,
                             BaseXmlSpawner.KeywordTag tag, Mobile from )
            : this( invoker, gumptext, gumptitle, gumptype, tag, from, null )
        {
        }

        public XmlSimpleGump( object invoker, string gumptext, string gumptitle, int gumptype,
                             BaseXmlSpawner.KeywordTag tag, Mobile from, XmlGumpCallback gumpcallback )
            : base( 0, 0 )
        {
            ...
            if( gumptype == 4 )
            {
                ...
                nselections = m_GumpSelections.Count;
            ...
            if( !string.IsNullOrEmpty( gumptitle ) && gumptype != 5 )
            {
            ...
            else if( gumptype == 4 )
            {
                ...
                for( int i = 0; i < m_GumpSelections.Count; i++ )
                {
                    int y = 360 + i * 40;
                    AddRadio( 101, y, 9721, 9724, i == 0 ? true : false, i ); // accept/yes radio
                    AddHtml( 137, y + 4, 250, 40, Color( m_GumpSelections[ i ].Selection, "FFFFFF" ), false,
                            false );
                }
            ...
            else if( gumptype == 5 )
            {
                ...
                for( int i = 0; i < m_GumpSelections.Count; i++ )
                {
                    string selection = m_GumpSelections[ i ].Selection;
                    string response = m_GumpSelections[ i ].Response;
                    ...
                    if( args != null && args.Length > 1 )
                    {
                        ...
                        {
                            m_GumpSelections[ i ].GumpItemType = 1;
                    ...
                    switch( m_Gumptype )
                    {
                        ...
                        case 4: // multiple option gump
                            if( info.Switches != null && info.Switches.Length > 0 )
                            {
                                ...
                                if( select >= 0 && select < m_GumpSelections.Count )
                                {
                                    // return the response string for that selection
                                    m_Gumpcallback( from, m_Invoker, m_GumpSelections[ select ].Response );
                                }
                            }
                            break;
                        case 5:
                            ...
                            if( info.ButtonID >= 1000 )
                            {
                                ...
                                {
                                    // return the response string for that selection
                                    buttonresponse = m_GumpSelections[ select ].Response;
                                }
                            }
                            ...
                            if( info.Switches != null && info.Switches.Length > 0 )
                            {
                                ...
                                if( radiostate >= 0 && radiostate < m_GumpSelections.Count )
                                {
                                    radioresponse = m_GumpSelections[ radiostate ].Response;
                                }
                            }

                            // check for any textentries
                            for( int j = 0; j < m_GumpSelections.Count; j++ )
                            {
                                if( m_GumpSelections[ j ].GumpItemType == 1 )
                                {
                                    try
                                    {
                            ...
                            if( !string.IsNullOrEmpty( buttonresponse ) )
                            {
                                responsestring = buttonresponse;
                            }
                            if( !string.IsNullOrEmpty( radioresponse ) )
                            {
