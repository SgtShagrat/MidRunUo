...
namespace Server.Engines.XmlSpawner2
{
    public class XmlDialog : XmlAttachment
    {
        ...
        private SpeechEntry FindMatchingKeyword( Mobile from, string keyword, int currententryid )
        {
            if( m_SpeechEntries == null )
                return null;
            var matchlist = new List<SpeechEntry>();
        ...
        {
            if( string.IsNullOrEmpty( gumpstring ) )
                return;
            ...
            var loc = new Point3D( 0, 0, 0 );
            Map map = null;

            if( AttachedTo is Mobile )
            {
                var m = AttachedTo as Mobile;
                loc = m.Location;
                map = m.Map;
            }
            ...
            if( typeName == "GUMP" )
            {
                BaseXmlSpawner.SpawnTypeKeyword( this, TheSpawn, typeName, substitutedtypeName, true, mob, loc, map,
                                                new XmlGumpCallback( DialogGumpCallback ), out status_str );
        ...
        private void ExecuteAction( Mobile mob, string action )
        {
            ...
            {
                BaseXmlSpawner.SpawnTypeKeyword( AttachedTo, TheSpawn, typeName, substitutedtypeName, true, mob, loc, map,
                                                out status_str );
            }
            else
            {
        ...
        private static bool IsInRange( IEntity e1, IEntity e2, int range )
        {
            if( e1 == null || e2 == null )
                return false;
        ...
        private void DelayedSpeech( object state )
        {
            ...
            var matchentry = (SpeechEntry)states[ 0 ];
            var m = (Mobile)states[ 1 ];
            ...
            if( matchentry != null )
            {
                ...
                if( !string.IsNullOrEmpty( CurrentEntry.Action ) )
                {
        ...
        public bool ProcessSpeech( Mobile m, string speech )
        {
            ...
            if( ( speech != null || CheckDependsOn( matchentry, -2 ) ) && !string.IsNullOrEmpty( TriggerOnCarried ) )
            {
            ...
            if( ( speech != null || CheckDependsOn( matchentry, -2 ) ) && !string.IsNullOrEmpty( NoTriggerOnCarried ) )
            {
        ...
        public void DoLoadNPC( Mobile from, string filename )
        {
            ...
            if( File.Exists( dirname ) )
            {
                ...
                // Create the data set
                var ds = new DataSet( NPCDataSetName );
                ...
                if( ds.Tables != null && ds.Tables.Count > 0 )
                {
                    ...
                    if( ds.Tables[ NPCPointName ] != null && ds.Tables[ NPCPointName ].Rows.Count > 0 )
                    {
                        ...
                        foreach( DataRow dr in ds.Tables[ SpeechPointName ].Rows )
                        {
                            ...
                            {
                                s.SpeechStyle =
                                    (MessageType)Enum.Parse( typeof( MessageType ), (string)dr[ "SpeechStyle" ] );
                            }
                            catch
                            {
                            }
        ...
        private class SaveNPCTarget : Target
        {
            ...
            protected override void OnTarget( Mobile from, object targeted )
            {
                ...
                var xa = XmlAttach.FindAttachment( targeted, typeof( XmlDialog ) ) as XmlDialog;
