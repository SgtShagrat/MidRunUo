...
using Config = Server.Engines.XmlSpawner2.Config;
...
namespace Server.Mobiles
{
    public class XmlSpawner : Item, ISpawner
    {
        ...
        private static int DefaultTriggerSound = 0x1F4;
        // click and sparkle sound by default  (0x1F4) , click sound (0x3A4)

        public static string XmlSpawnDir = "Spawns";
        // default directory for saving/loading .xml files with [xmlload [xmlsave
        ...
        public static AccessLevel DiskAccessLevel = AccessLevel.Seer;
        ...
        public static AccessLevel ConstructableAccessLevel = AccessLevel.GameMaster;
        // only allow spawning of objects that have Constructable access restrictions at this level or lower. Must define RESTRICTCONSTRUCTABLE to enable this.
#endif
        ...
        public static AccessLevel SmartSpawnAccessLevel = AccessLevel.Player; // mod by Dies Irae
        ...
        private Mobile m_MobWhoTriggered;
        ...
        private SkillName m_SkillThatTriggered;
        ...
        public List<BaseXmlSpawner.TypeInfo> PropertyInfoList = null;
        // used to optimize property info lookup used by set and get property methods.

        private Hashtable m_SpawnPositionWayTable = null; // used to optimize #waypoint lookup

        private bool m_Inrespawn = false;

        private List<Sector> sectorList = null;

        private bool m_Debug;

        private Point3D m_MostRecentSpawnPosition = Point3D.Zero;
        ...
        {
            get { return m_MostRecentSpawnPosition; }
            set { m_MostRecentSpawnPosition = value; }
        }
        ...
        public TimeSpan GameTOD
        {
            get
            {
                ...
                return
                    ( new DateTime( DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, hours, minutes, 0 ).TimeOfDay );
            }
        }
        ...
        {
            get { return Clock.GetMoonPhase( Map, Location.X, Location.Y ); }
        }
        ...
        */
        ...
        private static int m_TotalSectorsMonitored = 0;
                    ...
                    for( int x = x1; x <= x1 + width; x += Map.SectorSize )
                    {
                        for( int y = y1; y <= y1 + height; y += Map.SectorSize )
                        {
                            ...
                            if( !duplicate )
                            {
                                ...
                                {
                                    var spawnerlist = (ArrayList)GlobalSectorTable[ Map.MapID ][ s ];
                                    if( spawnerlist == null )
                                    {
                                ...
                                {
                                    var spawnerlist = new ArrayList();
                                    spawnerlist.Add( this );
                                    // add a new entry to the table
                                    GlobalSectorTable[ Map.MapID ].Add( s, spawnerlist );
                                }

                                m_TotalSectorsMonitored++;
                                ...
                                if( sectorList.Count > MaxSmartSectorListSize )
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        using( var op = new StreamWriter( "Logs/badspawn.log", true ) )
                                        {
        ...
        public Skill TriggerSkill
        {
            get
            {
                if( TriggerMob != null && (int)m_SkillThatTriggered >= 0 )
                {
                    return TriggerMob.Skills[ m_SkillThatTriggered ];
                }
                else
                {
                    return null;
            ...
            set
            {
                m_SkillThatTriggered = value != null ? value.SkillName : XmlSpawnerSkillCheck.RegisteredSkill.Invalid;
            }
        }
                    ...
                    foreach( SpawnObject so in value )
                    {
                        ...
                        if( alreadyInList == false )
                        {
                            ...
                            m_SpawnObjects.Add( new SpawnObject( so.TypeName, so.ActualMaxCount, so.SubGroup,
                                                               so.SequentialResetTime, so.SequentialResetTo,
                                                               so.KillsNeeded,
                                                               so.RestrictKillsToSubgroup, so.ClearOnAdvance,
                                                               so.MinDelay, so.MaxDelay, so.SpawnsPerTick, so.PackRange ) );
                        }
                    }
        ...
        */
        ...
        public TimeSpan TOD
        {
            get
            {
                if( m_TODMode == TODModeType.Gametime )
                {
                    ...
                    Clock.GetTime( Map, Location.X, Location.Y, out hours, out minutes );
                    return
                        ( new DateTime( DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, hours, minutes, 0 ).
                            TimeOfDay );
                }
                else
        ...
        {
            get { return m_MobWhoTriggered; }
            set { m_MobWhoTriggered = value; }
        }
            ...
            foreach( SpawnObject so in m_SpawnObjects )
            {
                for( int i = 0; i < so.SpawnedObjects.Count; ++i )
                {
                    if( so.SpawnedObjects[ i ] == spawn )
                    {
                        ...
                        // Mod by -=Derrick=- This was not previously checking RK
        ...
        public override void OnMapChange()
        {
            ...
            XmlSpawnerSkillCheck.UnRegisterSkillTrigger( this, m_SkillTriggerName, m_Currentmap, false );
        ...
        public static void ExecuteAction( object attachedto, Mobile trigmob, string action )
        {
            ...
            if( string.IsNullOrEmpty( action ) || attachedto == null || map == null )
                return;
        ...
        private static void RemoveFromSectorTable( Sector s, XmlSpawner spawner )
        {
            ...
            {
                var spawnerlist = (ArrayList)GlobalSectorTable[ s.Owner.MapID ][ s ];
        ...
        public void ReportStatus()
        {
            ...
            for( int i = 0; i < XmlSpawnerSkillCheck.RegisteredSkill.MaxSkills + 1; i++ )
            {
                if( XmlSpawnerSkillCheck.RegisteredSkill.TriggerList( (SkillName)i, Map.Ilshenar ).Count > 0 )
                    Console.WriteLine( "\t{0} : {1}", (SkillName)i,
                                      XmlSpawnerSkillCheck.RegisteredSkill.TriggerList( (SkillName)i, Map.Ilshenar ).
                                          Count );
        ...
        private string asetname9 = _traceName[ 9 ] = "Defrag";
        ...
        public static void _TraceEnd( int index )
        {
            if( index < MaxTraces )
            {
                _traceTotal[ index ] = _traceTotal[ index ].Add( DateTime.Now - _traceStart[ index ] );
        ...
        public void CheckTriggers( Mobile m, Skill s, bool hasproximity )
        {
            ...
            if( AllowTriggering && !m_ProximityActivated )
            {
                ...
                if( Utility.RandomDouble() < m_TriggerProbability )
                {
                    ...
                    if( !string.IsNullOrEmpty( m_ProximityTriggerMessage ) && m != null &&
                        !m.Deleted )
                    ...
                    if( s != null )
                    {
                        m_SkillThatTriggered = s.SkillName;
                    }
                    else
                    {
                        m_SkillThatTriggered = XmlSpawnerSkillCheck.RegisteredSkill.Invalid;
                    }
                }
                else
                {
        ...
        {
            get { return ( m_Running && !string.IsNullOrEmpty( m_SkillTrigger ) ); }
        }
        ...
        {
            get { return ( m_Running && !string.IsNullOrEmpty( m_SpeechTrigger ) ); }
        }
                            ...
                            for( int i = 0; i < keywordlist.Length; i++ )
                            {
                                ...
                                if( namelist != null && namelist.Length > 1 )
                                {
                                    ...
                                    if( namelist.Length > 2 )
                                    {
                                        ...
                                        {
                                            access =
                                                (AccessLevel)Enum.Parse( typeof( AccessLevel ), namelist[ 2 ].Trim(), true );
                                            validaccess = true;
                                        }
                                        catch
                                        {
                                            Console.WriteLine( "{0}: invalid accesslevel {1} for {2}", argname,
                                                              namelist[ 2 ], newname );
                                        }
                                    }
        ...
        public static void LoadSettings( AssignSettingsHandler settingshandler, string section )
        {
            ...
            Config.Pkg.LogInfo( "Loading {0} configuration...", section );
                    ...
                    if( split.Length >= 2 )
                    {
                        ...
                        try
                        {
                            if( settingshandler( argname, value ) )
                                ...
                                Config.Pkg.LogError( "'{0}' setting is invalid in section [{1}]", argname, currentsection );
                        }
                        catch( Exception e )
                        {
                            Config.Pkg.LogError( "Config error '{0}'='{1}'", argname, value );
                            Config.Pkg.LogError( "Error: {0}", e.Message );
                        }
                    }
                }
                if( nsettings > 0 )
                {
                    Config.Pkg.LogInfoLine( "done! {0} settings processed", nsettings );
                }
            }
        }
        ...
        public static void InitSystem()
        {
            ...
            // standard commands
            CommandSystem.Register( "XmlSpawnerShowAll", AccessLevel.Developer,
                                   new CommandEventHandler( ShowSpawnPoints_OnCommand ) );
            CommandSystem.Register( "XmlSpawnerHideAll", AccessLevel.Developer,
            ...
            CommandSystem.Register( "XmlSpawnerWipeAll", AccessLevel.Developer,
            ...
            CommandSystem.Register( "XmlSpawnerRespawn", AccessLevel.GameMaster,
                                   new CommandEventHandler( Respawn_OnCommand ) );
            CommandSystem.Register( "XmlSpawnerRespawnAll", AccessLevel.Developer,
            ...
            CommandSystem.Register( "XmlDefaults", AccessLevel.Developer, new CommandEventHandler( XmlDefaults_OnCommand ) );
            ...
            CommandSystem.Register( "OptimalSmartSpawning", AccessLevel.Developer,
                                   new CommandEventHandler( OptimalSmartSpawning_OnCommand ) );
            CommandSystem.Register( "SmartStat", AccessLevel.Developer, new CommandEventHandler( SmartStat_OnCommand ) );
            ...
            CommandSystem.Register( "XmlResetTrace", AccessLevel.Developer,
        ...
        public static void ShowSpawnPoints_OnCommand( CommandEventArgs e )
        {
            ...
            foreach( Item i in ToShow )
            {
                ...
                {
                    var root_item = xml_item.RootParent as Container;
        ...
        public static void SmartStat_OnCommand( CommandEventArgs e )
        {
            ...
            e.Mobile.SendMessage(
                ...
                percent, Version, m_TotalSectorsMonitored, SmartSpawnAccessLevel );
        }
                                               ...
                                               out int processedmaps, out int processedspawners )
        {
            ...
            if( from != null )
                from.SendMessage( string.Format( "UnLoading {0} objects{1} from file {2}.",
                                               "XmlSpawner",
                                               ( ( !string.IsNullOrEmpty( SpawnerPrefix ) )
            ...
            if( ds.Tables != null && ds.Tables.Count > 0 )
            {
                ...
                if( ds.Tables[ SpawnTablePointName ] != null && ds.Tables[ SpawnTablePointName ].Rows.Count > 0 )
                {
                    foreach( DataRow dr in ds.Tables[ SpawnTablePointName ].Rows )
                    {
                        ...
                        if( string.IsNullOrEmpty( SpawnerPrefix ) ||
        ...
        public static void XmlImportMap( string filename, Mobile from, out int processedmaps, out int processedspawners )
        {
            ...
            int total_processed_maps = 0;
            int total_processed_spawners = 0;
            if( string.IsNullOrEmpty( filename ) || from == null || from.Deleted )
                return;
            // Check if the file exists
            if( File.Exists( filename ) )
            {
                        ...
                        while( ( line = sr.ReadLine() ) != null )
                        {
                            ...
                            if( newformat )
                            {
                                ParseNewMapFormat( from, filename, line, args, linenumber, ref spawnercount,
                                                  ref badspawnercount, ref overridemap, ref overridemintime,
                                                  ref overridemaxtime );
                            }
                            else
                            {
        ...
        private static void ParseNewMapFormat( Mobile from, string filename, string line, string[] args, int linenumber,
                                              ref int spawnercount, ref int badspawnercount, ref int overridemap,
                                              ref double overridemintime, ref double overridemaxtime )
        {
            ...
            else if( args.Length == 2 && ( args[ 0 ].ToLower() == "overridemaxtime" ) )
            {
                ...
                if( args.Length > 0 && ( args[ 0 ] == "*" ) )
                {
                    ...
                    for( int k = 0; k < 6; k++ )
                    {
                        ...
                        {
                            if( string.IsNullOrEmpty( typenames[ k ][ i ] ) )
                                continue;
                    ...
                    if( !badspawn && totalspawns > 0 )
                    {
                        ...
                        for( int k = 0; k < 6; k++ )
                        {
                            ...
                            for( int i = 0; i < typenames[ k ].Length; i++ )
                            {
                                if( string.IsNullOrEmpty( typenames[ k ][ i ] ) || count > totalspawns )
                                    continue;
                        ...
                        var spawner = new XmlSpawner( SpawnId, x, y, 0, 0, spawnername, totalmaxcount,
                                                            ...
                                                            TimeSpan.FromMinutes( maxdelay ), TimeSpan.FromMinutes( 0 ), -1,
                                                            DefaultTriggerSound, 1,
                                                            0, homerange, false, so, TimeSpan.FromMinutes( 0 ),
                                                            TimeSpan.FromMinutes( 0 ), TimeSpan.FromMinutes( 0 ),
                                                            TimeSpan.FromMinutes( 0 ), null, null, null, null, null,
                                                            null, null, null, null, 1, null, false, defTODMode,
                                                            defKillReset, false, -1, null, false, false, false, null,
                        ...
                        try
                        {
                            ...
                            ScriptCompiler.EnsureDirectory( "Logs/SpawnsImportLogs/" );
                            using( var op = new StreamWriter( Path.Combine( "Logs/SpawnsImportLogs/", string.Format( "importedMap_{0}.log", name ) ), true ) )
                            {
                                foreach( SpawnObject spawnObject in spawner.SpawnObjects )
                                {
                                    if( spawnObject != null )
                                        op.WriteLine( string.Format( "{0}/Location/{1}", spawnObject.TypeName, spawner.Location ) );
                                }
                            }
                        }
                        catch( Exception e )
                        ...
                        if( map == 0 )
                        {
                            ...
                            spawner = new XmlSpawner( SpawnId, x, y, 0, 0, spawnername, totalmaxcount,
                                                     TimeSpan.FromMinutes( mindelay ), TimeSpan.FromMinutes( maxdelay ),
                                                     TimeSpan.FromMinutes( 0 ), -1, DefaultTriggerSound, 1,
                                                     0, homerange, false, so, TimeSpan.FromMinutes( 0 ),
                                                     TimeSpan.FromMinutes( 0 ), TimeSpan.FromMinutes( 0 ),
                                                     TimeSpan.FromMinutes( 0 ), null, null, null, null, null,
                                                     null, null, null, null, 1, null, false, defTODMode, defKillReset,
                                                     false, -1, null, false, false, false, null,
                                                     TimeSpan.FromHours( 0 ), null, false, null );
        ...
        public static void XmlImportSpawners_OnCommand( CommandEventArgs e )
        {
            if( e.Arguments.Length >= 1 )
            {
                ...
                if( File.Exists( filePath ) )
                {
                    ...
                    e.Mobile.SendMessage( "{0} spawners loaded successfully from {1}, {2} failures.", successes, filePath,
                                         failures );
                }
                else
        ...
        private static void ImportSpawner( XmlNode node, Mobile from )
        {
            ...
            var spawner = new XmlSpawner( SpawnId, location.X, location.Y, spawnwidth, spawnwidth, name, count,
                                                ...
                                                team, homeRange, false, so, TimeSpan.FromMinutes( 0 ),
                                                TimeSpan.FromMinutes( 0 ), TimeSpan.FromMinutes( 0 ),
                                                TimeSpan.FromMinutes( 0 ), null, null, null, null, null,
                                                null, null, null, null, 1, null, group, defTODMode, defKillReset, false,
                                                -1, null, false, false, false, null, defDespawnTime, null, false, null );
        ...
        private static void ImportMegaSpawner( Mobile from, XmlNode node )
        {
                        ...
                        if( entrynode != null )
                        {
                            ...
                            else
                            {
                                ...
                                if( group != bool.Parse( GetText( entrynode[ "GroupSpawn" ], "False" ) ) )
                                {
                                    ...
                                    try
                                    {
                                        using( var op = new StreamWriter( "Logs/badimport.log", true ) )
                                        {
        ...
        public static void XmlLoadFromFile( string filename, string SpawnerPrefix, Mobile from, Point3D fromloc,
                                           Map frommap, bool loadrelative, int maxrange, bool loadnew,
                                           out int processedmaps, out int processedspawners )
        {
            processedmaps = 0;
            processedspawners = 0;
        ...
        public static void XmlLoadFromFile( string filename, string SpawnerPrefix, Point3D fromloc, Map frommap,
                                           bool loadrelative, int maxrange, bool loadnew, out int processedmaps,
                                           out int processedspawners )
        {
            processedmaps = 0;
            processedspawners = 0;
        ...
        public static void XmlLoadFromStream( Stream fs, string filename, string SpawnerPrefix, Mobile from,
                                             Point3D fromloc, Map frommap, bool loadrelative, int maxrange, bool loadnew,
                                             out int processedmaps, out int processedspawners )
        {
            XmlLoadFromStream( fs, filename, SpawnerPrefix, from, fromloc, frommap, loadrelative, maxrange, loadnew,
        ...
        public static void XmlLoadFromStream( Stream fs, string filename, string SpawnerPrefix, Mobile from,
                                             Point3D fromloc, Map frommap, bool loadrelative, int maxrange, bool loadnew,
                                             out int processedmaps, out int processedspawners, bool verbose )
        {
            processedmaps = 0;
            processedspawners = 0;
                    ...
                    foreach( DataRow dr in ds.Tables[ SpawnTablePointName ].Rows )
                    {
                            ...
                            SpawnName.StartsWith( SpawnerPrefix ) )
                        {
                            ...
                            if( bad_spawner )
                            {
                                ...
                                try
                                {
                                    using( var op = new StreamWriter( "Logs/badxml.log", true ) )
                                    {
                            ...
                            if( !bad_spawner )
                            {
                                ...
                                var TheSpawn = new XmlSpawner( SpawnId, SpawnX, SpawnY, SpawnWidth, SpawnHeight,
                                                                     ...
                                                                     SpawnMinDelay, SpawnMaxDelay, SpawnDuration,
                                                                     SpawnProximityRange, SpawnProximityTriggerSound,
                                                                     SpawnAmount,
                                                                     SpawnTeam, SpawnHomeRange, SpawnIsRelativeHomeRange,
                                                                     Spawns, SpawnMinRefractory, SpawnMaxRefractory,
                                                                     SpawnTODStart,
                                                                     SpawnTODEnd, SpawnObjectPropertyItem,
                                                                     SpawnObjectPropertyName, SpawnProximityMessage,
                                                                     SpawnItemTriggerName, SpawnNoItemTriggerName,
                                                                     SpawnSpeechTrigger, SpawnMobTriggerName,
                                                                     SpawnMobPropertyName, SpawnPlayerPropertyName,
                                                                     SpawnTriggerProbability,
                                                                     SpawnSetPropertyItem, SpawnIsGroup, SpawnTODMode,
                                                                     SpawnKillReset, SpawnExternalTriggering,
                                                                     SpawnSequentialSpawning,
                                                                     SpawnRegionName, SpawnAllowGhost, SpawnAllowNPC,
                                                                     SpawnSpawnOnTrigger, SpawnConfigFile,
                                                                     SpawnDespawnTime, SpawnSkillTrigger,
                        ...
                        if( found_spawner && OldSpawner != null && !OldSpawner.Deleted )
                        {
                            ...
                            if( !string.IsNullOrEmpty( setObjectName ) )
                            {
                                ...
                                if( loadnew )
                                {
                                    ...
                                    OldSpawner.m_SetPropertyItem = BaseXmlSpawner.FindItemByName( null, tmpsetObjectName,
                                                                                                 typestr );
                                }
                                    ...
                                    try
                                    {
                                        using( var op = new StreamWriter( "Logs/badxml.log", true ) )
                                        {
                                            ...
                                            op.WriteLine( "{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}",
                                                         ...
                                                         OldSpawner.Location.Z, OldSpawner.Map, OldSpawner.Name,
                                                         filename );
                                            op.WriteLine();
                                        }
                                    }
                            ...
                            if( !string.IsNullOrEmpty( triggerObjectName ) )
                            {
                                ...
                                if( loadnew )
                                {
                                    ...
                                    OldSpawner.m_ObjectPropertyItem = BaseXmlSpawner.FindItemByName( null,
                                ...
                                {
                                    OldSpawner.m_ObjectPropertyItem = BaseXmlSpawner.FindItemByName( null, namestr,
                                                                                                    typestr );
                                }
                                if( OldSpawner.m_ObjectPropertyItem == null )
                                {
        ...
        public static bool SaveSpawnList( Mobile from, List<XmlSpawner> savelist, string dirname, Stream stream, bool oldformat,
                                         bool verbose )
        {
        ...
        public static void XmlTrace_OnCommand( CommandEventArgs e )
        {
            ...
            TimeSpan runningtime = DateTime.Now - _traceStartTime;
            ...
            for( int i = 0; i < MaxTraces; i++ )
            {
                if( _traceCount[ i ] > 0 )
                {
                    ...
                    Console.WriteLine( "{0} ({4}) {1,21} / {2} calls = {3:####.####} ms/call, {5:p3}",
                                      i, _traceTotal[ i ], _traceCount[ i ],
                                      ( (double)_traceTotal[ i ].TotalMilliseconds ) / _traceCount[ i ],
                                      _traceName[ i ], load );
                }
            }
        }
            ...
            : base( BaseItemId )
        {
            ...
            InitSpawn( 0, 0, m_Width, m_Height, string.Empty, 0, defMinDelay, defMaxDelay, defDuration,
                      ...
                      defTODStart, defTODEnd, null, null, null, null, null, null, null, null, null,
                      defTriggerProbability, null, defIsGroup, defTODMode,
        ...
        public XmlSpawner( Guid uniqueId, int x, int y, int width, int height, string name, int maxCount,
                          ...
                          TimeSpan minRefractory, TimeSpan maxRefractory, TimeSpan todstart, TimeSpan todend,
                          Item objectPropertyItem, string objectPropertyName, string proximityMessage,
                          ...
                          Item setPropertyItem, bool isGroup, TODModeType todMode, int killReset,
                          bool externalTriggering, int sequentialSpawning, string regionName,
        ...
        public void InitSpawn( int x, int y, int width, int height, string name, int maxCount, TimeSpan minDelay,
                              ...
                              TimeSpan minRefractory, TimeSpan maxRefractory, TimeSpan todstart, TimeSpan todend,
                              Item objectPropertyItem, string objectPropertyName, string proximityMessage,
                              string itemTriggerName, string noitemTriggerName, string speechTrigger,
                              string mobTriggerName, string mobPropertyName, string playerPropertyName,
                              double triggerProbability,
                              Item setPropertyItem, bool isGroup, TODModeType todMode, int killReset,
                              bool externalTriggering, int sequentialSpawning, string regionName, bool allowghost,
                              bool allownpc, bool spawnontrigger,
                              string configfile, TimeSpan despawnTime, string skillTrigger, bool smartSpawning,
                              WayPoint wayPoint )
        {
            Visible = false;
            Movable = false;
            ...
            foreach( SpawnObject so in m_SpawnObjects )
            {
                for( int x = 0; x < so.SpawnedObjects.Count; x++ )
                {
                    ...
                    if( o is Item )
                    {
                        ...
                        if( DespawnTime.TotalHours > 0 && !item.Deleted && ( item.LastMoved < DateTime.Now - DespawnTime ) &&
                            ( item.Parent == Parent )
        ...
        public void OnTick()
        {
            ...
            if( World.Saving )
                return; // modifica by Dies Irae

            #region mod by Dies Irae
            if( BaseWar.DisableTimersDuringBattle && Midgard.Engines.WarSystem.Core.Instance.WarPending )
                return;
            #endregion
        ...
        public bool Spawn( int index, bool smartspawn, int packrange, Point3D packcoord, bool ignoreloopprotection )
        {
            ...
            if( TheSpawn != null )
            {
                ...
                string substitutedtypeName = BaseXmlSpawner.ApplySubstitution( this, this, m_MobWhoTriggered,
                                                                              TheSpawn.TypeName );
                            ...
                            switch( keyvalueargs[ 0 ] )
                            {
                                case "#NOTILES":
                                    spawnpositioning.Add( new SpawnPositionInfo( SpawnPositionType.NoTiles,
                                                                               m_MobWhoTriggered, keyvalueargs ) );
                                    break;
                                case "#TILES":
                                ...
                                case "#CONDITION":
                                    ...
                                    if( ckeyvalueargs.Length > 1 )
                                    {
                                        ...
                                        if(
                                            !BaseXmlSpawner.CheckPropertyString( this, this, ckeyvalueargs[ 1 ],
                                                                                m_MobWhoTriggered, out status_str ) )
                                            return false;
                                    }
                                    else
                                    {
                ...
                if( BaseXmlSpawner.IsTypeOrItemKeyword( typeName ) )
                {
                    ...
                    bool completedtypespawn = BaseXmlSpawner.SpawnTypeKeyword( this, TheSpawn, typeName,
                                                                              substitutedtypeName, requiresurface,
                                                                              spawnpositioning,
                                                                              m_MobWhoTriggered, Location, Map,
                        ...
                        !( Parent != null && ( type == typeof( Mobile ) || type.IsSubclassOf( typeof( Mobile ) ) ) ) )
                    {
                        ...
                        try
                        {
                            if( o is Mobile )
                            {
                                ...
                                */
                                ...
                                BaseXmlSpawner.ApplyObjectStringProperties( this, substitutedtypeName, m,
                                                                           m_MobWhoTriggered, this, out status_str );
                            ...
                            else if( o is Item )
                            {
                                ...
                                BaseXmlSpawner.AddSpawnItem( this, TheSpawn, item, Location, map, m_MobWhoTriggered,
                                                            requiresurface, spawnpositioning, substitutedtypeName,
        ...
        public void Reset()
        {
            ...
            m_killcount = 0;
            OnHold = false;
            m_MostRecentSpawnPosition = Point3D.Zero;
            m_SpawnPositionWayTable = null;
        ...
        */
        ...
        private static void FindTileLocations( ref List<Point3D> locations, Map map, int startx, int starty, int width,
                                              int height, List<int> includetilelist, List<int> excludetilelist,
                                              TileFlag tileflag )
        {
                ...
                for( int y = starty; y <= starty + height; y++ )
                {
                    ...
                    if( excludetilelist != null && excludetilelist.Count > 0 )
                    {
                        ...
                        excludetile = ( ( lflags & TileFlag.Impassable ) != 0 ) ||
                                      excludetilelist.Contains( ltile.ID & 0x3FFF );
                    }
                    else
                    {
                        excludetile = false;
                    ...
                    for( int i = 0; i < statictiles.Length; ++i )
                    {
                        ...
                        if( excludetilelist != null && excludetilelist.Count > 0 )
                        {
                            excludetile = ( ( sflags & TileFlag.Impassable ) != 0 ) ||
                                          excludetilelist.Contains( stile.ID & 0x3FFF );
                        }
                        else
                        {
                            excludetile = false;
        ...
        private void FindRegionTileLocations( ref List<Point3D> locations, Region r, List<int> includetilelist,
                                             List<int> excludetilelist, TileFlag tileflag )
        {
                    ...
                    switch( s.PositionType )
                    {
                        ...
                        case SpawnPositionType.Waypoint:
                            ...
                            if( prefix != null )
                            {
                                ...
                                if( m_SpawnPositionWayTable == null )
                                {
                                    m_SpawnPositionWayTable = new Hashtable();
                                }

                                // try to find the waypoint list in the local table
                                WayList = (ArrayList)m_SpawnPositionWayTable[ prefix ];
                                ...
                                if( WayList == null )
                                {
                                    ...
                                    m_SpawnPositionWayTable.Add( prefix, WayList );
                                }
                            }
                            break;
                ...
                else
                {
                    switch( positioning )
                    {
                        ...
                        case SpawnPositionType.RelXY:
                            ...
                            x = m_MostRecentSpawnPosition.X + xinc;
                            y = m_MostRecentSpawnPosition.Y + yinc;
                            defaultZ = m_MostRecentSpawnPosition.Z + zinc;
                            break;
                        case SpawnPositionType.DeltaLocation:
                        ...
                        case SpawnPositionType.RowFill:
                            ...
                            x = m_MostRecentSpawnPosition.X + fillinc;
                            y = m_MostRecentSpawnPosition.Y;
                        ...
                        case SpawnPositionType.ColFill:
                            ...
                            x = m_MostRecentSpawnPosition.X;
                            y = m_MostRecentSpawnPosition.Y + fillinc;
                        ...
                        case SpawnPositionType.Waypoint:
                            ...
                            if( WayList != null && WayList.Count > 0 )
                            {
                                ...
                                var waypoint = (Item)WayList[ index ];
                                if( waypoint != null )
                                {
                    ...
                    m_MostRecentSpawnPosition = new Point3D( x, y, defaultZ );
                }
        ...
        private static void DeleteFromList( ArrayList list )
        {
            if( list == null )
                return;
        ...
        {
            ClearSubgroup( subgroup, false );
        }

        public void ClearSubgroup( int subgroup, bool IgnoreClearOverride )
        {
            if( m_SpawnObjects == null ) return;
            ...
            {
                if( so.SubGroup != subgroup || !( IgnoreClearOverride || so.ClearOnAdvance ) )
                    continue;
                    ...
                    if( delete_this_entry )
                    {
                        ...
                        if( from != null )
                            CommandLogging.WriteLine( from,
                                                     ...
                                                     from.AccessLevel, CommandLogging.Format( from ), Serial, Name,
                                                     GetWorldLocation().X, GetWorldLocation().Y, Map, SpawnObjectName );
                    }
                }
                    ...
                    foreach( NetState state in NetState.Instances )
                    {
                        ...
                        if( m != null && ( m.AccessLevel <= SmartSpawnAccessLevel || !m.Hidden ) )
                        {
                            ...
                            if( m.Map != null && m.Map != Map.Internal )
                            {
                                ...
                                {
                                    var spawnerlist = (ArrayList)GlobalSectorTable[ m.Map.MapID ][ s ];
                                    if( spawnerlist != null )
                                    {
        ...
        public override void Serialize( GenericWriter writer )
        {
            ...
            writer.Write( (int)m_SkillThatTriggered );
            writer.Write( m_FreeRun );
            writer.Write( m_MobWhoTriggered );
            // Version 21
            ...
            for( int i = 0; i < m_KeywordTagList.Count; i++ )
            {
                ...
                if( ( ( (BaseXmlSpawner.KeywordTag)( m_KeywordTagList[ i ] ) ).Flags & BaseXmlSpawner.KeywordFlags.Serialize ) !=
                    0 )
                    tagcount++;
            }
                ...
                case 22:
                    {
                        ...
                        m_SkillThatTriggered = (SkillName)reader.ReadInt();
                        m_FreeRun = reader.ReadBool();
                        m_MobWhoTriggered = reader.ReadMobile();
                        goto case 21;
                    }
                case 21:
                    {
        ...
        internal string GetSerializedObjectList()
        {
            ...
            foreach( SpawnObject so in m_SpawnObjects )
            {
                ...
                sb.AppendFormat( "{0}={1}", so.TypeName, so.ActualMaxCount );
                // '=' separates object name from maximum amount
            }
        ...
        internal string GetSerializedObjectList2()
        {
            ...
            foreach( SpawnObject so in m_SpawnObjects )
            {
                ...
                sb.AppendFormat( "{0}:MX={1}:SB={2}:RT={3}:TO={4}:KL={5}:RK={6}:CA={7}:DN={8}:DX={9}:SP={10}:PR={11}",
                                so.TypeName, so.ActualMaxCount, so.SubGroup, so.SequentialResetTime,
                                so.SequentialResetTo, so.KillsNeeded,
