...
namespace Server.Mobiles
{
    ...
    public class BaseXmlSpawner
    {
        ...
        private enum valuemodKeyword
        {
            ...
            PLAYERSINRANGE,
            RANDNAME,
            RNDHUE // mod by Dies Irae
        }

        #region mod by Dies Irae
        public enum HueTypes
        {
            None,
            Pink,
            Blue,
        ...
        public static void InitializeHash()
        {
            ...
            AddValuemodKeyword( "RNDHUE" ); // mod by Dies Irae
                ...
                if( arglist.Length > 1 )
                {
                    ...
                    else
                    {
                        if( IsValuemodKeyword( value_keywordargs[ 0 ] ) )
                        {
                            ...
                            else if( kw == valuemodKeyword.RNDHUE )
                            {
                                // generate a random hue id and use it as the property value.  Use the format /RNDHUE,hueType/
                                if( value_keywordargs.Length > 1 )
                                {
                                    HueTypes hue = HueTypes.None;
                                    int randValue = 0;

                                    try
                                    {
                                        hue = (HueTypes)Enum.Parse( typeof( HueTypes ), value_keywordargs[ 1 ], true );
                                    }
                                    catch { status_str = "Invalid hue"; }

                                    switch( hue )
                                    {
                                        case HueTypes.Pink:
                                            randValue = Utility.RandomPinkHue();
                                            break;
                                        case HueTypes.Blue:
                                            randValue = Utility.RandomBlueHue();
                                            break;
                                        case HueTypes.Green:
                                            randValue = Utility.RandomGreenHue();
                                            break;
                                        case HueTypes.Orange:
                                            randValue = Utility.RandomOrangeHue();
                                            break;
                                        case HueTypes.Red:
                                            randValue = Utility.RandomRedHue();
                                            break;
                                        case HueTypes.Yellow:
                                            randValue = Utility.RandomYellowHue();
                                            break;
                                        case HueTypes.Nondyed:
                                            randValue = Utility.RandomNondyedHue();
                                            break;
                                        case HueTypes.Neutral:
                                            randValue = Utility.RandomNeutralHue();
                                            break;
                                        case HueTypes.Snake:
                                            randValue = Utility.RandomSnakeHue();
                                            break;
                                        case HueTypes.Bird:
                                            randValue = Utility.RandomBirdHue();
                                            break;
                                        case HueTypes.Slime:
                                            randValue = Utility.RandomSlimeHue();
                                            break;
                                        case HueTypes.Animal:
                                            randValue = Utility.RandomAnimalHue();
                                            break;
                                        case HueTypes.Metal:
                                            randValue = Utility.RandomMetalHue();
                                            break;
                                        default:
                                            break;
                                    }
                                    ...
                                    // set the property value using the random number as the value
                                    string result = SetPropertyValue( spawner, o, arglist[ 0 ], String.Format( "{0}", randValue ) );
                                ...
                                {
                                    status_str = "Invalid RNDHUE args : " + arglist[ 1 ];
                                    no_error = false;
                                }
                                if( arglist.Length < 3 )
                                    break;
                            ...
                            else if( kw == valuemodKeyword.GETONNEARBY )
                            {
                                ...
                                if( value_keywordargs.Length > 3 )
                                {
                                    ...
                                    if( no_error )
                                    {
                                        ...
                                        List<IEntity> nearbylist = GetNearbyObjects( relativeto, targetname, targettype, typestr, range, searchcontainers, null );

                                        string resultstr = null;

                                        // apply the properties from the first valid thing on the list
                                        foreach( IEntity nearbyobj in nearbylist )
                                        {
                        ...
                        else if( IsTypemodKeyword( keywordargs[ 0 ] ) )
                        {
                            ...
                            else if( kw == typemodKeyword.BCAST )
                            {
                                ...
                                #region mod by Dies Irae
                                // syntax is BCAST[,hue][,font][,radius]/message
                                int radius = -1;
                                #endregion
                                ...
                                if( keywordargs.Length > 3 )
                                {
                                    try { radius = int.Parse( keywordargs[ 3 ] ); }
                                    catch { status_str = "Invalid radius : " + arglist[ 1 ]; no_error = false; }
                                }
                                #endregion
                                ...
                                if( radius > 0 )
                                {
                                    // broadcast a message to all players in a given radius
                                    BroadcastMessageArea( spawner, AccessLevel.Player, hue, font, radius, true, arglist[ 1 ] );
                                }
                                #endregion
        ...
        private static void GetItemsIn( Item source, string targetname, Type targettype, string typestr, ref List<IEntity> nearbylist, string proptest )
        {
            string status_str;
        ...
        private static List<IEntity> GetNearbyObjects( object invoker, string targetname, Type targettype, string typestr, int range, bool searchcontainers, string proptest )
        {
            IPooledEnumerable itemlist = null;
            IPooledEnumerable mobilelist = null;
            List<IEntity> nearbylist = new List<IEntity>();
            string statusStr;
                    ...
                    foreach( Item i in itemlist )
                    {
                        ...
                        if( searchcontainers )
                        {
                            ...
                            if( !i.Deleted && CheckNameMatch( targetname, i.Name ) && ( typestr == null ||
                                ( itemtype != null && targettype != null && ( itemtype.Equals( targettype ) || itemtype.IsSubclassOf( targettype ) ) ) ) )
                            {
                                if( proptest == null || CheckPropertyString( null, i, proptest, null, out statusStr ) )
                                    nearbylist.Add( i );
                            }
            ...
            if( targettype == null || targettype == typeof( Mobile ) || targettype.IsSubclassOf( typeof( Mobile ) ) )
            {
                ...
                if( mobilelist != null )
                {
                    ...
                    foreach( Mobile m in mobilelist )
                    {
                        ...
                        {
                            if( proptest == null || CheckPropertyString( null, m, proptest, null, out statusStr ) )
                                nearbylist.Add( m );
                        }
                    }
                }
        ...
        public static bool SingleCheckForNotCarried( Mobile m, string objectivestr )
        {
            ...
            {
                if( !string.IsNullOrEmpty( objstr[ objoffset ] ) )
                {
                                        ...
                                        if( item != null )
                                        {
                                            if( equip && m != null )
                                            {
                                                if( !m.EquipItem( item ) )
                                                {
                                                    ...
                                                    CheckInstanceOwner( o, item ); // mod by Dies Irae
                                                }
                                            }
                                            else
                                            {
        ...
        #region mod by Dies Irae
        public static void CheckInstanceOwner( object o, Item item )
        {
            if( o != null && item != null && o is BaseCreature )
                item.InstanceOwner = o;
        }

        public static void BroadcastMessageArea( Item item, AccessLevel ac, int hue, int font, int range, bool ascii, string message )
        {
            if( item != null && item.Map != null )
            {
                Point3D worldLoc = item.GetWorldLocation();
                IPooledEnumerable eable = item.Map.GetClientsInRange( worldLoc, range );

                foreach( NetState state in eable )
                {
                    Mobile mob = state.Mobile;

                    if( mob != null && mob.AccessLevel >= ac && mob.CanSee( item ) )
                    {
                        if( ascii )
                            mob.Send( new AsciiMessage( Serial.MinusOne, -1, MessageType.Regular, hue, font, "System", message ) );
                        else
                            mob.SendMessage( hue, message );
                    }
                }
            }
        }
        ...
        public static void AddSpawnItem( XmlSpawner spawner, SpawnObject theSpawn, Item item, Point3D location, Map map, Mobile trigmob, bool requiresurface,
    string propertyString, out string status_str )
        ...
        public static void AddSpawnItem( XmlSpawner spawner, object invoker, SpawnObject theSpawn, Item item, Point3D location, Map map, Mobile trigmob, bool requiresurface,
            string propertyString, out string status_str )
        {
        ...
        public static void InvokeOnAfterSpawnAndModify( object o )
        {
            ...
            {
                ((BaseCreature)o).OnAfterSpawnAndModify();
                return;
            }
        ...
        public static bool SpawnTypeKeyword( object invoker, SpawnObject TheSpawn, string typeName, string substitutedtypeName, bool requiresurface,
                    ...
                    case typeKeyword.SETONNEARBY:
                        {
                            ...
                            List<IEntity> nearbylist = GetNearbyObjects( invoker, targetname, targettype, typestr, range, searchcontainers, proptest );
                    ...
                    case typeKeyword.SETONPETS:
                        {
                            ...
                            List<IEntity> nearbylist = GetNearbyObjects( triggermob, targetname, targettype, typestr, range, searchcontainers, null );
                    ...
                    case typeKeyword.SETONSPAWNENTRY:
                        {
                            ...
                            {
                                SpawnObject targetobj = targetspawner.SpawnObjects[ i ];
                    ...
                    case typeKeyword.DESPAWN:
                        {
                            ...
                            {   // Mod by =-Derrick=- The DESPAWN keyword now ignores the Clear flag on the spawn entry
                                targetspawner.ClearSubgroup( subgroup, true );
                            }
                            else
                            {
