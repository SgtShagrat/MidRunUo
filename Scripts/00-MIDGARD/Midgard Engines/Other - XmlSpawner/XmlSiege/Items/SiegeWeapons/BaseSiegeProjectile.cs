...
namespace Server.Items
{
	public class BaseSiegeProjectile : Item, ISiegeProjectile
	{
		...
		public virtual double MobDamageMultiplier { get { return 0.3; } } // default damage multiplier for creatures
		...
		public BaseSiegeProjectile() : this( 1, 0xE74 )
		{
		}

		public BaseSiegeProjectile( int amount ) : this( amount, 0xE74 )
		{
		}

		public BaseSiegeProjectile( int amount, int itemid ) : base( itemid )
		{
			Weight = 5;
			Stackable = true;
			Amount = amount;
		...
		public BaseSiegeProjectile( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnAfterDuped( Item newItem )
		{
			...
			var s = newItem as BaseSiegeProjectile;
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			LabelTo( from, 1061169, Range.ToString() ); // range ~1_val~
			LabelTo( from, 1060658, string.Format( "Speed\t{0}", FiringSpeed ) ); // ~1_val~: ~2_val~
			LabelTo( from, 1060659, string.Format( "Accuracy bonus\t{0}", AccuracyBonus ) ); // ~1_val~: ~2_val~
			LabelTo( from, 1060660, string.Format( "Area\t{0}", Area ) ); // ~1_val~: ~2_val~
			LabelTo( from, 1060661, string.Format( "Physical damage\t{0}", PhysicalDamage ) ); // ~1_val~: ~2_val~
			LabelTo( from, 1060662, string.Format( "Fire damage\t{0}", FireDamage ) ); // ~1_val~: ~2_val~
		}
		...
		public void OnHit( Mobile from, ISiegeWeapon weapon, IEntity target, Point3D targetloc )
		{
			...
			var damagelist = new List<XmlSiege>();
			...
			if( itemlist != null )
			{
				foreach( Item item in itemlist )
				{
					...
					var a = (XmlSiege)XmlAttach.FindAttachment( item, typeof( XmlSiege ) );
			...
			var scaledfiredamage = (int)( FireDamage * StructureDamageMultiplier * weapon.WeaponDamageFactor );
			var scaledphysicaldamage = (int)( PhysicalDamage * StructureDamageMultiplier * weapon.WeaponDamageFactor );

			foreach( var a in damagelist )
			{
				// apply siege damage
			...
			if( totaldamage > 0 )
			{
				var scaledmobdamage = (int)( totaldamage * MobDamageMultiplier * weapon.WeaponDamageFactor );
