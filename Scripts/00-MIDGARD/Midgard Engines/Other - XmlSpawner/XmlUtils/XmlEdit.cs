    ...
    public class XmlEditDialogGump : Gump
    {
        ...
        private class EditDialogTarget : Target
        {
            ...
            protected override void OnTarget( Mobile from, object targeted )
            {
                ...
                var xd = XmlAttach.FindAttachment( targeted, typeof( XmlDialog ) ) as XmlDialog;
            ...
            : base( X, Y )
        {
            ...
            {
                m_Name = ( (Item)m_Dialog.AttachedTo ).Name;
            }
            else if( m_Dialog.AttachedTo is Mobile )
            {
                m_Name = ( (Mobile)m_Dialog.AttachedTo ).Name;
            }
            if( m_Name == null && m_Dialog.AttachedTo != null )
            {
                m_Name = m_Dialog.AttachedTo.GetType().Name;
            }
            AddLabelCropped( x, y, w, 21, 0, m_Name );
            ...
            if( m_SearchList != null )
                if( m_Selected >= 0 && m_Selected + m_DisplayFrom >= 0 && m_Selected + m_DisplayFrom < m_SearchList.Count )
                {
                    sentry = m_SearchList[ m_Selected + m_DisplayFrom ];
                }
            ...
            if( from.AccessLevel >= XmlSpawner.DiskAccessLevel )
            {
                ...
                AddTextEntry( 300, y, 180, 19, 0, 300, m_SaveFilename );
            }
            ...
            if( m_SearchList != null )
            {
                ...
                int last = m_DisplayFrom + MaxEntries < m_SearchList.Count ? m_DisplayFrom + MaxEntries : m_SearchList.Count;
                if( last > 0 )
                    AddLabel( 595, y, 68, String.Format( "Displaying {0}-{1}", m_DisplayFrom, last - 1 ) );
            }
            ...
            AddButton( 510 + 25 * ( MaxEntries / MaxEntriesPerPage ), y, 0x15E1, 0x15E5, 201, GumpButtonType.Reply, 0 );
            // block forward
            ...
            AddTextEntry( 595, y, 60, 21, 0, 400, m_DisplayFrom.ToString() );
        ...
        private void SaveList( Mobile from, string filename )
        {
            ...
            string dirname;
            if( Directory.Exists( XmlDialog.DefsDir ) && filename != null && !filename.StartsWith( "/" ) &&
                !filename.StartsWith( "\\" ) )
            {
        ...
        {
            var g = new XmlEditDialogGump( m_From, false, m_Dialog, m_Selected,
                                                        m_DisplayFrom, m_SaveFilename, m_SelectAll, m_SelectionList, X, Y );
        ...
        public static void ProcessXmlEditBookEntry( Mobile from, object[] args, string text )
        {
            ...
            var dialog = (XmlDialog)args[ 0 ];
            var entry = (XmlDialog.SpeechEntry)args[ 1 ];
            var textid = (int)args[ 2 ];
            var selected = (int)args[ 3 ];
            var displayfrom = (int)args[ 4 ];
            var savefile = (string)args[ 5 ];
        ...
        public override void OnResponse( NetState state, RelayInfo info )
        {
            ...
            if( m_SearchList != null && m_Selected >= 0 && m_Selected + m_DisplayFrom >= 0 &&
                m_Selected + m_DisplayFrom < m_SearchList.Count )
            {
                // entry information
                XmlDialog.SpeechEntry entry = m_SearchList[ m_Selected + m_DisplayFrom ];
                ...
                case 155: // add new entry
                    {
                        if( m_SearchList != null )
                        {
                            ...
                            lastentry += 10;
                            var se = new XmlDialog.SpeechEntry();
                            se.EntryNumber = lastentry;
                            se.ID = lastentry;
                            m_SearchList.Add( se );
                            m_Selected = m_SearchList.Count - 1;
                        }
                        break;
                    }
                ...
                case 156: // Delete selected entries
                    {
                        ...
                        state.Mobile.SendGump( new XmlConfirmDeleteGump( state.Mobile, g, m_SearchList, m_SelectionList,
                                                                       m_DisplayFrom, m_SelectAll, allcount ) );
                        return;
                    }
                ...
                case 159: // save to a .npc file
                    {
                        ...
                        // try to save
                        if( m_Dialog != null )
                            m_Dialog.DoSaveNPC( state.Mobile, m_SaveFilename, true );
                ...
                case 201: // forward block
                    {
                        ...
                        if( m_SelectionList != null && !m_SelectAll )
                ...
                default:
                    {
                        ...
                        else if( info.ButtonID >= 5000 && info.ButtonID < 5100 )
                        {
                            ...
                            if( m_SearchList != null && m_Selected >= 0 && m_Selected + m_DisplayFrom >= 0 &&
                                m_Selected + m_DisplayFrom < m_SearchList.Count )
                            {
                                entry = m_SearchList[ m_Selected + m_DisplayFrom ];
                            }
                            ...
                            args[ 2 ] = textid;
                            args[ 3 ] = m_Selected;
                            args[ 4 ] = m_DisplayFrom;
                            args[ 5 ] = m_SaveFilename;

                            var book = new XmlTextEntryBook( 0, String.Empty, m_Dialog.Name, 20, true,
                                                                         new XmlTextEntryBookCallback(
                                                                             ProcessXmlEditBookEntry ), args );
        ...
        {
            private List<XmlDialog.SpeechEntry> m_SearchList;
            private bool[] m_SelectedList;
            private Mobile m_From;
            private int m_DisplayFrom;
            private bool m_SelectAll;
            private XmlEditDialogGump m_Gump;

            public XmlConfirmDeleteGump( Mobile from, XmlEditDialogGump gump, List<XmlDialog.SpeechEntry> searchlist, bool[] selectedlist,
                                        int displayfrom, bool selectall, int allcount )
                : base( 0, 0 )
            {
                ...
                {
                    for( int i = 0; i < m_SelectedList.Length; i++ )
                    {
                        if( m_SelectedList[ i ] )
                            count++;
                    }
                    ...
                    default:
                        {
                            ...
                            {
                                // accept
                                var dlist = new List<XmlDialog.SpeechEntry>();
                                for( int i = 0; i < m_SearchList.Count; i++ )
                                {
                                    int index = i - m_DisplayFrom;
                                    if( ( index >= 0 && index < m_SelectedList.Length && m_SelectedList[ index ] ) || m_SelectAll )
                                    {
                                        object o = m_SearchList[ i ];
                                        // delete the entry;
                                        dlist.Add( o as XmlDialog.SpeechEntry );
                                    }
                                }

                                foreach( XmlDialog.SpeechEntry o in dlist )
                                {
                                    m_SearchList.Remove( o );
                                }

                                // clear the selections
                                Array.Clear( m_SelectedList, 0, m_SelectedList.Length );
