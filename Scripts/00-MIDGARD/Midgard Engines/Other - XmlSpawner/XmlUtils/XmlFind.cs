    ...
    public class XmlFindGump : Gump
    {
        public class XmlFindThread
        {
            ...
            public void XmlFindThreadMain()
            {
                ...
                List<SearchEntry> results = Search( m_SearchCriteria, out statusStr );

                var gump = new XmlFindGump( m_From, m_From.Location, m_From.Map, true, true, false, m_SearchCriteria, results, -1, 0, null,
                                                   m_Commandstring, false, false, false, false, false, false, 0, 0 );

                // display the updated gump synched with the main server thread
                Timer.DelayCall( TimeSpan.Zero, new TimerStateCallback( GumpDisplayCallback ), new object[] { m_From, gump, statusStr } );
            }
            ...
            public void GumpDisplayCallback( object state )
            {
                ...
                if( from != null && !from.Deleted )
                {
                    ...
                    if( statusStr != null )
                    {
                        from.SendMessage( 33, "XmlFind: {0}", statusStr );
                    }
                }
            }
        }
        ...
        public class SearchCriteria
        {
            ...
            public SearchCriteria( bool dotype, bool doname, bool dorange, bool doregion, bool doentry, bool doentrytype, bool docondition, bool dofel,
                                  bool dotram, bool domal, bool doilsh, bool dotok, bool doint, bool donull, bool doerr, bool doage, bool dowithattach,
                                  bool doattach, bool dohidevalid, bool agedirection, double age, int range, string region, string condition,
                                  string type, string attachtype, string name, string entry )
            {
                Dosearchtype = dotype;
                Dosearchname = doname;
        ...
        private bool m_Sortrange;
        private bool m_Sortname;
        private bool m_Sortmap;
        private bool m_Sortselect;
        private Mobile m_From;
        private Point3D m_StartingLoc;
        ...
        private static void IgnoreManagedInternal( object i, ref List<IEntity> ignoreList )
        {
        ...
        private static bool TestValidInternal( object o )
        {
            if( o is Mobile )
            {
                ...
                if( m.Map != Map.Internal || m.Account != null || ( m is IMount && ( (IMount)m ).Rider != null ) ||
                    ( m is BaseCreature && ( (BaseCreature)m ).IsStabled ) || ( m is BaseCreature && ( (BaseCreature)m ).IsHitched ) ||
                    ( m is BaseCreature && ( (BaseCreature)m ).IsShrinked ) )
                    return true;
            }
            else if( o is Item )
            {
                ...
                if( i is DisguisePersistance || i is Ethics.EthicsPersistance || i is Factions.FactionPersistance || i is Midgard.Engines.HardLabour.HardLabourPersistance
                    || i is Midgard.Misc.Midgard2Persistance || i is Midgard.Engines.MurderInfo.MurderInfoPersistance || i is Midgard.Engines.MidgardTownSystem.TownSystemPersistance
                    || i is Misc.TreasuresOfTokunoPersistance || i is Midgard.Engines.WorldForging.WorldForgingPersistance )
                    return true;
            }
        ...
        public static List<SearchEntry> Search( SearchCriteria criteria, out string statusStr )
        {
            statusStr = null;
            var newarray = new List<SearchEntry>();
            var ignoreList = new List<IEntity>();

            if( criteria == null )
            {
                statusStr = "Empty search criteria";
                return newarray;
            }
            ...
            if( criteria.Dosearchtype && criteria.Searchtype != null )
            {
                ...
                if( targetType == null )
                {
                    statusStr = "Invalid type: " + criteria.Searchtype;
                    return newarray;
                }
            }
            ...
            if( criteria.Dosearchattach && !string.IsNullOrEmpty( criteria.Searchattachtype ) )
            {
                ...
                {
                    statusStr = "Invalid type: " + criteria.Searchattachtype;
                    return newarray;
                }
            }
            ...
            if( !criteria.Dosearchattach )
            {
                ...
                lock( itemvalues.SyncRoot )
                {
                    try
                    {
                        itemarray = new List<Item>( World.Items.Values );
                    }
                    catch( SystemException e )
                    {
                        statusStr = "Unable to search World.Items: " + e.Message;
                    }
                }
                ...
                if( itemarray != null )
                {
                    foreach( Item i in itemarray )
                    {
                        ...
                        // check for name
                        if( criteria.Dosearchname && ( i.Name != null ) && ( criteria.Searchname != null ) && ( i.Name.IndexOf( criteria.Searchname, 0, StringComparison.OrdinalIgnoreCase ) >= 0 ) )
                        {
                            hasname = true;
                        }
                        ...
                        if( criteria.Dosearchcondition && ( criteria.Searchcondition != null ) )
                        {
                            ...
                            hascondition = BaseXmlSpawner.CheckPropertyString( null, i, criteria.Searchcondition, null, out statusStr );
                        }
                        ...
                        if( criteria.Dosearchspawnentry )
                        {
                            ...
                            else
                            {
                                ...
                                if( i is XmlSpawner )
                                {
                                    ...
                                    foreach( SpawnObject so in ( (XmlSpawner)i ).m_SpawnObjects )
                                    {
                                        if( criteria.Dosearchspawntype )
                                        {
                                        ...
                                        else
                                        {
                                            ...
                                            if( so.TypeName != null && so.TypeName.IndexOf( criteria.Searchspawnentry, 0, StringComparison.OrdinalIgnoreCase ) >= 0 )
                                            {
                                                hasentry = true;
                                                break;
                                            }
                                ...
                                else if( i is Spawner )
                                {
                                    ...
                                    foreach( string so in ( (Spawner)i ).CreaturesName )
                                    {
                                        ...
                                        {
                                            if( so != null && so.IndexOf( criteria.Searchspawnentry, 0, StringComparison.OrdinalIgnoreCase ) >= 0 )
                                            {
                                                hasentry = true;
                                                break;
                                            }
            ...
            if( !criteria.Dosearcherr && !criteria.Dosearchattach )
            {
                ...
                lock( mobilevalues.SyncRoot )
                {
                    ...
                    {
                        statusStr = "Unable to search World.Mobiles: " + e.Message;
                    }
                }
                ...
                if( mobilearray != null )
                {
                    foreach( Mobile i in mobilearray )
                    {
                        ...
                        if( i.AccessLevel > AccessLevel.Seer )
                            continue;
            ...
            var removelist = new List<SearchEntry>();
        ...
        public XmlFindGump( Mobile from, Point3D startloc, Map startmap, string type, int range, bool dorange, int x, int y )
            : this( from, startloc, startmap, true, false, false, new SearchCriteria( true, // dotype
                                                                                    false, // doname
                                                                                    dorange, // dorange
                                                                                    false, // doregion
                                                                                    false, // doentry
                                                                                    ...
                                                                                    null, // name
                                                                                    null // entry 
                                                                     ), null, -1, 0, null, null, false, false, false, false, false, false, x, y )
        {
        }


        public XmlFindGump( Mobile from, Point3D startloc, Map startmap, bool firststart, bool extension, bool descend, SearchCriteria criteria,
                           List<SearchEntry> searchlist, int selected, int displayfrom, string savefilename, string commandstring, bool sorttype,
                           bool sortname, bool sortrange, bool sortmap, bool sortselect, bool selectall, int startX, int startY )
            : base( startX, startY )
        {
            m_StartingMap = startmap;
            m_StartingLoc = startloc;
            if( from != null && !from.Deleted )
            {
                m_From = from;
                if( firststart )
                {
                    m_StartingMap = from.Map;
                    m_StartingLoc = from.Location;
                }
            }
            ...
            if( m_ShowExtension )
            {
                ...
                AddTextEntry( 270, y - 25, 180, 19, 0, 300, m_SaveFilename );
                ...
                AddTextEntry( 560, y - 25, 180, 19, 0, 301, m_CommandString );
                ...
                AddTextEntry( 501, y, 60, 21, 0, 400, m_DisplayFrom.ToString() );
                ...
                if( m_SearchList != null )
                {
                    ...
                    AddLabel( 400, y - 50, 68,
                             String.Format( "Displaying {0}-{1}", m_DisplayFrom,
                                           ( m_DisplayFrom + MaxEntries < m_SearchList.Count ? m_DisplayFrom + MaxEntries : m_SearchList.Count ) ) );
                ...
                AddButton( 670, y, ( m_SelectAll ? 0xD3 : 0xD2 ), ( m_SelectAll ? 0xD2 : 0xD3 ), 3998, GumpButtonType.Reply, 0 );

                for( int i = 0; i < MaxEntries; i++ )
                {
                    int index = i + m_DisplayFrom;
        ...
        private void SortFindList()
        {
            if( m_SearchList != null && m_SearchList.Count > 0 )
            {
                ...
                {
                    m_SearchList.Sort( new ListMapSorter( m_Descendingsort ) );
                }
                else if( m_Sortrange )
                {
                    m_SearchList.Sort( new ListRangeSorter( m_From, m_Descendingsort ) );
                }
                else if( m_Sortselect )
                {
                    m_SearchList.Sort( new ListSelectSorter( m_Descendingsort ) );
                }
            }
        }
        ...
        /*
                private class ListEntrySorter : IComparer<SearchEntry>
                {
                    private bool Dsort;
        ...
        */
        ...
        private class ListRangeSorter : IComparer<SearchEntry>
        {
            ...
            public int Compare( SearchEntry e1, SearchEntry e2 )
            {
                ...
                if( xmap != m_From.Map && ymap != m_From.Map )
                    return 0;
                ...
                if( m_Dsort )
                {
                    ...
                    if( xmap != m_From.Map && ymap == m_From.Map )
                        return -1;
                    return m_From.GetDistanceToSqrt( yloc ).CompareTo( m_From.GetDistanceToSqrt( xloc ) );
                }
                else
                {
                    if( xmap == m_From.Map && ymap != m_From.Map )
                    ...
                    if( xmap != m_From.Map && ymap == m_From.Map )
                        return 1;
                    return m_From.GetDistanceToSqrt( xloc ).CompareTo( m_From.GetDistanceToSqrt( yloc ) );
                }
            }
        }
        ...
        {
            state.Mobile.SendGump( new XmlFindGump( m_From, m_StartingLoc, m_StartingMap, false, m_ShowExtension, m_Descendingsort, m_SearchCriteria,
                                                  m_SearchList, m_Selected, m_DisplayFrom, m_SaveFilename, m_CommandString, m_Sorttype, m_Sortname, m_Sortrange,
                                                  m_Sortmap, m_Sortselect, m_SelectAll, X, Y ) );
        }
            ...
            if( !string.IsNullOrEmpty( command ) )
            {
                ...
                if( args != null && args.Length > 1 )
                {
                    ...
                    foreach( BaseCommand c in TargetCommands.AllCommands )
                    {
                        ...
                        if( string.Equals( c.Commands[ 0 ], args[ 0 ], StringComparison.OrdinalIgnoreCase ) )
                        {
                            ...
                            c.ExecuteList( e, new ArrayList( executelist ) );
        ...
        public override void OnResponse( NetState state, RelayInfo info )
        {
            ...
            if( tr != null )
                m_SaveFilename = tr.Text;

            tr = info.GetTextEntry( 301 ); // commandstring info
            if( tr != null )
                m_CommandString = tr.Text;
            ...
            switch( info.ButtonID )
            {
                ...
                case 3: // Search
                    {
                        ...
                        var tobj = new XmlFindThread( state.Mobile, m_SearchCriteria, m_CommandString );
                        var find = new Thread( new ThreadStart( tobj.XmlFindThreadMain ) );
                        find.Name = "XmlFind Thread";
                        find.Priority = ThreadPriority.BelowNormal;
                        Trace.WriteLine( "Starting XMLFind Thread." );
                        find.Start();
                ...
                case 4: // SubSearch
                    {
                        // do the search
                        string statusStr;
                        m_SearchList = Search( m_SearchCriteria, out statusStr );
                        break;
                    }
                ...
                case 155: // Return the player to the starting loc
                    {
                        m_From.Location = m_StartingLoc;
                        m_From.Map = m_StartingMap;
                        break;
                    }
                ...
                case 159: // xmlsave selected spawners
                    {
                        SaveList( state.Mobile, m_SaveFilename );
                        break;
                    }
                case 160: // execute the command on the selected items
                    {
                        ExecuteCommand( state.Mobile, m_CommandString );
                        break;
                    }
                ...
                default:
                    {
                        if( info.ButtonID >= 1000 && info.ButtonID < 1000 + MaxEntries )
                        {
                            ...
                            m_Selected = info.ButtonID - 1000;
                            // then go to it
                            DoGoTo( info.ButtonID - 1000 + m_DisplayFrom );
                        }
                        if( info.ButtonID >= 2000 && info.ButtonID < 2000 + MaxEntries )
                        {
                            // flag the entry selected
                            m_Selected = info.ButtonID - 2000;
                            // then open the gump
                            Refresh( state );
                            DoShowGump( info.ButtonID - 2000 + m_DisplayFrom );
                            return;
                        }
                        ...
                        if( info.ButtonID == 3999 )
                        {
                            ...
                            if( m_SearchList != null )
                            {
                                for( int i = 0; i < MaxEntries; i++ )
                                {
                                    if( i + m_DisplayFrom < m_SearchList.Count )
                                    {
                                        SearchEntry e = m_SearchList[ i + m_DisplayFrom ];
                        ...
                        if( info.ButtonID >= 4000 && info.ButtonID < 4000 + MaxEntries )
                        {
                            ...
                            if( m_SearchList != null && i >= 0 && m_SearchList.Count > i + m_DisplayFrom )
                            {
                                SearchEntry e = m_SearchList[ i + m_DisplayFrom ];
        ...
        public class XmlConfirmBringGump : Gump
        {
            ...
            public XmlConfirmBringGump( List<SearchEntry> searchlist )
                : base( 0, 0 )
            {
                m_SearchList = searchlist;
            ...
            public override void OnResponse( NetState state, RelayInfo info )
            {
                ...
                switch( info.ButtonID )
                {
                    default:
                        {
                            if( radiostate == 1 && m_SearchList != null )
                            {
                                // accept
                                for( int i = 0; i < m_SearchList.Count; i++ )
                                {
        ...
        public class XmlConfirmDeleteGump : Gump
        {
            ...
            public XmlConfirmDeleteGump( List<SearchEntry> searchlist )
                : base( 0, 0 )
            {
                m_SearchList = searchlist;
