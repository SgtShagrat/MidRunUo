...
namespace Server.Engines.XmlSpawner2
{
    public class XmlSpawnerAddon : BaseAddon
    {
        ...
        public static XmlSpawnerAddon ReadMultiFile( string filename, out string statusStr )
        {
            statusStr = null;
            ...
            if( File.Exists( dirname ) )
            {
                ...
                try
                {
                    ncomponents = LoadAddonFromMulti( newaddon, dirname, out statusStr );
                }
                catch
                {
                    newaddon.Delete();
                    statusStr = "Bad Multi file : " + filename;
                    return null;
                }
            ...
            {
                statusStr = "No such file : " + filename;
            }
        ...
        public static int LoadAddonFromMulti( XmlSpawnerAddon newaddon, string filename, out string statusStr )
        {
            statusStr = null;

            if( filename == null )
            {
                statusStr = "Invalid filename";
                return 0;
            }

            if( newaddon == null )
            {
                statusStr = "Invalid addon";
                return 0;
            }
                ...
                using( var sr = new StreamReader( filename ) )
                {
                    ...
                    while( ( line = sr.ReadLine() ) != null )
                    {
                        ...
                        if( args != null && args.Length >= 5 )
                        {
                            ...
                            {
                                statusStr = String.Format( "Error line {0}", linenumber );
                                break;
                            }
