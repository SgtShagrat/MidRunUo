...
namespace Server.Items
{
    ...
    public class TimedLever : XmlLatch, ILinkable
    {
        ...
        public void Activate( Mobile from, int state, List<ILinkable> links )
        {
            ...
            if( state < 0 )
                state = 0;
            if( state > 1 && m_LeverType == leverType.TwoState )
                state = 1;
            if( state > 2 )
                state = 2;
            ...
            if( state == 0 && m_TargetItem0 != null && !m_TargetItem0.Deleted && !string.IsNullOrEmpty( m_TargetProperty0 ) )
            {
                BaseXmlSpawner.ApplyObjectStringProperties( null, m_TargetProperty0, m_TargetItem0, null, this,
                                                           out statusStr );
            }
            if( state == 1 && m_TargetItem1 != null && !m_TargetItem1.Deleted && !string.IsNullOrEmpty( m_TargetProperty1 ) )
            {
                BaseXmlSpawner.ApplyObjectStringProperties( null, m_TargetProperty1, m_TargetItem1, null, this,
                                                           out statusStr );
            }
            if( state == 2 && m_TargetItem2 != null && !m_TargetItem2.Deleted && !string.IsNullOrEmpty( m_TargetProperty2 ) )
            {
        ...
        public override void OnDoubleClick( Mobile from )
        {
            ...
            if( newstate > 1 && m_LeverType == leverType.TwoState )
                newstate = 0;
            if( newstate > 2 )
                newstate = 0;
