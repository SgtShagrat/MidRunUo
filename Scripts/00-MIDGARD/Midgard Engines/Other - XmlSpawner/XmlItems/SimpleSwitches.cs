...
namespace Server.Items
{
    ...
    public class SimpleLever : Item, ILinkable
    {
        ...
        private leverType m_LeverType = leverType.TwoState;
        ...
        private bool m_AlreadyBeingActivated;
        ...
        public int LeverState
        {
            ...
            set
            {
                ...
                if( !m_AlreadyBeingActivated )
                {
                    m_AlreadyBeingActivated = true;
                    Activate( null, value, null );
                    m_AlreadyBeingActivated = false;
                }
                ...
                case 0:
                    {
                        ...
                        switch( ltype )
                        {
                            case (int)leverType.TwoState:
                                m_LeverType = leverType.TwoState;
                                break;
                            case (int)leverType.ThreeState:
                                m_LeverType = leverType.ThreeState;
                                break;
                        }
        ...
        public void SetLeverStatic()
        {
            switch( Direction )
            {
                ...
                case Direction.Up:
                    if( m_LeverType == leverType.TwoState )
        ...
        public void Activate( Mobile from, int state, List<ILinkable> links )
        {
            ...
            if( m_LeverState < 0 )
                m_LeverState = 0;
            if( m_LeverState > 1 && m_LeverType == leverType.TwoState )
                m_LeverState = 1;
            if( m_LeverState > 2 )
                m_LeverState = 2;
            ...
            if( m_LeverState == 0 && m_TargetItem0 != null && !m_TargetItem0.Deleted && !string.IsNullOrEmpty( m_TargetProperty0 ) )
            {
                BaseXmlSpawner.ApplyObjectStringProperties( null, m_TargetProperty0, m_TargetItem0, from, this,
                                                           out statusStr );
            }
            if( m_LeverState == 1 && m_TargetItem1 != null && !m_TargetItem1.Deleted && !string.IsNullOrEmpty( m_TargetProperty1 ) )
            {
                BaseXmlSpawner.ApplyObjectStringProperties( null, m_TargetProperty1, m_TargetItem1, from, this,
                                                           out statusStr );
            }
            if( m_LeverState == 2 && m_TargetItem2 != null && !m_TargetItem2.Deleted && !string.IsNullOrEmpty( m_TargetProperty2 ) )
            {
            ...
            if( statusStr != null && from != null && from.AccessLevel > AccessLevel.Player )
            {
                from.SendMessage( "{0}", statusStr );
            }
        }
    ...
    public class SimpleSwitch : Item, ILinkable
    {
        ...
        public void Activate( Mobile from, int state, List<ILinkable> links )
        {
            ...
            // if a target object has been specified then apply the property modification
            if( m_SwitchState == 0 && m_TargetItem0 != null && !m_TargetItem0.Deleted && !string.IsNullOrEmpty( m_TargetProperty0 ) )
            {
                BaseXmlSpawner.ApplyObjectStringProperties( null, m_TargetProperty0, m_TargetItem0, from, this,
                                                           out statusStr );
            }

            if( m_SwitchState == 1 && m_TargetItem1 != null && !m_TargetItem1.Deleted && !string.IsNullOrEmpty( m_TargetProperty1 ) )
            {
                BaseXmlSpawner.ApplyObjectStringProperties( null, m_TargetProperty1, m_TargetItem1, from, this,
                                                           out statusStr );
            }
    ...
    public class CombinationLock : Item
    {
        ...
        public int CheckDigit( object o, string property )
        {
            ...
            if( argtest.Length > 1 )
            {
                ...
                string statusStr;
                if( BaseXmlSpawner.CheckPropertyString( null, o, property, null, out statusStr ) )
                {
                    return 1; // true
                }
                else
