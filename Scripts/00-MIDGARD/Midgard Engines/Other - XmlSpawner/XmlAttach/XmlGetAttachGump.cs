...
namespace Server.Mobiles
{
    public class XmlGetAttGump : Gump
    {
        ...
        private bool m_Dosearchtype;
        private bool m_Dosearchname;

        private bool m_Dosearchage;

        private bool m_Searchagedirection;
        ...
        private bool TestAge( object o )
        {
            ...
            if( o is XmlAttachment )
            {
                ...
                if( m_Searchagedirection )
                {
                    // true means allow only mobs greater than the age
                    if( ( DateTime.Now - a.CreationTime ) > TimeSpan.FromHours( m_Searchage ) )
                        return true;
                }
                else
                {
        ...
        private ArrayList Search( object target, out string statusStr )
        {
            statusStr = null;
            ...
            if( attachments != null )
                foreach( XmlAttachment i in attachments )
                {
                    ...
                    // check for type
                    if( m_Dosearchtype && ( i.GetType().IsSubclassOf( targetType ) || i.GetType().Equals( targetType ) ) )
                    {
                        hastype = true;
                    }
                    ...
                    // check for name
                    if( m_Dosearchname && ( i.Name != null ) && ( m_Searchname != null ) &&
                        ( i.Name.ToLower().IndexOf( m_Searchname.ToLower() ) >= 0 ) )
                    {
                        hasname = true;
                    }
            ...
            : base( x, y )
        {
            ...
            {
                string statusStr;
                m_SearchList = Search( m_TargetObject, out statusStr );
            }
            ...
            if( m_SearchList != null )
            {
                ...
                AddLabel( 500, 425, 68, String.Format( "Displaying {0}-{1}", m_DisplayFrom,
                                                     ( m_DisplayFrom + MaxEntries < m_SearchList.Count
            ...
            for( int i = 0; i < MaxEntries; i++ )
            {
                ...
                if( o is XmlAttachment )
                {
                    var a = (XmlAttachment)m_SearchList[ index ];
        ...
        private sealed class ListTypeSorter : IComparer
        {
        ...
        private sealed class ListNameSorter : IComparer
        {
            private bool m_Dsort;
                ...
                case 156: // Delete selected items
                    {
                        ...
                        state.Mobile.SendGump( new XmlConfirmDeleteGump( state.Mobile, m_TargetObject, m_SearchList,
                                                                       m_SelectionList, m_DisplayFrom, m_SelectAll, allcount ) );
                        return;
                    }
                ...
                default:
                    {
                        ...
                        if( info.ButtonID >= 5000 && info.ButtonID < 5000 + MaxEntries )
                        {
                            ...
                            if( m_SelectionList != null && i >= 0 && i < m_SelectionList.Length && !m_SelectAll )
                            {
                                ...
                                if( m_SearchList != null && ( m_SearchList.Count - m_DisplayFrom > i ) )
                                {
                                    var a = m_SearchList[ i + m_DisplayFrom ] as XmlAttachment;
                                    if( a != null )
                                    {
