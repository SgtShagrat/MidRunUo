...
namespace Server.Engines.XmlSpawner2
{
    ...
    public class XmlAttach
    {
        ...
        public static bool HasAttachments( object o )
        {
            ...
            {
                var alist = (ArrayList)ItemAttachments[ o ];
            ...
            {
                var alist = (ArrayList)MobileAttachments[ o ];
                // see if the attachment list is empty
                if( alist == null || alist.Count == 0 )
                    return false;
            ...
            if( writer != null && imawriter != null && fpiwriter != null )
            {
                ...
                if( AllAttachments != null )
                {
                    ...
                    for( int i = 0; i < keyarray.Length; i++ )
                    {
                        ...
                        var a = valuearray[ i ] as XmlAttachment;
                ...
                if( MobileAttachments != null )
                {
                    ...
                    for( int i = 0; i < keyarray.Length; i++ )
                    {
                        ...
                        var alist = (ArrayList)valuearray[ i ];
        ...
        private static ArrayList m_Desererror;

        private static void ReportDeserError( string typestr, string detailstr )
        {
            if( m_Desererror == null )
                m_Desererror = new ArrayList();

            m_Desererror.Add( new DeserErrorDetails( typestr, detailstr ) );
        }
        ...
        private static bool AlreadyReported( string typestr )
        {
            if( m_Desererror == null )
                return false;
            foreach( DeserErrorDetails s in m_Desererror )
            {
                if( s.Type == typestr )
                    return true;
            }
        ...
        /// <summary>
        /// Handler called one time in OnBeforeDeath of BaseCreature
        /// </summary>
        /// <param name="creature">creature to call this handler on</param>
        public static void HandleOnBeforeDeath( BaseCreature creature )
        {
            if( creature == null || creature.Deleted )
                return;

            ArrayList attachList = FindAttachments( creature );
            if( attachList != null )
            {
                foreach( XmlAttachment a in attachList )
                {
                    if( a != null && !a.Deleted && a.HandlesOnDeath )
                    {
                        a.OnBeforeDeath( creature );
                    }
                }
            }
        }

        public static void HandleOnBeforeDeath( Midgard2PlayerMobile player )
        {
            if( player == null || player.Deleted )
                return;

            ArrayList attachList = FindAttachments( player );
            if( attachList != null )
            {
                foreach( XmlAttachment a in attachList )
                {
        ...
        /// <summary>
        /// Handler called one time in OnDeath of BaseCreature
        /// </summary>
        /// <param name="creature">creature to call this handler on</param>
        /// <param name="container"></param>
        public static void HandleOnDeath( BaseCreature creature, Container container )
        {
            if( creature == null || creature.Deleted )
                return;
            ...
            if( attachList != null )
            {
                foreach( XmlAttachment a in attachList )
                {
                    if( a != null && !a.Deleted && a.HandlesOnDeath )
                    {
                        a.OnDeath( creature, container );
                    }
                }
            }
        }

        public static void HandleOnDeath( Midgard2PlayerMobile player, Container container )
        {
            if( player == null || player.Deleted )
                return;
            ...
            if( attachList != null )
            {
                foreach( XmlAttachment a in attachList )
                {
                    if( a != null && !a.Deleted && a.HandlesOnDeath )
                    {
                        a.OnDeath( player, container );
                    }
                }
            }

            List<Item> equiplist = player.Items;
            if( equiplist != null )
            {
        ...
        public static void CheckOnBeforeKill( Mobile killed, Mobile killer )
        {
            // do not register creature vs creature kills, nor any kills involving staff
            if( killer == null || killed == null || !( killer.Player || killed.Player ) ||
                ( killer.AccessLevel > AccessLevel.Player ) || ( killed.AccessLevel > AccessLevel.Player ) )
                return;

            if( killer != null )
            {
                // check the killer
                ArrayList alist = FindAttachments( killer );
                if( alist != null )
                {
                    foreach( XmlAttachment a in alist )
                    {
                        ...
                        {
                            a.OnBeforeKill( killed, killer );
                        }
                    }
                }

                // check any equipped items
                List<Item> equiplist = killer.Items;
                if( equiplist != null )
                {
                    foreach( Item i in equiplist )
                    {
            ...
            {
                // check the killed
                ArrayList alist = FindAttachments( killed );
                if( alist != null )
                {
                    foreach( XmlAttachment a in alist )
                    {
                        ...
                        {
                            a.OnBeforeKilled( killed, killer );
                        }
                    }
                }
            }
        ...
        public static void CheckOnKill( Mobile killed, Mobile killer )
        {
            // do not register creature vs creature kills, nor any kills involving staff
            //            if (killer == null || killed == null || !(killer.Player || killed.Player) /*|| (killer.AccessLevel > AccessLevel.Player) || (killed.AccessLevel > AccessLevel.Player) */)
            //				return;
        ...
        private static void Match( Type matchtype, Type[] types, ICollection<Type> results )
        {
            if( matchtype == null )
                return;
        ...
        private static bool AttachTo( object from, object o, IXmlAttachment attachment, bool first )
        {
        ...
        public static ArrayList FindAttachments( Hashtable attachments, object o, Type type, string name, bool original )
        {
            ...
            else
            {
                ...
                var list = (ArrayList)attachments[ o ];
        ...
        public static XmlAttachment FindAttachment( Hashtable attachments, object o, Type type, string name )
        {
            ...
            var list = attachments[ o ] as ArrayList;
            if( type == null && name == null )
            {
        ...
        public static XmlAttachment FindAttachmentBySerial( int serialno )
        {
            if( serialno <= 0 )
                return null;
            var a = AllAttachments[ serialno ] as XmlAttachment;
        ...
        private static void FullDefrag( IDictionary attachments )
        {
        ...
        private static void FullSerialDefrag( IDictionary attachments )
        {
            // go through the item attachments
            var keyarray = new object[ attachments.Count ];
        ...
        private static void SerialDefrag( IXmlAttachment a )
        {
            if( a != null && a.Deleted )
        ...
        private static void Defrag( IDictionary attachments, object o )
        {
            if( o == null || attachments == null )
                return;
        ...
        public static void AddAttachmentProperties( object parent, ObjectPropertyList list )
        {
            ...
            if( plist != null && plist.Count > 0 )
            {
                ...
                {
                    var a = plist[ i ] as XmlAttachment;
