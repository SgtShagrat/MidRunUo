...
using StringList = Ultima.StringList;
...
namespace Midgard.Engines.Bestiary
{
    ...
    public sealed class MobileEntry : IComparable
    {
        #region [Static]
        ...
        /// </summary>
        private static MethodHandler m_BlankHandler = delegate( MobileEntry m, int paramCount )
                                                          {
                                                              /*supress the unregistered handler message in debug mode, TODO!*/
                                                          };
        ...
        static MobileEntry()
        {
			...
			********************************************************/
            ...
            // Set up the encoder
            m_EncoderParameter.Param[ 0 ] = new EncoderParameter( Encoder.Quality, new long[] { 0x46 } );
        ...
        {
            string huesFile = Server.Core.FindDataFile( "hues.mul" );
        ...
        public string Html
        {
            get
            {
                ...
                sb.AppendFormat( m_Templates[ "tableTop" ], MasterType.Name );
                ...
                if( m_Immunity != null )
                {
                    sb.AppendFormat( m_Templates[ "immunity" ], m_Immunity );
                }
                ...
                sb.AppendFormat( m_Templates[ "barding" ], bard );
                ...
                if( PrevLink != null )
                {
                    sb.AppendFormat( "<span style=\"padding: 3px;border: 1px solid #555; background-color: #333; margin: 2px;\">{0}</span>", PrevLink );
                }

                if( NextLink != null )
                {
                    sb.AppendFormat( "&nbsp;&nbsp;&nbsp;<span style=\"padding: 3px;border: 1px solid #555; background-color: #333; margin: 2px;\">{0}</span>", NextLink );
                }
        ...
        public MobileEntry( Type type )
        {
            ...
            m_Name = Core.TypeRegistry[ MasterType ].Name;
            m_Slayer = SlayerGroup();

            // Get constructor info
            ConstructorInfo ctor = HandleSpecialType( type );
...

            {
                ...
                m_Difficulty = BaseInstrument.GetBaseDifficulty( layla );
                layla.Delete();
            }
        ...
        {
            MethodInfo method = MasterType.GetMethod( methodName );
        ...
        /// <returns></returns>
        private static ConstructorInfo HandleSpecialType( Type type )
        {
        ...
        /// <returns></returns>
        private static bool MatchMetadata( int descriptor, MetadataToken token )
        {
        ...
        private SlayerEntry SlayerGroup()
        {
            ...
            {
                if( Array.IndexOf( group.Super.Types, MasterType ) != -1 )
                {
                    return group.Super;
                }
                ...
                {
                    if( Array.IndexOf( entry.Types, MasterType ) != -1 )
                    {
                        return entry;
                    }
                }
        ...
        private void GenerateImage()
        {
            ...
            if( Core.UseFixes )
            {
                string fixPath = Path.Combine( "Bestiary/fixes", MasterType.Name + ".jpg" );
                ...
                using( Bitmap mobImage = new Bitmap( bmp ?? animFrames[ 0 ].Bitmap ) )
                {
                    using( Bitmap result = new Bitmap( m_BorderImage.Width, m_BorderImage.Height ) )
                    {
                        using( Bitmap title = ASCIIText.DrawText( 3, m_Name ?? "", 0x3B2 ) )
                        {
                            ...
                            {
                                using( Bitmap tz = ASCIIText.DrawText( 3, string.Format( " - {0} - Bestiary -", Core.ShardName ?? "" ), 1260 ) )
                                {
                                    using( SolidBrush blackBrush = new SolidBrush( Color.FromArgb( 60, 1, 1, 1 ) ) )
                                    {
                                        ...
                                        string customBg = Core.TypeRegistry[ MasterType ].Background;
                                        ...
                                        result.Save( Path.Combine( "./Bestiary/images/", MasterType.Name + ".jpg" ), m_Encoder, m_EncoderParameter );
                                    }
                                }
                            }
                        }
        ...
        public int CompareTo( object obj )
        {
            ...
            MobileEntry reg = obj as MobileEntry;
            if( reg == null )
                throw new ArgumentException( "obj is not a MobileEntry", "obj" );

            return Insensitive.Compare( MasterType.Name, reg.MasterType.Name );
        }
    } ;
}