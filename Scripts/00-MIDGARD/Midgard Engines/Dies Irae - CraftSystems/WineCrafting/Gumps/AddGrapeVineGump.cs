    ...
    public class AddGrapeVineGump : Gump
    {
        ...
        public class InternalTarget : Target
        {
            ...
            protected override void OnTarget( Mobile from, object o )
            {
                ...
                if( p != null )
                {
                    object obj = Activator.CreateInstance( m_Types[ m_IType ].m_Type, from );

                    if( obj != null && obj is BaseGrape )
                    {
                        if( m_IVariety < 0 || m_IVariety > m_Grapes.Length )
                            m_IVariety = 0;

                        BaseGrape vine = (BaseGrape)obj;
                        ...
                        if( WineSystemHelper.ValidatePlacement( loc, from, o ) )
                        {
                            if( ( from.AccessLevel == AccessLevel.Player && WineSystemHelper.PayForVine( from ) ) || from.AccessLevel > AccessLevel.Player )
                            {
            ...
            protected override void OnTarget( Mobile from, object o )
            {
                ...
                if( p != null )
                {
                    ...
                    if( m_Object is Item )
                    {
                        ...
                        if( WineSystemHelper.ValidatePlacement( loc, from, m_Object ) )
        ...
        public class VineDeleteTarget : Target
        {
            ...
            protected override void OnTarget( Mobile from, object o )
            {
                ...
                if( o is BaseGrapeVine )
                {
                    ...
                    if( vine.Placer == from )
                    {
                        if( WineSystemHelper.RefundForVine( from ) )
                            ( (Item)o ).Delete();
                    }
                    else
                    {
        ...
        public override void OnResponse( NetState sender, RelayInfo info )
        {
            ...
            double reqSkill = WineSystemHelper.GetHarvestSkill( (BrewVariety)m_VarietyIndex );
            double fromSkill = m_From.Skills[ SkillName.Lumberjacking ].Value;

            string skillMess = string.Format( "{0:F2} skill is required for this grape variety", reqSkill );
                ...
                case 1: // Add Vine
                    {
                        ...
                        {
                            if( fromSkill < reqSkill )
                                m_From.SendGump( new AddGrapeVineGump( m_From, skillMess, m_VarietyIndex, GumpPage.None ) );
                            else
        ...
        private static GrapeInfo[] m_Grapes = new GrapeInfo[]
            {
                new GrapeInfo( "Cabernet Sauvignon", BrewVariety.CabernetSauvignon, typeof( CabernetSauvignonGrapes ) ),
                new GrapeInfo( "Chardonnay", BrewVariety.Chardonnay, typeof( ChardonnayGrapes ) ),
                new GrapeInfo( "Chenin Blanc", BrewVariety.CheninBlanc, typeof( CheninBlancGrapes ) ),
                new GrapeInfo( "Merlot", BrewVariety.Merlot, typeof( MerlotGrapes ) ),
                new GrapeInfo( "Pinot Noir", BrewVariety.PinotNoir, typeof( PinotNoirGrapes ) ),
                new GrapeInfo( "Riesling", BrewVariety.Riesling, typeof( RieslingGrapes ) ),
                new GrapeInfo( "Sangiovese", BrewVariety.Sangiovese, typeof( SangioveseGrapes ) ),
                new GrapeInfo( "Sauvignon Blanc", BrewVariety.SauvignonBlanc, typeof( SauvignonBlancGrapes ) ),
                new GrapeInfo( "Shiraz", BrewVariety.Shiraz, typeof( ShirazGrapes ) ),
                new GrapeInfo( "Viognier", BrewVariety.Viognier, typeof( ViognierGrapes ) ),
                new GrapeInfo( "Zinfandel", BrewVariety.Zinfandel, typeof( ZinfandelGrapes ) )
            };
    }
}