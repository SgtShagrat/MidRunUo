...
namespace Midgard.Engines.BrewCrafing
{
    public class BreweryLabelMaker : Item
    {
        ...
        public override void OnDoubleClick( Mobile from )
        {
            ...
            {
                from.SendMessage(
                    "Click on this label tool to name your brewery or a keg/bottle you crafted to add a new label." );
        ...
        private class RenameItemTarget : Target
        {
            ...
            protected override void OnTarget( Mobile from, object target )
            {
                if( target is Mobile )
                {
                    from.SendMessage( "Invalid Target.  Only crafted kegs, bottles, or this label maker can be labeled." );
                }
                else if( target is Item )
                {
                    Item item = (Item)target;

                    if( target == m_LabelMaker || target is BaseBrewContainer || target is BaseBrewContainerKeg )
                    {
                        ...
                        {
                            if( item is BaseBrewContainer )
                            {
                                BaseBrewContainer brew = (BaseBrewContainer)item;
                                if( brew.Crafter != from )
                                {
                                    from.SendMessage(
                            ...
                            else if( item is BaseBrewContainerKeg )
                            {
                                BaseBrewContainerKeg keg = (BaseBrewContainerKeg)item;
                                if( keg.Crafter != from )
                                {
                                    from.SendMessage(
        ...
        private class LabelPrompt : Prompt
        {
            ...
            {
                m_Item.Name = text;
                from.SendMessage( "The brewery has been labeled" );
            }
        }
        ...
        private class LabelPrompt2 : Prompt
        {
            ...
            public override void OnResponse( Mobile from, string text )
            {
                ...
                from.SendMessage(
                    "As long as you keep this tool in your pack, all future products you create will be labeled with this brewery name." );
            }
        }
    }
}