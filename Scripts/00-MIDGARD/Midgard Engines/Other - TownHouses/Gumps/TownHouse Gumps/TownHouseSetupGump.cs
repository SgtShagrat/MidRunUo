...
namespace Midgard.Engines.TownHouses
{
    public class TownHouseSetupGump : GumpPlusLight
    {
        ...
        private void QuickPage( int width, ref int y )
        {
            ...
            AddHtml( 0, y += 25, width, "<CENTER>Base Floor: " + m_Sign.MinZ );
            ...
            AddHtml( 0, y += 17, width, "<CENTER>Top Floor: " + m_Sign.MaxZ );
        ...
        private void WelcomePage( int width, ref int y )
        {
            ...
            if( m_Sign != null )
            {
                ...
                {
                    helptext =
                        String.Format( "  Welcome to the TownHouse setup menu!  This menu will guide you through " +
        ...
        private void FloorsPage( int width, ref int y )
        {
            ...
            AddHtml( 0, y += 20, width, "<CENTER>Top Floor: " + m_Sign.MaxZ );
            AddButton( width / 2 - 80, y + 3, 0x2716, "Top Floor", new GumpCallback( MaxZSelect ) );
            AddButton( width / 2 + 70, y + 3, 0x2716, "Top Floor", new GumpCallback( MaxZSelect ) );
        ...
        private void SkillsPage( int width, ref int y )
        {
            ...
            AddTextField( width / 2 + 20, y, 100, 20, 0x480, 0xBBC, "Skill", m_Sign.Skill );
        ...
        private bool SkillNameExists( string text )
        {
            try
            {
                return Enum.IsDefined( typeof( SkillName ), text );
            }
            catch( Exception ex )
            {
        ...
        {
            int volume = m_Sign.CalcVolume();
            m_Sign.Secures = Math.Max( volume / 50, 1 );
            m_Sign.Locks = Math.Max( volume / 3, 1 );
        ...
        {
            m_Sign.Price = m_Sign.CalcVolume() * Config.SuggestionFactor;

            if( m_Sign.RentByTime == TimeSpan.FromDays( 1 ) )
                m_Sign.Price /= 60;
            if( m_Sign.RentByTime == TimeSpan.FromDays( 7 ) )
                m_Sign.Price = (int)( m_Sign.Price / 8.57 );
