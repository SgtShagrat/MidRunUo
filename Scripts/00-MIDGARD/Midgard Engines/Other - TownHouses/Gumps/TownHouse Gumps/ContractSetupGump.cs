...
namespace Midgard.Engines.TownHouses
{
    public class ContractSetupGump : GumpPlusLight
    {
        ...
        private void BlocksPage( int width, ref int y )
        {
            ...
            if( !Core.HasOtherContract( m_Contract.ParentHouse, m_Contract ) )
            {
        ...
        private void LocSecPage( int width, ref int y )
        {
            ...
            AddHtml( 30, y += 25, width / 2 - 20, "<DIV ALIGN=RIGHT>Secures (Max: " + ( Core.RemainingSecures( m_Contract.ParentHouse ) + m_Contract.Secures ) + ")" );
            ...
            AddHtml( 30, y += 20, width / 2 - 20, "<DIV ALIGN=RIGHT>Lockdowns (Max: " + ( Core.RemainingLocks( m_Contract.ParentHouse ) + m_Contract.Locks ) + ")" );
        ...
        private void PricePage( int width, ref int y )
        {
            ...
            helptext += String.Format( "Cost per unit: {0} gold</CENTER>", Config.SuggestionFactor );
        ...
        private void SuggestPrice()
        {
            ...
            m_Contract.Price = m_Contract.CalcVolume() * Config.SuggestionFactor;

            if( m_Contract.RentByTime == TimeSpan.FromDays( 1 ) )
                m_Contract.Price /= 60;
            if( m_Contract.RentByTime == TimeSpan.FromDays( 7 ) )
                m_Contract.Price = (int)( m_Contract.Price / 8.57 );
        ...
        {
            int price = m_Contract.CalcVolume() * Config.SuggestionFactor;
        ...
        private void EntireHouse()
        {
            ...
            if( m_Contract.EntireHouse )
            {
                ...
                bool once = false;
                foreach( Rectangle3D rect in RegionHelper.RegionArea( m_Contract.ParentHouse.Region ) )
                {
        ...
        private void FinalizeSetup()
        {
            ...
            if( m_Contract.EntireHouse )
            {
                ...
                m_Contract.Secures = Server.Core.AOS ? m_Contract.ParentHouse.GetAosMaxSecures() : m_Contract.ParentHouse.MaxSecures;
                m_Contract.Locks = Server.Core.AOS ? m_Contract.ParentHouse.GetAosMaxLockdowns() : m_Contract.ParentHouse.MaxLockDowns;
            }
