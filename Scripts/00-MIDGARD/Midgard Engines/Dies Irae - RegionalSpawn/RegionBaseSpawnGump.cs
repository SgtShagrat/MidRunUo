...
{
    public abstract class RegionBaseSpawnGump : Midgard.Gumps.MidgardComplexGump
    {
        // Background
        ...
        protected RegionBaseSpawnGump( BaseRegion region, Mobile owner )
            : this( region, owner, 50, 50 )
        {
        }

        protected RegionBaseSpawnGump( BaseRegion region, Mobile owner, int offsetX, int offsetY )
        ...
        {
            if( Owner != null  && Enum.IsDefined( typeof( AccessLevel ), flag ) )
            {
                return Owner.AccessLevel >= (AccessLevel)flag;
            }
