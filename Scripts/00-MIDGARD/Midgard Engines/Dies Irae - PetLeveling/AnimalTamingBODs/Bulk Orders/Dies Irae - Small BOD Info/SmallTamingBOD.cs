    ...
    public class SmallTamingBOD : SmallMobileBOD
    {
        ...
        public static SmallTamingBOD CreateRandomFor( Mobile m )
        {
            ...
            if( entries.Length > 0 )
            {
                ...
                try
                {
                    // prova 10 volte a trovare una entry che abbia una taming richiesta
                    // minore del valore di taming del pg che richiede il BOD.
                    BaseCreature bc;
                    for( int i = 0; i < 10; i++ )
                    {
                        bc = Activator.CreateInstance( entry.Type ) as BaseCreature;
                        if( bc != null && bc.MinTameSkill < m.Skills[ SkillName.AnimalTaming ].Base )
        ...
        public SmallTamingBOD( Serial serial )
            : base( serial )
        {
        }
