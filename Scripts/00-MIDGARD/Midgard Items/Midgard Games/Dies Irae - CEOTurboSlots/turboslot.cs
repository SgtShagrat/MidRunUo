...
//		private DateTime m_BlinkTimerEnd;
		...
		public bool Active
		{
			...
			set
			{
				if (!m_Active && value)
				{
					...
					this.PublicOverheadMessage(0, this.Hue, true, "CEOTurboSlot online!");
				}
				else if (m_Active && !value)
				{
					m_OrigHue = this.Hue;
					this.Hue = 1001;
					this.PublicOverheadMessage(0, this.Hue, true, "CEOTurboSlot offline.");
				}
				m_Active = value;
		...
		public void JackpotHit(Mobile from, int jackpot, TurboSlot slot)
		{
			...
			this.PublicOverheadMessage(0, this.Hue, true, text);
			if (m_SlotSlaves != null)
			{
				...
				foreach (TurboSlot s in m_SlotSlaves)   // Announce the jackpot, but not to every machine to avoid a messy screen
				{
					...
					if (announce)
					{
						s.PublicOverheadMessage(0, this.Hue, true, text);
				...
				if (r1index == m_Symbols[8] && m_BonusRound != BonusRoundType.None && !m_Profile)
				{
					switch (m_BonusRound)
					{
						case BonusRoundType.MinerMadness:
							{
								m_jackpotStats[5]++;
								this.PublicOverheadMessage(0, this.Hue, true, "-Bonus Round!-");
		...
		private void DoSpinWin(Mobile from, int r1index, int winnings, int mobileSound, int jackpotSound, string slotText, int JackpotEffect)
		{
			...
			if (slotText != null)
				this.PublicOverheadMessage(0, this.Hue, true, slotText);
			if (JackpotEffect != 0)
		...
		{
			this.PublicOverheadMessage(0, this.Hue, true, "Share the wealth!");
		...
		private void Profile(bool setuptheme, int count)
		{
			...
			{
				this.PublicOverheadMessage(0, this.Hue, true, "The slot machine must be in use by a GM or higher in order to profile it.");
				return;
			}
			if (setuptheme)
			{
