...
namespace Server.Items
{
    ...
    public class KenoBoard : Item
    {
        ...
        public bool Active
        {
            ...
            set
            {
                if( !m_Active && value )
                {
                    ...
                    PublicOverheadMessage( 0, ( Hue == 907 ? 0 : Hue ), true, "CEOKeno online!" );
        ...
        public void DoWinSound( Mobile from, bool female, int amount )
        {
            ...
            {
                text = String.Format( "{0} wins {1}!", from.Name, amount );
                PublicOverheadMessage( 0, ( Hue == 907 ? 0 : Hue ), true, text );
            }
            if( amount > 499999 )
            {
                for ( int i = 0; i < 5; i++ )
        ...
        public float[] GetPayTable( int method, int Cost, int totalselected )
        {
            ...
            if( totalselected == 0 )
                return paytable; // Return zeroed table when nothing is selected
        ...
        public void SecurityCamera( int chatter, string text )
        {
            ...
            if( m_SecurityCamMobile.Player )
                ...
                m_SecurityCamMobile.PublicOverheadMessage( 0, ( Hue == 907 ? 0 : Hue ), true, text );
        }

        public void ProfileTable( KenoBoard Keno, PlayerInfo player, bool played, int PayTable, float[] PayOutTable, int CPayMethod, byte QuickPickCount, bool PickButtons, byte[] Selected, byte TotalSelected, byte[] MachinePicks )
        {
#if PROFILE
            ResetTotals = true;
            if (TotalSelected == 0)
