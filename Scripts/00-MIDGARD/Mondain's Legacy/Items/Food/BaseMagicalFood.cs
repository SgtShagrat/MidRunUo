...
namespace Server.Items
{
    ...
    public abstract class BaseMagicalFood : Food
    {
        ...
        public virtual TimeSpan GetDuration( Mobile from )
        {
        ...
        public virtual object EatMessage { get { return 0; } }
        #endregion
        ...
        public override bool Eat( Mobile from )
        {
            if( !IsUnderInfluence( from, FoodID ) )
            {
                ...
                {
                    if( EatMessage is int )
                        from.SendLocalizedMessage( (int)EatMessage );
                    else
                        from.SendMessage( (string)EatMessage );
        ...
        {
            if( m_Cooldown != null && mob != null && m_Cooldown.ContainsKey( mob ) )
                m_Cooldown[ mob ] = (int)m_Cooldown[ mob ] & ~( (int)id );
        }
