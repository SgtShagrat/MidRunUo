	...
	{				
		...
		*/
		...
		public BaseQuiver( int itemID ) : base( itemID )
		{
			Weight = 2;
			Capacity = 500;
			Layer = Core.ML ? Layer.Cloak : Layer.Waist; // mod by Dies Irae
		...
		{
			if ( Core.ML && m.NetState != null && !m.NetState.SupportsExpansion( Expansion.ML ) )
			{
		...
		private enum SaveFlag
		{
			...
			UnSetHue			= 0x10000000, // modifica by Dies Irae
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			SetSaveFlag( ref flags, SaveFlag.UnSetHue,			m_UnSetHue != 0 ); // modifica by Dies Irae
			#endregion
			...
			if ( GetSaveFlag( flags, SaveFlag.UnSetHue ) )
		...
		{
		    typeof( Arrow ), typeof( Bolt ), typeof( BasePoisonAmmonition ), typeof( BaseScoutArrow ) // mod by Dies Irae
		};
		#endregion
        ...
        private int m_UnSetHue; // mod by Dies Irae
