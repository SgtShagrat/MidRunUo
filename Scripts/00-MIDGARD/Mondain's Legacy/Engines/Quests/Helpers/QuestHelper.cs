...
namespace Server.Engines.Quests
{
	...
	{					
		...
		public static bool InProgress( PlayerMobile player, IMidgardQuester quester ) // mod by Dies Irae
		{								
		...
		public static bool DeliveryArrived( PlayerMobile player, /* BaseVendor */ Mobile vendor ) // mod by Dies Irae
		{
			for ( int i = 0; i < player.Quests.Count; i ++  )
			{
				BaseQuest quest = player.Quests[ i ];
					...
					if ( objective is DeliverObjective )
					{
						...
						if ( deliver.Update( vendor ) )
						{
							...
							{						
								...
								if ( vendor is IMidgardQuester ) // mod by Dies Irae
									player.SendGump( new MondainQuestGump( player, quest, MondainQuestGump.Section.Complete, false, true, (IMidgardQuester) vendor ) );
								else
									player.SendGump( new MondainQuestGump( quest, MondainQuestGump.Section.Complete, false, true ) );									
							}
		...
		public static bool FirstChainQuest( BaseQuest quest, object quester )
		{
			...
			if ( quester is IMidgardQuester ) // mod by Dies Irae
			{
				IMidgardQuester mQuester = (IMidgardQuester) quester;
		...
		public static Type FindFirstChainQuest( BaseQuest quest )
		{
			...
			if ( quest.Quester is IMidgardQuester )
			{
				IMidgardQuester mQuester = (IMidgardQuester) quest.Quester;
